CREATE OR REPLACE FUNCTION public.fc_saldoextfonte(integer, integer, integer, integer, integer)
...
declare
  ...
  sinal_cadastro	   CHAR(1);
  sinal_ant            CHAR(1);
  sinal_final          CHAR(1);

begin

  --	SINAL SALDO DA CONTA (CONFORME CADASTRO)
  ...
  WHERE( c61_reduz, c61_anousu )=( iConta, iAnousu )
  INTO sinal_cadastro;
  ...
  IF nSaldoInicialMes < 0 THEN
    sinal_ant := 'C';
  ELSEIF nSaldoInicialMes = 0 THEN
    sinal_ant := sinal_cadastro;
  ELSE
    sinal_ant := 'D';
  END IF;
  ...
  IF nSaldoFinalMes < 0 THEN
    sinal_final := 'C';
  ELSEIF nSaldoFinalMes = 0 THEN
    sinal_final := sinal_cadastro;
  ELSE
    sinal_final := 'D';
  END IF;
