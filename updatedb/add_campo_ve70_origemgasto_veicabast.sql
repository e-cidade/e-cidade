...
alter table veicabast add column ve70_origemgasto integer ;
...
select 							 (select max(codcam)+1 from db_syscampo) as codcam
                               ,'ve70_origemgasto'::character(40) nomecam
...
select
	(select codarq from db_sysarquivo where nomearq = 'veicabast'),
	(select codcam from w_db_syscampo),
	(select max(seqarq)+1 from db_sysarqcamp where codarq in (select codarq from db_sysarquivo where nomearq = 'veicabast')) seqarq,
	0;
