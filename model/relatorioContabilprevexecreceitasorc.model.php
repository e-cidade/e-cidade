...
class relatorioContabilPrevExecReceitasOrc {
  ...
  */
  ...
  function parseFormula($sVarNome, $sFormula, $iColuna, $aLinhas) {
    ...
    for ($i = 0; $i < $iTamanhoFormula; $i++) {
      ...
      if (in_array($iCaracterAtual,array('c','f','l'))) {
        /**
         * verificamos se o proximo caracter é um '['
         */
        $iCodigoLinha = null;
        if ($iCaracterProximo == "[") {
           ...
           */
          if ($iCaracterAtual == "c") {
            $iVirgula     = strpos($sFormula, ",", $i);
            $iCelColuna = substr($sFormula, $iVirgula+1, ($iFimChave-$iVirgula)-1);
            $iCelColuna = (int)$iCelColuna - 1;
            $iCelLinha     = substr($sFormula, $iInicioChave+1, ($iVirgula-$iInicioChave)-1);

            $sCelColuna = '';

            switch ($iCelColuna){
              case 0:
                $sCelColuna = 'previnirecbruta';
                break;
              case 1:
                $sCelColuna = 'prevatualrecbruta';
                break;
              case 2:
                $sCelColuna = 'recrealizadabruta';
                break;
              case 3:
                $sCelColuna = 'deducrec';
                break;
              case 4:
                $sCelColuna = 'recrealizadabasecalcpercaplicasps';
                break;
              case 5:
                $sCelColuna = 'deducformacfundeb';
                break;
              case 6:
                $sCelColuna = 'totgeralrecliqrealiza';
                break;
            }

            if(${$sVarNome}[$iCelLinha]->colunas[$iCelColuna]->o116_formula[0] === '#'){
              $sFormulaRetorno .= "\$aLinhas[$iCelLinha]->$sCelColuna";
            }else{
              $sFormulaRetorno .= $this->parseFormula($sVarNome,
                ${$sVarNome}[$iCelLinha]->colunas[$iCelColuna]->o116_formula,
                $iCelColuna,
                $aLinhas
              );
            }
