...
class Recibo
{
  ...
  function estornarRecibo($iNumpre, $sCaracteristicaPeculiar=null, $iSequencialEmpenho = null, $dataLancamento = null) {
  ...
  if (!db_utils::inTransaction()) {
    ...
    $oAutenticacaoArrecadacao = new AutenticacaoArrecadacao($iNumpre, "0", $this->getConta(), $this->getGrupoArrecadacao(), $dataLancamento ?? null, $sCaracteristicaPeculiar, $iSequencialEmpenho);
    if (count($this->aRecursos)  == 1) {
      $oAutenticacaoArrecadacao->setCodigoRecurso($this->aRecursos[0]);
    }
    $oAutenticacaoArrecadacao->estornar($dataLancamento ?? null);
    return true;
  }
