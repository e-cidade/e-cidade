...
class cda {
  ...
  function getProcessoParcelamento() {
    ...
    if ( count($aAgrupaProcParc) > 0 ) {
      ...
      } elseif ( @$iProcedenciaCDA == 13 or @$iProcedenciaCDA == 253 ) {
  ...
  protected function getDebitosParcelamento($lRemissao = false) {
    ...
    foreach ($aOrigens as $oOrigem) {
      ...
      if ($lRemissao){
        ...
        $sSqlCertter       = $oDaoCertter->sql_query(null,null,"distinct v13_dtemis",null,"v07_numpre = {$oOrigem->numpre}");
        $rsConsultaCertter = $oDaoCertter->sql_record($sSqlCertter);
        $dtEmissao         = db_utils::fieldsMemory($rsConsultaCertter,0)->v13_dtemis;

        $dataemis = mktime(0,0,0,substr($dtEmissao,5,2),
                                 substr($dtEmissao,8,2),
                                 substr($dtEmissao,0,4));
        $anoemis  = substr($dtEmissao,0,4);
      } else {
