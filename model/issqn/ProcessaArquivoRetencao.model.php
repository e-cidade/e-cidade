...
class ProcessaArquivoRetencao{
  ...
  public function validarArquivo(){
    ...
    foreach ($this->aRegistrosRetencao as $oRegistroRetencao) {
      ...
      /**
       * Validamos se existe CGM cadastrado para o cnpj do PRESTADOR
       */
      $oCgmPrestador = CgmFactory::getInstanceByCnpjCpf($oRegistroRetencao->q91_cpfcnpjprestador);
      if ($oCgmPrestador == false) {

        $this->aRegistrosInconsistentes[] = array( "sequencial_registro" => $oRegistroRetencao->q91_sequencialregistro,
                                                   "registro"            => $oRegistroRetencao->q91_cpfcnpjprestador,
                                                   "erro"                => self::PRESTADOR_SEM_CGM
                                                 );
        continue;
      }

      /**
       * Validamos se existe Inscrição Ativa cadastrada para o cnpj do PRESTADOR
       */
      $oDaoIssbase        = new cl_issbase;
      $sSqlBuscaInscricao = $oDaoIssbase->sql_queryInscricaoAtivaCgm( $oCgmPrestador->getCodigo() );
      $rsBuscaInscricao   = $oDaoIssbase->sql_record( $sSqlBuscaInscricao );
      if ( !empty($oDaoIssbase->erro_banco) ) {
        throw new Exception( _M( self::MENSAGENS . 'erro_validar_inscricao') );
      }

      $aInscricoesAtivas = array();

      /**
       * Busca todas as inscrições ativas vinculadas ao cgm retornado a partir do cpf/cnpj
       */
      $aInscricoesAtivas = db_utils::getCollectionByRecord( $rsBuscaInscricao );
      if( count($aInscricoesAtivas) == 0 ){

        $this->aRegistrosInconsistentes[] = array( "sequencial_registro" => $oRegistroRetencao->q91_sequencialregistro,
                                                   "registro"            => $oRegistroRetencao->q91_cpfcnpjprestador,
                                                   "erro"                => self::PRESTADOR_SEM_INSCRICAOATIVA
                                                 );
      }
    }
  }
  ...
  public function processarRegistros() {
    ...
    foreach ($this->aRegistrosRetencao as $iIndice => $oRegistroRetencao) {
      ...
      if ($rsCgm == false || $oDaoCgm->numrows == 0) {
        throw new DBException( _M( self::MENSAGENS . 'erro_cgm_nao_encontrado_prestador' ) );
      }
      ...
      if ( $rsIssBase == false || $oDaoIssBase->numrows == 0 ) {
        ...
        $oMensagem->sCnpj = $oRegistroRetencao->q91_cpfcnpjprestador;
        throw new DBException( _M( self::MENSAGENS . 'erro_inscricao_nao_encontrada_prestador', $oMensagem ) );
      }
      ...
      if ($rsTabAtiv == false || $oDaoTabAtiv->numrows == 0) {
        throw new DBException( _M( self::MENSAGENS . 'erro_atividade_nao_encontrada') );
      }
