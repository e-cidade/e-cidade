...
class retencaoNota
{
    ...
    /** Código ordem de pagamento */
    private $iCodOrd = null;


    /**
     * Controle de retencoes
     * @param integer $iCodNota Código da nota (empnota.e69_codnota)
     ...
     * @param boolean $lInSession define se o objeto sera adicionado apenas na sessão, ou sera apenas adicionado a classe.
     * @param boolean $isUpdate define se devera ser modificado ou nao os registros (true para apenas modificar e false para adicionar novo.
     * @return boolean
     */
    ...
    function addRetencao($oRetencao, $lInSession = false, $isUpdate = false)
    {
        ...
        /** Algumas regras:
         *
         * 1 - caso o user cadastrou uma retenção de tipo 1, ou 2 (Imposto de Renda)
         * ele não pode mais cadastrar uma retenção do tipo 3 ou 4 (INSS),
         * pois as retenções do tipo 3, 4 deduzem da base de cálculo.
         *
         * 2 - Não Podemos lançar uma retenção duas vezes.
         *
         * 3 - Sempre devemos ver o cálculo da retenção por CGM(cnpj/cpf) dentro do mês, nunca por nota.
        */

        /**
         * Fazemos uma cópia das retenções cadastradas, para fazermos algumas validações.
         */
        ...
        */
        if (($oDadosRetencao->e21_retencaotipocalc == 3 || $oDadosRetencao->e21_retencaotipocalc == 4 || $oDadosRetencao->e21_retencaotipocalc == 7) && !$isUpdate) {

            foreach ($aRetencoes as $oRetencaoAtiva) {

                if ($oRetencaoAtiva->e21_retencaotipocalc == 1 || $oRetencaoAtiva->e21_retencaotipocalc == 2) {
        ...
        if ($isUpdate) {
            $this->alterarRetencao($oRetencao);
        }

        return true;
    }

    public function alterarRetencao($oRetencao): void
    {
        require_once("classes/db_retencaoreceitas_classe.php");

        $oDaoRetencaoReceita = new cl_retencaoreceitas;
        $sSqlRetencao = $oDaoRetencaoReceita->sql_query_notas(
            null,
            "retencaoreceitas.*",
            null,
            "e71_codnota = $oRetencao->iCodNota 
                   AND e23_retencaotiporec = $oRetencao->iCodigoRetencao 
                   AND e23_ativo != false
                   AND e27_empagemov = $oRetencao->iCodMov"
        );

        $rsRetencao = $oDaoRetencaoReceita->sql_record($sSqlRetencao);

        $iNumRowsRetencao = pg_num_rows($oDaoRetencaoReceita->sql_record($sSqlRetencao));
        ...
        if ($iNumRowsRetencao > 0) {
            ...
            foreach ($aRetencoesAntigas as $oRetencaoAntiga) {
                ...
                $oDaoRetencaoReceita->e23_valorretencao = $oRetencao->nValorRetencao;
                $oDaoRetencaoReceita->e23_aliquota = $oRetencao->nAliquota;
    ...
    /**
     * Persiste a retenção na base de dados.
     *
     * @param integer $iNotaLiquidacao Código da nota de liquidação (e50_codord)
     * @param array $aMovimentosAuxiliares outros movimentos que compõem a base de cálculo.
     * @return boolean
     */
    function salvar($iNotaLiquidacao, $aMovimentosAuxiliares = null)
    {
        ...
        if (count($aRetencoes) > 0) {
            ...
            /**
             * Percorremos as retenções cadastradas, e verificamos se já foi recolhida ou já existe.
             * Caso já exista, marcamos ela como inativa, e incluímos novamente;
             * Caso já esteja recolhido dentro do mes, apenas passamos para a próxima retenção,
             * senão, incluímos nas tabelas retencaopagordem e retencaoreceitas.
             */
            foreach ($aRetencoes as $oRetencao) {
                ...
                $sSqlRetencao = $oDaoRetencaoReceita->sql_query_notas(
                    null,
                    "e23_sequencial, e23_ativo, e23_dtcalculo, e23_recolhido",
                    null,
                ...
                // Percorremos as retenções encontradas e que não foram pagas, e vinculamos a OP.
                if ($iNumRowsRetencao == 0) {

                    // Incluindo na retencaopagordem
                    ...
                    // Ligando a retenção ao movimento da agenda.
    ...
    {$sWhere}");
            ...
            if ($oDaoRetencaoReceitas->numrows > 0) {
                ...
                for ($iInd = 0; $iInd < $oDaoRetencaoReceitas->numrows; $iInd++) {
                     ...
                     */
                    $sSqlMovimentos = $oDaoRetencaoEmpAgeMov->sql_query_file(null, "*", null, "e27_retencaoreceitas = {$oRetencao->e23_sequencial}
     ...
     * @param integer $iNotaLiquidacao [$iNotaLiquidacao]
     * @throws Exception
     */
    function excluiRetencoes($iCodigoRetencao, $iNotaLiquidacao = null)
    {
        ...
        if (!empty($iNotaLiquidacao)) {
            ...
            $oDaoRetencaoReceitas = db_utils::getDao("retencaoreceitas");
            $sSqlRetencaoRecieitas = $oDaoRetencaoReceitas->sql_query_notas(
                null,
                "*",
                null,
                "e20_pagordem = {$iNotaLiquidacao} AND e23_recolhido = false AND e23_retencaotiporec = {$iCodigoRetencao} AND e23_ativo = true AND e71_anulado = false"
            );
            ...
            if (pg_num_rows($rsRetencoes) > 0) {
                ...
                $oDaoRetencaoEmpagemov->excluir($oRetencaoAtiva->e27_sequencial);
                $oDaoRetencaoReceitas->excluir($oRetencaoAtiva->e23_sequencial);
                $oDaoRetencaoPagordem->excluir($oRetencaoAtiva->e20_sequencial);

                if ($oDaoRetencaoReceitas->erro_status == 0) {

                    throw new Exception("Erro [1] - Não foi possivel excluir retenção\nErro: {$oDaoRetencaoReceitas->erro_status}");
                }
            }
        }
            ...
            */
            ...
            if ($oRetencao->e23_valorretencao > 0) {
                ...
                */
                ...
                $oDotacao = new Dotacao($oNotaLiquidacao->oDadosOrdem->e60_coddot, $oNotaLiquidacao->oDadosOrdem->e60_anousu);
                ...
                if ($oRetencao->e21_retencaotipocalc != 5 || $oInstit->prefeitura == "f") {
                    ...
                    if (isset($oReciboAvulso)) {
                        ...
                        */
                        ...
                        $sSqlDotacao = $oDaoDotacao->sql_query_file($oNotaLiquidacao->oDadosOrdem->e60_anousu, $oNotaLiquidacao->oDadosOrdem->e60_coddot, "o58_codigo");
                    ...
                    */
                    ...
                    $oDaoDotacao = db_utils::getDao("orcdotacao");
                    $rsRecurso = $oDaoDotacao->sql_record($oDaoDotacao->sql_query_file($oNotaLiquidacao->oDadosOrdem->e60_anousu, $oNotaLiquidacao->oDadosOrdem->e60_coddot, "o58_codigo"));
                    if ($oDaoDotacao->numrows == 1) {
                ...
                $sSqlCorgrupo = $oDaoCorrenteGrupo->sql_query_file(null, "*", "k105_autent desc limit 1", "k105_corgrupo = " . $this->getGrupoAutenticacao());
    ...
    function estornarRetencoes($oRetencao)
    {
        ...
        require_once("classes/db_empagemovslips_classe.php");
        require_once("classes/db_retencaoreceitas_classe.php");
        ...
        if (strpos($oRetencao->sDataLancamento, '/')) {
            $dataLancamento = DateTime::createFromFormat('d/m/Y', $oRetencao->sDataLancamento);
        ...
        if ($oRetencao->e23_valorretencao > 0) {
            ...
            if ($oRetencao->e21_retencaotipocalc != 5 || $oInstit->prefeitura == "f") {
                ...
                if (isset($oReciboAvulso)) {
                    ...
                    $sSqlBuscaCaracteristica = "SELECT empempenho.e60_concarpeculiar FROM retencaoreceitas
                    ...
                    $oReciboAvulso->estornarRecibo($iNumpre, $sCaracteristicaPeculiar, null, $dataLancamento ?? null);
                ...
                $oReciboDebito->estornarRecibo($iNumpre, $sCaracteristicaPeculiar, null, $dataLancamento ?? null);
    ...
    function getNumpreRetencao($iRetencao)
    {
        ...
        $oDaoRetencaoCorrente = db_utils::getdao("retencaocorgrupocorrente");
        $iNumpre = 0;
        $sSqlNumpre = $oDaoRetencaoCorrente->sql_query_numpre(null, "k12_numpre", null, "e23_sequencial = {$iRetencao}
    ...
    function getRetencaoByCodigo($iCodigoRetencao, $lRecolhida = false)
    {
        ...
        $sSqlRetencaoReceitas = $oDaoRetencaoReceitas->sql_query_notas(
            null,
            "distinct tabrec.*,retencaotiporec.*, retencaoreceitas.*, e27_principal, k108_slip, e27_empagemov, k107_empagemov ",
            "e21_sequencial ",
    ...
    /**
     * Retorna o código da ordem de pagamento baseado no código da nota (e69_codnota)
     * @return int
     */
    ...
    /**
     * Retorna as retenções (tabela: retencaoreceitas) baseadas no $iCodOrd
     * @param boolean $lValorTotal se 'true', retorna a soma do valor das retenções
     * @return mixed
     */
    public function getRetencoesFromCodOrd($lValorTotal = false)
    {
        $sCampos = $lValorTotal ? ' SUM(e23_valorretencao) as valor_total ' : ' * ';

        $sSqlRReceita = " SELECT {$sCampos} FROM retencaoreceitas "
            . " LEFT JOIN retencaopagordem ON e20_sequencial = e23_retencaopagordem "
            . " WHERE e20_pagordem = {$this->getCodOrd()} AND e23_ativo = TRUE; ";

        if ($lValorTotal) {
            $nRetorno = db_utils::fieldsMemory(db_query($sSqlRReceita), 0)->valor_total;
            return floatval($nRetorno);
        }

        return db_utils::getCollectionByRecord(db_query($sSqlRReceita));
    }

    /**
     * Retorna o valor da Ordem de Pagamento, considerando o desconto aplicado
     * @return float
     */
    ...
    /**
     * Retorna soma do valor das retençoes na seçao
     * @return float
     */
    public function getValorRetSession()
    {
        $nTotal = 0;

        foreach ($this->getRetencoes() as $retencaoSession) {
            $nTotal += floatval($retencaoSession->e23_valorretencao);
        }
    ...
    /**
     * Calcula ((valor da movimentação) - (valor retenções) - (valor da nova retenção)).
     * Retorna 'true' se o cálculo for > 0, 'false' se < 0
     * @param $oNovaRetencao retenção a ser inserida
     * @return boolean
     */
    public function podeInserirRetencao($oNovaRetencao)
    {
        $nValorDessa = floatval($oNovaRetencao->nValorRetencao);
        $nValorMovimento = floatval($oNovaRetencao->nValorbase);
