...
class CalculoBem {
  ...
  public function calcular() {
    ...
    /**
     * caso seja o ultimo mes de depreciação do bem,
     * devemos verificar se é necessário algum ajuste no valor do benm.
     * Bens nao pode ter depreciação negativa.
     */
    if (($iNumeroMeses + 1) == ($this->getBem()->getVidaUtil() * 12)) {
      ...
      if (round($nValorFinalBem, 2) != 0) {
        ...
        /**
         * Valor da diferença a ser verificada
         * a Diferença é o valor depreciado no mes.
         */
        ...
        if($nValorDepreciado > 0) {
          ...
          /**
        	 * Calculamos o percentual da Diferença entre o valor final do bem, e o valor da
        	 * da depreciaçaõ mes. com esse percentual, conseguimos chegar ao valor que a
        	 * depreciaçaõ deve ser no mes, para zeramos o valor da depreciação do bem.
        	 */
          $nPercentualDaDiferenca = round(($nValorFinalBem * 100) / $nValorDepreciado, 2);

          /**
        	 * Calcul do valor da Diferença;
        	 */
          $nValorDiferenca = round(($nPercentualDaDiferenca * $nValorDepreciado) / 100 ,2);
        }

        /**
         * Somamos o valor da Diferença ao bem.
         */
        $nValorFinalBem += $nDiferenca;

        /**
         * Corrigimos o valor da Depreciação no mes.
         */
        $nValorDepreciado += $nValorDiferenca;

        /**
         * Acertamos o percentual da Depreciaçao no mes, em relação ao valor
