...
class tableDataManager {
    ...
    /**
     * Adicionado neste array os tipos de dados que no podem entrar string quando os campos destes tipo
    ...
    public  function setByLineOfDBUtils($oDBUtils, $lInsertValue = false){
        ...
        foreach ( $this->aTableProperties as $aCampo ) {
            ...
            if (!array_key_exists($aCampo[0], (array) $oDBUtils) && $aCampo[0] != $this->sCampoPk) {
                throw new Exception("[tableDataManager] Campo ".$aCampo[0].":".$aCampo[1]." no informado!");
            }
    ...
    function persist() {
        ...
        $iCont = 0;
        @pg_query($this->pConexao, "copy {$this->sTableName} from stdin") ;
    ...
    public function persistSequenceValue() {
        $rsSetVal   = @pg_query("select setval('{$this->sSequenceName}', {$this->getLastPk()});");
    }
}
