 ...
 *
 */
abstract class RelatorioFolhaPagamentoPontoFixo {
  ...
  protected function retornaSQLBaseRelatorio($sWhere = null, $sOrderBy = null, $sCampos = '*') {
    ...
    $sWhereSqlBase = " rh01_regist is not null and r53_instit = " . db_getsession("DB_instit"); 
    if (!empty($this->sWhereSelecao)) {
      $sWhereSqlBase .= $this->sWhereSelecao;
    }
    if (!empty($sWhere)) {
      $sWhereSqlBase .= " and {$sWhere}";
    }
    ...
    foreach($this->aTipoFolha as $iIndiceTipoFolha => $oTipoFolha) {
      ...
      $sSqlBase  .= "        rh30_descr   as descr_regime,                                                           \n";
  ...
  public    function addTipoFolha($iTipoFolha) {
    ...
    switch ($iTipoFolha){
        ...
        case "7":
    		  $oTipoFolha->iCodigo = $iTipoFolha;
    		  $oTipoFolha->sTabela = "gerffx";
    		  $oTipoFolha->sSigla  = "r53";
        break;
    }
