...
class AjustePrevidencia {
  ...
  public static function ajustar($arquivo, $rubrica_base, $sequencia, $sigla_ajuste) {
    ...
    global $tipo_arquivo;
    global $previden,$inssirf,$Iinssirf,$previden_,$campos_pessoal,$opcao_geral,$db21_codcli,$subpes,$perc_inss;
  ...
  public static function ajustarValorInativosQueNaoAlcancaramTeto($sRubrica, $sTipoArquivo) {
    ...
    while ( list($iCgm, $aRubricas ) = each( self::$aValoresBase ) ) {
      ...
      foreach ($aValores as $iMatricula => $nValor) {
        ...
        $aChaves[1]  = "{$sSigla}_valor";
        $aValores[1] = "{$nValorBaseIndividual}";

        db_update($sTabela, $aChaves, $aValores, bb_condicaosubpes("{$sSigla}_")." and {$sSigla}_regist = {$iMatricula} and r14_rubric in ('{$sRubrica}','R992')");
