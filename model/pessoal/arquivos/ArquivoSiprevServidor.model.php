...
class ArquivoSiprevServidor extends  ArquivoSiprevBase {
    ...
    foreach ($aListaDados as $oIndiceDados => $oValorDados) {
       ...
       */
       if ( !$lCpfValido || !$lSexoValido || !$lPisValido) {
        
         $sPis  = $oValorDados->rh16_pis;

         if (!$lCpfValido && !$lSexoValido && !$lPisValido) {
            $sErro = "PIS, Sexo e CPF Inválido";
         } elseif (!$lCpfValido && !$lSexoValido) {
            $sErro = "Sexo e CPF Inválido";
         } elseif (!$lCpfValido && !$lPisValido ) {
            $sErro = "CPF e PIS Inválido";
         } elseif (!$lSexoValido && !$lPisValido) {
        ...
        // Dados Pessoais
          $oLinha->dadosPessoais->nome                       = DBString::removerCaracteresEspeciais($oValorDados->z01_nome);
          ...
          $oLinha->dadosPessoais->sexo                       = strtoupper($oValorDados->z01_sexo);
          $oLinha->dadosPessoais->nomeMae                    = DBString::removerCaracteresEspeciais($oValorDados->z01_mae);
          $oLinha->dadosPessoais->nomePai                    = DBString::removerCaracteresEspeciais($oValorDados->z01_pai);
          ...
          if ($oValorDados->rh16_pis == null ||$oValorDados->rh16_pis == '00000000000' ){
