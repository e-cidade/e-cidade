...
abstract class ArquivoConsignetRepository {
 ...
 public static function hasArquivoCompetencia (DBCompetencia $oCompetencia, Instituicao $oInstituicao) {
   ...
   $sWhereConsignetMovimento = "rh151_ano = '{$oCompetencia->getAno()}' and rh151_mes = '{$oCompetencia->getMes()}' and rh151_instit = {$oInstituicao->getCodigo()}";
  ...
  public static function getUltimoArquivo(Instituicao $oInstituicao, DBCompetencia $oCompetencia, $lProcessado = null) {
    ...
    $sWhere .= "rh151_instit = {$oInstituicao->getCodigo()}";
  ...
  public static function getUltimosArquivos(Instituicao $oInstituicao, DBCompetencia $oCompetencia, $lProcessado = null) {
    ...
    $sWherePreSql         = "     rh151_ano          = '{$oCompetencia->getAno()}'
    ...
    if ( is_resource($rsResultadoPreSql) && pg_num_rows($rsResultadoPreSql) > 0 ){
      ...
      throw new BusinessException(_M('recursoshumanos.pessoal.ProcessamentoPonto.'. 'nenhum_dado_encontrado'));
    }
  }
    ...
    foreach ($aArquivos as $oArquivo) {
      ...
      foreach ($oArquivo->getRegistros() as $oRegistro) {
        ...
        if ( $fMargem === false){
          ...
          $oVariavelErro = new stdClass();
          $oVariavelErro->nomeServidor = $oRegistro->getServidor()->getCgm()->getNome();

          throw new BusinessException(_M(ArquivoConsignetRepository::MENSAGEM . 'servidor_sem_margem', $oVariavelErro));

        } else {

          $fValorConsignar = $oRegistro->getValorParcela();
          if ( $fMargem < $oRegistro->getValorParcela() ){
            $fValorConsignar = $fMargem;
          }
        }
