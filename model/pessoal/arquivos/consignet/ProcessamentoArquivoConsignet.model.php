...
class ProcessamentoArquivoConsignet {
 ...
 /**                                                                                                                                                                         
  ...
  private function validarAfastamento(RegistroArquivoImportacao $oRegistro) {
     ...
     * 
     */
    if ($oRegistro->getValorParcela() > $oRegistro->getValorDescontado()) {
      ...
      if (!$rsAfastamento) {
        throw new BusinessException(_M(self::MENSAGEM . 'erro_alterar_rhconsignadomovimentoservidor'));
      }
      
      if (pg_num_rows($rsAfastamento) > 0) {
        
        $oRegistro->setMotivo(ArquivoConsignet::MOTIVO_SERVIDOR_AFASTADO);
        $oDaoMovimentoServidor->rh152_consignadomotivo = ArquivoConsignet::MOTIVO_SERVIDOR_AFASTADO;         
        $oDaoMovimentoServidor->rh152_sequencial       = $oRegistro->getSequencial();
