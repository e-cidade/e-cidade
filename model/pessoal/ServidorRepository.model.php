...
abstract class ServidorRepository
{
   ...
   */
    ...
    public static function persistServidor(Servidor $oServidor, $lSalvouVinculado = null)
    {
        ...
        /**
         * Caso servidor possua mais de um vínculo popula os atributos da classe cl_rhpessoalmov
         * com os dados atuais da base de dados adicionando apenas o(s) dado(s) comum(ns) com o
         * outro servidor vinculado, no caso o abono permanência
         */
        if ($lSalvouVinculado === true) {
            $oDadosPessoalMov = db_utils::fieldsMemory($rsRHPessoalMov, 0);
            foreach ($propriedadesRhPessoalMov as $key => $value) {
                if (isset($oDaoRHPessoalMov->$key)) {
                    $oDaoRHPessoalMov->$key = $oDadosPessoalMov->$key;
                }
            }

            $oDaoRHPessoalMov->rh02_abonopermanencia          = $oServidor->hasAbonoPermanencia();

            if (!$oDaoRHPessoalMov->rh02_abonopermanencia) {
                $oDaoRHPessoalMov->rh02_abonopermanencia = 'f';
            }
        }
        ...
        if (is_resource($rsRHPessoalMov) && pg_numrows($rsRHPessoalMov) > 0) {
            ...
            if ($oServidor->hasServidorVinculado()) {
                ...
                if ($lSalvouVinculado !== true) {
                    if ($oServidorVinculado->getTabelaPrevidencia() == $oDaoRHPessoalMov->rh02_tbprev
            &&  $oServidor->getTabelaPrevidencia() != $oDaoRHPessoalMov->rh02_tbprev) {
                        $oDaoRHPessoalMov->rh02_abonopermanencia = $oServidorVinculado->hasAbonoPermanencia();
                        $GLOBALS["HTTP_POST_VARS"]['rh02_abonopermanencia'] = $oDaoRHPessoalMov->rh02_abonopermanencia;
                    }
                }
            }
        ...
        /**
         * Verifica se tem servidor vinculado e chama esta mesma função
         * recursivamente afim de salvar o servidor vinculado
         */
        if ($oServidor->hasServidorVinculado()) {
            $oServidorVinculado = $oServidor->getServidorVinculado();

            if ($lSalvouVinculado !== true) {
                if ($oServidorVinculado->getTabelaPrevidencia() == $oServidor->getTabelaPrevidencia()) {
                    $oServidor->setAbonoPermanencia($oDaoRHPessoalMov->rh02_abonopermanencia);
                    $oServidorVinculado->setAbonoPermanencia($oServidor->hasAbonoPermanencia());
                    ServidorRepository::persistServidor($oServidorVinculado, true);
                }
            }
        }
