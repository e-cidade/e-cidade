...
class dadosEmpenhoFolha {
  ...
  public function getExcessoesEmpenhoFolha($sRubric='',$iAnoUsu='',$iInstit=''){
  	...
  	if ( trim($sRubric) == '' ) {
  		throw new Exception('Rubrica não informada!');
  	}
  ...
  */ 
       ...
       */
	    ...
	    if ( $iLinhasEmpenhos > 0 ) {
	    	...
	    	for ( $iInd=0; $iInd < $iLinhasEmpenhos; $iInd++ ) {
		      ...
		      if ( $oDaorhEmpenhoFolha->erro_status == "0") {
		      	echo $oDaorhEmpenhoFolha->erro_msg.' '.$sMsgErro;
    	...
    	if ( $iLinhasPagExtra > 0 ) {
    		...
    		for ( $iInd=0; $iInd < $iLinhasPagExtra; $iInd++ ) {
			    ...
			    if ( $oDaorhLotaVinc->numrows == 0 ){
			      throw new Exception("Verifique recurso e projeto atividade da lotação {$oPagExtra->lotacao}");
			    } else {
  ...
  public function geraDevolucoesEmpenhosFolha($sSigla='',$iAnoUsu='',$iMesUsu='',$iInstit='',$sSemestre=''){
    ...
    if ( $rsDevolucao ) {
      ...
      if ( $iLinhasDevolucao > 0 ) {
        ...
        for ( $iInd=0; $iInd < $iLinhasDevolucao; $iInd++ ) {
          ...
          if ( $oDaorhLotaVinc->numrows == 0 ){
            throw new Exception("Verifique recurso e projeto atividade da lotação {$oDevolucao->lotacao}");
          } else {
            $oProjAtivRec =  db_utils::fieldsMemory($rsProjAtivRec,0);
            $iRecurso = $oProjAtivRec->recurso;
          }               
    ...
    if ( trim($iLotacao) == '' ) {
      throw new Exception('Lotação não informada!');
    }
    if ( trim($sVinculo) == '' ) {
    ...
    if ( $oDaorhLotaExe->numrows == 0 ) {
      throw new Exception("{$sMsgErro}, verifique órgão e unidade da lotação {$iLotacao}");
    } else {
    ...
    if ( $oDaorhLotaVinc->numrows == 0 ){
      throw new Exception("Verifique recurso e projeto atividade da lotação {$iLotacao}");
    } else {
      $oProjAtivRec =  db_utils::fieldsMemory($rsProjAtivRec,0);
    }           
  ...
  public function geraDadosEmpenhosFGTS($sTipo='',$iAnoUsu='',$iMesUsu='',$iInstit=''){
    ...
    if ( trim($sTipo) == '' ) {
      throw new Exception("{$sMsgErro}, tipo de folha não informado!");
    }
    if ( trim($iAnoUsu) == '' ) {
      $iAnoUsu = db_anofolha();
    }   
    ...
    if ( $oDaoOrcParametro->numrows > 0 ) {
      ...
      throw new Exception("{$sMsgErro}, configure os parâmetros do orçamento para o ano {$iAnoUsu}!");
    }  
   ...
   */
	public function getSiglasFGTS($sTipo=''){
		...
		if ( trim($sTipo) == '') {
			throw new Exception("Tipo de folha não informado!");
		}
    ...
    if ( $sTipo == 'm') {
      ...
      $aSiglas = array('r35');
    	...
    	throw new Exception("Tipo de folha difere de mensal e 13º!");
    }
    ...
    if ( trim($sListaPrev) == '' ) {
      throw new Exception("{$sMsgErro}, previdência não informada!");
    }    
    if ( trim($iMesUsu) == '' ) {
      $iAnoUsu = db_mesfolha();
    }
  ...
  public function isLiberada($sSigla='',$sTipoEmp='',$iAnoUsu='',$iMesUsu='',$sSemestre='',$sListaPrev='') {
    ...
    if ( trim($sTipoEmp) == '' ) {
      throw new Exception("{$sMsgErro}, tipo não informado!");
    }  
    if ( trim($iAnoUsu) == '' ) {
      $iAnoUsu = db_anofolha();
    }   
   ...
   * @param integer $iAnoUsu   Exercício
   * @return integer Código da Dotação
   */
   ...
   */
  public function geraPlanilhaGeral( $sSigla='',$iAnoUsu='',$iMesUsu='',$iInstit='',$sSemestre='' ){
