...
class EmpenhoFinanceiroRepository {
  ...
  /**
   * Retorno uma instancia do empenho pelo Codigo e ano
   *
   * @param integer $iCodigoEmpenho
   * @param integer $iAnoEmpenho
   * @return EmpenhoFinanceiro
   */
  public static function getEmpenhoFinanceiroPorCodigoAno($iCodigoEmpenho, $iAnoEmpenho) {

    $oDaoEmpempenho = new cl_empempenho();
    $sSqlEmpempenho = $oDaoEmpempenho->sql_query_file(null, "e60_numemp", null, "e60_codemp = '{$iCodigoEmpenho}' and e60_anousu = {$iAnoEmpenho}");
    $rsEmpenho      = $oDaoEmpempenho->sql_record( $sSqlEmpempenho );

    if ($oDaoEmpempenho->numrows > 0) {
      return self::getEmpenhoFinanceiroPorNumero( db_utils::fieldsMemory($rsEmpenho, 0)->e60_numemp );
    }
