...
require_once("classes/db_testadanumero_classe.php");
...
class Lote {
  ...
  */
  ...
  public function getNumeroLote($codigoLote)
    {

        $testadaInstancia = new cl_testadanumero();

        $sql = "select j15_numero from testpri t inner join testadanumero tn on t.j49_face = tn.j15_face and t.j49_idbql = tn.j15_idbql where t.j49_idbql = $codigoLote";

        $testadaRegistro = $testadaInstancia->sql_record($sql);
        $testadaNumero = db_utils::fieldsMemory($testadaRegistro);

        return $testadaNumero->j15_numero;
    }
