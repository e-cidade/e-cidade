 ...
 */
  ...
  static function findCep($sCep, $sNomeBairro = null) {
    ...
    if (!empty($sCep)) {
      ...
      $oCep       = db_utils::getDao('cadenderruacep');
      $sWhere     = "  db86_cep = '".$sCep."'";
      $sCampos    = "  db70_sequencial as ipais,   db70_descricao as spais                         ";
      $sCampos   .= " ,db71_sequencial as iestado, cadendermunicipio.db72_sequencial as imunicipio ";
      $sCampos   .= " ,cadendermunicipio.db72_descricao as smunicipio, db73_sequencial as ibairro  ";
      $sCampos   .= " ,db73_descricao  as sbairro, db74_sequencial as irua, db74_descricao as srua ";
      $sCampos   .= " ,db85_ruastipo as iruatipo,  db85_sequencial as iruastipo                    ";
      //$sCampos   .= " ,db86_sequencial as icodigocep,      db86_cep as scep ";


      if ($sNomeBairro != null) {
        $sWhere .= " and translate(cadenderbairro.db73_descricao,'áéíóúàèìòùãõâêîôôäëïöüçÁÉÍÓÚÀÈÌÒÙÃÕÂÊÎÔÛÄËÏÖÜÇ','aeiouaeiouaoaeiooaeioucAEIOUAEIOUAOAEIOOAEIOUC') = '{$sNomeBairro}'";
      }

      $sQueryCep  = $oCep->sql_query_cep(null, $sCampos, 'db70_sequencial ', $sWhere);
      $rsQueryCep = $oCep->sql_record($sQueryCep);
      ...
      if ($rsQueryCep !== false) {
       ...
       /*
        $oCep       = db_utils::getDao('ceplocalidades');
        $sWhere     = " cp05_cepinicial = '".$sCep."' or cp05_cepfinal = '".$sCep."'";
        $sQueryCep  = $oCep->sql_query(null,"*",null,$sWhere);
        $rsQueryCep = $oCep->sql_record($sQueryCep);
        */
  ...
  static function findMunicipioByEstado($iCodigoEstado) {
    ...
    if (!empty($iCodigoEstado)) {
      ...
      // $sWhere       .= " and (select length(cast(db72_sequencial as varchar(6)))) = 4 ";

      $sSqlMunicipio = $oDaoMunicipio->sql_query(null, $sCampos, $sOrder, $sWhere);
      //print_r($sSqlMunicipio);
      $rsMunicipio   = $oDaoMunicipio->sql_record($sSqlMunicipio);

      /*

SELECT p.db70_sequencial,
       p.db70_descricao,
       a.db71_sequencial,
       a.db71_descricao,
       db72_sequencial,
       db72_cadenderestado,
       to_ascii(db72_descricao) as db72_descricao
FROM cadenderparam
...
INNER JOIN cadendermunicipio ON cadendermunicipio.db72_cadenderestado = cadenderestado.db71_sequencial
INNER JOIN cadenderpais AS p ON p.db70_sequencial = cadenderestado.db71_cadenderpais
INNER JOIN cadenderestado AS a ON a.db71_sequencial = cadendermunicipio.db72_cadenderestado
...
WHERE
db72_cadenderestado=11 and db125_db_sistemaexterno = 4 and
db72_sequencial =
        (SELECT db125_cadendermunicipio
         FROM db_config
         JOIN cadendermunicipio ON munic = to_ascii(db72_descricao)
         JOIN cadendermunicipiosistema ON cadendermunicipiosistema.db125_cadendermunicipio = cadendermunicipio.db72_sequencial
         AND codigo = 1 AND db72_cadenderestado =11 and db125_db_sistemaexterno = 4)
  ...
  static function findCgmEnderecoByCgm($iNumCgm) {
    ...
    $oDaoCgmEndereco = db_utils::getDao('cadenderlocal');
...
///      echo($oDaoEndereco->sql_query_completo(null,$sCampos,null,$sWhere));
  ...
  static function buscaEnderecoCidadao($iOv02_sequencial, $iOv02_seq, $lEncode=true) {
    ...
    *
    */
  static function buscaBairroRuaMunicipio($iCodigoBairroMunicipio, $iCodigoRuaMunicipio, $lEncode=true) {
    ...
    if (trim($iCodigoBairroMunicipio) != "") {
      ...
      $oDaoRuaRuas = db_utils::getDao('cadenderruaruas');
      $sCampos  = " db74_descricao as sRua, db73_descricao as sbairro ";
      $sCampos .= " ,db74_sequencial as irua ,db73_sequencial as ibairro ";
      $sCampos .= " ,db85_sequencial as iruastipo, db85_ruastipo as iruatipo ";

      $sWhere  = " j14_codigo = ".$iCodigoRuaMunicipio;
      $sWhere .= " and trim(db73_descricao) in (select upper(trim(j13_descr)) from bairro where j13_codi = ".$iCodigoBairroMunicipio.")";

      $sQueryRuaBairro  = $oDaoRuaRuas->sql_queryBairroRuaMunicipio(null,$sCampos,null,$sWhere);
