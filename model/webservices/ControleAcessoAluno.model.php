...
/**
 * Classe singleton para  a configuração do serviço da coleta
...
class ControleAcessoAluno {
  ...
  private function __construct() {
    ...
    require_once ("webservices/legacy_config/integracaoControleAcessoAluno.wsdl.php");
    $this->sUrlWebservice = $sUrlWebService;
    $this->lUsaWsdl       = $lUsaWsdl;
  }

  /**
   * marcamos o método como private, evitando e termos
  ...
  public function alunoEstaNaEscola($iAluno, $dtDiaAula, $sInicioAula, $sTerminoAula) {
  ...
  static function getMensagemNotificacao() {
    ...
    $sArquivoConfiguracao = "legacy_config/notificacao/servicos/notificacaoaluno.xml";
    if (!file_exists($sArquivoConfiguracao)) {
      throw new FileException('Arquivo de configuração para a notificação dos alunos não encontrado.');
    }
