...
class DadosCensoTurmaEspecial extends DadosCenso {
	...
	public function getDados() {
		...
		$oDaoTurma     = db_utils::getDao("turmaac");
		...
		$sCamposTurma .= "   WHERE ed17_i_turno  = ed268_i_turno";
		$sCamposTurma .= "     AND ed17_i_escola = ed268_i_escola) AS horario_inicial, ";
		$sCamposTurma .= " (SELECT max(ed17_h_fim) ";
		$sCamposTurma .= "    FROM periodoescola ";
		$sCamposTurma .= "   WHERE ed17_i_turno = ed268_i_turno";
		$sCamposTurma .= "     AND ed17_i_escola = ed268_i_escola) AS horario_final, ";
    ...
    $oDadosTurma->quantidade_de_atividades_na_turma  = count($aAtividades);
		...
		$oDadosTurma->aee_ensino_uso_recursos_opticos_nao_opticos        = substr($oDadosTurma->atividades_apoio, 1, 1);
		$oDadosTurma->aee_estrategias_desenvolvimento_processos_mentais  = substr($oDadosTurma->atividades_apoio, 2, 1);
		$oDadosTurma->aee_tecnicas_orientacao_mobilidade                 = substr($oDadosTurma->atividades_apoio, 3, 1);
		$oDadosTurma->aee_ensino_lingua_brasileira_sinais_libras         = substr($oDadosTurma->atividades_apoio, 4, 1);
		$oDadosTurma->aee_ensino_comunicacao_alternativa_aumentativa     = substr($oDadosTurma->atividades_apoio, 5, 1);
		$oDadosTurma->aee_estrategia_enriquecimento_curricular           = substr($oDadosTurma->atividades_apoio, 6, 1);
		$oDadosTurma->aee_ensino_uso_soroban                             = substr($oDadosTurma->atividades_apoio, 7, 1);
		$oDadosTurma->aee_ensino_usabilidade_funcionalidades_informatica = substr($oDadosTurma->atividades_apoio, 8, 1);
		$oDadosTurma->aee_ensino_lingua_portuguesa_modalidade_escrita    = substr($oDadosTurma->atividades_apoio, 9, 1);
		$oDadosTurma->aee_estrategias_autonomia_ambiente_escolar         = substr($oDadosTurma->atividades_apoio, 10, 1);
	...
	public function getDiasDaSemanaComAula () {
		...
		$aDiasDaSemana       = array();
		$oDaoRegenciaHorario =  new cl_dialetivo();
