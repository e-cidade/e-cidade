...
class Turma {
  ...
  public function getAlunosProgressaoParcial(Etapa $oEtapa = null) {
    ...
    if( count($this->aAlunosProgressaoParcial) == 0 || $oEtapa != null ) {
      ...
      if ($oEtapa != null) {
        ...
        $sWhere          .= " and ed115_regencia in ({$sRegenciasTurma}) and ed114_serie = {$oEtapa->getCodigo()}";
      }
   ...
   */
  public function getVagasDisponiveis ($iTurnoReferente = null ) {
  ...
  public function encerradaParcial(Etapa $oEtapa) {
    ...
    $rsDiariosEncerrados            = $oDaoDiario->sql_record($sSqlTurmaEncerradaParcialmente);

    if ($rsDiariosEncerrados && $oDaoDiario->numrows > 0) {
   ...
   * @param  boolean $lOrdenarNome - Verifica se os dados devem ser ordenados pelo nome do aluno
   * @return Matricula[]
   */
   ...
   * @return boolean
   */
  public function possuiMesmoProcedimentosAvaliacao( Turma $oTurmaNova ) {

    foreach( $this->getDisciplinas() as $oRegenciaAtual ) {

      foreach( $oTurmaNova->getDisciplinas() as $oRegenciaNova ) {
