 ...
 * @version 1.0
 */
class ContaCorrenteDotacao extends ContacorrenteBase implements IContaCorrente {

  /**
   * Registro na tabela contacorrente
   * @var integer
   */
  const CONTA_CORRENTE = 101;
  ...
  public function salvar($dtLancamento = null) {
    ...
    $iRecurso     = $this->oContaCorrenteDetalhe->getDotacao()->getRecurso();
    //$iRecurso     = $this->oContaCorrenteDetalhe->getRecurso()->getCodigo();
    $iDotacao     = $this->oContaCorrenteDetalhe->getDotacao()->getCodigo();
    $iAnoDotacao  = $this->oContaCorrenteDetalhe->getDotacao()->getAno();
    $sEstrutural  = $this->oContaCorrenteDetalhe->getDotacao()->getElemento();
  ...
  private function validarInformacoes() {
    /*echo "<pre>";
    print_r($this->oContaCorrenteDetalhe);
    ...
    if ( !$this->oContaCorrenteDetalhe->getDotacao() instanceof Dotacao) {
      throw new ParameterException("Dotação não informada para execução do conta corrente {$this->oContaCorrente->getDescricao()}.");
    }
