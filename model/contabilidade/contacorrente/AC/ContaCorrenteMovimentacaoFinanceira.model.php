 ...
 * @version 1.0
 */
class ContaCorrenteMovimentacaoFinanceira extends ContacorrenteBase implements IContaCorrente {

  /**
   * Registro na tabela contacorrente
   * @var integer
   */
  const CONTA_CORRENTE = 105;
  ...
  public function salvar($dtLancamento = null) {
    ...
    $iContaBancaria = $this->oContaCorrenteDetalhe->getContaBancaria()->getSequencialContaBancaria();
    ...
    if ($oDaoContaCorrenteDetalhe->numrows == 1) {
      ...
      if ($oDaoContaCorrenteDetalhe->erro_status == 0) {
        throw new DBException("Não foi possível inserir os dados da conta corrente {$this->oContaCorrente->getDescricao()}\n {$oDaoContaCorrenteDetalhe->erro_msg}");
      }
  ...
  private function validarInformacoes() {
    ...
    if ( ! $this->oContaCorrenteDetalhe->getContaBancaria() instanceof ContaBancaria) {
      throw new ParameterException("Conta bancária não informada para execução do conta corrente {$this->oContaCorrente->getDescricao()}.");
    }
    return true;
  }
}