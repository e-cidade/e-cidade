...
/**
 * Class BalancoFinanceiroDCASP2024
 */
class BalancoFinanceiroDCASP2024 extends RelatoriosLegaisBase {

  /**
   * @type int
   */
  const CODIGO_RELATORIO        = 1522024;

  const LINHA_INICIO_DISPENDIOS = 33;

  const LINHA_FINAL_DISPENDIOS  = 64;
   ...
   * @var array
   */
  private $aLinhasFinais = array(32, 64);

    /**
   * Linhas em negrito não totalizadoras. Utilizado somente para formatar a linha
   *
   * @var array
   */
  private $aLinhasNegrito = array(2, 34);
  ...
  private function escreverLinha(stdClass $oLinha) {
     ...
     */
    if ($oLinha->totalizar || in_array($oLinha->ordem, $this->aLinhasNegrito)) {
      $this->oPdf->setBold(true);
    }
  ...
  public function emitir() {
    ...
    foreach ($aDados as $oLinha) {
      ...
      if ($oLinha->ordem == self::LINHA_FINAL_DISPENDIOS && $this->oPdf->getAvailHeight() < 58) {
        $this->adicionarPagina($sNomeSecao);
      }
