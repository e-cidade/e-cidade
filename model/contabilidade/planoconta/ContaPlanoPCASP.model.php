...
class ContaPlanoPCASP extends ContaPlano {
    ...
    public function salvar($iTipoConta) {
        ...
        /*
         * Verifica se o sistema de conta selecionado é o "Informações Patrimoniais". Caso seja
         * o indicador de superavit não pode ser o "Não Se Aplica"
         */
        if ($this->getSubSistema()->getCodigo() == 2 && $this->getIdentificadorFinanceiro() == "N") {

            $sMsgErro  = "O subsistema de conta selecionado é de Informações Patrimoniais. Selecione o ";
        ...
        $oDaoConPlano->c60_nregobrig               = $this->getNRegObrig();
    ...
    public function persistirReduzido() {
        ...
        for ($iAno = $this->getAno(); $iAno <= $iUltimoAno; $iAno++) {
            ...
            $oDaoReduzido->c61_codtce        = $this->getCodigoTce();
    ...
    public function removerReduzido($iCodigoReduzido) {
        ...
        for ($iAnoFor = $iAnoUso; $iAnoFor <= $iUltimoAno; $iAnoFor++) {
            ...
            /**
             * Valida se existem lançamentos contábeis para o ano e reduzido informado
             */
            if ($this->hasLancamentosContabeis($iCodigoReduzido, $iAnoFor)) {
                throw new Exception("Conta possui lançamentos, não pode ser excluida.");
            }
            ...
            /**
             * alteração realizada para excluir os reduzidos a partir do codcon a qual pertence,
    ...
    /**
     * Busca todos os reduzidos do plano de conta passado por parâmetro
    ...
    public function getDescrNaturezareceita($icodNatureza){
        $oDaoContaPlanoOrcamento = db_utils::getDao("conplanoorcamento");
        $sWhere  = "c60_codcon = {$icodNatureza}";
        $sWhere .= " limit 1";
        $sSqlBuscarDescr = $oDaoContaPlanoOrcamento->sql_query_file(null,null,"c60_descr",null,$sWhere);
        $rsBuscarDescr = $oDaoContaPlanoOrcamento->sql_record($sSqlBuscarDescr);
        if($oDaoContaPlanoOrcamento->numrows > 0){
            return db_utils::fieldsMemory($rsBuscarDescr, 0)->c60_descr;
        }
    }
}
