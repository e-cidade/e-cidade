...
/**
 * Busca as contas da anulacao/estorno de um lancamento contabil para slip
 * @author matheus felini
 * @package contabilidade
 * @version $Revision: 1.8 $
 */
class RegraAnulacaoSlip {
  ...
  public function getRegraLancamento($iCodigoDocumento, $iCodigoLancamento, ILancamentoAuxiliar $oLancamentoAuxiliar) {
    ...
    $aDocumentosEstorno = array(121, 131, 141, 152, 153, 162, 163, 165, 167);
    ...
    if ($oDaoTransacao->numrows > 0) {
      ...
      if ($oDadosTransacao->c46_ordem == 1) {
        ...
        if (method_exists($oLancamentoAuxiliar, "getCodigoSlip")) {
          ...
          if ($oLancamentoAuxiliar->isEstorno() || in_array($iCodigoDocumento, $aDocumentosEstorno)) {

            $oRegraLancamentoContabil->setContaCredito($oDadoSlip->k17_debito);
            $oRegraLancamentoContabil->setContaDebito($oDadoSlip->k17_credito);
          }
