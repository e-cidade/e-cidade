...
/**
 * Realiza as Escrituracoes de resto a pagar (RP) em liquidacao
 * @author Andrio Costa 
 * @package contabilidade
 * @subpackage lancamento
 */
class EscrituracaoRestosAPagarEmLiquidacao{
	...
	private function salvar($lProcessar = true) {
		...
		$oDaoCabecalhoInscricaoRP = db_utils::getDao('inscricaorestosapagaremliquidacao');
		...
		if ($oDaoCabecalhoInscricaoRP->numrows == 0 && $lProcessar) {
			...
			$sMsgErro  = "Não é possível estornar a inscrição de restos a pagar EM LIQUIDAÇÃO do ano {$this->iAno} e a ";
			$sMsgErro .= "instituicao {$sInstituicao}.";
		...
		if (!$lIncluir) {
			...
			if ( !$lProcessar && $oDadosCabecalho->c107_processado == 'f' ) {
				...
				$sMsgErro  = "Não é possível desprocessar novamente lançamentos da inscrição dos RPs EM LIQUIDAÇÃO do ano {$this->iAno} e a ";
				$sMsgErro .= " instituicao {$sInstituicao}.";
				throw new BusinessException($sMsgErro);
				
			} else if ($lProcessar && $oDadosCabecalho->c107_processado == 't') {
				
				$sMsgErro  = "Não é possível processar novamente lançamentos da inscrição dos RPs EM LIQUIDAÇÃO do ano {$this->iAno} e a ";
				$sMsgErro .= " instituicao {$sInstituicao}.";
				throw new BusinessException($sMsgErro);
			}
		}
		...
		if ($oDaoCabecalhoInscricaoRP->erro_status == '0') {
			...
			$sMsgErro  = "Erro Técnico: Não foi possível escriturar/cancelar a inscricao dos restos a pagar EM LIQUIDAÇÃO do "; 
	...
	public static function getValorLancamento($iAno, $iInstituicao, $sProcessados) {
		...
		$oDaoRP  = db_utils::getDao('conlancaminscrestosapagaremliquidacao');
		$sWhere  = " c107_instit          = {$iInstituicao}      ";
		$sWhere .= " and c107_ano        = {$iAno}               ";
		$sOrder  = " c210_sequencial desc limit 1                ";
