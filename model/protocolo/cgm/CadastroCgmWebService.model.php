...
class CadastroCgmWebService {
  ...
  protected function prencherDadosCgm(CgmBase $oCgm) {
    ...
    $oRetorno->cgccpf       = $oCgm->getCgccpf();
    $oRetorno->tipo_emissao = $this->getTipoEmissaoByCgm($oCgm->getCodigo());
 ...
 /**
  * Funcao que retorna o tipo de emissao cadastrado na aba Informacoes complementares do cadastro de Alvara, modulo ISSQN.
  */
 public function getTipoEmissaoByCgm($iCgm){
   $sSqlTipoEmissao = "select db140_sequencial
                     from issbasecaracteristica
                     inner join issbase on issbase.q02_inscr = q138_inscr
                     ...
                     where db139_sequencial = 2
                      and issbase.q02_numcgm = {$iCgm}";

  $oTipoEmissao = pg_fetch_object(pg_query($sSqlTipoEmissao));
  return $oTipoEmissao->db140_sequencial;
 }
