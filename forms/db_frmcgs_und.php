...
<form name="form1" method="post" action="" class="container">
    ...
    <fieldset class='separator'>
      ...
      <table class="form-container">
        ...
        <tr>
          ...
          <td>
            <input type="checkbox" name="z01_b_faleceu" id="z01_b_faleceu" value="t"<?php echo $GLOBALS['z01_b_faleceu'] == 't' ? ' checked="checked"' : ''?>> Faleceu
        ...
        <tr>
          <td></td>
          <td>
            <input type="checkbox" name="z01_b_descnomemae" id="z01_b_descnomemae" value="t"<?php echo $GLOBALS['z01_b_descnomemae'] == 't' ? ' checked="checked"' : ''?>> Desconhece o nome da mãe
          </td>


          <td><strong>Nacionalidade:</strong></td>
          <td>
            <input type="radio" name="z01_i_naturalidade" value="0"<?php echo empty($GLOBALS['z01_i_naturalidade']) ? ' checked="checked"' : ''?>> Brasileiro
            <input type="radio" name="z01_i_naturalidade" value="1"<?php echo $GLOBALS['z01_i_naturalidade'] == 1 ? ' checked="checked"' : ''?>> Naturalizado
            <input type="radio" name="z01_i_naturalidade" value="2"<?php echo $GLOBALS['z01_i_naturalidade'] == 2 ? ' checked="checked"' : ''?>> Estrangeiro
          </td>
        ...
        <tr>
          ...
          <td>
            <select name="z01_i_paisorigem">
              <option value="">Selecione</option>
              <?php
              $sSql = "SELECT * FROM plugins.paises ORDER BY pais";
              $result = db_query($sSql);
              while ($row = pg_fetch_assoc($result)):
              ?>
                <option value="<?=$row['cod']?>"><?=$row['pais']?></option>
              <?php endwhile; ?>
            </select>
  ...
  ?>
  <input name="exportar-csv" type="button" id="exportar-csv" value="Exportar para CSV" onclick="window.location.href='sau1_cgs_und002_csv?chavepesquisa=<?=$_GET['chavepesquisa']?>'" <?=$db_opcao==1?"disabled":""?>>
...
function abrirPDF() {
  window.open('sau1_cgs_und002_pdf?chavepesquisa=<?=$_GET['chavepesquisa']?>', '', 'width=' + (screen.availWidth - 5) + ',height=' + (screen.availHeight - 40) + ', scrollbars=1, location=0 ');
}
...
if( $('z01_v_telef').value != '' ) {
  $('z01_v_telef').value = tratamentoMascaraTelefone( $('z01_v_telef').value, true );
}

if( $('z01_v_telcel').value != '' ) {
  $('z01_v_telcel').value = tratamentoMascaraTelefone( $('z01_v_telcel').value, true );
}
</script>