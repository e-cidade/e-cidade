...
<fieldset style="width: 500px; height: 180px; margin-top: 20px;"><legend><b>Preço Médio</b></legend>
  <table cellspacing="5px">
  <tr>
  <td><strong>Código</strong></td>
  <td> 
  ...
  <tr>
  <td><strong>Data da Cotação:</strong></td>
  <td>
  <input type="text" name="dtCotacao" id="dtCotacao" value="<?=$dtCotacao ?>" onfocus="js_validaEntrada(this);" onkeyup="return js_mascaraData(this,event)" 
  autocomplete="off" onblur="js_validaDbData(this);" maxlength="10">
  <input id="dtCotacao_dia" type="hidden" maxlength="2" size="2" value="" title="">
  <input id="dtCotacao_mes" type="hidden" maxlength="2" size="2" value="" title="">
  <input id="dtCotacao_ano" type="hidden" maxlength="4" size="4" value="" title="">
  <script>
    var PosMouseY, PosMoudeX;
    function js_comparaDtCotacao(dia,mes,ano){
      var objData = document.getElementById('dtCotacao');
 ...
 */
 function js_pesquisa_liclicita(mostra){
	...
	function pesquisar() {
		...
		var oAjax = new Ajax.Request("con4_pesquisarxmlprecomedio.php",
				{
			method:"post",
	  ...
	  var jsonObj = eval("("+json.responseText+")"); 
		...
		try {
			...
			for (var i = 0; i < jsonObj.length; i++) {
				...
				tabela += "</td><td id=\"I00\" bgcolor=\""+color+"\" nowrap=\"\" style=\"text-decoration: none; color: rgb(0, 0, 0);\">";
				tabela += "<a onclick=\"pegar_valor('"+jsonObj[i].codigo+"','"+jsonObj[i].nroProcessoLicitatorio+"','"
				+jsonObj[i].dtCotacao+"')\">"+jsonObj[i].dtCotacao+"</a>";
