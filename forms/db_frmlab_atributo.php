...
?>
<fieldset tyle='width: 75%;'><legend><b>Estrutural</b></legend>
<form name="form1" method="post" action="">
<center>
<?db_input('la49_c_estrutural',10,$Ila25_i_codigo,true,'hidden',3,"");?>
<table border="0">
  <tr>
    <td nowrap title="<?=@$Tla25_i_codigo?>">
      <?=@$Lla25_i_codigo?>
    </td>
    <td>
<?
...
<?
  ...
  if($db_opcao==1){
    ...
    $sProximo="1";
    $sSql=$cllab_atributo->sql_query_file("","*",""," la25_i_nivel=1 ");
    $cllab_atributo->sql_record($sSql);
    if($cllab_atributo->numrows>0){
...
?>
  ...
  <tr>
    <td nowrap title="<?=@$Tla26_i_exameatributopai?>">
       <?
       if(($db_opcao==2)||($db_opcao==22)){$db_opcao2=3;}else{$db_opcao2=$db_opcao;}
