   ...
   <table border="0" width="100%" cellspacing="1">
    ...
    <tr>
     ...
     <tr>
          ...
          <td>
            <?php db_input('z01_munic', 30, $Iz01_munic, true, 'text', 3);?>
          </td>
     </tr>

     <tr>
          <td title="<?=@$Tsd02_i_cidade?>" class='bold'>
            IBGE:
          </td>
          <td >
            <?php db_input('sd02_i_cidade', 15, $Isd02_i_cidade, true, 'text', 3);?>
          </td>
    ...
    <tr>
     <td colspan="4" align="center" valign="top">
      <input id="db_opcao" type="hidden" name="<?=($db_opcao==1?"incluir":($db_opcao==2||$db_opcao==22?"alterar":"excluir"))?>">
      <input onclick="validarCampos()"  type="button" value="<?=($db_opcao==1?"Incluir":($db_opcao==2||$db_opcao==22?"Alterar":"Excluir"))?>" <?=($db_botao==false?"disabled":"")?> >
...
<script>
document.form1.sd02_v_cnes.style.backgroundColor = 'white';
...
function js_pesquisasd02_i_codigo(mostra){
 if(mostra==true){
  js_OpenJanelaIframe('','db_iframe_db_depart','func_db_depart.php?funcao_js=parent.js_mostradb_depart1|coddepto|descrdepto&lFiltrarUnidadeSaude=true','Pesquisa',true);
 }else{
  if(document.form1.sd02_i_codigo.value != ''){
   js_OpenJanelaIframe('','db_iframe_db_depart','func_db_depart.php?pesquisa_chave='+document.form1.sd02_i_codigo.value+'&funcao_js=parent.js_mostradb_depart&lFiltrarUnidadeSaude=true','Pesquisa',false);
  }else{
...
function js_buscaMunicipio() {
  ...
  var sUrl  = "func_cadendermunicipiosistema.php";
      sUrl += "?iTipoSistema=4";
      sUrl += "&funcao_js=parent.js_retornoMunicipio|db72_descricao|db125_codigosistema";
  js_OpenJanelaIframe('', 'db_iframe_cadendermunicipiosistema', sUrl, 'Pesquisa Município', true);
}

function js_retornoMunicipio() {
  document.form1.z01_munic.value     = arguments[0];
  document.form1.sd02_i_cidade.value = arguments[1];
