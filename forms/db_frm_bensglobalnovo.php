	        ...
	        <tr>
	          ...
	          <td>
	            <?
	              ...
	              db_input('t41_placa', 10, "", true, "text", 1, "onkeypress='return SomenteNumero(event);'");
	            ?>
	          </td>
	          <td colspan="2"></td>
	        </tr>
...
function salvarDados() {
   ...
   if (oObject.garantia == "") {
     ...
     alert("garantia é um campo obrigatório");
     return false;
...
var oDadosImovel                     = new DBToogle('dadosdoimovel', true);
var oDadosMaterial                   = new DBToogle('dadosdomaterial', true);
    ...
    for (var iField = 0; iField < aFields.length; iField++) {
      ...
      with (aFields[iField]) {
        if (oRetorno.dados[id]) {
          ...
          value = oRetorno.dados.dadosbem[id].urlDecode();
        }
...
function js_retornoDadosNota(oAjax) {
  ...
  var oRetorno = eval('('+oAjax.responseText+')');
  if (typeof oRetorno.dados.e62_vlrun != "undefined") {
    $('vlAquisicao').value      = oRetorno.dados.e62_vlrun;
  }
  if ( typeof oRetorno.dados.e72_qtd != "undefined") {
