...
<form name="form1" method="post">
  ...
  <fieldset style='text-align: left'>
    ...
    ?>
      ...
      <tr>
        ...
        <td>
          <?php
            db_input('coddeptoconsumo', 10, "", true, 'text', "", " onchange='js_pesquisa_coddeptoConsu(false);'", "", "", "");
      ...
      <td width="50%" valign="top">
        <fieldset style='text-align: left'>
          ...
          <table border='0' width='100%' cellspacing="0" cellpadding="0" style='border: 2px inset white;table-layout: fixed;' id='gridItens'>
            <tr>
              <th class='table_header' width='20'><input type='checkbox' checked style='display: none' id='mtodos' onclick='js_marca()'> <b><a onclick='js_marca()' style='cursor: pointer'>M</a></b></th>
          ...
          <table>
            ...
            <tr>
              ...
              <td>
                ...
                ?>
                <!--  <select id='matmater' style='width:200pt' disabled="disabled"
  ...
  <hr>
  <button type='button' style='border: 1px solid #999999' title='Salvar informações do item' id='salvar' onClick='js_saveMaterial(false)'><u>S</u>alvar</button>
  <? if (db_permissaomenu(db_getsession("DB_anousu"), 480, 3981) == "true") { ?>
    <button type='button' style='border: 1px solid #999999' title='Cadastradar Novo item de Entrada' id='novoItem' value='' onclick="js_novomatmater()">Novo</button>
  <? } ?>
  <button type='button' style='border: 1px solid #999999' title='Escolher Item de Entrada' id='escolher' onclick='js_escolherMater();'>Escolher</button>
  <button type='button' style='border: 1px solid #999999' title='Fracionar Entrada do item' id='doFracionar' onclick='js_saveMaterial(true)'><u>F</u>racionar</button>
  <button type='button' style='border: 1px solid #999999' title='Cancela o fracionamento Escolhido' id='excluirFracionar' onclick='js_cancelarFracionamento();' disabled value=''>Cancelar Fracionamento</button>
  <br>
  <button type='button' style='border: 1px solid #999999' title='' id='acertoQtdValor' onclick='js_acertaQtdValor(this.value);' value='acerta' disabled>Acerto Qtd/Valor</button>
  </fieldset>
  </td>
  </tr>
  <tr>
    ...
    <center><input name="Confirmar" id='confirmar' type="button" value="Confirmar" onclick='js_confirmaEntrada()'> <input name="pesquisar" id='pesquisar' type="button" value="Pesquisar" onclick="js_pesquisa_matordem()"></center>
...
<script type="text/javascript">
  function js_pesquisa_coddeptoConsu(mostra) {
    if (mostra == true) {
      js_OpenJanelaIframe('', 'db_iframe_depart', 'func_db_depart.php?funcao_js=parent.js_mostradb_departConsumo1|coddepto|descrdepto', 'Pesquisa', true);
    } else {
      if (document.form1.coddeptoconsumo.value != '') {
        js_OpenJanelaIframe('', 'db_iframe_depart', 'func_db_depart.php?pesquisa_chave=' + document.form1.coddeptoconsumo.value + '&funcao_js=parent.js_mostradb_departConsumo', 'Pesquisa', false);
      } else {
        document.form1.coddeptoconsumo.value   = '';
        document.form1.descrdeptoconsumo.value = '';
      }
    }
  }

  function js_mostradb_departConsumo(chave, erro) {
    document.form1.descrdeptoconsumo.value = chave;
    if (erro == true) {
      document.form1.coddeptoconsumo.focus();
      document.form1.coddeptoconsumo.value = '';
    }
  }

  function js_mostradb_departConsumo1(chave1, chave2) {
    document.form1.coddeptoconsumo.value = chave1;
    document.form1.descrdeptoconsumo.value = chave2;
  ...
  function js_retornoSend(oAjax) {
    ...
    if ($F('sJson') != '' && lSalvo == false) {
      ...
      if (lModificado) {
        if (!confirm('Existem Modificações no item Anterior que nao foram salvas.\nContinuar?')) {
    ...
    //$('coddeptoconsumo').value = oItemAtivo.coddeptoconsumo;
    ...
    if (oItemAtivo.aMateriaisEstoque.length > 0) {
      ...
      for (var iInd = 0; iInd < oItemAtivo.aMateriaisEstoque.length; iInd++) {
        ...
        oOptions.descr = oItemAtivo.aMateriaisEstoque[iInd].m60_descr; //Propriedade Original da Requisiï¿½aio com URLENCODE
  ...
  function js_saveMaterial(lFraciona) {
    ...
    if (lAcerta) {
      ...
      if (!confirm('Este é um lançamento de acerto, deseja confirmar a entrada?')) {
        return false;
      }
    ...
    if (new Number($F('consumoImediato')) > new Number($F('qtdeRecebido'))) {
      alert('Consumo imediato não pode ser maior que a quantidade recebida.');
      return false;
    }
    ...
    /*
     * caso o usuário estiver fracionando o item, não podemos deixar ele incluir todo o saldo do item,
    ...
    oItemAtivo.m76_nome = encodeURIComponent($F('m76_nome'));
    ...
    //oItemAtivo.coddeptoconsumo = $F('coddeptoconsumo');
    oItemAtivo.descrdeptoconsumo = encodeURIComponent($F('descrdeptoconsumo'));
  ...
  function js_confirmaEntrada() {
    ...
    if (confirm('Confirma a entrada dos Itens Selecionados no estoque?')) {
      ...
      /*   if(new Date('<?php echo date("Y-m-d", db_getsession("DB_datausu")); ?>') > new Date($F('m51_data'))){
            alert('A data da emissïssão é inferior a data do sistema!');
      ...
      if ($F('e69_dtnota') == '') {
        ...
        alert('data da nota deve ser preenchida!');
        $('e69_dtnota').focus();
        return false;
      ...
      for (var i = 0; i < aItens.length; i++) {
        ...
        if (iIndice[1] > 0) {
          ...
          } else if (new Number($('saldovalor' + aItens[i].value + "_0").innerHTML).toFixed(2) > 0 ||
            new Number($('saldoItem' + aItens[i].value + "_0").innerHTML).toFixed(2) > 0) {
      ...
      if (iSemPai) {
        var sMsg = "Existem Itens que foram fracionados que  nao estão selecionados.\n";
       ...
       * Caso o controle do pit estiver ativo, o tipo do documento fiscal é obrigatorio.
       */
      ...
      /**
       *
       * Removido a obrigatoriedade do campo m80_obs. Solictado por Deborah@contass.
       *
       */
      /*
      if ($F('m53_obs').trim() == "") {
      ...
      js_divCarregando("Aguarde, Efetuando Entrada dos Itens", "msgBox");
      js_bloqueiaLiberaBotao(true);
      ...
      oAjax = new Ajax.Request(
        url, {
          method: 'post',
          parameters: 'json=' + sJson,
          onComplete: js_retorndoConfirmaEntrada
        }
      );
   ...
   * no estoque
   */
  function js_retorndoConfirmaEntrada(oAjax) {
  ...
  // Função para liberar o campo e69_chaveacesso caso seja Nota Fiscal Eletrï¿½nica
  // Acrescentado por causa do sicom
  ...
  //  Função para verificar a chave de acesso Nota Fiscal Eletrï¿½nica
  // Acrescentado por causa do sicom
