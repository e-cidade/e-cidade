...
if($ed110_i_ptavalpedag==0 || $ed110_i_ptgeral==0){
 db_msgbox("Pontuação para Avaliação Pedagógica ou Pontuação Geral está com valor zero! (Configurações)");
 $db_opcao = 3;
 $db_opcao1 = 3;
 $db_botao = false;
}
...
<table border="0">
 <tr>
  <td nowrap title="<?=@$Ted117_i_progmatricula?>">
   <?db_ancora(@$Led117_i_progmatricula,"js_pesquisaed117_i_progmatricula(true);",$db_opcao);?>
  </td>
  <td>
 ...
 <tr>
  <td colspan="2">
   <table border="1" width="730" cellspacing="0" cellpading="2">
    ...
    $cor2 = "#DBDBDB";
    $cor = "";
    $result = $clquestaoaval->sql_record($clquestaoaval->sql_query("","*","ed108_i_sequencia"," ed108_c_tipoaval = 'P' AND ed108_c_ativo = 'S'"));
       ...
       <td style="font-size:10px">
        ...
        if($clopcaoquestao->numrows>0){
         ...
         for($y=0;$y<$clopcaoquestao->numrows;$y++){
          ...
          ?>
          <input type="radio" name="ed117_i_opcaoquestao<?=$x?>" value="<?=$ed106_i_codigo?>" <?=$checked?> onclick="js_pontuacao(<?=$clquestaoaval->numrows?>,<?=$clopcaoquestao->numrows?>)">
...
function js_pesquisa(){
 js_OpenJanelaIframe('CurrentWindow.corpo','db_iframe_progavalpedag','func_progavalpedag.php?funcao_js=parent.js_preenchepesquisa|ed117_i_codigo','Pesquisa de Avaliação Pedagógica',true);
}
