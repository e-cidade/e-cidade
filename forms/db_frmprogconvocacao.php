...
<table border="0">
 ...
 <?if(isset($ano_chave)){?>
  ...
  <td colspan="2">
   ...
   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
   <b>Convocações NÃO cadastradas no Ano <?=$ano_chave?>:</b>
   <br>
   <?
   ...
   <?
   $sql1 = "SELECT ed111_i_codigo,ed111_c_titulo,ed111_d_data
           FROM convocacao
           WHERE ed111_i_ano = '$ano_chave'
           AND ed111_d_data >= '$ed112_d_datainicio'
           EXCEPT
           SELECT ed111_i_codigo,ed111_c_titulo,ed111_d_data
           FROM progconvocacao
            inner join convocacao on ed111_i_codigo = ed115_i_convocacao
           WHERE ed111_i_ano = '$ano_chave'
           AND ed111_d_data >= '$ed112_d_datainicio'
   ...
   <select name="convocacoes" id="convocacoes" size="18"  multiple style="font-size:9px;width:350px;">
    <?
    if($linhas1>0){
     while($row=pg_fetch_array($sql_result1)){
      $cod_jatem=$row["ed111_i_codigo"];
      $desc_jatem=$row["ed111_c_titulo"];
      $data_jatem=db_formatar($row["ed111_d_data"],'d');
      ?>
     ...
     ?>
      <option value="">Todas convocações cadastradas no ano <?=$ano_chave?></option>
     <?
    }
    ?>
   </select>
...
<input name="<?=($db_opcao==1?"incluir":($db_opcao==2||$db_opcao==22?"incluir":"incluir"))?>" type="button" id="db_opcao" value="<?=($db_opcao==1?"Incluir":($db_opcao==2||$db_opcao==22?"Incluir":"Incluir"))?>" <?=($db_botao==false||$linhas1==0?"disabled":"")?> onclick="js_selecionar();">
</form>
<script>
...
function js_ano(valor,codmatricula){
 if(valor!=""){
  location.href = "edu1_progconvocacao001.php?codmatricula="+codmatricula+"&ano_chave="+valor;
 }
}
function js_selecionar(){
 ...
 if(convoca==""){
  ...
  location.href = "edu1_progconvocacao001.php?incluir&codmatricula="+document.form1.ed115_i_progmatricula.value+"&ano_chave="+document.form1.ano.value+"&convoca="+convoca;
 }
}
</script>
