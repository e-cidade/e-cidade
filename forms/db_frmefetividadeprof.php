...
<b>Informe os dados da efetividade - Mês: <?=db_mes($ed98_i_mes,1)?> Ano: <?=$ed98_i_ano?> Tipo: <?=$ed98_c_tipo=="P"?"PROFESSORES":"FUNCIONÁRIOS"?></b><br>
<table border="1" width="97%" cellspacing="0" cellpading="1">
 ...
 if($linhas>0){
  ...
  if ( !$rsLicenca ) {
   $sErroMensagem .= "Não foi possivel verificar as licenças do docente: {$z01_nome}.\n";
  }
  ...
  if ( $iTotalLicencas > 0 ) {
   ...
   <tr bgcolor="<?=$cor?>">
    ...
    <td align="center">
     <input type="text" size="3" name="ed97_i_faltanjust" id="ed97_i_faltanjust" value="<?=@$ed97_i_faltanjust?>" style="text-transform:uppercase;background:<?=@$ed97_i_codigo=='' && $lPossuiLicencaPeriodo === false  ?'#DEB887':'#E6E4F1'?>;text-align:center;" <?=@$ed97_i_codigo=="" && $lPossuiLicencaPeriodo === false  ?"disabled":""?> onKeyUp="js_ValidaCampos(this,1,'Faltas não Justificadas','f','f',event);">
    </td>
    <td align="center">
