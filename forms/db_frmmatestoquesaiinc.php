...
<div class="container">
  <form name="form1" method="post" action="">
              ...
              <table border="0">
                <tr>
                  <td nowrap title="<?= @$Tm70_codmatmater ?>" align="right">
                    ...
                    db_ancora(
                      @$Lm70_codmatmater,
                      "js_pesquisam70_codmatmater(true);",
                ...
                <tr>
                  ...
                  <td>
                    ...
                    <select name='tipo_saida' id='tipo_saida'>
                      <?php foreach ($aTiposSaida as $key => $value) : ?>
                ...
                <tbody id='dadosrequisicao' style='height:80;width:95%;overflow:scroll;overflow-x:hidden;background-color:white'>
                  ...
                  if (isset($aSaldoItem) && count($aSaldoItem) > 0) {
                    ...
                    foreach ($aSaldoItem as $oSaldoMaterial) {
                      ...
                      echo "   id='saida{$oSaldoMaterial->m70_codmatmater}' value=\"{$oSaldoMaterial->m70_quant}\" onblur='js_valDev(this.value, this.id)'>";
                      echo "  </td>";
                      echo "</tr>";
                    }
                  } else {
    ...
    <input name="<?= ($db_opcao == 1 ? "incluir" : ($db_opcao == 2 || $db_opcao == 22 ? "alterar" : "excluir")) ?>" type="button" id="db_opcao" value="<?= ($db_opcao == 1 ? "Incluir" : ($db_opcao == 2 || $db_opcao == 22 ? "Alterar" : "Excluir")) ?>" <?= ($db_botao == false ? "disabled" : "") ?> onclick='return js_saidaMaterial();'>
...
<script>
  ...
  function js_saidaMaterial() {
    ...
    for (var i = 0; i < aItens.length; i++) {
      ...
      aJsonItens.push({
        iCodMater: $('m70_codmatmater').value,
        nQuantidade: nValor,
        sObs: $F('m80_obs'),
        iCriterioCustoRateio: iCodigoCriterioCusto
      });
    ...
    if (confirm('Confirma saída do material?')) {
      ...
      oJSON = {
        ...
        tipoSaida: document.getElementById('tipo_saida').value
      };
      var url = 'mat4_requisicaoRPC.php';
      var oAjax = new Ajax.Request(
        url, {
          method: 'post',
          parameters: 'json=' + JSON.stringify(oJSON),
