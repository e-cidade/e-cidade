...
db_input('ve33_descr',40,$Ive33_descr,true,'text',3,'')
    ...
    <tr>
        <?
        $ve05_instit = db_getsession("DB_instit");
        db_input('ve05_instit',10,$Ive05_instit,true,'hidden',3,"");
        ?>
    </tr>
  </table>
  </center>
...
function js_pesquisave05_numcgm(mostra){
  if(mostra==true){
    js_OpenJanelaIframe('CurrentWindow.corpo.iframe_veicmotoristas','db_iframe_cgm','func_veicnomealt.php?pessoal=<?=$pessoal?>&funcao_js=parent.js_mostracgm1|z01_numcgm|z01_nome','Pesquisa',true,'0');
  }else{
     if(document.form1.ve05_numcgm.value != ''){
        js_OpenJanelaIframe('CurrentWindow.corpo.iframe_veicmotoriastas','db_iframe_cgm','func_veicnomealt.php?pessoal=<?=$pessoal?>&pesquisa_chave='+document.form1.ve05_numcgm.value+'&funcao_js=parent.js_mostracgm','Pesquisa',false);
     }else{
