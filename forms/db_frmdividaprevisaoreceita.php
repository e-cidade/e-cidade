<div class="registro_empinscritosemrp">
    <?php
    // Mantendo querys e padrão de funcionamento da tela de convenio
    $iAnousu =  db_getsession('DB_anousu');
    $clorcfontes = new cl_orcfontes();
    
    if ($iAnousu > 2022) {
        $sWhere = "o70_codigo in ('15740000', '16340000', '17540000') and o70_anousu = ".$iAnousu." and o70_instit = ".db_getsession("DB_instit")." and o70_valor > 0 group by 1, 2, 3, 4, 5";
    }
    ...
    <form name="form1" method="post" action="" style="" onsubmit="return validaForm(this);">
        <table>
            <tr>
                <td>
                    <center><strong>Previsão das Receitas de Operação de Crédito</strong></center>
                </td>
            </tr>
            <tr>
                <td></td>
            ...
            foreach ($aReceitas as $index => $oRec):
                ...
                <table class="DBGrid">
                    ...
                    <td class="linhagrid" style="width: 750px; text-align: left;">
                        ...
                        db_ancora("<b>{$sRec}</b>", "js_associacaoPrevisaoReceita({$oRec->o70_codrec}, '{$sRec}', {$oRec->o70_valor}, {$index}, {$oRec->o70_codigo}, {$iAnousu});", 1);
                        ?>
                    </td>
...
<script>
    ...
    function js_associacaoPrevisaoReceita(iCodRec, sReceita, fValorPrev, index, iFonte, iAnousu){

        if (iAnousu >= 2020) {
            js_OpenJanelaIframe('CurrentWindow.corpo','db_iframe_conconvprevrec','func_dividaprevisaoreceitaarrecad.php?c229_fonte='+iCodRec+'&sReceita='+sReceita+'&fValorPrevAno='+fValorPrev+'&index='+index+'&iFonte='+iFonte,'Associação de Operação de Crédito à Previsão da Receita',true);
        } else {
            js_OpenJanelaIframe('CurrentWindow.corpo','db_iframe_conconvprevrec','func_dividaprevisaoreceita.php?c229_fonte='+iCodRec+'&sReceita='+sReceita+'&fValorPrev='+fValorPrev+'&index='+index+'&iFonte='+iFonte,'Associação de Operação de Crédito à Previsão da Receita',true);
        }
