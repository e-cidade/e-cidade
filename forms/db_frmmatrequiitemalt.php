...
if (isset($m40_codigo)&&trim($m40_codigo)!=""){
	...
	$res_matrequi = $clmatrequi->sql_record($clmatrequi->sql_query_file($m40_codigo,"m40_auto,m40_finalizado"));
...
if (isset($m41_codmatmater)&&$m41_codmatmater!=""){
  ...
  /**
   * Ocorrencia 791
   * Verifica a quantidade vencida do item
   */
  $sqlQuantAnulado = "select (sum(m71_quant)-sum(m71_quantatend)) as vencidos
                        FROM matestoque
                        INNER JOIN matestoqueitem ON m71_codmatestoque = m70_codigo
                        INNER JOIN matestoqueitemlote ON m77_matestoqueitem = m71_codlanc
                      where m70_codmatmater = {$codmater} and m70_coddepto = ".db_getsession("DB_coddepto")." and m71_quantatend < m71_quant
                      and m77_dtvalidade <= '".date("Y-m-d")."'";

  $oQuantAnulado = pg_fetch_object(pg_query($sqlQuantAnulado));
  // Tratamento para o calculo
  if(isset($oQuantAnulado->vencidos)&&($oQuantAnulado->vencidos!="")){
    $quant_venc = $oQuantAnulado->vencidos;
  }else{
    $quant_venc = 0;
  }
...
<table>
<input style="margin-left: 3px; display: <?php echo ($m40_finalizado == 'f') ? 'inline-block' : 'none'; ?>" name="<?=($db_opcao==1?"incluir":($db_opcao==2||$db_opcao==22?"alterar":"excluir"))?>" type="submit" id="db_opcao" value="<?=($db_opcao==1?"Incluir":($db_opcao==2||$db_opcao==22?"Alterar":"Excluir"))?>" <?=($db_botao==false?"disabled":"")?> <?=(($db_opcao==1||$db_opcao==2||$db_opcao==22)&&$testquan=='f'?"onclick='return js_testaquant();'":"")?>  >
<? if ($db_opcao==1||$db_opcao==2) : ?>
  <input style="margin-left: 3px;" name='pesquisar' type='button' value='Emite Requisição' onclick='js_abre();'>
<?endif;?>
<? if ($db_opcao==1) : ?>
