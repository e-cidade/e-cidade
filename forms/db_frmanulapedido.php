...
<table width='100%'>
  <tr>
    <td>
      <br></br>
      <fieldset><legend><b>Anulação da Solicitação</b></legend>
        <table border="0">
          <tr>
  ...
  <tr>
    <td align="center">
      ...
      db_input( "m82_codigo", 100, 0, true, "hidden", 3 );
      ?>
    </td>
  </tr>
</table>
...
function js_janelaSolicitacao(oAjax) {
  ...
  aWidth[2]  = '30%';
  aWidth[11] = '3%';
  oDBGridSolicitacao.setCellWidth(aWidth);
  ...
  if (obj) {
    ...
    for (var iInd = 0; iInd < obj.itens.length; iInd++) {
      ...
      with (obj.itens[iInd]) {
        ...
        aLinha[9]  =" <input readonly "+lhabilitado+" name='atendido"+iInd+"' id='atendido"+iInd+"' value='"+(qtdpendente)+"'size=10 type='text' onblur='js_verificaQuantidade(this.value, \""+(qtdpendente)+"\",\"Quantidade maior que o saldo.\",\"Quantidade deve ser maior que zero.\")'>";
        aLinha[10] = eval("oMotivo"+iInd+".toInnerHtml();");
        aLinha[11] = m98_sequencial;
...
function js_atendeSolicitacao() {
  ...
  if (confirm('Confirma Anulação da Solicitação?')) {
    ...
    for (var i = 0; i < aItens.length; i++) {
      ...
      if (aItens[i]==true) {
        ...
        var sItemMotivo      = oDBGridSolicitacao.aRows[i].aCells[10].getValue();
        var iMatPedidoItem   = oDBGridSolicitacao.aRows[i].aCells[11].getValue();
        ...
        if (js_verificaQuantidade(nTotalDigitado, nSaldo, sMsg, sMsg1)) {
          ...
          sJsonItem += "'iCodMater':"+iCodMater+",'iCodEstoque':"+$F('m97_coddepto')+",'iCentroDeCusto':"+iCodigoCriterio+",'sItemMotivo':'"+sItemMotivo+"'}";
          sVirgula  = ",";
...
function js_mostraLotes(iItem) {
  ...
  iCodItem      = oDBGridSolicitacao.aRows[iItem].aCells[1].getValue();//código do material
  ...
  nValorSolItem = oDBGridSolicitacao.aRows[iItem].aCells[4].getValue();
