...
$resultVerificaCgcPref = pg_query($sqlVerificaCgcPref);
...
> 10% </option>
		...
		if (isset($passaproximo)&& $passaproximo=="sim"){
		  ...
		  $result3 = pg_exec("select * from issplan where q20_ano = $ano and q20_mes=$mes and q20_numcgm= $z01_numcgm order by q20_mes");
          if(pg_numrows($result3)!=0){
            db_fieldsmemory($result3,0);
            // tem planilha
						//die($q20_nomecontri);
    ...
    if (isset($planilha)){
      ...
      $result4 = pg_exec("select * from issplan where q20_planilha = $planilha");
     ...
     $cancelaexclusao = 0;
		 if($linhasnum == 1){
		  ...
		  if($cancelaexclusao != 1){
			  ...
			  echo"<script> document.form1.proximo.style.visibility='visible';
				...
				$resultealtera = pg_query($sqlaltera);
      ...
      db_inicio_transacao();
		  ...
		  if(isset($q21_retido)){
			...
			$result_valorissplanit = pg_query($sql_valorissplanit);
