...
include("classes/db_configdbpref_classe.php");
...
function db_getsession($str) {
  ...
  switch($str) {
    ...
    case "DB_id_usuario":
      return 1;
    case "DB_login":
      return @$HTTP_SESSION_VARS["DB_login"];
    case "DB_acesso":
      return @$HTTP_SESSION_VARS["DB_acesso"];
    case "hora":
      return @$HTTP_SESSION_VARS["hora"];
...
//$result= sql_record("select w01_corfundomenu from confsite");
//db_fieldsmemory($result,0);
...
$hora = db_getsession("hora");
if($usuario != ""){
  $result = pg_exec("select nome,d.id_usuario as id_usuario,u.cgmlogin as cgmlogin
                     from db_usuarios d
                       inner join db_usuacgm u on u.id_usuario = d.id_usuario
                     where u.cgmlogin = ".@$usuario);
  $nomeusuario = pg_result($result,0,'nome');
  $id_usuario  = pg_result($result,0,'id_usuario');
  $cgmlogin    = pg_result($result,0,'cgmlogin');
  $cgmlogin_teste    = pg_result($result,0,'cgmlogin');
...
function db_lovrot($query, $numlinhas, $arquivo = "", $filtro = "%", $aonde = "_self", $campos_layer = "", $NomeForm = "NoMe", $variaveis_repassa = array (), $automatico = true, $totalizacao = array()) {
  ...
  if ( ( empty ($$tot_registros) && !empty ($query) ) || isset($recomecar)) {
    ...
    if(count($totalizacao)>0 || isset($totalizacao_rep)){
      ...
      reset($totalizacao);
      $tot = pg_exec("select count(*),$total_campos
                        from ($query) as temp");
    }else{
	  ...
	  Retornado")."</form>
		...
		if(count($totalizacao) > 0 ){
		  ...
		  for ($j = 0; $j < $NumFields; $j ++) {
		    ...
		    if( $key_elemento == true && pg_fieldname($result, $j) == $key_elemento ){
