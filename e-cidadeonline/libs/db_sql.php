...
function debitos_tipos_matricula($matricula){
  ...
  $result = pg_query($sql) or die("Sql : ".pg_ErrorMessage());
  return (pg_numrows($result)==0?false:$result);
}
...
*/
  ...
  //die($sql);
  $result = @pg_query($sql) or die("Sql Error! Contate o Administrador.<br><br>".pg_errormessage());
  if($limite == 0 ) {
     if(pg_numrows($result) == 0 ) {
        return false;
     }
...
function db_mensagem($cabecalho,$rodape) {
  global $DB_mens1, $DB_align1, $DB_mens2, $DB_align2;
  $result = pg_query("select mens,alinhamento from db_confmensagem where cod = '$cabecalho' or cod = '$rodape' order by cod");
  if (pg_numrows($result) == 0 ){
...
function debitos_iptu($matricula,$limite,$anoexe){
  ...
  if ( $anoexe != 0 ) {
     ...
     $result_u = pg_query($sqlu) or die("Sql : ".pg_ErrorMessage($result_u));
