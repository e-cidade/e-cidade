...
$rsRetornoConfig  = pg_query($sqlRetornoConfig);
...
$rsVerificaEscrito = pg_query($sVerificaEscrito);
...
$sqlmenucert .= "	 and id_item_filho = 5473 $where                                ";
...
$resultmenuissqn = pg_query ( $sqlmenuissqn );
...
$resultmenuretido = pg_query ( $sqlmenuretido );
...
//	                &nbsp;<font class=\"links\">Opções Imóvel &gt;</font>\n";
	...
	if (! isset ( $matricula )) {
		...
		$result = pg_query ($sqlIptuBase) or die ( "Erro: " . pg_ErrorMessage ( $conn ) );
...
//		                &nbsp;<font class=\"links\">Opções Alvará &gt;</font>\n";
	...
	if (! isset ( $inscricao )) {
		...
		$result = pg_query ( $sql_exe ) or die ( "Erro: " . pg_ErrorMessage ( $conn ) );
	  ...
	  if (isset($inscricao) != "") {
		...
		$result = pg_query ($sql) or die ( "Erro: " . pg_ErrorMessage ( $conn ) );
	  }
		$cgccpf = trim ( pg_result ( $result, 0, 'z01_cgccpf' ) );
	}
...
//			}
		...
		<table width="100%" border="0" bordercolor="#cccccc" cellpadding="5"
			...
			for($x = 0; $x < $linhas; $x ++) {
				...
				$resultmostra = pg_query ( $sqlmostra );
