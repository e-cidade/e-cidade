...
try {
  ...
  switch ( $oParam->exec ) {
    ...
    case 'salvarConferencia':
       ...
       */
      $sConsideracao = "";
      if ( !empty($oParam->sConsideracao) ) {
        $sConsideracao = db_stdClass::normalizeStringJsonEscapeString( $oParam->sConsideracao );
      }
      ...
      foreach ($oParam->aExames as $oExame) {
        ...
        if ( !$oParam->lConferido ) {
          ...
          $oDaoConferencia->la47_t_observacao = $sConsideracao;
        }
        ...
        $oItemRequisicao->setObservacao( $sConsideracao );
        $oItemRequisicao->salvar();

        if ( !empty($sConsideracao) && $oParam->lConferido) {

          $oResultado = $oItemRequisicao->getResultado();
          $oResultado->setConsideracao( $sConsideracao );
          $oResultado->salvar();
        }
