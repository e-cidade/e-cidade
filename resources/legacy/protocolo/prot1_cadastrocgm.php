...
//    document.form1.z01_ender.value = '';
    ...
    js_OpenJanelaIframe('<?=(!isset($testanome)?"CurrentWindow.corpo":"")?>','db_iframe_ruas','func_ruas_alt.php?nomerua='+ document.form1.z01_ender.value+'&rural=1&funcao_js=parent.js_preenchepesquisa|j14_codigo|j14_nome|cep','Pesquisa',true);
    return false;
  }else{
...
function js_cep(abre){
  ...
  if(abre == true){
    js_OpenJanelaIframe('<?=(!isset($testanome)?"CurrentWindow.corpo":"")?>','db_iframe_cep','func_cep.php?funcao_js=parent.js_preenchecep|cep|cp06_logradouro|cp05_localidades|cp05_sigla|cp01_bairro','Pesquisa',true);
  }else{
    js_OpenJanelaIframe('<?=(!isset($testanome)?"CurrentWindow.corpo":"")?>','db_iframe_cep','func_cep.php?<?=($municipio == 't'?"municipio=sim&":"")?>pesquisa_chave='+document.form1.z01_cep.value+'&funcao_js=parent.js_preenchecep|cep|cp06_logradouro|cp05_localidades|cp05_sigla|cp01_bairro|codigo','Pesquisa',false);
  }
}
...
function js_logradcep(){
  js_OpenJanelaIframe('<?=(!isset($testanome)?"CurrentWindow.corpo":"")?>','db_iframe_ruas','func_ruas_alt.php?rural=1&funcao_js=parent.js_preenchepesquisa|j14_codigo|j14_nome|cep|','Pesquisa',true);
}

function js_ruas(){
  js_OpenJanelaIframe('<?=(!isset($testanome)?"CurrentWindow.corpo":"")?>','db_iframe_ruas','func_ruas.php?rural=1&funcao_js=parent.js_preenchepesquisaruas|j14_codigo|j14_nome','Pesquisa',true);
}
...
function js_cepcon(abre){
  //if(document.form1.z01_cep.value != "")
    //document.getElementById('teste').style.visibility = 'visible';
  if(abre == true){
    js_OpenJanelaIframe('<?=(!isset($testanome)?"CurrentWindow.corpo":"")?>','db_iframe_cep','func_cep.php?funcao_js=parent.js_preenchecepcon|cep|cp06_logradouro|cp05_localidades|cp05_sigla|cp01_bairro','Pesquisa',true);
  }else{
    js_OpenJanelaIframe('<?=(!isset($testanome)?"CurrentWindow.corpo":"")?>','db_iframe_cep','func_cep.php?pesquisa_chave='+document.form1.z01_cepcon.value+'&funcao_js=parent.js_preenchecepcon|cep|cp06_logradouro|cp05_localidades|cp05_sigla|cp01_bairro','Pesquisa',false);
  }
}
