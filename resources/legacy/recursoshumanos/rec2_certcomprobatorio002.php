     ...
     || db_selectmax("vinculos","select * from vinculos where h11_tipo = " . db_sqlformat($tipo_vantagem) . " and h11_regime = " . db_sqlformat(db_str($pessoal[$Ipessoal]["r01_regime"],1))." and h11_funcao = '99999'")){
      ...
      if( busca_lei($vinculos[0]["h11_lei1"],$tipo_vantagem)){
         ...
         if( !db_empty($anos)){
            ...
            global $assenta;
            if( db_selectmax("assenta","select * from assenta where h16_regist = " . db_sqlformat(db_str($oDadosPessoal->r01_regist,6))." order by h16_dtconc, h16_assent ")){
   ...
   for($i=1;$i<=18;$i++){
      ...
      if( $anos >= $anos_perc && !db_empty($anos_perc) && $anos_perc > $anos_ant){
	    ...
	    $mar2 = array();
            ...
            $mar2[1] = $oDadosPessoal->r01_regist;
            $mar2[2] = $oDadosPessoal->r01_lotac ;
            $mar2[3] = $data_admissao;
            $mar2[4] = $data_consessao;
            $mar2[5] = $anos_perc;
            $mar2[6] = $percentual;
            $mar2[7] = $tipo_vantagem;
            $mar2[8] = $dias_protelar;
            $mar2[9] = $inform;
            $mar2[10] = bb_space(6);
