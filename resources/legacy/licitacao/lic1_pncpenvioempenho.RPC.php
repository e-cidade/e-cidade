    ...
    case 'ExcluirEmpenho':
        ...
        try {
            foreach ($oParam->aEmpenhos as $aEmpenho) {
                ...
                $statusExclus�o = $clliclicitaPNCP->excluirContrato($sequencialpncp->e213_sequencialpncp, $sequencialpncp->e213_ano, $sequencialpncp->e213_numerocontrolepncp, $oParam->justificativa);

                if ($statusExclus�o->status == null)
                    $clempcontrolepncp->excluir($e123_sequencial = null, "e213_contrato = $aEmpenho->codigo");

                if ($statusExclus�o->status == 404) {
                    throw new Exception(utf8_decode($statusExclus�o->message));
                }
                if ($statusExclus�o->status == 422) {
                    throw new Exception(utf8_decode($statusExclus�o->message));
                }
                if ($statusExclus�o->status == 500) {
                    throw new Exception(utf8_decode($statusExclus�o->message));
                }
            }
        } catch (Exception $eErro) {
