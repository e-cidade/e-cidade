...
try {
  ...
  switch($oParam->sExecucao) {
    ...
    case 'getDadosPedido':
      ...
      $sCamposPedido  = " distinct tf01_i_cgsund, z01_v_nome as nome_paciente, tf25_i_destino, tf03_c_descr, tf10_i_prestadora, ";
    ...
    case 'getAcompanhantes':
      ...
      $sCamposAcompanhantes  = " tf13_i_cgsund, z01_v_nome,  ";
      $sCamposAcompanhantes .= " case when tf19_i_codigo is not null then true else false end as vinculado_carro, ";
      $sCamposAcompanhantes .= " tf19_i_fica ";
      $sSqlAcompanhantes     = $oDaoAcompanhantes->sql_query_acompanhantes_cgs( null, $sCamposAcompanhantes, null, $sWhereAcompanhantes);
      ...
      for ( $iContador = 0; $iContador < $iLinhas; $iContador++ ) {
        ...
        $oAcompanhante->iFica           = $oDadosAcompanhante->tf19_i_fica;
    ...
    case 'getVagasVeiculo':
      ...
      if ( empty($oParam->dtSaida) && empty($oParam->dtRetorno) && empty($oParam->horaSaida) &&
           empty($oParam->horaRetorno) && empty($oParam->iVeiculo) && empty($oParam->iDestino) ) {
      ...
      $aWhere[] = " tf18_i_destino     = {$oParam->iDestino} ";
    ...
    case 'salvar' :
      ...
      */
       ...
       */
      $iCodigoVeiculoDestino = buscaCodigoVeiculoDestino($oDtSaida, $oDtRetorno, $oParam->horaRetorno,
                                                         $oParam->horaSaida, $oParam->iCodigoVeiculo,
                                                         $oParam->iDestinoPrestadora, $oParam->iCodigoMotorista);
      /**
       * Inclui os passageiros
       */
      foreach ($oParam->aPassageiros as $oPassageiro) {
        ...
        $oDaoPassageiroVeiculo->tf19_i_pedidotfd      = $oParam->iPedidoTFD;
 ...
 * @param  int    $iAgendamentoPrestadora
 * @param  int    $iPrestadora
...
/**
 * Remove a agenda do paciente
 *
 * @throws Exception
 *
 * @param  integer $iPedido
 * @return boolean
 */
function removeAgendaSaida($iPedido = null) {
  ...
  $oDaoAgendaSaida    = new cl_tfd_agendasaida();
  $sWhereRemoveAgenda = "tf17_i_pedidotfd = {$iPedido} ";
  $oDaoAgendaSaida->excluir(null, $sWhereRemoveAgenda);
  $oErroMsg = new stdClass();
  if ( $oDaoAgendaSaida->erro_status == 0 ) {
