...
$sCampos .= " , (case";
$sCampos .= "      when z01_v_telcel <> '' then z01_v_telcel";
$sCampos .= "      when z01_v_telef <> '' then z01_v_telef";
$sCampos .= "      else ''";
$sCampos .= "  end) as telefone";
...
function cabecalhoViagem($oPdf) {
  ...
  $oPdf->cell( 105, 4, "PACIENTE",    1, 0, "C", 1);
  $oPdf->cell(  15, 4, "DT SAÍDA",    1, 0, "C", 1);
...
function imprimeModeloAjudaCusto( $aPedidos, $oPdf, $oGet ) {
  ...
  foreach ($aPedidos as $oPrestadora) {
    ...
    if ($lPrimeiro || $oPdf->getY() >= ($oPdf->h - 36)) {
...
function imprimeModeloViagem( $aPedidos, $oPdf ) {
  ...
  foreach ($aPedidos as $oPrestadora) {
    ...
    foreach ($oPrestadora->aPacientes as $oPaciente ) {
      ...
      $oPdf->cell(  15, 4, $oPaciente->cgs,                        1, 0, "C");
      $oPdf->cell( 105, 4, $oPaciente->paciente,                   1, 0, "L");
      $oPdf->cell(  15, 4, $oData->convertTo(DBDate::DATA_PTBR),   1, 0, "C");
      ...
      if ( count($oPaciente->aAcompanhante) > 0 ) {
        ...
        foreach ($oPaciente->aAcompanhante as $oAcompanhante ) {
          ...
          $oPdf->cell( 105, 4, $oAcompanhante->paciente, 1, 0, "L");
          $oPdf->cell(  15, 4, "",                       1, 0);
          $oPdf->cell(  15, 4, "",                       1, 0);
          $oPdf->cell(  60, 4, "",                       1, 0);
          $oPdf->cell(  20, 4, "",                       1, 0);
