  ...
  switch ($oParam->exec) {
    ...
    case "buscaMatriculas": 
      ...
      if ($oDadosFormulario->opcao_gml == 'l' && isset($oDadosFormulario->opcao_filtro)) {
        ...
        switch ($oDadosFormulario->opcao_filtro) {
          case 'i':
            $sWhereServidores .= " and rh02_lota between {$oDadosFormulario->r110_lotaci} and {$oDadosFormulario->r110_lotacf} ";
          break;
          case 's':

            $slotacoes         = implode(',', $oDadosFormulario->sellotac);
            $sWhereServidores .= " and rh02_lota in ({$slotacoes}) ";
          break;
        }
      }
      ...
      if (!$rsServidores) {
        throw new BusinessException(_M(MENSAGENS . 'nenhum_servidor' ));
      }
      ...
      for ($iServidores = 0; $iServidores < pg_num_rows($rsServidores); $iServidores++) {
        ...
        $iMatricula = db_utils::fieldsMemory($rsServidores, $iServidores)->rh01_regist; 
        $aMatriculas[$iMatricula] = $iMatricula;
      }

      rsort($aMatriculas);
     
      $oRetorno->aServidores = $aMatriculas;
    break;
