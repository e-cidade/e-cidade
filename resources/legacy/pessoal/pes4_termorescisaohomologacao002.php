...
$sCampos  = "DISTINCT rh16_pis,rh01_admiss,h13_tpcont,z01_nome,z01_mae,rh16_ctps_n,rh16_ctps_s,rh16_ctps_uf,rh01_sexo,rh01_instru,rh01_nasc,rh02_hrssem,";
$sCampos .= "rh37_cbo,rh116_codigo,rh116_cnpj,rh116_descricao,";
...
for($iCont = 0; $iCont < $oDaoRhpesrescisao->numrows; $iCont++) {
	...
	$oPdf->cell(192,5,"TERMO DE HOMOLOGAÇÃO DE RESCISÃO DO CONTRATO DE TRABALHO",0,1,"C",1);
	...
	//LINHA
    $oDados->label = "16 Código Sindical";
    $oDados->value = $oResult->rh116_codigo;
    $oDados->size = 40;
    $oDados->ln = false;
    addField($oPdf, $oDados);

    $oDados->label = "17 CNPJ e Nome da Entidade Sindical Laboral";
	...
	$text = "Foi prestada, gratuitamente, assistência na rescisão do contrato de trabalho, nos termos do artigo n.o 477, § 1o, da Consolidação das Leis do
	Trabalho (CLT), sendo comprovado neste ato o efetivo pagamento das verbas rescisórias especificadas no corpo do TRCT, no valor líquido de R$          
	, o qual, devidamente rubricado pelas partes, é parte integrante do presente Termo de Homologação.";
	addText($oPdf, $text);

    $oPdf->Ln(15);
	$text = "As partes assistidas no presente ato de rescisão contratual foram identificadas como legítimas conforme previsto na Instrução
	Normativa/SRT n.o 15/2010.";
	addText($oPdf, $text);
    
    $oPdf->Ln(7);
	$text = "Fica ressalvado o direito de o trabalhador pleitear judicialmente os direitos informados no campo 155, abaixo.";
	...
	$oPdf->cell(96,4,$underlinaText,0,0,"L",0);
	$oPdf->cell(96,4,$underlinaText,0,1,"L",0);
	$text = "      Carimbo e Assinatura do Assistente";
	$oPdf->cell(96,4,$text,0,0,"L",0);
	$text = "      Nome do Órgão Homologador";
	$oPdf->cell(96,4,$text,0,1,"L",0);

	$oPdf->Ln(4);
	$oPdf->cell(192,4,"    Ressalvas:","RTL",1,"L",0);
	$oPdf->cell(192,16,"","RBL",1,"L",0);
