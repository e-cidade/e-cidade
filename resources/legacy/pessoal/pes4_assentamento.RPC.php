...
try {
  ...
  case "lancarAssentamentosPonto":
    ...
    foreach ($aRegistros as $oRegistro) {
      ...
      if($oRegistro->sOpcao == 'incluir') {
        ...
        lancarAssentamento( $oTipoAssentamento, $oServidor, $oAssentamento, $oFolhaPagamento );
      }
    ...
    $oRetorno->message    = urlencode(_M(MENSAGENS ."sucesso_processar_lancamento_assentamento"));
    $oRetorno->erro       = false;
  ...
  case 'getAssentamento':
    ...
    $oRetorno->oAssentamento = $oAssentamento;

    break;

  case 'getNaturezaAssentamento':

    if ( isset($oParam->iTipoAssentamento) ) {
...
function getServidoresEAssentamentoPorTipoAssentamento($iTipo) {
  ...
  $aServidores         = ServidorRepository::getServidoresPorTipoAssentamento($iTipo, new DBDate("2015-06-01") );
  ...
  foreach ($aServidores as $oServidor) {
    ...
    $aAssentamentos                = AssentamentoRepository::getAssentamentosPorServidor($oServidor, $iTipo, new DBDate("2015-06-01"));
    ...
    foreach ($aAssentamentos as $oAssentamento ) {
      ...
      if(!$oLote || DBPessoal::getCompetenciaFolha()->comparar($oAssentamento->getLote()->getCompetencia(), DBCompetencia::COMPARACAO_IGUAL) ){
        ...
        $oDadosAssentamento->data_concessao  = $oAssentamento->getDataConcessao()->getDate();
...
function lancarAssentamento(TipoAssentamento $oTipoAssentamento, Servidor $oServidor, Assentamento $oAssentamento, FolhaPagamento $oFolhaPagamento) {
  ...
  $oLoteRegistro->setTipoLancamentoPonto(LoteRegistrosPonto::SOMAR_RUBRICA);
...
function cancelarLancamentoAssentamento(TipoAssentamento $oTipoAssentamento, Servidor $oServidor, Assentamento $oAssentamentoCancelamento) {
  ...
  $oCompetencia           = DBPessoal::getCompetenciaFolha();
  $oInstituicao           = InstituicaoRepository::getInstituicaoSessao();
  $aAssentamentosServidor = AssentamentoRepository::getAssentamentosPorServidor($oServidor, $oTipoAssentamento->getCodigo());
  $aLotesRetorno          = array();
