...
if(!isset($rh34_where) || (isset($rh34_where) && trim($rh34_where) == "")){
  ...
  if( $rh34_codban == "041" && $tipo == 's' ){
    $rh34_where .= " and ( trim(r38_conta) = '' or r38_conta is null or to_number(coalesce(r38_conta,'0'),'99999999999999') = 0  )";
  }elseif( $rh34_codban == "041" && $tipo == 'c' ){
    $rh34_where .= " and to_number(coalesce(r38_conta,'0'),'99999999999999') > 0  ";
  }
}
if($sqlerro == false){
  $sql = $clfolha->sql_query_gerarqbag(null,"folha.*,cgm.*,
                                             ...
                                             case  
                                                  ...
                                                  when r70_estrut between '0700' and '0704' then 'SEC. SADE'
                                                  when r70_estrut between '5000' and '5006' then 'FAPS'
                                                  else 'PREFEITURA' 
                                             end as secretaria, 
                                             length(trim(z01_cgccpf)) as tam,
                                             r38_liq as valorori",
                                            "secretaria,r38_banco,r38_nome",
                                            "$rh34_where");
  ...
  if($numrows > 0 && $rh34_codban == "041"){
    ...
    $head6 = "GERAO  :  ".db_formatar($datagera,"d").' AS '.date("H").':'.date("i").':'.date("s").' HS';
	...
	if($r38_liq>=5000){
      ...
      $contasapagarT = db_formatar(str_replace('.','',str_replace('-','',$r38_conta)),'s','0',10,'e',0);
      if($acodigodobanco == '041'){
    ...
    $pdf->cell(170,$alt,"Totalização (".$secretaria.")","TLB",0,"R",1);
    $pdf->cell(20,$alt,$secquanttotal,"TB",0,"R",1);
    $pdf->cell(20,$alt,db_formatar($secvalortotal,"f"),"TB",0,"R",1);
    $pdf->cell(70,$alt,"","TRB",1,"C",1);
    $pdf->ln(2);
...
//      db_setaPropriedadesLayoutTxt(&$db_layouttxt, 3, "B");
      ...
      if($i == 0 || $pdf->gety() > $pdf->h - 30 || $secretariaant != $secretaria){
        $pdf->setfont('arial','b',8);
        ...
        if($secretariaant != $secretaria){
          if($secretariaant != ""){
            $pdf->cell(140,$alt,"Totalização (".$secretariaant.")","TLB",0,"R",1);
            $pdf->cell(20,$alt,$secquanttotal,"TB",0,"R",1);
            $pdf->cell(20,$alt,db_formatar($secvalortotal,"f"),"TB",0,"R",1);
            $pdf->cell(50,$alt,"","TRB",1,"C",1);
            $pdf->ln(2);
          }
          $secquanttotal = 0;
          $secvalortotal = 0;
        ...
        $pdf->cell(230,$alt,$secretaria,1,1,"L",1);
        $secretariaant = $secretaria;
      }
    ...
    $pdf->setfont('arial','b',8);
    $pdf->cell(140,$alt,"Totalização (".$secretaria.")","TLB",0,"R",1);
    $pdf->cell(20,$alt,$secquanttotal,"TB",0,"R",1);
    $pdf->cell(20,$alt,db_formatar($secvalortotal,"f"),"TB",0,"R",1);
    $pdf->cell(50,$alt,"","TRB",1,"C",1);
    $pdf->ln(2);
