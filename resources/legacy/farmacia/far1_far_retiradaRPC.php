...
if ($objParam->exec == 'getAvisoRetirada') {
  ...
  if ($clfar_retirada->numrows > 0) {
    ...
    $oRetirada                 = db_utils::fieldsmemory($rsRetirada,0);
    $objRetorno->dData         = db_formatar($oRetirada->fa04_d_data,"d");
    $objRetorno->sDepartamento = $oRetirada->fa04_i_unidades." - ".$oRetirada->descrdepto;
    $objRetorno->sDepartamento = utf8_encode($objRetorno->sDepartamento);
    $sSql                      = $clfar_retiradaitens->sql_query("",
                                                                 "m60_descr",
                                                                 "",
                                                                 " fa06_i_retirada = $oRetirada->fa04_i_codigo ");
    $sMedicamentos             = "";
    $sSep                      = "";
...
if ($objParam->exec == 'ConsultaSaldo') {
  ...
  $sSql =  "SELECT 
       sum((m71_quant - m71_quantatend)) as m71_quant,
       $sSubPontoPedido
FROM matestoque
