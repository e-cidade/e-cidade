...
if (isset($processa) && $processa == 't') {
  ...
  if ($origemori == 1) {
    ...
    /**
     * FUNDAMENTADO PELA LEI MUNICIPAL N. 756/2009 QUE DISPOE SOBRE A LEI GERAL DE MICRO E PEQUENA EMPRESA.
     */
    if ($oInstit->getCodigoCliente() == Instituicao::COD_CLI_PMCAPITAOENEAS) {
      $sSql .= "
       AND NOT EXISTS
       (
        SELECT 1 FROM isscadsimples
          LEFT JOIN isscadsimplesbaixa ON q39_isscadsimples = q38_sequencial
        where (q38_dtinicial <= datacad) and (q39_dtbaixa >= datacad or q39_dtbaixa is null)
        and q38_inscr = q02_inscr
        and q38_categoria in (1, 3)
       )
      ";
    }
  ...
  for ($iAno = $iAnoInicial; $iAno <= $iAnoFinal; $iAno++) {
    ...
    for ($i = 0; $i < $numrows; $i++) {
      ...
      if ($numrowscalc == 0) {
        ...
        $erro_msg = "Não foi possivel calcular a vistoria número - $y70_codvist -  erro na função fc_vistorias ";
        $sqlerro = true;
        break;
      }
