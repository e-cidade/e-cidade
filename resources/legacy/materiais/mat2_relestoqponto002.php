...
$sql_estoqponto = "select m91_codigo,
                          ...
                          (coalesce(m70_quant,0) - coalesce(m64_pontopedido,0)) as diferenca_ponto,
                          m64_estoqueminimo,
                          (coalesce(m70_quant,0) - coalesce(m64_estoqueminimo,0)) as diferenca_estoquemin
...
for($i = 0; $i < $numrows; $i++){
     db_fieldsmemory($resultado, $i);
	   ...
	   if ($pdf->gety() > $pdf->h - 30 || $troca != 0 || $imp == 1) {
          ...
          $pdf->cell(115,$alt,"Descricao do Material","TBRL",0,"C",1);
          $pdf->cell(30,$alt,"Estoque Atual",         "TBRL",0,"C",1);
          $pdf->cell(30,$alt,"Ponto de Pedido",       "TBRL",0,"C",1);
          $pdf->cell(30,$alt,"Dif Ponto de Pedido",             "TBRL",0,"C",1);
          $pdf->cell(30,$alt,"Estoque Mí­nimo",       "TBRL",0,"C",1);
          $pdf->cell(30,$alt,"Dif Estoque Mí­nimo",             "TBRL",1,"C",1);
     ...
     if ($codigo_ant != $m91_codigo){
          if ($codigo_ant > 0){
               ...
               $pdf->setfont("arial", "b", 8);
               $pdf->ln();
               $pdf->cell(135, $alt, "SUBTOTAL:",                      "TB", 0, "R", $p);
               $pdf->cell(30, $alt, db_formatar($total_estoque,"f"),   "TB", 0, "R", $p);
               $pdf->cell(30, $alt, db_formatar($total_pedido,"f"),    "TB", 0, "R", $p);
               $pdf->cell(30, $alt, db_formatar($total_diferenca_pedido,"f"), "TB", 0, "R", $p);
               $pdf->cell(30, $alt, db_formatar($total_minimo,"f"),    "TB", 0, "R", $p);
               $pdf->cell(30, $alt, db_formatar($total_diferenca_minimo,"f"), "TB", 1, "R", $p);
               $pdf->setfont("arial", "", 8);
     ...
     $pdf->cell(115,$alt, $m60_descr,                       0,0,"L",$p);
     $pdf->cell(30,$alt, db_formatar($m70_quant,"f"),       0,0,"R",$p);
     $pdf->cell(30,$alt, db_formatar($m64_pontopedido,"f"), 0,0,"R",$p);
     $pdf->cell(30,$alt, db_formatar($diferenca_ponto,"f"),       0,0,"R",$p);
     $pdf->cell(30,$alt, db_formatar($m64_estoqueminimo,"f"), 0,0,"R",$p);
     $pdf->cell(30,$alt, db_formatar($diferenca_estoquemin,"f"),       0,1,"R",$p);

     $total_estoque   += $m70_quant;
     $total_pedido    += $m64_pontopedido;
     $total_diferenca_pedido += $diferenca_ponto;
     $total_minimo    += $m64_estoqueminimo;
     $total_diferenca_minimo += $diferenca_estoquemin;
...
$pdf->setfont("arial", "b", 8);
$pdf->cell(135, ($alt+1), "TOTAL GERAL:",                         "TB", 0, "R", 0);
$pdf->cell(30, ($alt+1), db_formatar($total_geral_estoque,"f"),   "TB", 0, "R", 0);
$pdf->cell(30, ($alt+1), db_formatar($total_geral_pedido,"f"),    "TB", 0, "R", 0);
$pdf->cell(30, ($alt+1), db_formatar($total_geral_diferenca_pedido,"f"), "TB", 0, "R", 0);
$pdf->cell(30, ($alt+1), db_formatar($total_geral_minimo,"f"),    "TB", 0, "R", 0);
$pdf->cell(30, ($alt+1), db_formatar($total_geral_diferenca_minimo,"f"), "TB", 1, "R", 0);

$pdf->ln();
$pdf->cell(195, $alt, "TOTAL DE REGISTROS: ".$total_registros, 0, 1, "L", 0);
