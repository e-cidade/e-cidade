...
function contabilizaMateriais($rsMateriais, $dtInicial) {
  ...
  for ($iMaterial = 0; $iMaterial < $iNumeroMateriais; $iMaterial++) {
    ...
    /* Verifica se a quantidade_estoque for zero, o saldo final sera zerado
       Melhoria solicitada pelo Henrique Souza - Jeferson Santos
     */
    if ($oMaterial->quantidade_estoque == 0){
       $oItem->nSaldoFinal = 0;
    }else{
       $oItem->nSaldoFinal = 0 + ($oMaterial->saldo_inicial + $oMaterial->entrada - $oMaterial->saida);
    }

    if ($oMaterial->quantidade_entrada == 0){
       $oItem->nTotalEntradas           = 0;
    }else{
       $oItem->nTotalEntradas           = $oMaterial->entrada + 0;
    }

    if ($oMaterial->quantidade_saida == 0){
       $oItem->nTotalSaidas             = 0;
    }else{
...
function agruparPorConta(&$aItem, $lPatrimonial = false, $iAno, $sWhere) {
  ...
  foreach ($aItem as $oItem) {
    ...
    if ($aContas[$sEstrutural]->iQuantidadeInicial == 0){
       $aContas[$sEstrutural]->nSaldoAnterior    += 0;
    }else{
    ...
    if ($oAgrupamento->iQuantidadeInicial == 0){
       $oAgrupamento->nSaldoAnterior    += 0;
    }else{
...
function agruparPorGrupoSubGrupo(&$aMateriais) {
  ...
  foreach ($aMateriais as $oMaterial) {
    ...
    if ($oMaterial->iQuantidadeSaida == 0){
       $oGrupos->aAgrupamentos[$iCodigoGrupo]->nTotalSaidas      += 0;
    }else{
       $oGrupos->aAgrupamentos[$iCodigoGrupo]->nTotalSaidas      += $oMaterial->nTotalSaidas;
    }

    if ($oMaterial->iQuantidadeEntrada == 0){
       $oGrupos->aAgrupamentos[$iCodigoGrupo]->nTotalEntradas    += 0;
    }else{
       $oGrupos->aAgrupamentos[$iCodigoGrupo]->nTotalEntradas    += $oMaterial->nTotalEntradas;
    }


    if ($oMaterial->iQuantidadeEmEstoque == 0){
       $oGrupos->aAgrupamentos[$iCodigoGrupo]->nSaldoFinal       += 0;
    }else{
    ...
    if ($oMaterial->iQuantidadeInicial == 0){
       $oGrupos->aAgrupamentos[$iCodigoGrupo]->nSaldoAnterior    += 0;
    }else{
...
function montarArvoreGrupos($oArvore, $aGruposNivel, $iNivel) {
  ...
  while($iNivel != 0) {
    ...
    foreach ($aGruposNivel as $iGrupoFilho) {
      ...
      if ($oArvore->aAgrupamentos[$iGrupoFilho]->iQuantidadeSaida == 0){
         $oArvore->aAgrupamentos[$iCodigoPai]->nTotalSaidas      += 0;
      }else{
         $oArvore->aAgrupamentos[$iCodigoPai]->nTotalSaidas      +=  $oArvore->aAgrupamentos[$iGrupoFilho]->nTotalSaidas;
      }

      if ($oArvore->aAgrupamentos[$iGrupoFilho]->iQuantidadeEntrada == 0){
         $oArvore->aAgrupamentos[$iCodigoPai]->nTotalSaidas      += 0;
      }else{
      ...
      if ($oArvore->aAgrupamentos[$iGrupoFilho]->iQuantidadeEmEstoque == 0){
         $oArvore->aAgrupamentos[$iCodigoPai]->nSaldoFinal       += 0;
      }else{
      ...
      if ($oArvore->aAgrupamentos[$iGrupoFilho]->iQuantidadeInicial == 0){
         $oArvore->aAgrupamentos[$iCodigoPai]->nSaldoAnterior    += 0;
      }else{
...
function imprimirGrupo($oAgrupamento, $oGrupo, $oPdf, $iAlturaLinha, $lAnalitica,  $lImprimeCabecalho = false) {
  ...
  if ($lImprimeCabecalho) {
    ...
    $oPdf->cell(25, $iAlturaLinha, "Saldo Anterior ", "LTBR",0,"C", 1);
    $oPdf->cell(25, $iAlturaLinha, "Qtd. Inicial", "LTBR",0,"C", 1);
    $oPdf->cell(25, $iAlturaLinha, "Entradas", "LTBR",0,"C", 1);
    $oPdf->cell(25, $iAlturaLinha, "Qtd. Entrada", "LTBR",0,"C", 1);
    $oPdf->cell(25, $iAlturaLinha, "Saídas", "LTBR",0,"C", 1);
    $oPdf->cell(25, $iAlturaLinha, "Qtd. Saída", "LTBR",0,"C", 1);
    $oPdf->cell(25, $iAlturaLinha, "Saldo Final","LTBR",0,"C", 1);
  ...
  $oPdf->cell(25, $iAlturaLinha, db_formatar(abs($oGrupo->nSaldoAnterior),"f"), "LTBR",0,"R", 1);
  $oPdf->cell(25, $iAlturaLinha, db_formatar($oGrupo->iQuantidadeInicial,"f"), "LTBR",0,"R", 1);
  $oPdf->cell(25, $iAlturaLinha, db_formatar(abs($oGrupo->nTotalEntradas),"f"), "LTBR",0,"R", 1);
  $oPdf->cell(25, $iAlturaLinha, db_formatar($oGrupo->iQuantidadeEntrada,"f"), "LTBR",0,"R", 1);
  $oPdf->cell(25, $iAlturaLinha, db_formatar(abs($oGrupo->nTotalSaidas),"f"), "LTBR",0,"R", 1);
  $oPdf->cell(25, $iAlturaLinha, db_formatar($oGrupo->iQuantidadeSaida,"f"), "LTBR",0,"R", 1);
  $oPdf->cell(25, $iAlturaLinha, db_formatar(abs($oGrupo->nSaldoFinal),"f"), "LTBR",0,"R", 1);
  $oPdf->cell(25, $iAlturaLinha, db_formatar($oGrupo->iQuantidadeEmEstoque,"f"), "LTB",1,"R", 1);
...
function imprimirRodapeConta($oPdf, $iAlturaLinha, $oConta, $lAnalitica) {
  ...
  if (!$lAnalitica) {
    ...
    $oPdf->cell(25,$iAlturaLinha,"Saldo Inicial", "TBR",0,"C", 0);
    $oPdf->cell(25,$iAlturaLinha,"Qtd. Inicial", "TBR",0,"C", 0);
    $oPdf->cell(25,$iAlturaLinha,"Entradas", "TBR",0,"C", 0);
    $oPdf->cell(25,$iAlturaLinha,"Qtd. Entradas", "TBR",0,"C", 0);
    $oPdf->cell(25,$iAlturaLinha,"Saídas", "TBR",0,"C", 0);
    $oPdf->cell(25,$iAlturaLinha,"Qtd. Saídas", "TBR",0,"C", 0);
    $oPdf->cell(25,$iAlturaLinha,"Saldo final", "TBR",0,"C", 0);
    $oPdf->cell(25,$iAlturaLinha,"Qtd. Estoque", "TB",1,"C", 0);
  }

  $oPdf->setfont('arial','b',7);
  $oPdf->cell(80,$iAlturaLinha,"TOTAL", "TB",0,"R",1);
  $oPdf->cell(25,$iAlturaLinha, db_formatar(abs($oConta->nSaldoAnterior),"f"), "LTBR",0,"R", 1);
  $oPdf->cell(25,$iAlturaLinha, db_formatar($oConta->iQuantidadeInicial,"f"), "LTBR",0,"R", 1);
  $oPdf->cell(25,$iAlturaLinha, db_formatar(abs($oConta->nTotalEntradas),"f"), "LTBR",0,"R", 1);
  $oPdf->cell(25,$iAlturaLinha, db_formatar($oConta->iQuantidadeEntrada,"f"), "LTBR",0,"R", 1);
  $oPdf->cell(25,$iAlturaLinha, db_formatar(abs($oConta->nTotalSaidas),"f"), "LTBR",0,"R", 1);
  $oPdf->cell(25,$iAlturaLinha, db_formatar($oConta->iQuantidadeSaida,"f"), "LTBR",0,"R", 1);
  $oPdf->cell(25,$iAlturaLinha, db_formatar(abs($oConta->nSaldoFinal),"f"), "LTBR",0,"R", 1);
  $oPdf->cell(25,$iAlturaLinha, db_formatar($oConta->iQuantidadeEmEstoque,"f"), "LTB",1,"R", 1);
}
...
function imprimirItens($oPdf, $iAlturaLinha, $aItens) {
  ...
  foreach($aItens as $oItem) {
    ...
    if ($oItem->nTotalEntradas <= 0 && $oItem->nTotalSaidas <= 0) {
		  continue;
	  }
    ...
    if ($oTotal->iQuantidadeInicial == 0){
       $oTotal->nSaldoAnterior    += 0;
    }else{
...
function imprimirItem ($oPdf, $iAlturaLinha,$oItem) {
  ...
  if (db_formatar(abs($oItem->nTotalEntradas),"f") <= 0 && db_formatar(abs($oItem->nTotalSaidas),"f") <= 0) {
		return;
	}
  ...
  if (strlen($oItem->sDescricao) > 30 ) {               
	  $aDescricao = quebrar_texto($oItem->sDescricao,30);  
	  $alt_novo = count($aDescricao);                   
  } else {
	  $alt_novo = 1;
  }
  
  $oPdf->cell(20,$iAlturaLinha*$alt_novo,$oItem->iCodigo, "TBR",0,"C",0);
  ...
  $ctr = count($aDescricao);
	if (strlen($oItem->sDescricao) > 30) {
	  ...
	  foreach ($aDescricao as $descricao_nova) {
	    ...
	    $oPdf->cell(60,$iAlturaLinha,substr($descricao_nova, 0, 30),$borda,1,"L",0);
	    $oPdf->x=$pos_x;   
	  ...
	  $oPdf->cell(60,($iAlturaLinha*$alt_novo),substr($oItem->sDescricao, 0, 30),"TBR",0,"L",0);
	}
  $oPdf->cell(25,$iAlturaLinha*$alt_novo, db_formatar(abs($oItem->nSaldoAnterior),"f"), "TBR",0,"R", 0);
  $oPdf->cell(25,$iAlturaLinha*$alt_novo, db_formatar(abs($oItem->iQuantidadeInicial),"f"), "TBR",0,"R", 0);
  $oPdf->cell(25,$iAlturaLinha*$alt_novo, db_formatar(abs($oItem->nTotalEntradas),"f"), "TBR",0,"R", 0);
  $oPdf->cell(25,$iAlturaLinha*$alt_novo, db_formatar($oItem->iQuantidadeEntrada, "f") , "TBR",0,"R", 0);
  $oPdf->cell(25,$iAlturaLinha*$alt_novo, db_formatar(abs($oItem->nTotalSaidas),"f"), "TBR",0,"R", 0);
  $oPdf->cell(25,$iAlturaLinha*$alt_novo, db_formatar($oItem->iQuantidadeSaida, "f") , "TBR",0,"R", 0);
  $oPdf->cell(25,$iAlturaLinha*$alt_novo, db_formatar(abs($oItem->nSaldoFinal),"f"), "TBR",0,"R", 0);
  $oPdf->cell(25,$iAlturaLinha*$alt_novo, db_formatar($oItem->iQuantidadeEmEstoque, "f"), "TB",1,"R", 0);
}
...
function imprimirTotal($oPdf, $iAlturaLinha, $oTotal) {
  ...
  if (db_formatar(abs($oTotal->nTotalEntradas),"f") <= 0 && db_formatar(abs($oTotal->nTotalSaidas),"f") <= 0) {
		return;
	}
  ...
  $oPdf->cell(25,$iAlturaLinha, db_formatar(abs($oTotal->nSaldoAnterior),"f"),   "LTBR",0,"R", 1);
  $oPdf->cell(25,$iAlturaLinha, db_formatar($oTotal->iQuantidadeInicial, "f"),   "LTBR",0,"R", 1);
  $oPdf->cell(25,$iAlturaLinha, db_formatar(abs($oTotal->nTotalEntradas),"f"),   "LTBR",0,"R", 1);
  $oPdf->cell(25,$iAlturaLinha, db_formatar($oTotal->iQuantidadeEntrada, "f"),   "LTBR",0,"R", 1);
  $oPdf->cell(25,$iAlturaLinha, db_formatar(abs($oTotal->nTotalSaidas),"f"),     "LTBR",0,"R", 1);
  $oPdf->cell(25,$iAlturaLinha, db_formatar($oTotal->iQuantidadeSaida, "f"),     "LTBR",0,"R", 1);
  $oPdf->cell(25,$iAlturaLinha, db_formatar(abs($oTotal->nSaldoFinal),"f"),      "LTBR",0,"R", 1);
  $oPdf->cell(25,$iAlturaLinha, db_formatar($oTotal->iQuantidadeEmEstoque, "f"), "LTBR",1,"R", 1);
}
