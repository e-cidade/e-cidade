...
class db_calendario {
  ...
  function monta_calendario ($exercicio=null, $metodo=null){
    ...
    echo "<script>\n";
    echo "function js_mostra_alvo(codmod,codproced){
            js_OpenJanelaIframe('','db_iframe_calend','".$this->pagina_alvo."?codmod'+codmod+'&codproced='+codproced);
          }\n";
    ...
    if($this->clcalend->numrows>0){
      ...
      for($i=0; $i < count($matriz_imp);$i++){
        ...
        for($c=$this->diaatual;$c<$this->diaatual+7;$c++){
          ...
          if( isset($matriz_imp[key($matriz_imp)]['m'][substr($datat,8,2)]) ){
            echo "<td height=20 align='left' bgcolor='$color' width='10%' onclick='js_mostra_alvo(\"".$datat."\",\"".$identifica[0]."\")' >";
            echo $matriz_imp[key($matriz_imp)]['m'][substr($datat,8,2)];
          }else{
            echo "<td height=20 align='left' bgcolor='$color' width='10%'  >";
            echo "&nbsp";
          }
          echo "</td>";
  ...
  function monta_calendario_anual_semana ($exercicio=null, $metodo=null,$data=null){
    ...
    for($mes=$this->mesatual;$mes< 13;$mes ++){
      ...
      for($dia=1;$dia < (date('t',mktime(0,0,0,$mes,1,$this->anousu))+1);$dia++){
        ...
        if($qsem != $sem || $dia == 1){
          $sem = $qsem;
          if(!isset($qsemana[db_formatar($sem,'s','0',2)])){
            $qsemana[db_formatar($sem,'s','0',2)] = $sem;
            $qt += 1;
          }
        }
      }
    ...
    if($this->clcalend->numrows>0){
      ...
      $semana_compara = "";
      for($i=0;$i<$this->clcalend->numrows;$i++){
        global $codmod,$at60_codproced,$at62_cor,$at60_descricao, $at64_situacao;
        db_fieldsmemory($result,$i);

//        if( $semana_compara != "" && $semana_compara > $dl_datacalend){
//             break;
//        }        
//        $semana_compara = $dl_datacalend ;
      ...
      for($i=0; $i < count($matriz_imp);$i++){
        ...
        echo "<tr>\n";
        echo "<td align='left' title='".$matriz_projeto[key($matriz_imp)]."' bgcolor='lightgreen' nowrap>";
        echo key($matriz_imp);
        echo "</td>";
        ...
        for($c=1;$c<53;$c++){
         	...
         	$color = "";
          if ( isset($qsemana[db_formatar($c,'s','0',2)]) ) {
            if( isset($matriz_imp[key($matriz_imp)][db_formatar($c,'s','0',2)]) ){
             	$color = $matriz_cor[key($matriz_imp)][db_formatar($c,'s','0',2)];
            }else{
            	$color = "";
            }
          
            if( isset($matriz_imp[key($matriz_imp)][db_formatar($c,'s','0',2)]) ){
              echo "<td nowrap  height=10 align='left' bgcolor='$color' onclick='js_mostra_alvo(".$matriz_imp[key($matriz_imp)][db_formatar($c,'s','0',2)].")' title=\"".$matriz_imp[key($matriz_imp)][db_formatar($c,'s','0',2)]."\" >";
              if( date("Y") == $this->anousu && $c<date('W')){
                echo "*";
              }else{
                echo "";
              }
            ...
            if( isset($matriz_imp[key($matriz_imp)][db_formatar($c,'s','0',2)]) ){
              echo $matriz_situacao[key($matriz_imp)][db_formatar($c,'s','0',2)];
            }
            echo "</td>";
          }else{
