...
function buscaQuantidadeAnulada($iCodigoItemSolicitacao)
{
    $oDaoSolicitemanul        = db_utils::getDao("solicitemanul");
    $sCampos                  = "coalesce(sum(pc28_qtd),0) as total";
    $sWhere                   = "pc55_solicitempai = {$iCodigoItemSolicitacao}";
    $sSqlQuantidadesAnuladas  = $oDaoSolicitemanul->sql_query_vinculo(null, $sCampos, null, $sWhere);
    $rsQuantidadesAnuladas    = $oDaoSolicitemanul->sql_record($sSqlQuantidadesAnuladas);
    $iTotalAnulado            = 0;

    if ($oDaoSolicitemanul->numrows == 1) {
        $iTotalAnulado = db_utils::fieldsMemory($rsQuantidadesAnuladas, 0)->total;
    }
    return $iTotalAnulado;
}
...
for ($iInd = 0; $iInd  < $iRsSql; $iInd++) {
    ...
    $oSolicita->soma = $oSolicita->solicitada - $oSolicita->anulada;
    ...
    $oStdItemEstimativa->iEmpenhada   = $oSolicita->empenhada - $oSolicita->empAnulada;
    $oStdItemEstimativa->iLiquidada   = buscaQuantidadeLiquidada($oSolicita->pc11_codigo);
    $oStdItemEstimativa->iSolicitada  = ($oSolicita->solicitada - $oSolicita->anulada);
    $oStdItemEstimativa->iReservada   = buscaQuantidadeReservada($oSolicita->pc11_codigo);
    $oStdItemEstimativa->iAnulada     = buscaQuantidadeAnulada($oSolicita->pc11_codigo);
    $oStdItemEstimativa->iSolicitar   = ($oSolicita->pc11_quant - $oSolicita->solicitada + $oSolicita->anulada);
    ...
    $oStdItemEstimativa->iSolicitada  = ($lControlaValor ? $oItem->getValorSolicitado() : $oItem->getQuantidadesSolicitadas()) - $oSolicita->anulada;
    $oStdItemEstimativa->iReservada   = buscaQuantidadeReservada($oItem->getCodigoItemSolicitacao());
    $oStdItemEstimativa->iEmpenhada   = ($lControlaValor ? $oItem->getValoresEmpenhados() : $oItem->getQuantidadesEmpenhadas()) - $oSolicita->empAnulada;
    $oStdItemEstimativa->iLiquidada   = buscaQuantidadeLiquidada($oItem->getCodigoItemSolicitacao());
    $oStdItemEstimativa->iSolicitar   = (($oSolicita->pc11_quant + $oSolicita->qteexcedente) - $oStdItemEstimativa->iSolicitada);
    ...
    if ($lControlaValor) {
        ...
        $oStdItemEstimativa->iSolicitar   = $oStdItemEstimativa->nVlrUnitario - $oStdItemEstimativa->iSolicitada + $oSolicita->anulad;
    }
    ...
    /*
   *
   * Realiza o agrupamento pelos fornecedores usando o codigocgm como chave. Caso não exista fornecedor vencedor na
   * licitação, os itens serão agrupados na chave 'sem-fornecedor'
   *
   * */
