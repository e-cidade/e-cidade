...
/**
 * Verifica os parametros de configuração padrão do modulo
 */
...
switch ($oParam->sExecucao) {
        ...
        $oRetorno->lFormaNotifCarta = false;
          ...
          $oFornecedor                       = new fornecedor($oParam->iNumCgm);
	        ...
	        $iCodigoNotificaBloqueioFornecedor = $oFornecedor->notificar($oParam->lGerarNotificacaoDebito,
	                                                                     ...
	                                                                     $aDebitos);
	        /**
	         * Envia um e-mail de notificação para o fornecedor
	         */
	        if ($oParam->lFormaNotifEmail) {
	          ...
	          /**
	           * Verifica as notificações de debito ja processadas para o fornecedor
	           */
            ...
            $sSqlNotificaBloqueioFornecedor  = $oDaoNotificaBloqueioFornecedor->sql_debitos_notificados(null, $sCampos, null, $sWhere);
	          ...
	          if ($oDaoNotificaBloqueioFornecedor->numrows > 0) {
	            ...
	            if (empty($oNotificaBloqueioFornecedor->emailremetente)) {
	            	...
	            	$sMensagem  = "E-mail do remetente não encontrado!";
	            	$sMensagem .= " \nVerifique parametros de configuração.";
	            	throw new Exception($sMensagem);
	            }

	            if (empty($oNotificaBloqueioFornecedor->emaildestinatario)) {

	            	$sMensagem  = "E-mail do destinatário não encontrado!";
	            ...
	            $sAssunto            = "Notificação de Débitos Fornecedor";
  ...
  case "debitosEmAberto":
      ...
      /**
       *
       * Verifica o tipo de liberação que o servidor está fazendo e verifica se respectivo CGM informado
       * está na tabela de Liberação de Fornecedor (liberafornecedor).
       *
       * Caso esteja, transforma o débito do respectivo em 0 para permitir a geração de empenho, solicitacao, processo.
       *
       */
  ...
  case "verificaEmailFornecedor":
      ...
      /**
       * Verifica permissões de menu do fornecedor
       */
