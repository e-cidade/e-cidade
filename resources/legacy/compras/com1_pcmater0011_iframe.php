  ...
  if(isset($o50_subelem) && ($o50_subelem=="t")){
    ...
    for($i = 0;$i < $numrows;$i++) {
      ...
      $sql=" select distinct (c60_codcon) as o56_codele, c60_estrut as o56_elemento, c60_descr as o56_descr
	         ...
	         where c60_anousu = ".db_getsession("DB_anousu")." and c60_estrut like '$elemento%' 
		     order by c60_estrut";    
      $res=$auxiliar->sql_record($sql);
  	  if($auxiliar->numrows>0){
        for ($ix=0;$ix < $auxiliar->numrows ;$ix++){  
          db_fieldsmemory($res,$ix);	    
	      $result_trancaele = $clempautitem->sql_record($clempautitem->sql_query_autoriza(null,null,"e55_item,e55_codele","e55_item","e55_item=$codigomater and e55_codele=$o56_codele and e54_anulad is null"));
	      $trancaele = false;
      ...
      $result = $clpcsubgrupo->sql_record($clpcsubgrupo->sql_query_orcelement(null,"o56_descr,o56_elemento,o56_codele","o56_elemento","o56_anousu = ".db_getsession("DB_anousu")." and pc04_codsubgrupo =".@$codsubgrupo));
      $numrows = $clpcsubgrupo->numrows;
      echo "<table border=\"0\">\n";
      echo "<tr bgcolor=\"#6699cc\"><th>&nbsp;</th><th>Reduzido</th><th>Desdobramento</th><th>$RLo56_descr</th></tr>\n";
      for($i = 0;$i < $numrows;$i++) {
