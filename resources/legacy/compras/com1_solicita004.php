...
<body style="background-color: #CCCCCC; margin-top:30px;">
	...
	if (isset($incluir) || (isset($importar) && $confirma == true)) {
		...
		if (isset($importar) && trim($importar) != "" && $sqlerro == false) {
			...
			$sSqlItem = $clsolicitem->sql_query_file(
				null,
			...
			for ($i = 0; $i < $numrows_importacaoitem; $i++) {
				 ...
				 */
					...
					if ($sqlerro == false) {
						$result_pcmater = $clsolicitempcmater->sql_record($clsolicitempcmater->sql_query(
							null,
							null,
							"pc16_codmater",
							"",
					...
					if ($sqlerro == false) {
						...
						for ($ii = 0; $ii < $numrows_importacaodot; $ii++) {
							 ...
							 */
							$rsContrapartida = $oDaoContrapartida->sql_record($oDaoContrapartida->sql_query_file(
								null,
								"*",
								null,
