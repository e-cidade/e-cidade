...
require_once("classes/db_bensmotbaixa_classe.php");
...
if($lImprimeOrgao == 't'){
  ...
  $sOrdem = " o40_orgao , o41_unidade ";
  if (!empty($sOrderBy)) {
    $sOrdem = "{$sOrderBy} , {$sOrdem}";
  }
...
for($cont=0;$cont<$numrows;$cont++){
  ...
  if($pdf->gety() > $pdf->h - 30 || $troca != 0 ){
    ...
    $pdf->cell(45,$alt,$RLdescrdepto,1,1,"C",1);
    $pdf->cell(284,$alt,$RLt51_descr,1,1,"L",1);
    $pdf->cell(284,$alt,"Destino",1,1,"L",1);
  ...
  $pos_x = $pdf->x;
	$pos_y = $pdf->y;
  $pdf->multicell(85,$alt,$t52_descr,"T","L",0,0);
  $pdf->x = $pos_x+85;
  $pdf->y = $pos_y;
  ...
  $pdf->cell(15,$alt,$t44_valoratual,"T",0,"C",0);
  $pdf->multicell(45,$alt,$descrdepto,"T","L",0,0);
  $pdf->cell(284,$alt,$t51_descr,"T",1,"L",0);
  $pdf->cell(284,$alt,$destino,"T",1,"L",0);
  $pdf->multicell(284,$alt,$t55_obs,"T","L",0,0);
  $total++;
  $valor_total+=$t44_valoratual;
}
//$pdf->cell(278,1,"","T",1,"L",0);
$pdf->setfont('arial','b',8);
$pdf->cell(142,$alt,'TOTAL DE REGISTROS  :  '.$total,"T",0,"L",0);
$pdf->cell(142,$alt,'VALOR TOTAL :  '.$valor_total,"T",0,"L",0);
$pdf->Output();
?>
