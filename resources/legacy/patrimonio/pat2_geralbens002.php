    ...
    if (($quebra_por == 1 && $imp_classi == "S") || $imp_classi == "N") {
        ...
        if ($pdf->gety() > $pdf->h - 30 || $troca != 0) {
            ...
            if ($flag_forn == true) {
                ...
                $pdf->cell(143, $alt, $RLt52_obs, 1, 0, "C", 1);
                $pdf->cell(15, $alt, "", 1, 1, "C", 1);
            }
        ...
        $pdf->setfont('arial', '', 7);
        $at52_descr = quebrar_texto($t52_descr, 40);
        $tam = count($at52_descr);

        $pdf->cell(15, $alt * $tam, $t52_bem, 0, 0, "C", $p);

        if (strlen($t52_descr) > 40) {
            //echo $t52_bem." | ".$t52_descr." | ".$tam;print_r($at52_descr);exit;
            $pos_x = $pdf->x;
            $pos_y = $pdf->y;
            foreach ($at52_descr as $descr_nova) {
                $descr_nova = ltrim($descr_nova);
                $pdf->cell(65, $alt, $descr_nova, 0, 1, "L", $p);
                $pdf->x = $pos_x;
            }
            $pdf->x = $pos_x + 65;
            $pdf->y = $pos_y;
        ...
        $pdf->cell(20, $alt * $tam, db_formatar($t52_valaqu, "f"), 0, 0, "R", $p);
        $pdf->cell(20, $alt * $tam, db_formatar($t52_dtaqu, "d"), 0, 0, "C", $p);
        $pdf->cell(15, $alt * $tam, $iNumemp, 0, 0, "C", $p);
        ...
        $pdf->cell(15, $alt * $tam, $t52_bem, 0, 0, "C", $background);
        ...
        if (strlen($t52_descr) > 40) {
            ...
            foreach ($at52_descr as $descr_nova) {
                $descr_nova = ltrim($descr_nova);
                $pdf->cell(65, $alt, $descr_nova, 0, 1, "L", $background);
                $pdf->x = $pos_x;
            }
            $pdf->x = $pos_x + 65;
            $pdf->y = $pos_y;
        ...
        $pdf->cell(20, $alt * $tam, db_formatar($t52_dtaqu, "d"), 0, 0, "C", $background);
        $pdf->cell(15, $alt * $tam, $iNumemp, 0, 0, "C", $background);
        $pdf->cell(20, $alt * $tam, $t52_ident, 0, 0, "C", $background);
...
if (isset($imprime_total_parcial)) {
    ...
    $pdf->cell(100, $alt, 'VALOR TOTAL:  ' . $valortotal, "T", 0, "R", 0);
    $pdf->cell(173, $alt, 'TOTAL DE REGISTROS  :  ' . $contaregistro, "T", 1, "R", 0);
}

$pdf->setfont('arial', 'b', 8);
$pdf->Ln(1);
$pdf->cell(100, $alt, 'VALOR TOTAL:' . db_formatar($totalvalor, "f"), "T", 0, "R", 0);
$pdf->cell(178, $alt, 'TOTAL GERAL DE REGISTROS  :  ' . $total, "T", 1, "R", 0);
...
function orgao_cabecalho($pdf, $RLt52_descr, $RLt52_ident, $RLt52_depart, $alt)
{
    ...
    $pdf->cell(20, $alt, "Vlr Aquisição", 1, 0, "C", 1);
    $pdf->cell(20, $alt, "Dt. Aquisição", 1, 0, "C", 1);
