...
include("classes/db_benscadcedente_classe.php");
...
if ($clcfpatriplaca->numrows > 0){
  ...
  $sMsg = _M('patrimonial.patrimonio.pat2_relatorioqttbens002.nao_existem_placas_para_instituicao');
...
$head3  = "RELATÓRIO QUANTITATIVO DE BENS";
$head4  = $info;
...
if($tipo == 'a') {
  $campos = "t42_descr,t52_bem, t52_descr, round(t52_valaqu,2) as t52_valaqu, t52_dtaqu, cast( regexp_replace( coalesce(nullif(trim(t52_ident),''), '0') , '[^0-9.,-]' , '', 'g') as numeric) as t52_ident, 
  ...
  $sqlrelatorio = $clbens->sql_queryBensLote(null,"$campos",$ordem,"$where_instit");
  
}else{
   $campos = "t42_codigo,t42_descr, count(t52_descr) as quantbens, t52_depart, sum(round(t52_valaqu,2)) as t52_valaqu";
   $sqlrelatorio = $clbens->sql_queryBensLoteSint(null,"$campos",$ordem,"$where_instit");
}
...
if ($clbens->numrows == 0){
  ...
  $sMsg = _M('patrimonial.patrimonio.pat2_relatorioqttbens002.nao_existem_registros');
  db_redireciona('db_erros.php?fechar=true&db_erro=' . $sMsg);
  exit;
}
  ...
  //}
	...
	if (($quebra_por == 1 && $imp_classi == "S") || $imp_classi == "N") {
	  ...
	  if ($pdf->gety() > $pdf->h - 30 || $troca != 0 ) {
	    ...
	    if(isset($orgaos) && isset($unidades) && isset($departamentos)&& $q_pagina != "N"){
	    	...
	    	if ($tipo == 'a'){
	    	  ...
	    	  orgao_cabecalhoSintetico($pdf,$RLt52_descr,$RLt52_ident,$RLt52_depart,$alt);
	    	  $lCabecalho = true;
	    	  $p = 0;
	    	}
	    }
	  ...
	  if ($tipo == 'a') {
		  ...
		  $pdf->cell(163,$alt,substr($t52_descr,0,50)			,0,0,"L",$p);
		  $pdf->cell(50,$alt,$t42_descr,0,0,"L",$p);
		  $pdf->cell(20,$alt,db_formatar($t52_valaqu,"f")	,0,1,"R",$p);
		  ...
		  //$pdf->cell(15,$alt,$placaIdentificacao	,0,1,"C",$p);
	  	  ...
	  	  $pdf->setfont('arial','',7);
	  	  $pdf->cell(15,$alt,$t42_codigo,0,0,"L",$p);
		  $pdf->cell(163,$alt,$t42_descr,0,0,"L",$p);
		  $pdf->cell(50,$alt,$quantbens,0,0,"L",$p);
		  $pdf->cell(50,$alt,$t52_valaqu,0,1,"L",$p);
		  
		  $totalquant +=  $quantbens;
...
$pdf->setfont('arial','b',8);
$pdf->Ln(1);
$pdf->cell(70,$alt,'TOTAL GERAL DE REGISTROS  :  '.$total,"T",0,"R",0);

$ctr = 207.5;

if ($tipo != 'a') {

$pdf->cell(130,$alt,'QUANT TOTAL:'.$totalquant,"T",0,"R",0);	
$ctr = 77.5;
}
$pdf->cell($ctr,$alt,'VALOR TOTAL:'.db_formatar($totalvalor,"f"),"T",1,"R",0);
...
function orgao_cabecalho($pdf,$RLt52_descr,$RLt52_ident,$RLt52_depart,$alt){
  ...
  $pdf->cell(163,$alt,$RLt52_descr			,1,0,"C",1);
  $pdf->cell(50,$alt,"Lote"	,1,0,"C",1);
  $pdf->cell(20,$alt,"Vlr Aquisição"	,1,1,"C",1);
  //$pdf->cell(25,$alt,"Data Aquisição"	,1,0,"C",1);
...
function orgao_cabecalhoSintetico($pdf,$RLt52_descr,$RLt52_ident,$RLt52_depart,$alt){
	
  $pdf->setfont('arial','b',8);
  $pdf->cell(15,$alt,"Codigo",1,0,"C",1);
  $pdf->cell(163,$alt,"Lote"	,1,0,"C",1);
  $pdf->cell(50,$alt,"Quant",1,0,"C",1);
  $pdf->cell(50,$alt,"Valor Total",1,1,"C",1);
