...
//Imprime a linha final das totalizações
	...
	for ($iInd = 0; $iInd < $iNumRows; $iInd++) {
		...
		if (($oGet->iPeriodoImpr == 1 && $iInd == 8)){
				...
				if($iInd+1 == $iNumRows){
					$pdf->cell($oRelatorio->iPeriocidade[$iInd]->tamanho, $iAlt, db_formatar($iSomaTotalGeral,'f'),'LB',0,'R',1);
				}
	}
...
//Imprime a linha final das totalizações
	...
	for ($iInd = 0; $iInd < $iNumRows; $iInd++) {
		...
		if($iInd+1 == $iNumRows){
			$pdf->cell($oRelatorio->iPeriocidade[$iInd]->tamanho, $iAlt, db_formatar($iSomaTotalGeral,'f') , 'LTB',0,'R',1);
		}
	}
