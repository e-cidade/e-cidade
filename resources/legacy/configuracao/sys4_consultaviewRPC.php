...
if ($oPost->tipo == "consultaCampos") {
    ...
    foreach ($aObjCampos as $oCampos) {
        ...
        $oColunaRelatorio = new dbColunaRelatorio(
            $oCampos->iId,
    ...
    $oVariavel     = new dbVariaveisRelatorio(
        utf8_decode($oPostVariavel->sNome),
    ...
    foreach ($aObjCampos as $sInd => $oCampos) {
        ...
        if ($oCampos) {
            ...
            $oOrdemRelatorio = new dbOrdemRelatorio(
                $oCampos->iId,
                $oCampos->sNome,
                $oCampos->sAscDesc,
                $oCampos->sAlias
    ...
    if (!$lErro) {
        ...
        $aRetorno = array(
            "msg" => urlencode('Inclusão feita com sucesso!'),
            "erro" => false
        );
    } else {
        $aRetorno = array(
    ...
    $rsSequenciaMenu = $oDaoDbmenu->sql_record($oDaoDbmenu->sql_query_file(
        null,
