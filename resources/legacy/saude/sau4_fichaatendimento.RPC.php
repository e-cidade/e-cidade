  ...
  switch($oParam->sExecucao) {
    ...
    case 'buscaUltimaObservacaoDaMovimentacao':
      ...
      if ( !isset($oParam->iProntuario) || empty($oParam->iProntuario) ) {
        throw new ParameterExceptio( _M( MENSAGENS_FICHAATENDIMENTORPC . 'prontuario_nao_informado' ) );
      }

      if ( !isset($oParam->iTelaOrigem) || empty($oParam->iTelaOrigem) ) {
        throw new ParameterExceptio( _M( MENSAGENS_FICHAATENDIMENTORPC . 'tela_nao_informada' ) );
      }

      $oMovimentacaoDao    = new cl_movimentacaoprontuario();
      $sCamposMovimentacao = " sd102_observacao";
      ...
      if ( pg_num_rows($rsMovimentacao) > 0 ) {
        $oRetorno->sObservacao = urlencode(db_utils::fieldsMemory( $rsMovimentacao, 0 )->sd102_observacao);
      }
    ...
    case 'encaminharProntuario':
      ...
      if ( empty($oParam->iProntuario ) ) {
        throw new ParameterExceptioException( _M( MENSAGENS_FICHAATENDIMENTORPC . 'prontuario_nao_informado' ) );
      }

      if ( empty($oParam->iSetorDestino) ) {
        throw new ParameterExceptioException( _M( MENSAGENS_FICHAATENDIMENTORPC . 'setor_destino_nao_informado' ) );
      }
    ...
    case 'buscarCartoesSus':
      ...
      if ( !isset($oParam->iPaciente) || empty($oParam->iPaciente) ) {
        throw new ParameterExceptio( _M( MENSAGENS_FICHAATENDIMENTORPC . 'paciente_nao_informado' ) );
      }
