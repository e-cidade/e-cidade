...
for ($iX = 0; $iX < $iTam; $iX++){
  ...
  for( $y=1; $y<=2; $y++ ){
    ...
    $pdf->text( $pdf->getX() + 17, $pdf->getY() +  7, "SISTEMA DE INFORMAÇÕES" );//nome biblioteca
    ...
    $pdf->setfont('arial','b',5.5);
    $pdf->text( $lar + 2, $alt + 55, "CÓDIGO SIA/SUS:".$sd02_c_siasus);
    $pdf->setfont('arial','b',5.5);
    ...
    $pdf->text( $pdf->getX() + 131, $pdf->getY() + 21, "TRATAMENTO DESCRIÇÃO" );
    ...
    $pdf->setfont( 'arial', 'b', 5.5 );
    $pdf->text( $pdf->getX() + 1, $pdf->getY() + 71, "IDENTIFICAÇÃO DO PACIENTE" );
    $pdf->setfont( 'arial', 'b', 5.5 );
    ...
    $pdf->text( $pdf->getX() + 1, $alt + 29, "ENDEREÇO: ".substr($z01_v_ender.", ".$z01_i_numero.", ".$z01_v_compl,0,21));
    ...
    $pdf->text( $pdf->getX() + 1, $alt + 41, "MUNICÍPIO:".substr($z01_v_munic,0,40));
    ...
    $pdf->text( $pdf->getX() + 1 ,$alt + 65, "FAMÍLIA PSF:".$sd33_v_descricao);
    $pdf->text( $pdf->getX() + 1 ,$alt + 68, "MICROÁREA:".$sd34_v_descricao);
    ...
    $pdf->text( $pdf->getX() + 57, $pdf->getY() + 122, "DIAGNÓSTICO:" );
    ...
    $pdf->setfont( 'arial', 'b', 6 );
    $pdf->text( $pdf->getX() + 57, $pdf->getY() + 134, "ASSINATURA DO PACIENTE OU RESPONSÁVEL:" );
