...
try {
  ...
  switch($oParam->exec) {
    ...
    case 'salvarTriagem':
      ...
      $oTriagemAvulsa->setEvolucao( db_stdClass::normalizeStringJsonEscapeString( $oParam->sTextEvolucao ));
    ...
    case 'buscaTriagemValida':
      ...
      if ( pg_num_rows($rsTriagemConsulta) > 0 ) {
        ...
        $oRetorno->sEvolucao                = urlencode( $oTriagemAvulsa->getEvolucao() );
    ...
    case 'salvarEspecialidadeProcedimentos':
      ...
      if( !empty($oParam->iEspecialidade) ) {
        ...
        foreach ( $oParam->aProcedimentosTriagem as $iProcedimento ) {
          ...
          $oDaoProntproced->sd29_c_hora         = date('H:i',db_getsession("DB_datausu"));
