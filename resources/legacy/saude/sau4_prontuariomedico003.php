...
$sCamposProntuarios .= ' sd29_i_prontuario, sd29_d_data, sd24_v_pressao, sd24_f_temperatura, sd24_f_peso, s152_i_cintura, ';
$sCamposProntuarios .= ' sau_cid.sd70_c_cid, sau_cid.sd70_c_nome, sau_procedimento.sd63_c_procedimento, ';
$sCamposProntuarios .= ' sd58_i_codigo, sd02_i_codigo, descrdepto, s144_c_descr, s152_i_altura, s152_i_glicemia, sd24_t_diagnostico, ';
$sCamposProntuarios .= ' sd24_i_codigo, sd24_d_cadastro, sd24_c_cadastro, ';
$sCamposProntuarios .= ' sau_procedimento.sd63_c_nome, sd29_t_tratamento, m.z01_nome as profissional';
...
for ($p=0; $p < $linhas1; $p++) {
  ...
  if ($retorna_obs == 0) {
    ...
    for($iCont = 0; $iCont < $oDaoProntproced->numrows; $iCont++){
      ...
      if ($oDados->sd29_sigilosa == 't') {
        $sConteudoPrescricao = " SIGILOSA ";
      }

      $sConteudo .= "\nEvolução/Prescrição: {$sConteudoPrescricao}\n";
    }
    ...
    if ($oDaoSauReceitaMedica->numrows > 0) {
      ...
      $sConteudo .= str_pad('Forma de Administração', 30);
      $sConteudo .= str_pad('Posologia', 20);
      $sConteudo .= str_pad('Qtde.', 10);
      ...
      for( $iContador = 0; $iContador < $oDaoSauReceitaMedica->numrows; $iContador++ ) {
        ...
        $sConteudo .= "\n" . str_pad( $oDadosMedicamento->m60_descr, 40);
        $sConteudo .= str_pad($oDadosMedicamento->s160_c_descr, 30);
        $sConteudo .= str_pad($oDadosMedicamento->s159_t_posologia, 20);
        $sConteudo .= str_pad($oDadosMedicamento->s159_n_quant, 10);
      }
    }
