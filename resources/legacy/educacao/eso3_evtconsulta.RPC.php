...
require_once('model/esocial/MakeImportarDadosEvt.model.php');
...
try {
    switch ($oParam->exec) {
        ...
        case "importarDadosXml":
            if (!file_exists($oParam->sPath)) {
                throw new Exception("Houve um erro ao realizar upload do arquivo. Tente novamente.");
            }

            db_inicio_transacao();
            $oImportarDadosEvt = MakeImportarDadosEvt::make($oParam);
            $oRetorno->sequencial = $oImportarDadosEvt->processar();
