  ...
  case 'encerrarAvaliacoesPorAluno' :
    ...
    try {
      ...
      foreach ($oParam->aTurmas as $oTurmaEtapa) {
        ...
        $aMatriculas = getMatriculasAlunoTurma($oParam->aAlunos, $oTurma);
 ...
 * @throws Exception
 */
function getMatriculasAlunoTurma( $aCodigoAluno, $oTurma ) {

  $aMatriculas        = array();
  $aAlunosPercorridos = array();

  if (count($aCodigoAluno) == 0) {
    throw new Exception("Nenhum aluno selecionado.");
  }

  foreach ($aCodigoAluno as $iCodigoAluno) {
