...
if (strstr($disciplinas,",")) {
  ...
  $sqlRegencia =  $clregencia->sql_query("","ed232_c_descr as descr_disc","",
                                         " ed59_i_codigo = $disciplinas AND ed59_c_condicao = 'OB'");
  $rsResultRegencia = $clregencia->sql_record($sqlRegencia);
  db_fieldsmemory($rsResultRegencia,0);
  if ($punico == "yes") {
    $descr_disc = "PARECER ÚNICO";
  } else {
    $descr_disc = $descr_disc;
  }
}
  ...
  if ($punico == "yes") {
    ...
    foreach ($oTurma->getDisciplinas() as $oRegencia) {
      ...
      if (trim($oParecer->sParecerPadronizado) != '' || trim($oParecer->sParecer) != '') {
        $disciplinas = $oRegencia->getCodigo();
      }
    }
  ...
  for ($r = 0; $r < $linhas0; $r++) {
    ...
    if (($amparo == "N" || $amparo == "") && ($amparoum == "N" || $amparoum == "")) {
      ...
      if ($ed81_i_justificativa != "") {
        $pdf->cell(170,5,"Justificativa Legal: ".pg_result($result3,0,'justificativa'),0,0,"L",0);
      } else {
        $pdf->cell(170,5,pg_result($result3,0,'ed250_c_abrev'),0,0,"L",0);
      }
      $pdf->cell(10,5,"","",1,"L",0);
    }
  }
