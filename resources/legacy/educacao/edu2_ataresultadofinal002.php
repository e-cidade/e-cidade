  ...
  foreach ($aDisciplinasPorPagina as $iDisciplina => $aDisciplinasPagina) {
    ...
    for ($iContadorAluno = 0; $iContadorAluno < $iTotalAlunosMatriculados; $iContadorAluno++) {
      ...
      if ($lSequencialDiario) {
        ...
        $oPdf->Cell(5, 4, ++$iContadorSequencial, $sBordaAluno, 0, "C", $iCorLinha);
      }

      $sNomeAluno = substr($aListaDeAlunos[$iContadorAluno]->getAluno()->getNome(), 0, 38);

      $oPdf->Cell($oFiltroRelatorio->iTamanhoTotalColunaDisciplina, 4,
                  $sNomeAluno, $sBordaAluno, 0, "L", $iCorLinha);
      ...
      if ($aListaDeAlunos[$iContadorAluno]->getSituacao() != "MATRICULADO") {
        ...
        foreach ($aDisciplinasPagina as $oRegenciaTurma) {
          ...
          /**
           * Verificamos se o aluno foi reprovado em alguma disciplina. Caso tenha sido, o Resultado Final é 'R'.
        ...
        if ($oFiltroRelatorio->lCalculaFrequencia == 2 && $oFiltroRelatorio->iFrequencia != 1) {
          ...
          $oPdf->SetFontSize(5.5);
          $oPdf->Cell($oFiltroRelatorio->iTamanhoColunaResultado, 4, "{$nValorFaltas}", $sBordaAluno, 0, "C", $iCorLinha);
          $oPdf->SetFontSize(6);
          $oPdf->Cell($oFiltroRelatorio->iTamanhoColunaResultado, 4, "{$sResultadoGeral}", $sBordaAluno, 1, "C", $iCorLinha);
        } else {
