	   ...
	   */
		case 'salvarOrigemNota':
			...
			if ( isset($oParam->iMatricula) && isset($oParam->iDiarioAvaliacao) ) {
				...
				/**
				 * Verifica se existe a turma instanciada na sessão, e substitui a matrícula existente pela da sessão
				 */
				if (isset($_SESSION["oTurma"]) && $_SESSION["oTurma"] instanceof Turma) {

		      $oTurma = $_SESSION["oTurma"];

		      	foreach ($oTurma->getAlunosMatriculados() as $oMatriculaTurma) {

		      		if ($oMatriculaTurma->getCodigo() == $oMatricula->getCodigo()) {
