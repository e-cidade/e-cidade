...
if ( empty($oGet->dtfim) === false || empty($oGet->dtini) === false ) {
...
if ( empty($oGet->dtVencini) === false || empty($oGet->dtVemcfim) === false ) {

    $dtVencIni = implode("-",array_reverse(explode("/",$oGet->dtVencini)));
    $dtVencFim = implode("-",array_reverse(explode("/",$oGet->dtVemcfim)));

    if ( !empty($dtVencIni) && !empty($dtVencFim) ) {

        $sHeaderVencimento = "Vencimento : DE ".$oGet->dtVencIni." A ".$oGet->dtVencFim;
        $sWhere        .= " {$sAnd} it14_dtvenc between '{$dtVencIni}' and '{$dtVencFim}'";
        $sAnd           = " and ";
    } else if ( !empty($dtVencIni) ) {

        $sHeaderVencimento = "Vencimento : DATA MAIOR OU IGUAL A ".$oGet->dtVencIni;
        $sWhere        .= " {$sAnd} it14_dtvenc >= '{$dtVencIni}' ";
        $sAnd           = " and ";
    } else if ( !empty($dtVencFim) ) {

        $sHeaderVencimento = "Vencimento : DATA MENOR OU IGUAL A ".$oGet->dtVencFim;
        $sWhere        .= " {$sAnd} it14_dtvenc <= '{$dtVencFim}' ";
        $sAnd           = " and ";
    }
}

if (empty($oGet->sLogradouro) === false) {
  ...
  foreach ( $aDadosItbi as $iCodItbi => $aDados ) {
    ...
    if (isset($aDados['aAdquirentes'])) {
	    ...
	    foreach ( $aDados['aAdquirentes'] as $iInd => $aDadosAdquir ) {
	      ...
	      $pdf->cell(139.5,$iAlt,$aDadosAdquir                                                                ,0,1,"C",0);
	    }
    }
    ...
    if (isset($aDados['aTransmitentes'])) {
	    ...
	    foreach ( $aDados['aTransmitentes'] as $iInd => $aDadosTrans ) {
	      ...
	      imprimeCabecalho($pdf,$iAlt);
	    	$pdf->SetX(149.5);
	      $pdf->cell(139.5,$iAlt,$aDadosTrans                                                                 ,0,1,"C",0);
	    }
    }
