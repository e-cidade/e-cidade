 ...
 $sql="select termo.*,
      ...
      from termo
           ...
           left outer join db_usuarios
                   on db_usuarios.id_usuario = termo.v07_login
					 left join cadtipoparc on k40_codigo = v07_desconto
...
if ($termo_cabecalho == "") {
  $termo_cabecalho = "TERMO DE CONFISSÃO DE DÍVIDA E COMPROMISSO DE PAGAMENTO";
}

$pdf->MultiCell(0,4,"$termo_cabecalho: $v07_parcel",0,"C",0,0);
$pdf->Ln(4);
$pdf->SetFont('Arial','',11);
$pdf->MultiCell(0,5,$termo_p1,0,"J",0,0);
...
$sqlrepar = "select retermo.*, k00_matric as matric,k00_inscr as inscr 
             ...
             where v07_parcel=$parcel order by retermo.v07_dtlanc limit 1";
$result = pg_exec($sqlrepar);
//echo $sqlrepar;exit;
//print_r($result);exit;
$sql = "select * from termoreparc where v08_parcel = $parcel limit 1";
$result_reparc = pg_exec($sql) or die($sql);
...
if (pg_numrows($result_reparc) > 0) {
	...
	$sql =	"select distinct
									termoreparc.*,
									arreold.*,
									arreold.k00_numpar as v01_exerc,
									cadtipo.k03_tipo,
									arrecad.k00_tipo as tipo,
									arreold.k00_valor as valor,
									arreold.k00_valor as vlrcor,
									0 as juros,
									0 as multa,
									arreold.k00_dtvenc as v01_dtvenc,
									termoori.v07_numpre,
									(select coalesce(max(k00_matric), 0) from arrematric 
													inner join termo on v07_numpre = k00_numpre
													where v07_parcel = $parcel) as matric,
									(select coalesce(max(k00_inscr), 0) from arreinscr
													inner join termo on v07_numpre = k00_numpre
													where v07_parcel = $parcel) as inscr
					from termoreparc
					inner join termo termoori on v08_parcelorigem = termoori.v07_parcel
					inner join arreold on termoori.v07_numpre = arreold.k00_numpre
					...
					left join arrecad on arrecad.k00_numpre = termoatual.v07_numpre
					left join arrematric on termoatual.v07_numpre = arrematric.k00_numpre
					left join arreinscr on termoatual.v07_numpre = arreinscr.k00_numpre
					where v08_parcel = $parcel
					";
	...
	if ( pg_numrows($result) > 0 ) {
		// se for reparcelamento ou diversos...
		 ...
		 if ($k03_tipo == 13) { // parcelamento do foro
				 ...
				 $sql = "select distinct x.z01_nome as nomematric,
				x.v03_descr, 
				arreold.*, 
...
//die("np: $np  npa: $npa  k00_numpre: $k00_numpre  k00_numpar: $k00_numpar");
...
//     $multa = $multa + $vlrdescmul;
   ...
   $pdf->Cell(30,4,(@$v03_descr==''?"Parcelamento: ".(pg_numrows($result_reparc) > 0?$v08_parcelorigem:$parcel):$v03_descr),1,0,"L",0);
...
if($pdf->GetY() > ( $pdf->h - 40 )){
   ...
   $pdf->Cell(50,5,'Vencimento entrada','LRTB',0,"L",0);
   ...
   $pdf->Cell(50,5,'Valor da ultima parcela','LRB',0,"L",0);
   $pdf->Cell(45,5,$v07_ultpar,'LRB',1,"R",0);

   $pdf->SetXY(105,$y);
   $pdf->Cell(50,5,'Numero de Parcelas ',1,0,"L",0);
   $pdf->Cell(45,5,"Entrada + " . $numpar,1,1,"R",0);
   $pdf->SetX(105);
   $pdf->Cell(50,5,'Valor das parcelas a partir de ',1,0,"L",0);
   $pdf->Cell(45,5,db_formatar($vlrpar,'f'),1,1,"R",0);
   $pdf->SetX(105);
   $pdf->Cell(50,5,'Segundo vencimento',1,0,"L",0);
   $pdf->Cell(45,5,db_formatar($vencpar,'d'),1,1,"R",0);
   $pdf->SetX(105);
   $pdf->Cell(50,5,'Numpre do parcelamento',1,0,"L",0);
   $pdf->Cell(45,5,$v07_numpre,1,1,"R",0);
   $pdf->SetFont('Arial','',11);

$pdf->Ln(2);
$pdf->MultiCell(0,5,$termo_p2,0,"J",0,0);
if ( $pdf->GetY() > 248) {
   $pdf->AddPage();
}
$pdf->MultiCell(0,5,$termo_p3,0,"J",0,0);
if ( $pdf->GetY() > 248) {
   $pdf->AddPage();
}
$pdf->MultiCell(0,5,$termo_p4,0,"J",0,0);
if ( $pdf->GetY() > 248) {
   $pdf->AddPage();
}
