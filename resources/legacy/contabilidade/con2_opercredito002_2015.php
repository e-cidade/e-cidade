...
$head4 = "DEMONSTRATIVO DAS OPERAÇÕES DE CRÉDITO";
...
$aLinhasRelatorio[25]->label = "    Outras Operações de Crédito Não Sujeitas ao Limite ";
$aLinhasRelatorio[26]->label = "RECEITA CORRENTE LÍQUIDA - RCL ";
$aLinhasRelatorio[27]->label = "OPERAÇÕES VEDADAS";
...
if (!isset($arqinclude)) {
  ...
  $pdf->addpage();
  $pdf->setfont('arial','b',7);
  $pdf->cell(165,$alt,'RGF - ANEXO 4 (LRF, art. 55, inciso I, alínea "d" e inciso III alínea "c")','B',0,"L",0);
  ...
  $linhaReceitaCorrenteLiquida = 26;
  $aLinhasMaiores27            = array_slice($aLinhasRelatorio, 26, 9, true);
  foreach ($aLinhasMaiores27 as $key => $oLinha) {
    ...
    // Se é a linha da RCL imprime só o '-' (traço)
    if ($key == $linhaReceitaCorrenteLiquida) {
      $pdf->cell(30,$alt, $oLinha->atebimestre,"TBL",1,"C");

    // Caso contrário aplica a db_formatar no valor
    } else {
