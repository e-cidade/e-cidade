...
if (!isset($arqinclude)){
  ...
  if ($tipo_emissao!='datas'){
    ...
    if ($iMesInicialAnterior < 13) {
      ...
      $txt    = strtoupper(db_mes($iMesInicialAnterior));
      $txt   .= "  / ".$anousu_ant;
    } else {
      $txt   .= "  / ".$anousu;
    }
  ...
  switch($bimestre) {
    ...
    case 1:
      $txt   = strtoupper(db_mes('3'))."/".($anousu-1)." À ".strtoupper(db_mes('2'))."/".$anousu;
      break;
    case 2:
      $txt   = strtoupper(db_mes('5'))."/".($anousu-1)." À ".strtoupper(db_mes('4'))."/".$anousu;
      break;
    case 3:
      $txt   = strtoupper(db_mes('7'))."/".($anousu-1)." À ".strtoupper(db_mes('6'))."/".$anousu;
      break;
    case 4:
      $txt   = strtoupper(db_mes('9'))."/".($anousu-1)." À ".strtoupper(db_mes('8'))."/".$anousu;
      break;
    case 5:
      $txt   = strtoupper(db_mes('11'))."/".($anousu-1)." À ".strtoupper(db_mes('10'))."/".$anousu;
      break;
    case 6:
      $txt   = strtoupper(db_mes('1'))."/".($anousu)." À ".strtoupper(db_mes('12'))."/".($anousu);
      break;
  }
...
if (!isset($arqinclude)){
  ...
  for ($x = 1; $x <= 22; $x++) { // 18 é a qtd de parametros existentes
    ...
    if ($x == 1) {
      ...
      //-------------------------------
      $pdf->setfont('arial', '', 6);
      $pdf->cell(60, $alt, " Impostos, Taxas e Contribuições de Melhoria", 'R', 0, "L", 0);
      $total = 0;
      for ($y = $iMesInicialAnterior; $y <= 12; $y++) {
