...
$dt_ini = $dados[0]; // data inicial do período
$dt_fin = $dados[1]; // data final do período
...
if ($emite_balorc_rec==1){
  ...
  $m_oper_int_mobiliaria                = $orcparamrel->sql_parametro('15','39',"f",$instituicao); //operações de credito/refinanciamento
        ...
        if ($emite_alienacao==1){
          ...
          $sele_work     = ' o70_instit in (' . str_replace('-',', ',$db_selinstit) . ')';
              ...
              if ($emite_balorc_rec==1){
                ...
                $pdf->cell(90,$alt,"BALANÇO ORÇAMENTARIO - RECEITAS","BT",0,"C",0);
              ...
              if ($emite_balorc_desp==1){
                ...
                $pdf->cell(90,$alt,"BALANÇO ORÇAMENTARIO - DESPESAS","BT",0,"C",0);
                $pdf->cell(50,$alt,"No ".$dados["periodo"],"BTRL",0,"C",0);
                $pdf->cell(50,$alt,"Até o ".$dados["periodo"],"BT",1,"C",0);

                $pdf->cell(90,$alt,'Dotação Inicial','R',0,"L",0);
              ...
              if ($emite_rp==1){
                ...
                $pdf->cell(25,$alt,db_formatar(($total_proc_insc+$total_naoproc_insc),"f"),"TRL",0,"R",0);      // Total de Inscricoes
                $pdf->cell(25,$alt,db_formatar(($total_proc_canc+$total_naoproc_canc),"f"),"TRL",0,"R",0);      // Total de Cancelados
                $pdf->cell(25,$alt,db_formatar(($total_proc_pgto+$total_naoproc_pgto),"f"),"TR",0,"R",0);       // Total de Pagamentos
                $pdf->cell(25,$alt,db_formatar(($total_proc_a_pagar+$total_naoproc_a_pagar),"f"),"T",1,"R",0);  // Total de Saldo a Pagar
              }
              // MDE
              if ($emite_mde==1) {
                if ($num_rel > 0){
              ...
              if ($emite_proj==1){
                if ($num_rel > 0){
                  if ($num_rel > 7){
                    $pdf->cell(190,($alt+2),"Continuação na página 2",0,1,"L",0);
                    $pdf->addpage();
                    $pdf->cell(190,($alt+2),"Continuação da página 1",0,1,"L",0);
                  } else{
                    $pdf->cell(190,$alt,"","TB",1,"C",0);
                  }
                }
              ...
              if ($emite_ppp==1){
                ...
                $pdf->cell(90, $alt,"DESPESAS DE CARÁTER CONTINUADO DERIVADAS DE","TR",0,"C",0);
