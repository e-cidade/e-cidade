...
$head2  = "MUNICÍPIO DE {$oMunicipio->munic} - {$oMunicipio->uf}";
$head3  = "RELATÓRIO RESUMIDO DA EXECUÇÃO ORÇAMENTÁRIA";
$head4  = "DEMONSTRATIVO DA RECEITA DE ALIENAÇÃO DE ATIVOS E APLICAÇÃO DOS RECURSOS";
...
$oPdf->Cell(130, $iAltCell, 'RREO - ANEXO 11 (LRF, art. 53, § 1o, inciso III )', 'B', 0, "L", 0);
...
$oPdf->Cell(35, $iAltCell+9, 'SALDO A REALIZAR (c)=(a-b)', 'TBL', 1, "C", 0);

foreach ($aDadosAnexoXIV->quadroreceitas as $oQuadroReceita) {
...
$oPdf->Cell(35, $iAltCell+12, 'SALDO A EXECUTAR (g)=(d-(e+f))', 'TBL', 1, "C", 0);
...
foreach ($aDadosAnexoXIV->quadrodespesas as $oQuadroDespesas) {
...
$oPdf->SetFont('arial', 'b', $iTamFonte);
$oPdf->Cell(64, $iAltCell+9, 'SALDO FINANCEIRO A APLICAR',     'TBR', 0, "C", 0);
$oPdf->Cell(42, $iAltCell+9, "{$iAnoAnterior} (h)",            'TBRL', 0, "C", 0);
$oPdf->Cell(49, $iAltCell+9, "{$iAnoUsu} (i)=(Ib-(IIe+IIf))",  'TBRL', 0, "C", 0);
$oPdf->Cell(35, $iAltCell+9, 'SALDO ATUAL (j)=(IIIh+IIIi)',    'TBL', 1, "C", 0);

foreach ($aDadosAnexoXIV->quadrosaldofinanceiro as $oQuadroSaldoFinanceiro) {

	$oPdf->SetFont('arial', '', $iTamFonte);
	$oPdf->Cell(64, $iAltCell, substr($oQuadroSaldoFinanceiro->descricaolinha, 0, 70),       'TBR', 0, "L", 0);
	$oPdf->Cell(42, $iAltCell, db_formatar($oQuadroSaldoFinanceiro->exercicioanterior, 'f'), 'TBRL', 0, "R", 0);
	$oPdf->Cell(49, $iAltCell, db_formatar($oQuadroSaldoFinanceiro->valorexercicio, 'f'),    'TBRL', 0, "R", 0);
	$oPdf->Cell(35, $iAltCell, db_formatar($oQuadroSaldoFinanceiro->saldoatual, 'f'),        'TBL', 1, "R", 0);
}
