...
$resultinst = db_query("select codigo,munic,db21_tipoinstit,uf from db_config");
...
$head2 = "MUNICÍPIO DE " . strtoupper($munic) . " - {$uf}";
$head3 = "RELATÓRIO DE GESTÃO FISCAL";
$head4 = "DEMONSTRATIVO DA DÍVIDA CONSOLIDADA LÍQUIDA";
...
$aTotalizadores[9]["label"]           = "DÍVIDA CONSOLIDADA PREVIDENCIÁRIA (IX)";
...
$aTotalizadores[10]["label"]           = "DEDUÇÕES (X)¹";
...
$aTotalizadores[11]["label"]           = "DÍVIDA CONSOLIDADA LÍQUIDA PREVIDENCIÁRIA (XI) = (IX - X)";
...
$aTotalizadores[12]["label"]           = "DÍVIDA CONTRATUAL (IV = V + VI + VII + VIII)";
...
$aTotalizadores[13]["label"]           = "DÍVIDA COM INSTITUIÇÃO FINANCEIRA (VII)";
...
if (! isset($arqinclude)) {
  ...
  $pdf->cell(110, $alt, 'RGF - ANEXO 2 (LRF, art. 55, inciso I, alínea "b")', 'B', 0, "L", 0);
  ...
  if ($usa_datas == false) {
    $pdf->cell(84, $alt, "SALDO DO EXERCÍCIO  DE $anousu", 'TB', 1, "C", 0);
  } else {
    $pdf->cell(84, $alt, "SALDO DO PERIODO ", 'TB', 1, "C", 0);
  }
  $pdf->cell(73, $alt, "DETALHAMENTO DA DÍVIDA CONTRATUAL", 'BR', 0, "C", 0);
  $pdf->cell(28, $alt, "EXERCÍCIO ANTERIOR", 'RB', 0, "C", 0);
 ...
 for ($i = 7; $i <= 8; $i++) {
  ...
  $pdf->cell(73, $alt, "OUTROS VALORES NÃO INTEGRANTES DA DC", 'BR', 0, "C", 0);
  $pdf->cell(28, $alt, "EXERCÍCIO ANTERIOR", 'RB', 0, "C", 0);
  ...
  $pdf->Cell(190, $alt, 'Continua na página '.($pdf->PageNo() + 1)."/{nb}", '', 1, "R", 0);
  $pdf->AddPage();
  $pdf->ln();
