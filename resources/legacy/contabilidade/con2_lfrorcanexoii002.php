...
$result_dot = db_dotacaosaldo(4, 2 , 4  ,true ,$sele_work,$anousu,$dataini,$datafin,8,0,true,1,false);
//$result_dot1 = db_dotacaosaldo(4, 2 , 4  ,true ,$sele_work,$anousu,$dataini,$datafin);

 //db_criatabela($result_dot1);exit;
...
for($i=0;$i<pg_numrows($result);$i++){
  ...
  if($pdf->gety() > $pdf->h-40){
     $pdf->cell(190,$alt,'Continua Página 2',"T",1,"L",0); 
     $pdf->cell(190,$alt,'',"T",1,"L",0);
     
     $pdf->addpage();
     $pdf->ln(2);
     $pdf->cell(190,$alt,'Continuação da Página 1',"T",1,"L",0);
     $pdf->cell(01,$alt,'LRF, Art. 52, Inciso II, alínea"c" - Anexo II',"B",0,"L",0);
