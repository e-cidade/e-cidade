...
//  $saldo += (($e91_vlremp - $e91_vlranu - $vlranu) - ($e91_vlrpag + $vlrpag));
  ...
  /*TAREFA 29391
    Comentado este trecho como solicitado na tarefa 29391
    if ($c70_anousu == $anousu) {
    	$cancelado += $vlrliq;
    } 
  */
...
if ($sSiglaPeriodo != '1S' && $sSiglaPeriodo != '2S') {
	$sBimestre = "BIMESTRE ".db_mes($vdt_ini [1], 1)."-".db_mes($vdt_fin [1], 1);
} else if ($sSiglaPeriodo == '2S'){
  $vdt_ini [1] = "01";
}
$head5 = db_mes($vdt_ini [1], 1) . " A " . db_mes($vdt_fin [1], 1) . " de " . $anousu . $sBimestre;
...
if ($lGeraColunaRps) {
  ...
  @$nPercOperCred = ( ( $desp_p[3]['bimestre'] * 100 ) / $total_V_atebim );
}
...
if ($lGeraColunaRps) {
  $pdf->cell($iLarguraColunaValor, $alt, "","TB", 0, "R", 0);
  $total_V_atebim = $total_V_atebim + $total_IV_rp - $aTotalDespCustRecSaude['rpNaoProcessado'];
}
...
for($i = 0; $i < 7; $i ++) {
  if ($i == 0) {
    ...
    //echo "($sub301_atebim)/($total_acum)*100<br>";
