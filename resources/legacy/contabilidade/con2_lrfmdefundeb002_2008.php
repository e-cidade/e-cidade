...
$receita[37]["txt"] = "    9.1 - Cota-Parte FPM Destinada ao FUNDEB - (18,33% de 2.1)";
$receita[38]["txt"] = "    9.2 - Cota-Parte ICMS Destinada ao FUNDEB - (18,33% de 2.2)";
$receita[39]["txt"] = "    9.3 - ICMS-Desoneração Destinada ao FUNDEB - (18,33% de 2.3)";
$receita[40]["txt"] = "    9.4 - Cota-Parte IPI-Exportação Destinada ao FUNDEB - (18,33% de 2.4)";
$receita[41]["txt"] = "    9.5 - Cota-Parte ITR Destinada ao FUNDEB - (13,33% de 2.5)";
$receita[42]["txt"] = "    9.6 - Cota-Parte IPVA Destinada ao FUNDEB - (13,33% de 2.6)";
      ...
      if (count($m_despesa[$linha]["funcao"])      == 0 || in_array($v_funcao, $m_despesa[$linha]["funcao"])) {
        if (count($m_despesa[$linha]["subfunc"])   == 0 || in_array($v_subfuncao, $m_despesa[$linha]["subfunc"])) {
          if (count($m_despesa[$linha]["recurso"]) == 0 || in_array($v_recurso, $m_despesa[$linha]["recurso"])) {
            ...
            $m_despesa[$linha]["inscritas"]  += $empenhado_acumulado-$anulado_acumulado-$liquidado_acumulado;
          }
        }
      }
    }
...
// Cabecalho de receitas
  $pdf->cell(90,($alt*2),"RECEITA BRUTA DE IMPOSTOS",'TBR',0,"C",0);
...
// br
  ...
  for ($linha=1; $linha <= 6; $linha++) {
    ...
    $pdf->cell(16,$alt,db_formatar($despesa[$linha]['inicial'],'f'),'R',0,"R",0);
    $pdf->cell(16,$alt,db_formatar($despesa[$linha]['atualizada'],'f'),'R',0,"R",0);
    $pdf->cell(16,$alt,db_formatar($despesa[$linha]['bimestre'],'f'),'R',0,"R",0);
    $pdf->cell(16,$alt,db_formatar($despesa[$linha]['exercicio'],'f'),'R',0,"R",0);
    $pdf->cell(18,$alt,db_formatar($despesa[$linha]['inscritas'],'f'),'R',0,"R",0);
//  @$pdf->cell(16,$alt,db_formatar(($despesa[$linha]['exercicio']*100)/$despesa[$linha]['atualizada'],'f'),0,0,"R",0);
    @$pdf->cell(18,$alt,db_formatar( ( ( ($despesa[$linha]['exercicio'] + $despesa[$linha]['inscritas']) ) /$despesa[$linha]['atualizada'] )*100,'f'),0,0,"R",0);
    $pdf->Ln();
  }
  ...
  $somador14_inscritas   = $despesa[1]["inscritas"]  + $despesa[4]["inscritas"];
...
// br
  ...
  $pdf->cell(170,$alt,"15 - MÍNIMO DE 60% DO FUNDEB NA REMUNERAÇÃO DO MAGISTÉRIO COM EDUCAÇÃO INFANTIL E ENSINO FUNDAMENTAL(12/10)x100%","TBR",0,"L",0);
 ...
 $pdf->setfont('arial','',6); 
  ...
  $nValorAtualizado25  = ($somador3_atualizada*25)/100;
  $nValorAteBimestre25 = ($somador3_atebimestre*25)/100;
  ...
  @$pdf->cell(20,$alt,db_formatar((($nValorAteBimestre25/$nValorAtualizado25)*100),'f'),'TB',0,"R",0);
  $pdf->ln();
  ...
  if ($periodo_selecionado=='6B' || $periodo_selecionado=='2S'){
  $pdf->setfont('arial','',4);
  $pdf->cell(90,($alt*3),"DESPESAS COM AÇÕES TÍPICAS DE MANUTENÇÃO E DESENVOLVIMENTO DO ENSINO",'TBR',0,"C",0);
...
// br
  ...
  for ($linha=7; $linha <= 16; $linha++) {
    ...
    $pdf->cell(16,$alt,db_formatar($despesa[$linha]['exercicio'],'f'),'R',0,"R",0);
    $pdf->cell(18,$alt,db_formatar($despesa[$linha]['inscritas'],'f'),'R',0,"R",0);
    @$pdf->cell(18,$alt,db_formatar(($despesa[$linha]['exercicio']*100)/$despesa[$linha]['atualizada'],'f'),0,0,"R",0);
    $pdf->Ln();
  }
  ...
  $somador23_inscritas    = $despesa[7]["inscritas"]   + $despesa[10]["inscritas"]  + $despesa[13]["inscritas"]  +
...
// Variaveis do relatorio
  ...
  if ($clconrelinfo->numrows > 0){
    ...
    $pdf->cell(40,$alt,"0,00","TB",1,"R",0);
    $pdf->cell(150,$alt,"27 - DESPESAS VINCULADAS AO SUPERÁVIT FINANCEIRO DO ACRÉSCIMO E DA COMPLEMENTAÇÃO DO FUNDEB DO EXERCÍCIO ANTERIOR","TBR",0,"L",0);
    $pdf->cell(40,$alt,"0,00","TB",1,"R",0);
  }
// Fim das Variaveis
// RESTOS A PAGAR
...
if ($periodo_selecionado=='6B' || $periodo_selecionado=='2S'){
  $somador31_valor = @((($despesa[7]["exercicio"] + $despesa[10]["exercicio"] + $despesa[7]["inscritas"] + $despesa[10]["inscritas"] ) - $somador30_valor)/$somador3_atebimestre)*100;
}else{
...
// br
  ...
  $somador35_inscritas   = $despesa[17]["inscritas"]  + $despesa[18]["inscritas"]  +
                           $despesa[19]["inscritas"];
 ...
 @$pdf->cell(18,$alt,db_formatar((($somador35_atebimestre + $somador35_inscritas)/$somador35_atualizada)*100,'f'),'TB',0,"R",0);
  ...
  $somador36_inscritas   = $somador23_inscritas   + $somador35_inscritas;
  $somador35_DespEns     = $somador36_atebimestre + $somador36_inscritas;
...
// br
  ...
  $pdf->cell(60,($alt*2),"CANCELADO EM ".$anousu,"TB",0,"C",0);
  $pdf->ln();
...
//  $pdf->cell(190,($alt-2),"",0,1,"C",0);
  ...
  for($linha=1; $linha <= 5; $linha++){
    if ($linha == 5){
      $pdf->cell(150,$alt,$fluxo[$linha]['txt'],'RB',0,"L",0);
      $pdf->cell(40,$alt,db_formatar($fluxo[$linha]["valor"],"f"),"B",0,"R",0);
    } else {
...
// Rodape  
  ...
  notasExplicativas(&$pdf,31,"{$periodo_selecionado}",190);
