...
function construirArraysDespesas($sSql, $lDiferenciaTipo = true) {
  ...
  for ($i = 0; $i < $iNumeroResultados; $i++) {
    ...
    $oStdSubfuncao->nTotalLiquidar                  = ($oStdSubfuncao->nTotalDotacaoAtualizada - $oStdSubfuncao->nTotalDespesasLiquidadasPeriodo);
...
function imprimeLinha($oPdf,$oDespesas,$sPeriodo,$sTipoPeriodo,$iAlturaLinha,$sDescricao = null,$sNivel = null, $sTipoFonte =''){
  ...
  $oPdf->cell($iColunaDotacaoInicial,      $iAlturaLinha, db_formatar($oDespesas->nTotalDotacaoInicial, 'f'),"LR",0,"R",0);
  $oPdf->cell($iColunaDotacaoAtualizado,   $iAlturaLinha, db_formatar($oDespesas->nTotalDotacaoAtualizada,'f'),"LR",0,"R",0);
  $oPdf->cell($iColunaEmpenhadoNoPeriodo,  $iAlturaLinha, db_formatar($oDespesas->nTotalDespesasEmpenhadas, 'f'),"LR",0,"R",0);
  $oPdf->cell($iColunaEmpenhadoAtePeriodo, $iAlturaLinha, db_formatar($oDespesas->nTotalDespesasEmpenhadasPeriodo,'f'),"LR",0,"R",0);
  $oPdf->cell($iColunaLiquidadoNoPeriodo,  $iAlturaLinha, db_formatar($oDespesas->nTotalDespesasLiquidadas, 'f'),"LR",0,"R",0);
  $oPdf->cell($iColunaLiquidadoAtePeriodo, $iAlturaLinha, db_formatar($oDespesas->nTotalDespesasLiquidadasPeriodo,'f'),"LR",0,"R",0);
...
$sDotacao     = db_dotacaosaldo(8, 2 , 5  ,true ,$sele_work." {$sWherePadrao}",$dtAnousu,$sDataInicio,$sDataFim,8,0,true,1,false);
...
$oPdf->cell($iColunaEmpenhadoAtePeriodo, $iAltura, db_formatar($nTotalDespesasEmpenhadasPeriodo, 'f') ,"LRTB",0,"R",0);
$oPdf->cell($iColunaLiquidadoNoPeriodo , $iAltura, db_formatar($nTotalDespesasLiquidadas       , 'f'),"LRTB",0,"R",0);
$oPdf->cell($iColunaLiquidadoAtePeriodo, $iAltura, db_formatar($nTotalDespesasLiquidadasPeriodo, 'f'),"LRTB",0,"R",0);
