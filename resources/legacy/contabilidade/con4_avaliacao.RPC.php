  ...
  case "getPerguntasPorGrupo":
    ...
    foreach ($oAvaliacao->getPerguntas($oParam->iGrupo) as $oPerguntaTemp) {
       ...
       foreach ($oPergunta->respostas as $oResposta) {
           //Evita que o campo descricaoresposta seja encondado mais de uma vez.
           //No select um nova requisição ajax é feita para filtrar o grupo de pergunta, então
           //a descrição que recebe o encoding no primeiro carregamento é novamente encodada gerando o bug.
           if (!strstr($oResposta->descricaoresposta, '%')){
