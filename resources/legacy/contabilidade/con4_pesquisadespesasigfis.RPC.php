...
$oDomXml->load('legacy_config/sigfis/despesasigfis.xml');
$aContas = $oDomXml->getElementsByTagName("despesa");
$aDespesaRetorno = array();
foreach ($aContas as $oConta) {
  ...
  if (strpos(strtolower($sDescricao), strtolower($sName)) !== false ||
    ...
    $oDespesaRetorno->cod    = $iCodigo;
    $oDespesaRetorno->label  = "{$iCodigo} - {$sDescricao}";
    $aDespesaRetorno[]       = $oDespesaRetorno;
  }

}

echo $oJson->encode($aDespesaRetorno);
