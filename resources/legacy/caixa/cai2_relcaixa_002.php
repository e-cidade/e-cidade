   ...
   $head7 = "Exercício: ".db_getsession("DB_anousu");
    ...
    for($i=0;$i<$num;$i++) {
      	...
      	if($linha++ > $TPagina || $tipoConta == '' || $tipoConta != $oSaltes->tipoconta){
			...
			$pdf->Cell(24,4,"Cod. TCE","TRB",0,"C",$preenc);
			$pdf->Cell(14,4,"Banco",$bordat,0,"C",$preenc);
			$pdf->Cell(72,4,"Descrição",$bordat,0,"C",$preenc);
		...
		$pdf->Cell(24,4,$oSaltes->c61_codtce,"TRB",0,"R",$preenc);
		$pdf->Cell(14,4,$oSaltes->c63_banco,1,0,"R",$preenc);
		$pdf->Cell(72,4,$oSaltes->c60_descr,1,0,"L",$preenc);
