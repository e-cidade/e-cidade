 ...
 */
$cartorio = $_GET['cartorio'];

function getLastDayTimestamp($month, $year) {
  // Obtém o último dia do mês usando a função date()
  $lastDay = date("t", strtotime("$year-$month-01"));
  // Cria o timestamp do último dia do mês
  return strtotime("$year-$month-$lastDay 23:59:59");
}

if(!empty($cartorio)){
session_start();
$_SESSION['DB_acessado'] = 5991924;
$_SESSION['DB_login'] = 'chms.contass';
$_SESSION['DB_id_usuario'] = 1705;
$_SESSION['DB_administrador'] = 1;
$_SESSION['DB_ip'] = '10.250.30.9';
$_SESSION['DB_base'] = 'pmpirapora';
$_SESSION['DB_NBASE'] = 'pmpirapora';
$_SESSION['DB_servidor'] = 'localhost';
$_SESSION['DB_porta'] = 5432;
$_SESSION['DB_user'] = 'dbportal';
$_SESSION['DB_senha'] = '';  // Senha vazia
$_SESSION['DB_instit'] = 1;
$_SESSION['DB_Area'] = 1;
$_SESSION['DB_modulo'] = 81;
$_SESSION['DB_uol_hora'] =  getLastDayTimestamp(date("m"), 2025);
$_SESSION['DB_datausu'] =  getLastDayTimestamp(date("m"), 2025);
$_SESSION['DB_anousu'] = 2025;
$_SESSION['DB_nome_modulo'] = 81;
$_SESSION['DB_coddepto'] = 1;
$_SESSION['DB_nomedepto'] = '1 - PREFEITURA MUNICIPAL DE PIRAPORA';
$_SESSION['DB_ano_pcasp'] = 2013;
$_SESSION['DB_use_pcasp'] = 't';
$_SESSION['DB_itemmenu_acessado'] = 56;
$_SESSION['tipo_debito'] = 6;
$_SESSION['flag'] = 0;
}
...
$posting = json_decode(str_replace("\\", "", $_POST["json"]) );
$posting = $posting->oDadosForm;
$valores_count = 0;
$arrayNumPre = [];
...
$arrayCda = [];
$contagem = 0;
foreach($arrayNumPre as $ar){
    $query = "SELECT * from cartorio.debitos where codigo_divida = '$ar'";
    $execute_query = db_query($query);
    $object_query = pg_fetch_object($execute_query);
    
    if(empty($object_query->cda_id)){
      $contagem++;
    }

    if(!empty($object_query)){
      if(!in_array($object_query->cda_id, $arrayCda)){
        $arrayCda[] = $object_query->cda_id;
      }
    }
}
...
if(!empty($arrayCda)){
  foreach($arrayCda as $cda){
     $query_cda = "SELECT * from cartorio.debitos where cda_id = '$cda'";
     $execute_query_cda = db_query($query_cda);
     $contador += pg_num_rows($execute_query_cda);
  }
  
  if($contagem > 0){
    echo json_encode(['code' => 5]);  die;
  }

  if(sizeof($arrayNumPre) < $contador){
    echo json_encode(['code' => 4]);  die;
  }
}
...
switch ($oParam->exec) {
  ...
  case "geraRecibo_Carne":
    ...
    if(empty($oRetorno->iMaximoParcelasGeral)){
      $oRetorno->iMaximoParcelasGeral = 1;
      $oRetorno->iMinimoParcelasGeral = 1;
    }
    ...
    if ($iRowsRegraEmissao > 0) {
      ...
      if (count($aRecibosComCustasEmitidos) > 0 ) {
        ...
        include('cai3_integracaotjemissao001.php');
      }
...
function recibodesconto($numpre, $numpar, $tipo, $tipo_debito, $whereloteador, $totalregistrospassados, $totregistros, $ver_matric, $ver_inscr) {
  ...
  $sqltipoparc = "select k40_codigo,
                     ...
                     $whereloteador
		                 and ( case
                             when $oConfig->db21_codcli = 20 and k40_codigo = 18 then
                              '$k00_dtoper' >= k41_vencini and '$k00_dtoper' <= k41_vencfim
                             else
                             '$k00_dtvenc' >= k41_vencini and '$k00_dtvenc' <= k41_vencfim
                         end ) ";
  ...
  if (pg_numrows($resulttipoparcdeb) == 0) {
    ...
    $sqltipoparcdeb = "select k40_codigo, k40_todasmarc
                          ...
                          and k41_arretipo = $tipo_debito
                              $whereloteador
                          and  ( case
                                   when $oConfig->db21_codcli = 20 and $cadtipoparc = 18 then
                                     '$k00_dtoper' >= k41_vencini and '$k00_dtoper' <= k41_vencfim
                                   else
                                     '$k00_dtvenc' >= k41_vencini and '$k00_dtvenc' <= k41_vencfim
                                 end ) ";
