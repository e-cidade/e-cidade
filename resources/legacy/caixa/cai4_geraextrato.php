...
require_once("classes/db_extratosaldo_classe.php");
require_once("classes/db_extratolinha_classe.php");
require_once("classes/db_bancoshistmov_classe.php");
...
require_once("classes/extratoCnab240.php");
...
if (isset($arqname) && $arqname != "") {
  ...
  $rsHistoricosNaoEncontrados    = db_query($clbancoshistmovexcecao->sql_query(null," distinct k66_historico"));
  ...
  for ($i = 0; $i < $totalLinhas; $i++) {
    ...
    if ($clextratocnab240->isHeaderArquivo($i) && $clextratocnab240->isValido($i) && $iNumRowsVerificaDataConta == 0 && $lPrimeiro) {
    ...
    } else if ($clextratocnab240->isHeaderLote($i) && $clextratocnab240->isTrailerLote($i+1)) {
      continue;
    } else if ($clextratocnab240->isHeaderLote($i) && $clextratocnab240->isValido($i)) {
    ...
    } else if ($clextratocnab240->isDetalhe($i) && $clextratocnab240->isValido($i) &&  $contaatual != '') {

      $iBancoHistmov      = (int)$clextratocnab240->getBancoHistmov($i);
      $lEncontrouHistorico = db_stdClass::inCollection("k66_historico", $iBancoHistmov, $aDadosHistoricosNaoEncontrdos);
      if ( $lEncontrouHistorico !== false ) {
        continue;
      }
      $clextratolinha->k86_extrato = $clextrato->k85_sequencial;
      $sqlCodmovimento = $clbancoshistmov->sql_query_file( null,
        "k66_sequencial",
        null,
        "    k66_historico = " . ( int ) $clextratocnab240->getBancoHistmov($i) . "
                                                            and k66_codbco    = " . ( int ) $clextrato->k85_codbco);
      ...
      } else if ($clextratocnab240->getTipoLancamento($i) == 'D') {
    ...
    foreach ( $aExtratoSaldoArray as $iConta => $aLinhas ) {
      ...
      foreach ( $aLinhas as $sData => $aColunas ) {
        ...
        $sSqlSaldoAnt = "select coalesce(k97_saldofinal,0) as k97_saldofinal
	                         from extratosaldo
	                        where k97_contabancaria = {$iConta}
	                          and k97_dtsaldofinal < '{$sData}'
