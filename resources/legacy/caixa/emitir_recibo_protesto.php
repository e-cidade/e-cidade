...
if(!empty($cartorio)){
  $parcelamento = $_GET['Parcelamento'];
  $numpre_sql = "select * from termo where v07_parcel = $parcelamento";
  $exec_numpre = db_query($numpre_sql);
  $resultado_numpre = pg_fetch_assoc($exec_numpre);
  $numpre_resultado = $resultado_numpre['v07_numpre'];
  $query_parcelamento = "select k00_numcgm as z01_numcgm, z01_nome from arrenumcgm left join arreinstit on arreinstit.k00_numpre = arrenumcgm.k00_numpre and arreinstit.k00_instit = 1 inner join cgm on z01_numcgm = k00_numcgm where arrenumcgm.k00_numpre = $numpre_resultado limit 1";
  $exec_parcelamento = db_query($query_parcelamento);
  $resultado = pg_fetch_assoc($exec_parcelamento);
  $aDados['ver_numcgm'] = $resultado['z01_numcgm'];
...
{
     if(sizeof($_SESSION) == 24){
      $query_numpref = "select * from caixa.numpref where k03_anousu = " . db_getsession('DB_anousu') .
      " and k03_instit = " . db_getsession('DB_instit') . " limit 1";
      
      $executar = db_query($query_numpref);
      $numpref = pg_fetch_object($executar);
    }else{
    ...
    if($_SESSION['flag'] == 1){
      $query_recibo = "select * from arrecadacao.recibopaga_qrcode_pix where k176_numnov = $numnov limit 1";
      $executar_recibo = db_query($query_recibo);
      $recibopagaQrcodePix = pg_fetch_object($executar_recibo);
    }else{
...
$pdf1->objpdf->output('tmp/'.$salvar.".pdf");
