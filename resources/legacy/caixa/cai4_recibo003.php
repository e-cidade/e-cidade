...
if (!isset($lGerarOutput)) {
  include_once("fpdf151/recibopdf.php");
}
...
// 								       cgm.z01_cgccpf,
// 								       cgm.z01_bairro,
// 								       ruas.j14_nome as nomepri,
// 								       issruas.q02_compl as j39_compl,
// 								       issruas.q02_numero as j39_numero,
// 								       bairro.j13_descr
// 								 from issbase
...
try {
  $oConvenio = new convenio($oRegraEmissao->getConvenio(),$k03_numpre,1,$total_recibo,$db_vlrbar,$datavencimento,6);
} catch ( Exception $eExeption ){
  db_redireciona("db_erros.php?fechar=true&db_erro={$eExeption->getMessage()}");
}
...
if (isset($lReemissao) && $lReemissao ) {
   ...
   */
	if ($k12_codautent == '') {
		include("libs/db_libpostgres.php");
...
$pdf1->descr9 = $k03_numpre."001"; //$numpre; // cod. de arrecadação

$pdf1->predescr6 = $datavencimento;  // Data de Vencimento
$pdf1->predescr7 = db_formatar(@$valor_parm,'f');  // qtd de URM ou valor
$pdf1->predescr9 = $k03_numpre."001"; //$numpre; // cod. de arrecadação
/*************************************************************************************/
