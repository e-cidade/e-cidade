...
if (isset($HTTP_POST_VARS ["codrec"]) || isset($codrec)) {
  ...
  if ($naocalcula == true) {
    ...
    if (isset($valor_variavel)) {
      ...
      $sSqlIssVar .= " where q05_numpre = ".substr($codrec, 0, 8);
      $sSqlIssVar .= "   and q05_numpar = ".substr($codrec, 8, 3);
      $result = db_query($sSqlIssVar);
    }


    $sSqlCalcula = "select fc_calcula(substr('{$codrec}',1,8)::integer,
                                      substr('{$codrec}',9,3)::integer,
                                      0,
