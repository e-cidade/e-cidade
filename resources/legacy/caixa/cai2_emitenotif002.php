 ...
 */
$sSqlDadosInstituicao  = "select nomeinst, bairro, cgc, ender,                ";
        ...
        if ($tipo_arrecadacao > 0){
          ...
          for ($i = 0;$i < 2;$i++) {
            ...
            if ($i == 0) {
              ...
              $pdf->SetX($xcol+3);
              $pdf->Setfont('Arial','',5);
              $pdf->multicell(110,3,'HISTÓRICO :   '.$pdf->historico,0,"L");
              $pdf->SetX($xcol+3);
          ...
          $pdf->Line($x+10,   $y+17, $xx+$x+178, $y+17); //horiz
          $pdf->Line($x+10,   $y+24, $xx+$x+178, $y+24); //horiz
          $pdf->Line($x+10,   $y+31, $xx+$x+178, $y+31); //horiz
          $pdf->Line($x+10,   $y+38, $xx+$x+178, $y+38); //horiz
          $pdf->Line($x+136, $y+45, $xx+$x+178, $y+45); //horiz
          $pdf->Line($x+136, $y+52, $xx+$x+178, $y+52); //horiz
          $pdf->Line($x+136, $y+59, $xx+$x+178, $y+59); //horiz
          $pdf->Line($x+136, $y+66, $xx+$x+178, $y+66); //horiz
          $pdf->Line($x+10,     $y+73, $xx+$x+178, $y+73); //horiz

          $pdf->Line($x+136, $y+9,  $x+136, $y+87); //vert
          $pdf->Line($x+156, $y+9,  $x+156, $y+17); //vert

          $pdf->Line($x+30,  $y+24, $x+30,  $y+31); //vert
          $pdf->Line($x+73,  $y+24, $x+73,  $y+31); //vert
          $pdf->Line($x+99, $y+24,  $x+99, $y+31); //vert
          $pdf->Line($x+112, $y+24, $x+112, $y+31); //vert

          $pdf->Line($x+32,  $y+31, $x+32,  $y+38); //vert
          $pdf->Line($x+52,  $y+31, $x+52,  $y+38); //vert
          $pdf->Line($x+78,  $y+31, $x+78,  $y+38); //vert
          $pdf->Line($x+108, $y+31, $x+108, $y+38); //vert
          ...
          $pdf->Text($x+12,   $y+19,"Cedente");
          $pdf->Text($x+138, $y+19,"Agência/Código Cedente");

          $pdf->Text($x+12,   $y+26,"Data do Documento");
          $pdf->Text($x+31,  $y+26,"Número do Documento");
          $pdf->Text($x+75,  $y+26,"Espécie Doc.");
          $pdf->Text($x+101, $y+26,"Aceite");
          $pdf->Text($x+114, $y+26,"Data do Processamento");
          $pdf->Text($x+138, $y+26,"Nosso Número");

          $pdf->Text($x+12,   $y+33,"Notificação");
          $pdf->Text($x+34,  $y+33,"Carteira");
          $pdf->Text($x+54,  $y+33,"Espécie");
          $pdf->Text($x+80,  $y+33,"Quantidade");
          $pdf->Text($x+110, $y+33,"Valor");
          $pdf->Text($x+138, $y+33,"( = ) Valor do Documento");

          $pdf->Text($x+12,   $y+40,"Instruções");
          $pdf->sety($y+42);
          $pdf->setx(20);
          $pdf->multicell(112,4,''.$pdf->historico);
          ...
          $pdf->Text($x+12,   $y+75,"Sacado");
          $pdf->Text($x+12,   $y+85,"Sacador/Avalista");
          ...
          $pdf->Text($x+12,   $y+30,date('d/m/Y'));                            // data do documento
          $pdf->Text($x+29,  $y+30,@$pdf->numero_documento);                   // numero do documento
          $pdf->Text($x+75,  $y+30,@$pdf->especie_doc);                        // especie do documento
          $pdf->Text($x+101, $y+30,@$pdf->aceite);                             // aceite
          $pdf->Text($x+114, $y+30,@$pdf->data_processamento);                 // data de opercao   data do processamento
          ...
          $pdf->Text($x+12,   $y+37,@$pdf->tipo_exerc);                        // codigo do cedente //
          $pdf->Text($x+34,  $y+37,@$pdf->carteira);                           // carteira
          $pdf->Text($x+54,  $y+37,@$pdf->especie);                            // especie
          $pdf->Text($x+80,  $y+37,@$pdf->quantidade);                         // quantidade
          $pdf->Text($x+110, $y+37,@$pdf->valorhis);                           // valor
          if($pdf->valtotal <> 0){
            $pdf->Text($x+138, $y+37,@$pdf->valtotal);                         // valor do documento
          }else{
            $pdf->Text($x+138, $y+37,@$pdf->valtotal);                         // valor do documento
          }
          ...
          $pdf->SetFont('Arial','',8);
          $pdf->Text($x+24,  $y+77,@$pdf->descr11_1);            // $pdf->nome);    // sacado 1
          $pdf->Text($x+24,  $y+80,@$pdf->descr11_2);            // $pdf->ender);    // sacado 2

          $pdf->text($xcol+75, $xlin+19, 'CNPJ/CPF:');
          $pdf->text($xcol+90, $xlin+19, db_formatar(@$pdf->cgccpf,(strlen(@$pdf->cgccpf)<12?'cpf':'cnpj')));

          $pdf->Text($x+24,  $y+83,trim(@$pdf->munic)." / RS / CEP-".$pdf->cep); // $pdf->munic);    // sacado 3
          $pdf->SetLineWidth(0.2);
        }

        /**
         * Fim da Ficha de Compensação
         */
          ...
          if ($dadosopcoes[0] == "parte2") {
            ...
            if (trim($dadosopcoes[1]) == "AR" && $dadosopcoes[0] == "parte3") {
              ...
              if (!isset($ender) || empty($ender)) {
                $pdf->cell(50,4,strtoupper(trim($z01_ender)),0,1,"L",0);
              } else {
                $pdf->cell(50,4,strtoupper(trim(@$ender.", ".@$numero)),0,1,"L",0);
              }
