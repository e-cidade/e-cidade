...
///////////////////////////////////////////////////////////////////////
 ...
 if (strlen($data2) < 7){
    ...
    $campos = $campos."e63_codhist,e40_codhist,e40_descr,";
    $campos = $campos."e60_vlrpag,e60_anousu,e60_coddot,o58_coddot,o58_orgao,o40_orgao,o40_descr,o58_unidade,o41_unidade,o41_descr,o15_codigo,o15_descr";
...
// monta sql
   ...
   if (isset($lista)){
       if (isset($ver) and $ver=="com"){
           $txt_where= $txt_where." and e63_codhist in  $w";
       } else {
           $txt_where= $txt_where." and e63_codhist not in  $w";
       }
   }
...
if ($tipo =="a") {
       if ($quebra == "g"){
            $ordem = "e63_codhist, e60_emiss ";
       } else if  ($quebra == "o") {
            $ordem = "e63_codhist, o58_orgao, e60_emiss";
       } else if ($quebra =="u"){
            $ordem = "e63_codhist, o58_orgao,o58_unidade, e60_emiss";
       }

       //  echo  $clempemphist->sql_query_empenho(null,$campos,$ordem,$txt_where);
       //  exit;
       $res=$clempemphist->sql_record($clempemphist->sql_query_empenho(null,$campos,$ordem,$txt_where));
       if ($clempemphist->numrows > 0 ){
           $rows=$clempemphist->numrows;
       } else {
...
// é sintético monta sql direto
   if($quebra =="g"){
        $sql = "select e63_codhist,e40_descr,
                       sum(e60_vlremp) as e60_vlremp,
                       sum(e60_vlranu) as e60_vlranu,
          	       sum(e60_vlrliq) as e60_vlrliq,
       	               sum(e60_vlrpag) as e60_vlrpag
   	        from empemphist
		     inner join empempenho on e60_numemp = e63_numemp
	             inner join emphist on e40_codhist = e63_codhist
         ...
         if (isset($lista)){
              if (isset($ver) and $ver=="com"){
                 $sql = $sql ." and e63_codhist in  $w";
              } else {
                 $sql = $sql ." and e63_codhist not in  $w";
              }
         }
         $sql = $sql ." group by e63_codhist, e40_descr  order by e63_codhist ";
	 $res=$clempempenho->sql_record($sql);
       ...
       /*  sintético por orgao */
        $sql = "select e63_codhist,e40_descr,o40_orgao,o40_descr,
                       sum(e60_vlremp) as e60_vlremp,
                       sum(e60_vlranu) as e60_vlranu,
          	       sum(e60_vlrliq) as e60_vlrliq,
       	               sum(e60_vlrpag) as e60_vlrpag
           ...
           $sql = $sql ." group by e63_codhist,e40_descr,o40_orgao,o40_descr
	                  order by e63_codhist,o40_orgao ";
           // echo $sql;
	   // exit;
	   $res=$clempempenho->sql_record($sql);
           if ($clempempenho->numrows > 0 ){
      ...
      /* unidade sintética */
       ...
       /*  sintético por orgao */
        $sql = "select e63_codhist,e40_descr,o40_orgao,o40_descr,o41_unidade,o41_descr,
                       sum(e60_vlremp) as e60_vlremp,
                       sum(e60_vlranu) as e60_vlranu,
          	       sum(e60_vlrliq) as e60_vlrliq,
       	               sum(e60_vlrpag) as e60_vlrpag
           ...
           $sql = $sql ." group by e63_codhist,e40_descr,o40_orgao,o40_descr,o41_unidade,o41_descr
	                  order by e63_codhist,o40_orgao ";
           // echo $sql;
	   // exit;
	   $res=$clempempenho->sql_record($sql);
...
if ($quebra=="g" and $tipo=="a"){
       ...
       for ($x=0; $x < $rows;$x++){
		...
		$pdf->SetFont('Arial','B',9);
                $pdf->Cell(20,$tam,"$e63_codhist",0,0,"R",0);
                $pdf->Cell(100,$tam,"$e40_descr",0,1,"L",0);  //quebra inha
  		$pdf->SetFont('Arial','',7);
	    }
...
if ($quebra=="g" and $tipo=="s"){
       ...
       for ($x=0; $x < $rows;$x++){
	    ...
	    $pdf->Ln(1);
            $pdf->Cell(20,$tam,$e63_codhist,0,0,"R",0);
 	    $pdf->Cell(100,$tam,$e40_descr ,0,0,"L",0);
       ...
       for ($x=0; $x < $rows;$x++){
	    ...
	    // testa nova pagina
            ...
            if ($imprime_header==true)
    	    {
	         ...
	         $pdf->Cell(20,$tam,strtoupper($RLe60_codemp),1,0,"C",1);
                 ...
                 $pdf->Cell(100,$tam,strtoupper($RLDBtxt_estrutural),1,1,"C",1); //quebra linha
		...
		if ($x > 1 ){
		    ...
		    */
	        ...
	        $pdf->Ln();
		$pdf->SetFont('Arial','B',9);
                $pdf->Cell(20,$tam,$e40_codhist,0,0,"R",0);
                $pdf->Cell(100,$tam,$e40_descr,0,1,"L",0);  //quebra inha
  		$pdf->SetFont('Arial','',7);
	    }
	    if ($orgao != $o40_orgao) {
	    ...
	    if ($repete != $e63_codhist){
		 ...
		 //
	         ...
	         $pdf->Ln();
                 ...
                 $pdf->Cell(20,$tam,$e63_codhist,0,0,"R",0);
 	         $pdf->Cell(100,$tam,$e40_descr ,0,1,"L",0); //quebra linha
                 $pdf->SetFont('Arial','',7);
	    }
