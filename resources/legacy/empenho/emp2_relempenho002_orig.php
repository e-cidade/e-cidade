...
//	die($clempempenho->sql_query(null,$campos,$ordem,$txt_where));
	...
	/* 
	db_msgbox("Relatório sintético não disponível!!");
	db_redireciona('db_erros.php?fechar=true&db_erro=Relatório sintético não disponível!!');  
	*/
	...
	for ($x = 0; $x < $rows; $x ++) {
       ...
       /* --------  */
		...
		if ($x == ($rows -1)) {
			...
			$pdf->Ln();
			$pdf->Cell(150, $tam, "MOVIMENTAÇÃO CONTABIL NO PERIODO", "T", 0, "L", 1);
