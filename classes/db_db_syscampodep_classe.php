...
class cl_db_syscampodep { 
   ...
   function cl_db_syscampodep() { 
     //classes dos rotulos dos campos
     $this->rotulo = new rotulo("db_syscampodep"); 
   ...
   function atualizacampos($exclusao=false) {
     if($exclusao==false){
       ...
       $this->codcampai = ($this->codcampai == ""?@$GLOBALS["HTTP_POST_VARS"]["codcampai"]:$this->codcampai);
     }else{
       $this->codcam = ($this->codcam == ""?@$GLOBALS["HTTP_POST_VARS"]["codcam"]:$this->codcam);
     }
   }
   ...
   function incluir ($codcam){ 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,397,752,'','".AddSlashes(pg_result($resaco,0,'codcam'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,397,2437,'','".AddSlashes(pg_result($resaco,0,'codcampai'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
   ...
   function alterar ($codcam=null) { 
     ...
     if(trim($this->codcampai)!="" || isset($GLOBALS["HTTP_POST_VARS"]["codcampai"])){ 
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["codcam"]))
           $resac = db_query("insert into db_acount values($acount,397,752,'".AddSlashes(pg_result($resaco,$conresaco,'codcam'))."','$this->codcam',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["codcampai"]))
           $resac = db_query("insert into db_acount values($acount,397,2437,'".AddSlashes(pg_result($resaco,$conresaco,'codcampai'))."','$this->codcampai',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){ 
   ...
   function excluir ($codcam=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,397,752,'','".AddSlashes(pg_result($resaco,$iresaco,'codcam'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,397,2437,'','".AddSlashes(pg_result($resaco,$iresaco,'codcampai'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
