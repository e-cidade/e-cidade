...
class cl_numeracaotipoproc { 
   ...
   var $campos = "
                 p200_codigo = int4 = Código 
                 p200_ano = int4 = Ano 
:                 p200_numeracao = int4 = Numeração
   ...
   function incluir ($p200_codigo){
     ...
     if($this->p200_tipoproc == null ){ 
       $this->erro_sql = " Campo Tipo Proc não informado.";
     ...
     if($p200_codigo == "" || $p200_codigo == null ){
       ...
       if(($result != false) && (pg_result($result,0,0) < $p200_codigo)){
         $this->erro_sql = " Campo p200_codigo maior que último número da sequencia.";
     ...
     if($result==false){ 
       ...
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         $this->erro_sql   = "numeracaotipoproc ($this->p200_codigo) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_banco = "numeracaotipoproc já Cadastrado";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       }else{
         $this->erro_sql   = "numeracaotipoproc ($this->p200_codigo) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
   ...
   function excluir ($p200_codigo=null,$dbwhere=null) { 
     ...
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "numeracaotipoproc nao Excluído. Exclusão Abortada.\\n";
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "numeracaotipoproc nao Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$p200_codigo;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
