...
//CLASSE DA ENTIDADE conplanocontabancaria
class cl_conplanocontabancaria { 
   ...
   function cl_conplanocontabancaria() { 
   ...
   function incluir ($c56_sequencial){ 
     ...
     if($result==false){ 
       ...
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         $this->erro_sql   = "Ligação conta bancaria com a contabilidade ($this->c56_sequencial) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,2741,15627,'','".AddSlashes(pg_result($resaco,0,'c56_sequencial'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,2741,15628,'','".AddSlashes(pg_result($resaco,0,'c56_contabancaria'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,2741,15629,'','".AddSlashes(pg_result($resaco,0,'c56_codcon'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = db_query("insert into db_acount values($acount,2741,15630,'','".AddSlashes(pg_result($resaco,0,'c56_anousu'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["c56_sequencial"]) || $this->c56_sequencial != "")
           $resac = db_query("insert into db_acount values($acount,2741,15627,'".AddSlashes(pg_result($resaco,$conresaco,'c56_sequencial'))."','$this->c56_sequencial',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["c56_contabancaria"]) || $this->c56_contabancaria != "")
           $resac = db_query("insert into db_acount values($acount,2741,15628,'".AddSlashes(pg_result($resaco,$conresaco,'c56_contabancaria'))."','$this->c56_contabancaria',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["c56_codcon"]) || $this->c56_codcon != "")
           $resac = db_query("insert into db_acount values($acount,2741,15629,'".AddSlashes(pg_result($resaco,$conresaco,'c56_codcon'))."','$this->c56_codcon',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["c56_anousu"]) || $this->c56_anousu != "")
           $resac = db_query("insert into db_acount values($acount,2741,15630,'".AddSlashes(pg_result($resaco,$conresaco,'c56_anousu'))."','$this->c56_anousu',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "Ligação conta bancaria com a contabilidade nao Alterado. Alteracao Abortada.\\n";
         $this->erro_sql .= "Valores : ".$this->c56_sequencial;
       $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       $this->erro_status = "0";
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "Ligação conta bancaria com a contabilidade nao foi Alterado. Alteracao Executada.\\n";
         $this->erro_sql .= "Valores : ".$this->c56_sequencial;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
   ...
   function excluir ($c56_sequencial=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,2741,15627,'','".AddSlashes(pg_result($resaco,$iresaco,'c56_sequencial'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,2741,15628,'','".AddSlashes(pg_result($resaco,$iresaco,'c56_contabancaria'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,2741,15629,'','".AddSlashes(pg_result($resaco,$iresaco,'c56_codcon'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,2741,15630,'','".AddSlashes(pg_result($resaco,$iresaco,'c56_anousu'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     ...
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "Ligação conta bancaria com a contabilidade nao Excluído. Exclusão Abortada.\\n";
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "Ligação conta bancaria com a contabilidade nao Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$c56_sequencial;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
