...
class cl_bensmater { 
  ...
  /**
   *
   * Função que busca o estrutural do bem para verificar se a data de garantia é obrigatoria
   * True = obrigatório
   * False = não obrigatório
   *
   */
  
  public function verificaEstrutBem($codbem){
     /**
       *
       * Ocorrência:
       * Condição que Verifica o estrutural do bem, vinculado ao empenho.
       * Caso o estrutural do bem seja 44905208 e o cliente não seja BH, ou
       * Caso o estrutural do bem seja 44905207 e o cliente seja BH, o input garantia não será obrigatório.
       *
       */

      $oCliente = pg_fetch_object(db_query("select cgc from db_config where prefeitura is true"));

      $sSqlEstruBem = "SELECT distinct substring(conplanoorcamento.c60_estrut FROM 2 FOR 8) estrut
      FROM empnotaitembenspendente
      INNER JOIN matestoqueitem ON matestoqueitem.m71_codlanc = empnotaitembenspendente.e137_matestoqueitem
      INNER JOIN empnotaitem ON empnotaitem.e72_sequencial = empnotaitembenspendente.e137_empnotaitem
      INNER JOIN matestoque ON matestoque.m70_codigo = matestoqueitem.m71_codmatestoque
      INNER JOIN empempitem ON empempitem.e62_sequencial = empnotaitem.e72_empempitem
      INNER JOIN empnota ON empnota.e69_codnota = empnotaitem.e72_codnota 
      ...
      LEFT JOIN empnotaord ON empnotaord.m72_codnota = empnota.e69_codnota 
      LEFT JOIN matordemitem ON matordemitem.m52_codordem = empnotaord.m72_codordem 
      INNER JOIN cgm ON cgm.z01_numcgm = empempenho.e60_numcgm
      INNER JOIN pcmater ON pcmater.pc01_codmater = e62_item 
      LEFT JOIN bensempnotaitem ON bensempnotaitem.e136_empnotaitem = empnotaitem.e72_sequencial 
      LEFT JOIN matordemanu ON m53_codordem = m52_codordem 
      WHERE exists (select 1 from bensempnotaitem where e136_empnotaitem = e72_sequencial and e136_bens = {$codbem})";

      $oEstruturalBem = pg_fetch_object(db_query($sSqlEstruBem));

      if($oEstruturalBem->estrut == ""){

        $sSqlEstruBem = "SELECT 
                        substring(conplanoorcamento.c60_estrut FROM 2 FOR 8) estrut
                      FROM bensmater
                      INNER JOIN empelemento ON t53_empen = e64_numemp
                      INNER JOIN empempenho ON e64_numemp = e60_numemp
                      INNER JOIN conplanoorcamento ON c60_codcon = e64_codele
                      AND c60_anousu = e60_anousu
                      WHERE t53_codbem = {$codbem}";
        $oEstruturalBem = pg_fetch_object(db_query($sSqlEstruBem));
      }
      
      if(($oEstruturalBem->estrut == "44905208" && $oCliente->cgc != "17316563000196") 
        || ($oEstruturalBem->estrut == "44905207" && $oCliente->cgc == "17316563000196")){
        return false;
      }else{
        return true;
      }
