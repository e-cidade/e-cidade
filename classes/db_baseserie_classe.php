...
class cl_baseserie { 
   ...
   function atualizacampos($exclusao=false) {
     if($exclusao==false){
       $this->ed87_i_codigo = ($this->ed87_i_codigo == ""?@$GLOBALS["HTTP_POST_VARS"]["ed87_i_codigo"]:$this->ed87_i_codigo);
       $this->ed87_i_serieinicial = ($this->ed87_i_serieinicial == ""?@$GLOBALS["HTTP_POST_VARS"]["ed87_i_serieinicial"]:$this->ed87_i_serieinicial);
       $this->ed87_i_seriefinal = ($this->ed87_i_seriefinal == ""?@$GLOBALS["HTTP_POST_VARS"]["ed87_i_seriefinal"]:$this->ed87_i_seriefinal);
     }else{
       $this->ed87_i_codigo = ($this->ed87_i_codigo == ""?@$GLOBALS["HTTP_POST_VARS"]["ed87_i_codigo"]:$this->ed87_i_codigo);
     }
   }
   ...
   function alterar ($ed87_i_codigo=null) { 
     ...
     if($ed87_i_codigo!=null){
       $sql .= " ed87_i_codigo = $this->ed87_i_codigo";
     }
     $resaco = $this->sql_record($this->sql_query_file($this->ed87_i_codigo));
     if($this->numrows>0){
     ...
     if($result==false){ 
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "Série Inicial e Final da Base MPS nao foi Alterado. Alteracao Executada.\\n";
         $this->erro_sql .= "Valores : ".$this->ed87_i_codigo;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
   ...
   function excluir ($ed87_i_codigo=null,$dbwhere=null) { 
     ...
     if($dbwhere==null || $dbwhere ==""){
        if($ed87_i_codigo != ""){
          ...
          $sql2 .= " ed87_i_codigo = $ed87_i_codigo ";
        }
     }else{
       $sql2 = $dbwhere;
     }
     ...
     if($result==false){ 
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "Série Inicial e Final da Base MPS nao Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$ed87_i_codigo;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
