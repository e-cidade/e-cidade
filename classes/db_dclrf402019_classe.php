<?
//MODULO: sicom
//CLASSE DA ENTIDADE dclrf402019
class cl_dclrf402019 {
   ...
   function cl_dclrf402019() {
     //classes dos rotulos dos campos
     $this->rotulo = new rotulo("dclrf402019");
   ...
   function incluir ($si193_sequencial){
      $this->atualizacampos();
	   if($si193_sequencial == "" || $si193_sequencial == null ){
		   $result = @pg_query("select nextval('dclrf402019_si193_sequencial_seq')");
		   if($result==false){
			   $this->erro_banco = str_replace("\n","",@pg_last_error());
			   $this->erro_sql   = "Verifique o cadastro da sequencia: dclrf402019_si192_sequencial_seq do campo: si193_sequencial";
		   ...
		   $result = @pg_query("select last_value from dclrf402019_si193_sequencial_seq");
...
//     $sql2 = "si193_reg10 = $si193_reg10";
     ...
     if($result==false){
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "dclrf402019 nao Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
         return true;
   ...
   function sql_query ( $si193_sequencial = null,$campos="*",$ordem=null,$dbwhere=""){
     ...
     if($dbwhere==""){
       if( $si193_sequencial != "" && $si193_sequencial != null){
          $sql2 = " where dclrf402019.si193_sequencial = $si193_sequencial";
       }
     }else if($dbwhere != ""){
       $sql2 = " where $dbwhere";
     }
