...
class cl_lab_valorrefselgrupo { 
   ...
   var $campos = "
                 la51_i_codigo = int4 = Código 
                 la51_i_valorrefsel = int4 = Valor Referencial Selecionável 
                 la51_i_referencia = int4 = Referência 
                 ";
   ...
   function incluir ($la51_i_codigo){ 
     ...
     if($this->la51_i_valorrefsel == null ){ 
       $this->erro_sql = " Campo Valor Referencial Selecionável nao Informado.";
     ...
     if($la51_i_codigo == "" || $la51_i_codigo == null ){
       ...
       if(($result != false) && (pg_result($result,0,0) < $la51_i_codigo)){
         $this->erro_sql = " Campo la51_i_codigo maior que último número da sequencia.";
     ...
     if($result==false){ 
       ...
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         $this->erro_sql   = "lab_valorrefselgrupo ($this->la51_i_codigo) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_banco = "lab_valorrefselgrupo já Cadastrado";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       }else{
         $this->erro_sql   = "lab_valorrefselgrupo ($this->la51_i_codigo) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
   ...
   function excluir ($la51_i_codigo=null,$dbwhere=null) { 
     ...
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "lab_valorrefselgrupo nao Excluído. Exclusão Abortada.\\n";
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "lab_valorrefselgrupo nao Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$la51_i_codigo;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
