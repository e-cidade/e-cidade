<?
//MODULO: sicom
//CLASSE DA ENTIDADE regadesao102025
class cl_regadesao102025
{
  ...
  var $campos = "
                 ...
                 si67_codorgao = varchar(2) = C�digo do �rg�o
                 si67_codunidadesub = varchar(8) = C�digo da unidade
                 si67_nroprocadesao = varchar(12) = N�mero do  processo de  ades�o
                 si63_exercicioadesao = int8 = Exerc�cio do processo de ades�o
                 si67_dtabertura = date = Data de abertura  do processo
                 si67_cnpjorgaogerenciador = varchar(100) = Nome do �rg�o
                 si67_exerciciolicitacao = int8 = Exerc�cio em que   foi instaurado
                 si67_nroprocessolicitatorio = varchar(20) = N�mero sequencial  do processo
                 si67_codmodalidadelicitacao = int8 = Modalidade da Licita��o
                 si67_dtataregpreco = date = Data da Ata do  Registro de Pre�os
                 si67_dtvalidade = date = Data de validade
                 si67_naturezaprocedimento = int8 = Natureza do Procedimento
                 si67_dtpublicacaoavisointencao = date = Data de publica��o  do aviso
                 si67_objetoadesao = varchar(500) = Objeto da Ades�o
                 si67_cpfresponsavel = varchar(11) = CPF do  respons�vel
                 si67_processoporlote = int8 = Processo por Lote
                 si67_tipocadastro = int8 = Cadastro inicial
                 si67_nroedital = int8 = N�mero do edital
                 si67_exercicioedital = int8 = Exerc�cio do edital
                 si67_mes = int8 = M�s
                 si67_instit = int8 = Institui��o
                 si67_leidalicitacao = int8 = Lei da Licita��o
                 si67_regimecontratacao int8 = regime de contratacao
                 si67_tipocriterio int8 = tipo criterio
                 ";

  //funcao construtor da classe
  function cl_regadesao102025()
  {
    //classes dos rotulos dos campos
    $this->rotulo = new rotulo("regadesao102025");
  ...
  function incluir($si67_sequencial)
  {
    ...
    if ($si67_sequencial == "" || $si67_sequencial == null) {
      $result = db_query("select nextval('regadesao102025_si67_sequencial_seq')");
      if ($result == false) {
        $this->erro_banco = str_replace("", "", @pg_last_error());
        $this->erro_sql = "Verifique o cadastro da sequencia: regadesao102025_si67_sequencial_seq do campo: si67_sequencial";
        $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
        $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
        $this->erro_status = "0";
        return false;
      ...
      $result = db_query("select last_value from regadesao102025_si67_sequencial_seq");
      if (($result != false) && (pg_result($result, 0, 0) < $si67_sequencial)) {
        $this->erro_sql = " Campo si67_sequencial maior que �ltimo n�mero da sequencia.";
    ...
    $sql = "insert into regadesao102025(
                                       si67_sequencial
                                      ,si67_tiporegistro
                                      ,si67_codorgao
                                      ,si67_codunidadesub
    ...
    if ($result == false) {
      ...
      if (strpos(strtolower($this->erro_banco), "duplicate key") != 0) {
        $this->erro_sql = "regadesao102025 ($this->si67_sequencial) nao Inclu�do. Inclusao Abortada.";
        $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
        $this->erro_banco = "regadesao102025 j� Cadastrado";
        $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
      } else {
        $this->erro_sql = "regadesao102025 ($this->si67_sequencial) nao Inclu�do. Inclusao Abortada.";
        $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
  ...
  {
    $this->atualizacampos();
    $sql = " update regadesao102025 set ";
    $virgula = "";
    ...
    if (trim($this->si67_leidalicitacao) != "" || isset($GLOBALS["HTTP_POST_VARS"]["si67_leidalicitacao"])) {
      ...
      if (trim($this->si67_leidalicitacao) == null) {
        $this->erro_sql = " Campo da Licita��o n�o Informado.";
...
", "", @pg_last_error());
      $this->erro_sql = "regadesao102025 nao Alterado. Alteracao Abortada.\n";
      $this->erro_sql .= "Valores : " . $this->si67_sequencial;
      $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
      $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
      $this->erro_status = "0";
      ...
      if (pg_affected_rows($result) == 0) {
        $this->erro_banco = "";
        $this->erro_sql = "regadesao102025 nao foi Alterado. Alteracao Executada.\n";
        $this->erro_sql .= "Valores : " . $this->si67_sequencial;
        $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
        $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
        $this->erro_status = "1";
  ...
  function excluir($si67_sequencial = null, $dbwhere = null)
  {
    ...
    $sql = " delete from regadesao102025
                    where ";
    $sql2 = "";
    if ($dbwhere == null || $dbwhere == "") {
      if ($si67_sequencial != "") {
    ...
    if ($result == false) {
      $this->erro_banco = str_replace("
", "", @pg_last_error());
      $this->erro_sql = "regadesao102025 nao Exclu�do. Exclus�o Abortada.\n";
      ...
      if (pg_affected_rows($result) == 0) {
        $this->erro_banco = "";
        $this->erro_sql = "regadesao102025 nao Encontrado. Exclus�o n�o Efetuada.\n";
        $this->erro_sql .= "Valores : " . $si67_sequencial;
        $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
        $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
        $this->erro_status = "1";
...
", "", @pg_last_error());
    ...
    if ($this->numrows == 0) {
      $this->erro_banco = "";
      $this->erro_sql = "Record Vazio na Tabela:regadesao102025";
      $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
      $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
      $this->erro_status = "0";
      return false;
  ...
  function sql_query($si67_sequencial = null, $campos = "*", $ordem = null, $dbwhere = "")
  {
    ...
    $sql .= " from regadesao102025 ";
    $sql2 = "";
    if ($dbwhere == "") {
      if ($si67_sequencial != null) {
        $sql2 .= " where regadesao102025.si67_sequencial = $si67_sequencial ";
      }
    } else if ($dbwhere != "") {
      $sql2 = " where $dbwhere";
    }
