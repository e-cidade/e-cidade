<?
//MODULO: sicom
//CLASSE DA ENTIDADE reglic202025
class cl_reglic202025
{
  ...
  var $campos = "
                 ...
                 si45_codorgao = varchar(2) = C�digo do �rg�o
                 si45_regulamentart47 = int8 = Identifica
                 si45_nronormareg = varchar(6) = N�mero da norma
                 si45_datanormareg = date = Data da norma
                 si45_datapubnormareg = date = Data de publica��o
                 si45_regexclusiva = int8 = Identifica
                 si45_artigoregexclusiva = varchar(6) = Artigo da regulamenta��o exclusiva
                 si45_valorlimiteregexclusiva = float8 = Valor Limite
                 si45_procsubcontratacao = int8 = Identifica
                 si45_artigoprocsubcontratacao = varchar(6) = Artigo do percentual contrata��o
                 si45_percentualsubcontratacao = float8 = Percentual estabelecido
                 si45_criteriosempenhopagamento = int8 = crit�rios ?
                 si45_artigoempenhopagamento = varchar(6) = Artigo relativo
                 si45_estabeleceuperccontratacao = int8 = estabeleceu ?
                 si45_artigoperccontratacao = varchar(6) = Artigo do percentual contrata��o
                 si45_percentualcontratacao = float8 = Percentual estabelecido
                 si45_mes = int8 = M�s
                 si45_instit = int8 = Institui��o
                 ";

  //funcao construtor da classe
  function cl_reglic202025()
  {
    //classes dos rotulos dos campos
    $this->rotulo = new rotulo("reglic202025");
  ...
  function incluir($si45_sequencial)
  {
    ...
    if ($si45_sequencial == "" || $si45_sequencial == null) {
      $result = db_query("select nextval('reglic202025_si45_sequencial_seq')");
      if ($result == false) {
        $this->erro_banco = str_replace("
", "", @pg_last_error());
        $this->erro_sql = "Verifique o cadastro da sequencia: reglic202025_si45_sequencial_seq do campo: si45_sequencial";
        $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
        $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
        $this->erro_status = "0";
      ...
      $result = db_query("select last_value from reglic202025_si45_sequencial_seq");
      if (($result != false) && (pg_result($result, 0, 0) < $si45_sequencial)) {
        $this->erro_sql = " Campo si45_sequencial maior que �ltimo n�mero da sequencia.";
...
", "", @pg_last_error());
      if (strpos(strtolower($this->erro_banco), "duplicate key") != 0) {
        $this->erro_sql = "reglic202025 ($this->si45_sequencial) nao Inclu�do. Inclusao Abortada.";
        $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
        $this->erro_banco = "reglic202025 j� Cadastrado";
        $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
      } else {
        $this->erro_sql = "reglic202025 ($this->si45_sequencial) nao Inclu�do. Inclusao Abortada.";
        $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
  ...
  function alterar($si45_sequencial = null)
  {
    ...
    if ($result == false) {
      $this->erro_banco = str_replace("
", "", @pg_last_error());
      $this->erro_sql = "reglic202025 nao Alterado. Alteracao Abortada.\n";
      $this->erro_sql .= "Valores : " . $this->si45_sequencial;
      $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
      $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
      $this->erro_status = "0";
      ...
      if (pg_affected_rows($result) == 0) {
        $this->erro_banco = "";
        $this->erro_sql = "reglic202025 nao foi Alterado. Alteracao Executada.\n";
        $this->erro_sql .= "Valores : " . $this->si45_sequencial;
        $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
        $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
        $this->erro_status = "1";
  ...
  function excluir($si45_sequencial = null, $dbwhere = null)
  {
    ...
    if ($result == false) {
      $this->erro_banco = str_replace("
", "", @pg_last_error());
      $this->erro_sql = "reglic202025 nao Exclu�do. Exclus�o Abortada.\n";
      ...
      if (pg_affected_rows($result) == 0) {
        $this->erro_banco = "";
        $this->erro_sql = "reglic202025 nao Encontrado. Exclus�o n�o Efetuada.\n";
        $this->erro_sql .= "Valores : " . $si45_sequencial;
        $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
        $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
        $this->erro_status = "1";
...
", "", @pg_last_error());
    ...
    if ($this->numrows == 0) {
      $this->erro_banco = "";
      $this->erro_sql = "Record Vazio na Tabela:reglic202025";
      $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
      $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
      $this->erro_status = "0";
  ...
  function sql_query($si45_sequencial = null, $campos = "*", $ordem = null, $dbwhere = "")
  {
    ...
    $sql .= " from reglic202025 ";
    $sql2 = "";
    if ($dbwhere == "") {
      if ($si45_sequencial != null) {
        $sql2 .= " where reglic202025.si45_sequencial = $si45_sequencial ";
      }
    } else if ($dbwhere != "") {
      $sql2 = " where $dbwhere";
    }
