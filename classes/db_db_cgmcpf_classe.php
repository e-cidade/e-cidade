...
class cl_db_cgmcpf { 
   ...
   function atualizacampos($exclusao=false) {
     if($exclusao==false){
       ...
       $this->z01_cpf = ($this->z01_cpf == ""?@$GLOBALS["HTTP_POST_VARS"]["z01_cpf"]:$this->z01_cpf);
     }else{
       $this->z01_numcgm = ($this->z01_numcgm == ""?@$GLOBALS["HTTP_POST_VARS"]["z01_numcgm"]:$this->z01_numcgm);
     }
   }
   ...
   function incluir ($z01_numcgm){ 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,136,216,'','".AddSlashes(pg_result($resaco,0,'z01_numcgm'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["z01_numcgm"]))
           $resac = db_query("insert into db_acount values($acount,136,216,'".AddSlashes(pg_result($resaco,$conresaco,'z01_numcgm'))."','$this->z01_numcgm',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["z01_cpf"]))
           $resac = db_query("insert into db_acount values($acount,136,731,'".AddSlashes(pg_result($resaco,$conresaco,'z01_cpf'))."','$this->z01_cpf',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){ 
   ...
   function excluir ($z01_numcgm=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,136,216,'','".AddSlashes(pg_result($resaco,$iresaco,'z01_numcgm'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,136,731,'','".AddSlashes(pg_result($resaco,$iresaco,'z01_cpf'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $sql = " delete from db_cgmcpf
                    where ";
     $sql2 = "";
     if($dbwhere==null || $dbwhere ==""){
        if($z01_numcgm != ""){
   ...
   function sql_record($sql) { 
     ...
     $this->numrows = pg_numrows($result);
      if($this->numrows==0){
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:db_cgmcpf";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        $this->erro_status = "0";
        return false;
   ...
   function sql_query ( $z01_numcgm=null,$campos="*",$ordem=null,$dbwhere=""){ 
     ...
     $sql .= " from db_cgmcpf ";
     $sql .= "      inner join cgm  on  cgm.z01_numcgm = db_cgmcpf.z01_numcgm";
     $sql2 = "";
     if($dbwhere==""){
       if($z01_numcgm!=null ){
