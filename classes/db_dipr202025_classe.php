...
class cl_dipr202025
{
    ...
    function cl_dipr202025()
    {
        //classes dos rotulos dos campos
        $this->rotulo = new rotulo("dipr202025");
    ...
    function incluir($si231_sequencial)
    {
        ...
        if ($si231_sequencial == "" || $si231_sequencial == null) {
            $result = db_query("select nextval('dipr202025_si231_sequencial_seq');");
            if ($result == false) {
                $this->erro_banco = str_replace("", "", @pg_last_error());
                $this->erro_sql = "Verifique o cadastro da sequencia: dipr202025_si231_sequencial_seq do campo: si231_sequencial";
                $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
                $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
                $this->erro_status = "0";
            ...
            $result = db_query("select last_value from dipr202025_si231_sequencial_seq");
            if (($result != false) && (pg_result($result, 0, 0) < $si231_sequencial)) {
                $this->erro_sql = " Campo si231_sequencial maior que �ltimo n�mero da sequencia.";
        ...
        if ($result == false) {
            ...
            if (strpos(strtolower($this->erro_banco), "duplicate key") != 0) {
                $this->erro_sql = "dipr202025 ($this->si231_sequencial) nao Inclu�do. Inclusao Abortada.";
                $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
                $this->erro_banco = "dipr202025 j� Cadastrado";
                $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
            } else {
                $this->erro_sql = "dipr202025 ($this->si231_sequencial) nao Inclu�do. Inclusao Abortada.";
                $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
    ...
    function excluir($si231_sequencial = null, $dbwhere = null)
    {
        ...
        if ($result == false) {
            $this->erro_banco = str_replace("", "", @pg_last_error());
            $this->erro_sql = "dipr202025 nao Exclu�do. Exclus�o Abortada.\n";
            ...
            if (pg_affected_rows($result) == 0) {
                $this->erro_banco = "";
                $this->erro_sql = "dipr202025 nao Encontrado. Exclus�o n�o Efetuada.\n";
                $this->erro_sql .= "Valores : " . $si231_sequencial;
                $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
                $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
                $this->erro_status = "1";
    ...
    function sql_record($sql)
    {
        ...
        if ($this->numrows == 0) {
            $this->erro_banco = "";
            $this->erro_sql = "Record Vazio na Tabela:dipr202025";
            $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
            $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
            $this->erro_status = "0";
    ...
    function sql_query($si231_sequencial = null, $campos = "*", $ordem = null, $dbwhere = "")
    {
        ...
        if ($dbwhere == "") {
            if ($si231_sequencial != null) {
                $sql2 .= " where dipr202025.si231_sequencial = $si231_sequencial ";
            }
        } else if ($dbwhere != "") {
            $sql2 = " where $dbwhere";
        }
