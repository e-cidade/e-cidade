...
class empenho {
    ...
    if ($e60_anousu < $iAnoSessao){
      ...
      */
      ...
      if (USE_PCASP) {
        ...
        if ($oCodigoGrupoContaOrcamento && !$lIsPassivo) {
  ...
  function estornaLiq($numemp = "", $codele = "", $codnota = "", $valor = "", $historico = "", $dDataEstorno = null) {
    ...
    if ($iAnoEstorno == $oEmpenho->e60_anousu) {
      ...
      if ( ! cl_translan::possuiLancamentoDeControle($oEmpenhoFinanceiro->getNumero(), false) && in_array($documento, $aDocumentosControleLiquidacao) ) {
        $documento = 4;
      }
          ...
          if ($this->iNumRowsItens > 0){
            ...
            for ($i = 0;$i < $this->iNumRowsItens;$i++){
              ...
              $strJson["data"][] = array (
                ...
                "saldodiferenca"    => $objNotas->saldocentavos,
                "e62_vlrtot"        => round($objNotas->saldovalor,2),
                "servicoquantidade" => $objNotas->servicoquantidade,
                "libera"            => $checked
              );
            }//end for
          }
...
//        }
  ...
  function anularEmpenho($aItens , $nValorAnular = 0,$sMotivo = null, $aSolicitacoes = null, $iTipoAnulacao, $lTransacao = true, $id_usuario = null){
    ...
    $sSqlVerificacao .= date("Y-m-d",db_getsession("DB_datausu"))."',{$iCodigoDocumento},".round($nValorAnular,2).") as verificacao";
    ...
    if ((USE_PCASP && ParametroIntegracaoPatrimonial::possuiIntegracaoContrato($oDataImplantacao, $oInstituicao))) {
        ...
        if (!$this->lSqlErro && $oDaoEmpenhoContrato->numrows > 0) {
            ...
            try {
                // a partir de 2022 os lançamento serão feitos no contrato.
                $oAcordo = new Acordo(db_utils::fieldsMemory($rsContrato, 0)->e100_acordo);
                if(db_getsession("DB_anousu") < 2022 && $acordoLancamento == true){
...
//                $oEmpItem->saldovalor -= $aItensNota[$iItens]->e72_valor;
    ...
    switch ($oDadosLancamento->tipoevento) {
      ...
      case 20:
        ...
        $oNota = db_utils::fieldsMemory($this->getNotas($this->numemp,
          "e69_codnota = $oDadosLancamento->codigonotafiscal"),0);
        if ($oNota->e50_codord != "") {
  ...
  function excluirLancamento($iCodigoLancamento) {
    ...
    switch ($oDadosLancamento->tipoevento) {
      ...
      case 20: //Liquidacoes
         ...
         */
        $oNota = db_utils::fieldsMemory($this->getNotas($this->numemp,
          "e69_codnota = $oDadosLancamento->codigonotafiscal",false),0);
        if ($oNota->e50_codord != "") {
  ...
  protected function excluirLancamentoEstornoLiquidacao($oDadosLancamento) {
    ...
    $oDaoPagOrdemNota = db_utils::getDao("pagordemnota");
    $sSqlOrdens       = $oDaoPagOrdemNota->sql_query_valorordem(null, $oDadosLancamento->codigonotafiscal,
        ...
        foreach ($aMovimentos as $oMovimento) {
          ...
          if ($oMovimento->e97_codforma == 2 && $oMovimento->e91_cheque != "") {
            ...
            if (db_strtotime($dtLancamento) < db_strtotime($oMovimento->e86_data)) {
              ...
              $sMsgErro .= "com o movimento '{$oMovimento->e81_codmov}' com cheque  número ";
              $sMsgErro .= "'{$oMovimento->e91_cheque}' Emitido com a data de ".db_formatar($oMovimento->e86_data,"d").".\n";
   ...
   * @return bool
   */
  public static function possuiLancamentoDeControle($iNumeroEmpenho, $iAno, array $aDocumentos) {
