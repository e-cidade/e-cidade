...
class cl_cadvenc { 
   ...
   function alterar_alt ($q82_codigo=null,$q82_parc=null,$dbwhere="") { 
     ...
     if($this->numrows>0){       $resac = pg_query("select nextval('db_acount_id_acount_seq') as acount");
       $acount = pg_result($resac,0,0);
       $resac = pg_query("insert into db_acountkey values($acount,264,'$this->q82_codigo','A')");
       $resac = pg_query("insert into db_acountkey values($acount,265,'$this->q82_parc','A')");
       if(isset($GLOBALS["HTTP_POST_VARS"]["q82_codigo"]))
         $resac = pg_query("insert into db_acount values($acount,53,264,'".pg_result($resaco,0,'q82_codigo')."','$this->q82_codigo',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       if(isset($GLOBALS["HTTP_POST_VARS"]["q82_parc"]))
         $resac = pg_query("insert into db_acount values($acount,53,265,'".pg_result($resaco,0,'q82_parc')."','$this->q82_parc',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       if(isset($GLOBALS["HTTP_POST_VARS"]["q82_venc"]))
         $resac = pg_query("insert into db_acount values($acount,53,266,'".pg_result($resaco,0,'q82_venc')."','$this->q82_venc',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       if(isset($GLOBALS["HTTP_POST_VARS"]["q82_desc"]))
         $resac = pg_query("insert into db_acount values($acount,53,267,'".pg_result($resaco,0,'q82_desc')."','$this->q82_desc',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       if(isset($GLOBALS["HTTP_POST_VARS"]["q82_perc"]))
         $resac = pg_query("insert into db_acount values($acount,53,268,'".pg_result($resaco,0,'q82_perc')."','$this->q82_perc',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       if(isset($GLOBALS["HTTP_POST_VARS"]["q82_hist"]))
         $resac = pg_query("insert into db_acount values($acount,53,269,'".pg_result($resaco,0,'q82_hist')."','$this->q82_hist',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }     
