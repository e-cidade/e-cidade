...
class cl_sau_upsparalisada { 
   ...
   var $campos = "
                 s140_i_codigo = int4 = Código 
                 s140_i_unidade = int4 = Unidade 
                 s140_d_inicio = date = Início 
                 s140_d_fim = date = Fim 
                 s140_i_tipo = int4 = Motivo da paralisação 
   ...
   function incluir ($s140_i_codigo){ 
     ...
     if($s140_i_codigo == "" || $s140_i_codigo == null ){
       ...
       if(($result != false) && (pg_result($result,0,0) < $s140_i_codigo)){
         $this->erro_sql = " Campo s140_i_codigo maior que último número da sequencia.";
     ...
     if($result==false){ 
       ...
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         $this->erro_sql   = "sau_upsparalisada ($this->s140_i_codigo) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_banco = "sau_upsparalisada já Cadastrado";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       }else{
         $this->erro_sql   = "sau_upsparalisada ($this->s140_i_codigo) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
   ...
   function excluir ($s140_i_codigo=null,$dbwhere=null) { 
     ...
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "sau_upsparalisada nao Excluído. Exclusão Abortada.\\n";
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "sau_upsparalisada nao Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$s140_i_codigo;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
