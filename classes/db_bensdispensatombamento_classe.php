...
class cl_bensdispensatombamento
{
  ...
  var $campos = "
                 ...
                 e139_codordem = int4 = Cdigo da ordem de compra
                 ";
  ...
  function incluir($e139_sequencial)
  {
    ...
    if ($this->e139_empnotaitem == null) {
      $this->erro_sql = " Campo Item nota de empenho no informado.";
    ...
    if ($this->e139_codcla == null) {
      $this->erro_sql = " Campo Codigo da Classificacao no informado.";
    ...
    if ($this->e139_justificativa == null) {
      $this->erro_sql = " Campo Justificativa no informado.";
    ...
    if ($e139_sequencial == "" || $e139_sequencial == null) {
      ...
      if (($result != false) && (pg_result($result, 0, 0) < $e139_sequencial)) {
        $this->erro_sql = " Campo e139_sequencial maior que ltimo nmero da sequencia.";
    ...
    if ($result == false) {
      ...
      if (strpos(strtolower($this->erro_banco), "duplicate key") != 0) {
        $this->erro_sql   = "Dispensa de tombamento ($this->e139_sequencial) nao Includo. Inclusao Abortada.";
        $this->erro_msg   = "Usurio: \\n\\n " . $this->erro_sql . " \\n\\n";
        $this->erro_banco = "Dispensa de tombamento j Cadastrado";
        $this->erro_msg   .=  str_replace('"', "", str_replace("'", "",  "Administrador: \\n\\n " . $this->erro_banco . " \\n"));
      } else {
        $this->erro_sql   = "Dispensa de tombamento ($this->e139_sequencial) nao Includo. Inclusao Abortada.";
        $this->erro_msg   = "Usurio: \\n\\n " . $this->erro_sql . " \\n\\n";
  ...
  function alterar($e139_sequencial = null)
  {
    ...
    if (trim($this->e139_sequencial) != "" || isset($GLOBALS["HTTP_POST_VARS"]["e139_sequencial"])) {
      ...
      if (trim($this->e139_sequencial) == null) {
        $this->erro_sql = " Campo Codigo sequencial no informado.";
    ...
    if (trim($this->e139_matestoqueitem) != "" || isset($GLOBALS["HTTP_POST_VARS"]["e139_matestoqueitem"])) {
      ...
      if (trim($this->e139_matestoqueitem) == null) {
        $this->erro_sql = " Campo Item da entrada da ordem de compra no informado.";
  ...
  function excluir($e139_sequencial = null, $dbwhere = null)
  {
    ...
    if ($result == false) {
      $this->erro_banco = str_replace("\n", "", @pg_last_error());
      $this->erro_sql   = "Dispensa de tombamento nao Excludo. Excluso Abortada.\\n";
      ...
      if (pg_affected_rows($result) == 0) {
        $this->erro_banco = "";
        $this->erro_sql = "Dispensa de tombamento nao Encontrado. Excluso no Efetuada.\\n";
        $this->erro_sql .= "Valores : " . $e139_sequencial;
        $this->erro_msg   = "Usurio: \\n\\n " . $this->erro_sql . " \\n\\n";
        $this->erro_msg   .=  str_replace('"', "", str_replace("'", "",  "Administrador: \\n\\n " . $this->erro_banco . " \\n"));
        $this->erro_status = "1";
  ...
  function get_codpcmater($e139_sequencial)
  {

    if ($e139_sequencial == "") $e139_sequencial = "null";

    $result = db_query("select pc01_codmater from bensdispensatombamento
    ...
    inner join pcmater on pc01_codmater = m63_codpcmater where e139_sequencial = $e139_sequencial");
    ...
    return db_utils::fieldsMemory($result, 0)->pc01_codmater;
  }
