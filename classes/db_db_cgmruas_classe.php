...
class cl_db_cgmruas { 
   ...
   function incluir ($z01_numcgm){ 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,138,216,'','".AddSlashes(pg_result($resaco,0,'z01_numcgm'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["z01_numcgm"]))
           $resac = db_query("insert into db_acount values($acount,138,216,'".AddSlashes(pg_result($resaco,$conresaco,'z01_numcgm'))."','$this->z01_numcgm',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["j14_codigo"]))
           $resac = db_query("insert into db_acount values($acount,138,53,'".AddSlashes(pg_result($resaco,$conresaco,'j14_codigo'))."','$this->j14_codigo',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){ 
   ...
   function excluir ($z01_numcgm=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,138,216,'','".AddSlashes(pg_result($resaco,$iresaco,'z01_numcgm'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,138,53,'','".AddSlashes(pg_result($resaco,$iresaco,'j14_codigo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $sql = " delete from db_cgmruas
                    where ";
     $sql2 = "";
     if($dbwhere==null || $dbwhere ==""){
        if($z01_numcgm != ""){
   ...
   function sql_record($sql) { 
     ...
     $this->numrows = pg_numrows($result);
      if($this->numrows==0){
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:db_cgmruas";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        $this->erro_status = "0";
        return false;
   ...
   function sql_query ( $z01_numcgm=null,$campos="*",$ordem=null,$dbwhere=""){ 
     ...
     $sql .= " from db_cgmruas ";
     $sql .= "      inner join ruas  on  ruas.j14_codigo = db_cgmruas.j14_codigo";
     $sql .= "      inner join cgm  on  cgm.z01_numcgm = db_cgmruas.z01_numcgm";
     $sql2 = "";
     if($dbwhere==""){
       if($z01_numcgm!=null ){
