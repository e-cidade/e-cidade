...
class cl_lab_coletaitem
{
  ...
  var $campos = "
                 la32_i_codigo = int4 = CÃ³digo 
                 la32_i_usuario = int4 = Usuário 
                 la32_i_requiitem = int4 = RequisiÃ§Ã£o 
  ...
  function incluir($la32_i_codigo)
  {
    ...
    if ($this->la32_i_requiitem == null) {
      $this->erro_sql = " Campo RequisiÃ§ao nao Informado.";
    ...
    if ($result == false) {
      ...
      if (strpos(strtolower($this->erro_banco), "duplicate key") != 0) {
        $this->erro_sql   = "Coleta do Item ($this->la32_i_codigo) nao IncluÃ­do. Inclusao Abortada.";
        $this->erro_msg   = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
        $this->erro_banco = "Coleta do Item jÃ¡ Cadastrado";
        $this->erro_msg   .=  str_replace('"', "", str_replace("'", "",  "Administrador: \\n\\n " . $this->erro_banco . " \\n"));
      } else {
        $this->erro_sql   = "Coleta do Item ($this->la32_i_codigo) nao IncluÃ­do. Inclusao Abortada.";
        $this->erro_msg   = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
  ...
  function alterar($la32_i_codigo = null)
  {
    ...
    if (trim($this->la32_i_requiitem) != "" || isset($GLOBALS["HTTP_POST_VARS"]["la32_i_requiitem"])) {
      ...
      if (trim($this->la32_i_requiitem) == null) {
        $this->erro_sql = " Campo RequisiÃ§Ã£o nao Informado.";
        $this->erro_campo = "la32_i_requiitem";
        $this->erro_banco = "";
        $this->erro_msg   = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
        $this->erro_msg   .=  str_replace('"', "", str_replace("'", "",  "Administrador: \\n\\n " . $this->erro_banco . " \\n"));
  ...
  function excluir($la32_i_codigo = null, $dbwhere = null)
  {
    ...
    if ($result == false) {
      $this->erro_banco = str_replace("\n", "", @pg_last_error());
      $this->erro_sql   = "Coleta do Item nao ExcluÃ­do. ExclusÃ£o Abortada.\\n";
      ...
      if (pg_affected_rows($result) == 0) {
        $this->erro_banco = "";
        $this->erro_sql = "Coleta do Item nao Encontrado. ExclusÃ£o não Efetuada.\\n";
        $this->erro_sql .= "Valores : " . $la32_i_codigo;
        $this->erro_msg   = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
        $this->erro_msg   .=  str_replace('"', "", str_replace("'", "",  "Administrador: \\n\\n " . $this->erro_banco . " \\n"));
        $this->erro_status = "1";
