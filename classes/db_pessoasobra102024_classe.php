<?php
//MODULO: sicom
//CLASSE DA ENTIDADE pessoasobra102024
class cl_pessoasobra102024 {
  ...
  function __construct() {
    ...
    $this->rotulo = new rotulo("pessoasobra102024");
  ...
  function incluir () {
    ...
    if ($this->si194_sequencial == null ) {
      $result = db_query("select nextval('pessoasobra102024_si194_sequencial_seq')");
      if($result==false){
        $this->erro_banco = str_replace("\n","",@pg_last_error());
        $this->erro_sql   = "Verifique o cadastro da sequencia: pessoasobra102024_si194_sequencial_seq do campo: si194_sequencial";
  ...
  function alterar ( $si194_sequencial=null ) {
    $this->atualizacampos();
    $sql = " update pessoasobra102024 set ";
    $virgula = "";
  ...
  function excluir ( $si194_sequencial=null ,$dbwhere=null) {
    ...
    $sql = " delete from pessoasobra102024
                    where ";
    $sql2 = "";
  ...
  function sql_record($sql) {
    ...
    if ($this->numrows==0) {
      $this->erro_banco = "";
      $this->erro_sql   = "Record Vazio na Tabela:pessoasobra102024";
      $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
      $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
      $this->erro_status = "0";
      return false;
  ...
  function sql_query ( $si194_sequencial = null,$campos="*",$ordem=null,$dbwhere="") {
    ...
    $sql .= " from pessoasobra102024 ";
    $sql2 = "";
    if ($dbwhere=="") {
      if ( $si194_sequencial != "" && $si194_sequencial != null) {
        $sql2 = " where pessoasobra102024.si194_sequencial = '$si194_sequencial'";
      }
    } else if ($dbwhere != "") {
      $sql2 = " where $dbwhere";
    }
