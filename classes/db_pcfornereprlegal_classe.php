...
class cl_pcfornereprlegal
{
  ...
  var $campos = "
                 ...
                 pc81_obs = text = Observao
                 pc81_tipopart = int8 = Tipo participao 
                 ";
  ...
  function atualizacampos($exclusao = false)
  {
    if ($exclusao == false) {
      ...
      $this->pc81_tipopart = ($this->pc81_tipopart == "" ? @$GLOBALS["HTTP_POST_VARS"]["pc81_tipopart"] : $this->pc81_tipopart);
    } else {
      $this->pc81_sequencia = ($this->pc81_sequencia == "" ? @$GLOBALS["HTTP_POST_VARS"]["pc81_sequencia"] : $this->pc81_sequencia);
    }
  }
  ...
  function incluir($pc81_sequencia)
  {
    ...
    $rsCgmforn = db_query("select z01_cgccpf from cgm where z01_numcgm = $this->pc81_cgmforn");
    $rsCgmResp = db_query("select z01_cgccpf from cgm where z01_numcgm = $this->pc81_cgmresp");
    $sCnpjFornecedor  = db_utils::fieldsMemory($rsCgmforn, 0)->z01_cgccpf;
    $sCnpjResponsavel = db_utils::fieldsMemory($rsCgmResp, 0)->z01_cgccpf;
    if (strlen($sCnpjFornecedor) == 14 && $sCnpjFornecedor == $sCnpjResponsavel) {
    ...
    if ($this->pc81_tipopart == null) {
      $this->erro_sql = " Campo Tipo participao nao Informado.";
      $this->erro_campo = "pc81_tipopart";
      $this->erro_banco = "";
      $this->erro_msg   = "Usurio: \\n\\n " . $this->erro_sql . " \\n\\n";
      $this->erro_msg   .=  str_replace('"', "", str_replace("'", "",  "Administrador: \\n\\n " . $this->erro_banco . " \\n"));
      $this->erro_status = "0";
    ...
    if ($pc81_sequencia == "" || $pc81_sequencia == null) {
      ...
      if (($result != false) && (pg_result($result, 0, 0) < $pc81_sequencia)) {
        $this->erro_sql = " Campo pc81_sequencia maior que ltimo nmero da sequencia.";
    ...
    if ($result == false) {
      ...
      if (strpos(strtolower($this->erro_banco), "duplicate key") != 0) {
        $this->erro_sql   = "Representantes Legais do Cliente ($this->pc81_sequencia) nao Includo. Inclusao Abortada.";
        $this->erro_msg   = "Usurio: \\n\\n " . $this->erro_sql . " \\n\\n";
        $this->erro_banco = "Representantes Legais do Cliente j Cadastrado";
        $this->erro_msg   .=  str_replace('"', "", str_replace("'", "",  "Administrador: \\n\\n " . $this->erro_banco . " \\n"));
      } else {
        $this->erro_sql   = "Representantes Legais do Cliente ($this->pc81_sequencia) nao Includo. Inclusao Abortada.";
        $this->erro_msg   = "Usurio: \\n\\n " . $this->erro_sql . " \\n\\n";
    ...
    $this->erro_banco = "";
    $this->erro_sql = "$this->pc81_cgmforn - $this->pc81_cgmresp Inclusao efetuada com Sucesso\\n";
  ...
  function alterar($pc81_sequencia = null)
  {
    ...
    if (trim($this->pc81_tipopart) != "" || isset($GLOBALS["HTTP_POST_VARS"]["pc81_tipopart"])) {
      $sql  .= $virgula . " pc81_tipopart = $this->pc81_tipopart ";
      $virgula = ",";
      if (trim($this->pc81_tipopart) == null) {
        $this->erro_sql = " Campo Tipo participao nao Informado.";
        $this->erro_campo = "pc81_tipopart";
        $this->erro_banco = "";
        $this->erro_msg   = "Usurio: \\n\\n " . $this->erro_sql . " \\n\\n";
  ...
  function excluir($pc81_sequencia = null, $dbwhere = null)
  {
    ...
    if ($result == false) {
      $this->erro_banco = str_replace("\n", "", @pg_last_error());
      $this->erro_sql   = "Representantes Legais do Cliente nao Excludo. Excluso Abortada.\\n";
      ...
      if (pg_affected_rows($result) == 0) {
        $this->erro_banco = "";
        $this->erro_sql = "Representantes Legais do Cliente nao Encontrado. Excluso no Efetuada.\\n";
        $this->erro_sql .= "Valores : " . $pc81_sequencia;
        $this->erro_msg   = "Usurio: \\n\\n " . $this->erro_sql . " \\n\\n";
        $this->erro_msg   .=  str_replace('"', "", str_replace("'", "",  "Administrador: \\n\\n " . $this->erro_banco . " \\n"));
        $this->erro_status = "1";
