<?
//MODULO: sicom
//CLASSE DA ENTIDADE consor502025
class cl_consor502025
{
  ...
  var $campos = "
                 ...
                 si20_codorgao = varchar(2) = C�digo do �rg�o
                 si20_cnpjconsorcio = varchar(14) = CNPJ do Cons�rcio
  ...
  function cl_consor502025()
  {
    //classes dos rotulos dos campos
    $this->rotulo = new rotulo("consor502025");
  ...
  function incluir($si20_sequencial)
  {
    ...
    if ($this->si20_codorgao == null) {
      $this->erro_sql = " Campo C�digo do Orgao nao Informado.";
    ...
    if ($si20_sequencial == "" || $si20_sequencial == null) {
      $result = db_query("select nextval('consor502025_si20_sequencial_seq')");
      if ($result == false) {
        $this->erro_banco = str_replace("
", "", @pg_last_error());
        $this->erro_sql = "Verifique o cadastro da sequencia: consor502025_si20_sequencial_seq do campo: si20_sequencial";
        $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
        $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
        $this->erro_status = "0";
      ...
      $result = db_query("select last_value from consor502025_si20_sequencial_seq");
      if (($result != false) && (pg_result($result, 0, 0) < $si20_sequencial)) {
        $this->erro_sql = " Campo si20_sequencial maior que �ltimo n�mero da sequencia.";
...
", "", @pg_last_error());
      if (strpos(strtolower($this->erro_banco), "duplicate key") != 0) {
        $this->erro_sql = "consor502025 ($this->si20_sequencial) nao Inclu�do. Inclusao Abortada.";
        $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
        $this->erro_banco = "consor502025 j� Cadastrado";
        $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
      } else {
        $this->erro_sql = "consor502025 ($this->si20_sequencial) nao Inclu�do. Inclusao Abortada.";
        $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
  ...
  function alterar($si20_sequencial = null)
  {
    ...
    if (trim($this->si20_codconsorcio) != "" || isset($GLOBALS["HTTP_POST_VARS"]["si20_codconsorcio"])) {
      ...
      if (trim($this->si20_codconsorcio) == null) {
        $this->erro_sql = " Campo C�digo do  Cons�rcio nao Informado.";
    ...
    if ($result == false) {
      $this->erro_banco = str_replace("
", "", @pg_last_error());
      $this->erro_sql = "consor502025 nao Alterado. Alteracao Abortada.\n";
      $this->erro_sql .= "Valores : " . $this->si20_sequencial;
      $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
      $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
      $this->erro_status = "0";
      ...
      if (pg_affected_rows($result) == 0) {
        $this->erro_banco = "";
        $this->erro_sql = "consor502025 nao foi Alterado. Alteracao Executada.\n";
        $this->erro_sql .= "Valores : " . $this->si20_sequencial;
        $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
        $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
        $this->erro_status = "1";
  ...
  function excluir($si20_sequencial = null, $dbwhere = null)
  {
    ...
    if ($result == false) {
      $this->erro_banco = str_replace("
", "", @pg_last_error());
      $this->erro_sql = "consor502025 nao Exclu�do. Exclus�o Abortada.\n";
      ...
      if (pg_affected_rows($result) == 0) {
        $this->erro_banco = "";
        $this->erro_sql = "consor502025 nao Encontrado. Exclus�o n�o Efetuada.\n";
        $this->erro_sql .= "Valores : " . $si20_sequencial;
        $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
        $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
        $this->erro_status = "1";
...
", "", @pg_last_error());
    ...
    if ($this->numrows == 0) {
      $this->erro_banco = "";
      $this->erro_sql = "Record Vazio na Tabela:consor502025";
      $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
      $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
      $this->erro_status = "0";
  ...
  function sql_query($si20_sequencial = null, $campos = "*", $ordem = null, $dbwhere = "")
  {
    ...
    $sql .= " from consor502025 ";
    $sql2 = "";
    if ($dbwhere == "") {
      if ($si20_sequencial != null) {
        $sql2 .= " where consor502025.si20_sequencial = $si20_sequencial ";
      }
    } else if ($dbwhere != "") {
      $sql2 = " where $dbwhere";
    }
