...
//MODULO: ISSQN
//CLASSE DA ENTIDADE meiimportalinharesponsavel
class cl_meiimportalinharesponsavel { 
   ...
   var $campos = "
                 ...
                 q108_meiimportalinha = int4 = Linha de Importação do MEI 
   ...
   function cl_meiimportalinharesponsavel() { 
     //classes dos rotulos dos campos
     $this->rotulo = new rotulo("meiimportalinharesponsavel"); 
   ...
   function atualizacampos($exclusao=false) {
     if($exclusao==false){
       ...
       $this->q108_meiimportalinha = ($this->q108_meiimportalinha == ""?@$GLOBALS["HTTP_POST_VARS"]["q108_meiimportalinha"]:$this->q108_meiimportalinha);
   ...
   function incluir ($q108_sequencial){ 
     ...
     if($this->q108_meiimportalinha == null ){ 
       $this->erro_sql = " Campo Linha de Importação do MEI nao Informado.";
       $this->erro_campo = "q108_meiimportalinha";
       $this->erro_banco = "";
     ...
     if($q108_sequencial == "" || $q108_sequencial == null ){
       $result = db_query("select nextval('meiimportalinharesponsavel_q108_sequencial_seq')"); 
       if($result==false){
         $this->erro_banco = str_replace("\n","",@pg_last_error());
         $this->erro_sql   = "Verifique o cadastro da sequencia: meiimportalinharesponsavel_q108_sequencial_seq do campo: q108_sequencial"; 
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "0";
         return false; 
       ...
       $result = db_query("select last_value from meiimportalinharesponsavel_q108_sequencial_seq");
     ...
     $sql = "insert into meiimportalinharesponsavel(
                                       q108_sequencial 
                                      ,q108_municipio 
                                      ,q108_meiimportalinha 
                                      ,q108_nome 
                                      ,q108_cpf 
                                      ,q108_tipologradouro 
                                      ,q108_logradouro 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,2853,16277,'','".AddSlashes(pg_result($resaco,0,'q108_meiimportalinha'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
   ...
   function alterar ($q108_sequencial=null) { 
      $this->atualizacampos();
     $sql = " update meiimportalinharesponsavel set ";
     $virgula = "";
     ...
     if(trim($this->q108_meiimportalinha)!="" || isset($GLOBALS["HTTP_POST_VARS"]["q108_meiimportalinha"])){ 
       $sql  .= $virgula." q108_meiimportalinha = $this->q108_meiimportalinha ";
       $virgula = ",";
       if(trim($this->q108_meiimportalinha) == null ){ 
         $this->erro_sql = " Campo Linha de Importação do MEI nao Informado.";
         $this->erro_campo = "q108_meiimportalinha";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["q108_meiimportalinha"]) || $this->q108_meiimportalinha != "")
           $resac = db_query("insert into db_acount values($acount,2853,16277,'".AddSlashes(pg_result($resaco,$conresaco,'q108_meiimportalinha'))."','$this->q108_meiimportalinha',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
   ...
   function excluir ($q108_sequencial=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,2853,16277,'','".AddSlashes(pg_result($resaco,$iresaco,'q108_meiimportalinha'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     ...
     $sql = " delete from meiimportalinharesponsavel
                    where ";
     $sql2 = "";
     if($dbwhere==null || $dbwhere ==""){
        if($q108_sequencial != ""){
   ...
   function sql_record($sql) { 
     ...
     $this->numrows = pg_numrows($result);
      if($this->numrows==0){
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:meiimportalinharesponsavel";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        $this->erro_status = "0";
        return false;
   ...
   function sql_query ( $q108_sequencial=null,$campos="*",$ordem=null,$dbwhere=""){ 
     ...
     $sql .= " from meiimportalinharesponsavel ";
     $sql .= "      inner join meiimportalinha  on  meiimportalinha.q105_sequencial = meiimportalinharesponsavel.q108_meiimportalinha";
     $sql .= "      inner join meiimporta  on  meiimporta.q104_sequencial = meiimportalinha.q105_meiimporta";
     $sql2 = "";
     if($dbwhere==""){
       if($q108_sequencial!=null ){
         $sql2 .= " where meiimportalinharesponsavel.q108_sequencial = $q108_sequencial "; 
       } 
     }else if($dbwhere != ""){
       $sql2 = " where $dbwhere";
     }
