<?
//MODULO: pessoal
//CLASSE DA ENTIDADE regrapontorubrica
class cl_regrapontorubrica { 
   ...
   function cl_regrapontorubrica() { 
     //classes dos rotulos dos campos
     $this->rotulo = new rotulo("regrapontorubrica"); 
   ...
   function incluir ($rh124_sequencial){ 
     ...
     $sql = "insert into regrapontorubrica(
                                       rh124_sequencial 
                                      ,rh124_regraponto 
                                      ,rh124_rubrica 
                                      ,rh124_instituicao 
   ...
   function alterar ($rh124_sequencial=null) { 
      $this->atualizacampos();
     $sql = " update regrapontorubrica set ";
     $virgula = "";
   ...
   function excluir ($rh124_sequencial=null,$dbwhere=null) { 
     ...
     $sql = " delete from regrapontorubrica
                    where ";
     $sql2 = "";
     if($dbwhere==null || $dbwhere ==""){
        if($rh124_sequencial != ""){
   ...
   function sql_record($sql) { 
     ...
     $this->numrows = pg_numrows($result);
      if($this->numrows==0){
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:regrapontorubrica";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        $this->erro_status = "0";
        return false;
   ...
   function sql_query ( $rh124_sequencial=null,$campos="*",$ordem=null,$dbwhere=""){ 
     ...
     $sql .= " from regrapontorubrica ";
     $sql .= "      inner join rhrubricas  on  rhrubricas.rh27_rubric = regrapontorubrica.rh124_rubrica and  rhrubricas.rh27_instit = regrapontorubrica.rh124_instituicao";
     $sql .= "      inner join regraponto  on  regraponto.rh123_sequencial = regrapontorubrica.rh124_regraponto";
     ...
     $sql .= "      inner join gruposelecao  on  gruposelecao.rh122_sequencial = regraponto.rh123_selecao";
     $sql2 = "";
     if($dbwhere==""){
       if($rh124_sequencial!=null ){
         $sql2 .= " where regrapontorubrica.rh124_sequencial = $rh124_sequencial "; 
       } 
     }else if($dbwhere != ""){
       $sql2 = " where $dbwhere";
     }
