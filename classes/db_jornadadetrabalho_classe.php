...
class cl_jornadadetrabalho
{
    ...
    public $campos = "
                 ...
                 jt_descricao = varchar(100) = Descrição
                 ";
    ...
    function incluir($jt_sequencial)
    {
        ...
        if ($jt_sequencial == "" || $jt_sequencial == null) {
            ...
            if (($result != false) && (pg_result($result, 0, 0) < $jt_sequencial)) {
                $this->erro_sql = " Campo jt_sequencial maior que último número da sequencia.";
        ...
        if ($result == false) {
            ...
            if (strpos(strtolower($this->erro_banco), "duplicate key") != 0) {
                $this->erro_sql   = "jornadadetrabalho ($this->jt_sequencial) nao Incluído. Inclusao Abortada.";
                $this->erro_msg   = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
                $this->erro_banco = "jornadadetrabalho já Cadastrado";
                $this->erro_msg   .=  str_replace('"', "", str_replace("'", "",  "Administrador: \\n\\n " . $this->erro_banco . " \\n"));
            } else {
                $this->erro_sql   = "jornadadetrabalho ($this->jt_sequencial) nao Incluído. Inclusao Abortada.";
                $this->erro_msg   = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
    ...
    function excluir($jt_sequencial = null, $dbwhere = null)
    {
        ...
        if ($result == false) {
            $this->erro_banco = str_replace("\n", "", @pg_last_error());
            $this->erro_sql   = "jornadadetrabalho nao Excluído. Exclusão Abortada.\\n";
            ...
            if (pg_affected_rows($result) == 0) {
                $this->erro_banco = "";
                $this->erro_sql = "jornadadetrabalho nao Encontrado. Exclusão não Efetuada.\\n";
                $this->erro_sql .= "Valores : " . $jt_sequencial;
                $this->erro_msg   = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
                $this->erro_msg   .=  str_replace('"', "", str_replace("'", "",  "Administrador: \\n\\n " . $this->erro_banco . " \\n"));
                $this->erro_status = "1";
