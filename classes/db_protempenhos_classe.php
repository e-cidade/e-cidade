...
class cl_protempenhos {
  ...
  function incluir ($p103_sequencial) {
     ...
     if ($this->p103_protocolo == null ) {
       $this->erro_sql = " Campo Emp. Protocolo não informado.";
     ...
     if ($p103_sequencial == "" || $p103_sequencial == null ) {
       ...
       if (($result != false) && (pg_result($result,0,0) < $p103_sequencial)) {
         $this->erro_sql = " Campo p103_sequencial maior que último número da sequencia.";
     ...
     if ($result==false) {
       ...
       if ( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ) {
         $this->erro_sql   = "protempenhos ($this->p103_sequencial) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_banco = "protempenhos já Cadastrado";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       } else {
         $this->erro_sql   = "protempenhos ($this->p103_sequencial) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
  ...
  function alterar ($p103_sequencial=null) {
     ...
     if (trim($this->p103_sequencial)!="" || isset($GLOBALS["HTTP_POST_VARS"]["p103_sequencial"])) {
       ...
       if (trim($this->p103_sequencial) == null ) {
         $this->erro_sql = " Campo p103_sequencial não informado.";
  ...
  function excluir ($p103_sequencial=null,$dbwhere=null) {
     ...
     if ($result==false) {
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "protempenhos nao Excluído. Exclusão Abortada.\\n";
       ...
       if (pg_affected_rows($result)==0) {
         $this->erro_banco = "";
         $this->erro_sql = "protempenhos nao Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$p103_sequencial;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
