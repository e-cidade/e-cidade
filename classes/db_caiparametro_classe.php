...
class cl_caiparametro
{
    ...
    var $campos = "
                 ...
                 k29_exibeobservacao = bool = Exibir observação em ordem bancaria
    ...
    function incluir($k29_instit)
    {
        ...
        if ($this->k29_exibeobservacao == null) {
            $this->erro_sql = " Campo Exibir Observacao nao Informado.";
            $this->erro_campo = "k29_exibeobservacao";
            $this->erro_banco = "";
            $this->erro_msg   = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
            $this->erro_msg   .=  str_replace('"', "", str_replace("'", "",  "Administrador: \\n\\n " . $this->erro_banco . " \\n"));
            $this->erro_status = "0";
        ...
        if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
            ...
            if (($resaco != false) || ($this->numrows != 0)) {
                ...
                $resac = db_query("insert into db_acount values($acount,1503,14618,'','" . AddSlashes(pg_result($resaco, 0, 'k29_exibeobservacao')) . "'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
    ...
    function alterar($k29_instit = null)
    {
        ...
        if (trim($this->k29_exibeobservacao) != "" || isset($GLOBALS["HTTP_POST_VARS"]["k29_exibeobservacao"])) {
            $sql  .= $virgula . " k29_exibeobservacao = '$this->k29_exibeobservacao' ";
            $virgula = ",";
            if (trim($this->k29_exibeobservacao) == null) {
                $this->erro_sql = " Campo Exibir observacao nao Informado.";
                $this->erro_campo = "k29_exibeobservacao";
                $this->erro_banco = "";
                $this->erro_msg   = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
            ...
            if ($this->numrows > 0) {
                ...
                for ($conresaco = 0; $conresaco < $this->numrows; $conresaco++) {
                    ...
                    if (isset($GLOBALS["HTTP_POST_VARS"]["k29_contassemmovimento"]) || $this->k29_contassemmovimento != "")
                        if (isset($GLOBALS["HTTP_POST_VARS"]["k29_exibeobservacao"]) || $this->k29_exibeobservacao != "")
                            $resac = db_query("insert into db_acount values($acount,1503,14618,'" . AddSlashes(pg_result($resaco, $conresaco, 'k29_exibeobservacao')) . "','$this->k29_exibeobservacao'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
                    if (isset($GLOBALS["HTTP_POST_VARS"]["k29_contassemmovimento"]) || $this->k29_contassemmovimento != "")
        ...
        if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
            ...
            if (($resaco != false) || ($this->numrows != 0)) {
                ...
                for ($iresaco = 0; $iresaco < $this->numrows; $iresaco++) {
                    ...
                    $resac  = db_query("insert into db_acount values($acount,1503,14618,'','" . AddSlashes(pg_result($resaco, $iresaco, 'k29_exibeobservacao')) . "'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
