...
class cl_sau_tipounidade { 
   ...
   var $campos = "
                 sd42_i_tp_unid_id = int4 = Código tipo unidade 
                 sd42_v_descricao = varchar(60) = Descrição 
                 ";
   ...
   function incluir ($sd42_i_tp_unid_id){ 
     ...
     if($result==false){ 
       ...
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         $this->erro_sql   = "tipounidade ($this->sd42_i_tp_unid_id) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_banco = "tipounidade já Cadastrado";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       }else{
         $this->erro_sql   = "tipounidade ($this->sd42_i_tp_unid_id) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
   ...
   function alterar ($sd42_i_tp_unid_id=null) { 
     ...
     if(trim($this->sd42_i_tp_unid_id)!="" || isset($GLOBALS["HTTP_POST_VARS"]["sd42_i_tp_unid_id"])){ 
       ...
       if(trim($this->sd42_i_tp_unid_id) == null ){ 
         $this->erro_sql = " Campo Código tipo unidade nao Informado.";
   ...
   function excluir ($sd42_i_tp_unid_id=null,$dbwhere=null) { 
     ...
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "tipounidade nao Excluído. Exclusão Abortada.\\n";
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "tipounidade nao Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$sd42_i_tp_unid_id;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
