<?php
//MODULO: sicom
//CLASSE DA ENTIDADE exeobras102022
class cl_exeobras102022 {
  ...
  function __construct() {
    ...
    $this->rotulo = new rotulo("exeobras102022");
  ...
  function incluir () {
    ...
    if ($this->si197_sequencial == null ) {
      $result = db_query("select nextval('exeobras102022_si197_sequencial_seq')");
      if($result==false){
        $this->erro_banco = str_replace("\n","",@pg_last_error());
        $this->erro_sql   = "Verifique o cadastro da sequencia: exeobras102022_si197_sequencial_seq do campo: si197_sequencial";
  ...
  function excluir ( $si197_sequencial=null ,$dbwhere=null) {
    ...
    $sql = " delete from exeobras102022
                    where ";
    $sql2 = "";
  ...
  function sql_record($sql) {
    ...
    if ($this->numrows==0) {
      $this->erro_banco = "";
      $this->erro_sql   = "Record Vazio na Tabela:exeobras102022";
      $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
      $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
      $this->erro_status = "0";
      return false;
  ...
  function sql_query ( $si197_sequencial = null,$campos="*",$ordem=null,$dbwhere="") {
    ...
    $sql .= " from exeobras102022 ";
    $sql2 = "";
    if ($dbwhere=="") {
      if ( $si197_sequencial != "" && $si197_sequencial != null) {
        $sql2 = " where exeobras102022.si197_sequencial = '$si197_sequencial'";
      }
    } else if ($dbwhere != "") {
      $sql2 = " where $dbwhere";
    }
