<?
//MODULO: pessoal
//CLASSE DA ENTIDADE regrapontorhrubrica
class cl_regrapontorhrubrica { 
   ...
   var $rh124_instituicao = 0; 
   // cria propriedade com as variaveis do arquivo 
   var $campos = "
                 rh124_sequencial = int4 = Cod. Regra Ponto 
                 rh124_regraponto = int4 = Cod. Regra 
                 rh124_rubrica = char(4) = Cod. Rubrica 
                 rh124_instituicao = int4 = Cod. Instituicao 
                 ";
   //funcao construtor da classe 
   function cl_regrapontorhrubrica() { 
     //classes dos rotulos dos campos
     $this->rotulo = new rotulo("regrapontorhrubrica"); 
   ...
   function atualizacampos($exclusao=false) {
     if($exclusao==false){
       ...
       $this->rh124_instituicao = ($this->rh124_instituicao == ""?@$GLOBALS["HTTP_POST_VARS"]["rh124_instituicao"]:$this->rh124_instituicao);
     }else{
       $this->rh124_sequencial = ($this->rh124_sequencial == ""?@$GLOBALS["HTTP_POST_VARS"]["rh124_sequencial"]:$this->rh124_sequencial);
     }
   }
   ...
   function incluir ($rh124_sequencial){ 
     ...
     if($this->rh124_instituicao == null ){ 
       $this->erro_sql = " Campo Cod. Instituicao nao Informado.";
       $this->erro_campo = "rh124_instituicao";
       $this->erro_banco = "";
       $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       $this->erro_status = "0";
     ...
     if($rh124_sequencial == "" || $rh124_sequencial == null ){
       $result = db_query("select nextval('regrapontorubrica_rh124_sequencial_seq')"); 
       if($result==false){
         $this->erro_banco = str_replace("\n","",@pg_last_error());
         $this->erro_sql   = "Verifique o cadastro da sequencia: regrapontorubrica_rh124_sequencial_seq do campo: rh124_sequencial"; 
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "0";
         return false; 
       ...
       $result = db_query("select last_value from regrapontorubrica_rh124_sequencial_seq");
     ...
     $sql = "insert into regrapontorhrubrica(
                                       rh124_sequencial 
                                      ,rh124_regraponto 
                                      ,rh124_rubrica 
                                      ,rh124_instituicao 
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if(($resaco!=false)||($this->numrows!=0)){
         ...
         $resac = db_query("insert into db_acount values($acount,3612,20127,'','".AddSlashes(pg_result($resaco,0,'rh124_instituicao'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     return true;
   } 
   ...
   function alterar ($rh124_sequencial=null) { 
      $this->atualizacampos();
     $sql = " update regrapontorhrubrica set ";
     $virgula = "";
     ...
     if(trim($this->rh124_instituicao)!="" || isset($GLOBALS["HTTP_POST_VARS"]["rh124_instituicao"])){ 
       $sql  .= $virgula." rh124_instituicao = $this->rh124_instituicao ";
       $virgula = ",";
       if(trim($this->rh124_instituicao) == null ){ 
         $this->erro_sql = " Campo Cod. Instituicao nao Informado.";
         $this->erro_campo = "rh124_instituicao";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if($this->numrows>0){
         ...
         for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
           ...
           if(isset($GLOBALS["HTTP_POST_VARS"]["rh124_instituicao"]) || $this->rh124_instituicao != "")
             $resac = db_query("insert into db_acount values($acount,3612,20127,'".AddSlashes(pg_result($resaco,$conresaco,'rh124_instituicao'))."','$this->rh124_instituicao',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
     $result = db_query($sql);
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if (($resaco != false) || ($this->numrows!=0)) {
         ...
         for ($iresaco = 0; $iresaco < $this->numrows; $iresaco++) {
           ...
           $resac  = db_query("insert into db_acount values($acount,3612,20127,'','".AddSlashes(pg_result($resaco,$iresaco,'rh124_instituicao'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
     $sql = " delete from regrapontorhrubrica
                    where ";
     $sql2 = "";
     if($dbwhere==null || $dbwhere ==""){
        if($rh124_sequencial != ""){
   ...
   function sql_record($sql) { 
     ...
     $this->numrows = pg_numrows($result);
      if($this->numrows==0){
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:regrapontorhrubrica";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        $this->erro_status = "0";
        return false;
   ...
   function sql_query ( $rh124_sequencial=null,$campos="*",$ordem=null,$dbwhere=""){ 
     ...
     $sql .= " from regrapontorhrubrica ";
     $sql .= "      inner join rhrubricas  on  rhrubricas.rh27_rubric = regrapontorhrubrica.rh124_rubrica and  rhrubricas.rh27_instit = regrapontorhrubrica.rh124_instituicao";
     $sql .= "      inner join regraponto  on  regraponto.rh123_sequencial = regrapontorhrubrica.rh124_regraponto";
     ...
     $sql .= "      inner join selecao  on  selecao.r44_selec = regraponto.rh123_selecao and  selecao.r44_instit = regraponto.rh123_instituicao";
     $sql2 = "";
     if($dbwhere==""){
       if($rh124_sequencial!=null ){
         $sql2 .= " where regrapontorhrubrica.rh124_sequencial = $rh124_sequencial "; 
       } 
     }else if($dbwhere != ""){
       $sql2 = " where $dbwhere";
     }
