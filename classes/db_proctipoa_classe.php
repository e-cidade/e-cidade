...
class cl_proctipoa { 
   ...
   function alterar ( $procedimento, $tipoatend ) {
      $this->atualizacampos();
...
$result = @pg_exec($sql);
   ...
   function sql_query ( $oid = null,$campos="proctipoa.oid,*",$ordem=null,$dbwhere=""){ 
     ...
     $sql .= " from proctipoa ";
     $sql .= "      inner join procedimentos  on  procedimentos.sd09_i_codigo = proctipoa.sd20_i_procedimento";
     $sql .= "      inner join atendtipo  on  atendtipo.sd14_i_codigo = proctipoa.sd20_i_tipoatend";
     $sql .= "      inner join grupoproc  on  grupoproc.sd11_c_codigo = procedimentos.sd09_c_grupoproc";
     $sql2 = "";
     if($dbwhere==""){
       if( $oid != "" && $oid != null){
          $sql2 = " where proctipoa.oid = '$oid'";
       }
     }else if($dbwhere != ""){
       $sql2 = " where $dbwhere";
     }
