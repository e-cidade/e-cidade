...
//MODULO: Ambulatorial
//CLASSE DA ENTIDADE sau_prontprograma
class cl_sau_prontprograma { 
   ...
   var $campos = "
                 ...
                 s141_i_acaoprog = int4 = programatico 
                 s141_i_prontuario = int4 = Prontuario 
                 s141_d_data = date = Data 
                 s141_c_hora = char(5) = Hora 
                 ";
   //funcao construtor da classe 
   function cl_sau_prontprograma() { 
     //classes dos rotulos dos campos
     $this->rotulo = new rotulo("sau_prontprograma"); 
   ...
   function atualizacampos($exclusao=false) {
     if($exclusao==false){
       ...
       $this->s141_i_prontuario = ($this->s141_i_prontuario == ""?@$GLOBALS["HTTP_POST_VARS"]["s141_i_prontuario"]:$this->s141_i_prontuario);
       if($this->s141_d_data == ""){
   ...
   function incluir ($s141_i_codigo){ 
     ...
     if($this->s141_i_acaoprog == null ){ 
       $this->erro_sql = " Campo programatico nao Informado.";
     ...
     if($this->s141_i_prontuario == null ){ 
       $this->erro_sql = " Campo Prontuario nao Informado.";
       $this->erro_campo = "s141_i_prontuario";
       $this->erro_banco = "";
       $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       $this->erro_status = "0";
     ...
     if($s141_i_codigo == "" || $s141_i_codigo == null ){
       $result = db_query("select nextval('sau_prontprograma_codigo_seq')"); 
       if($result==false){
         $this->erro_banco = str_replace("\n","",@pg_last_error());
         $this->erro_sql   = "Verifique o cadastro da sequencia: sau_prontprograma_codigo_seq do campo: s141_i_codigo"; 
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "0";
         return false; 
       ...
       $result = db_query("select last_value from sau_prontprograma_codigo_seq");
     ...
     $sql = "insert into sau_prontprograma(
                                       s141_i_codigo 
                                      ,s141_i_acaoprog 
                                      ,s141_i_prontuario 
                                      ,s141_d_data 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,2700,15339,'','".AddSlashes(pg_result($resaco,0,'s141_i_prontuario'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
   ...
   function alterar ($s141_i_codigo=null) { 
      $this->atualizacampos();
     $sql = " update sau_prontprograma set ";
     $virgula = "";
     ...
     if(trim($this->s141_i_prontuario)!="" || isset($GLOBALS["HTTP_POST_VARS"]["s141_i_prontuario"])){ 
       $sql  .= $virgula." s141_i_prontuario = $this->s141_i_prontuario ";
       $virgula = ",";
       if(trim($this->s141_i_prontuario) == null ){ 
         $this->erro_sql = " Campo Prontuario nao Informado.";
         $this->erro_campo = "s141_i_prontuario";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["s141_i_prontuario"]) || $this->s141_i_prontuario != "")
           $resac = db_query("insert into db_acount values($acount,2700,15339,'".AddSlashes(pg_result($resaco,$conresaco,'s141_i_prontuario'))."','$this->s141_i_prontuario',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
   ...
   function excluir ($s141_i_codigo=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,2700,15339,'','".AddSlashes(pg_result($resaco,$iresaco,'s141_i_prontuario'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     ...
     $sql = " delete from sau_prontprograma
                    where ";
     $sql2 = "";
     if($dbwhere==null || $dbwhere ==""){
        if($s141_i_codigo != ""){
   ...
   function sql_record($sql) { 
     ...
     $this->numrows = pg_numrows($result);
      if($this->numrows==0){
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:sau_prontprograma";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        $this->erro_status = "0";
        return false;
   ...
   function sql_query ( $s141_i_codigo=null,$campos="*",$ordem=null,$dbwhere=""){ 
     ...
     $sql .= " from sau_prontprograma ";
     $sql .= "      inner join far_programa  on  far_programa.fa12_i_codigo = sau_prontprograma.s141_i_acaoprog";
     $sql .= "      inner join prontuarios  on  prontuarios.sd24_i_codigo = sau_prontprograma.s141_i_prontuario";
     ...
     if($dbwhere==""){
       if($s141_i_codigo!=null ){
         $sql2 .= " where sau_prontprograma.s141_i_codigo = $s141_i_codigo "; 
       } 
     }else if($dbwhere != ""){
       $sql2 = " where $dbwhere";
     }
