...
class cl_autprotslip {
  ...
  function incluir ($p108_sequencial) {
     ...
     if ($this->p108_autorizado == null ) {
       $this->erro_sql = " Campo Autorizado não informado.";
     ...
     if ($p108_sequencial == "" || $p108_sequencial == null ) {
       ...
       if (($result != false) && (pg_result($result,0,0) < $p108_sequencial)) {
         $this->erro_sql = " Campo p108_sequencial maior que último número da sequencia.";
     ...
     if ($result==false) {
       ...
       if ( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ) {
         $this->erro_sql   = "autprotslip ($this->p108_sequencial) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_banco = "autprotslip já Cadastrado";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       } else {
         $this->erro_sql   = "autprotslip ($this->p108_sequencial) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
  ...
  function alterar ($p108_sequencial=null) {
     ...
     if (trim($this->p108_sequencial)!="" || isset($GLOBALS["HTTP_POST_VARS"]["p108_sequencial"])) {
       ...
       if (trim($this->p108_sequencial) == null ) {
         $this->erro_sql = " Campo p108_sequencial não informado.";
  ...
  function excluir ($p108_sequencial=null,$dbwhere=null) {
     ...
     if ($result==false) {
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "autprotslip nao Excluído. Exclusão Abortada.\\n";
       ...
       if (pg_affected_rows($result)==0) {
         $this->erro_banco = "";
         $this->erro_sql = "autprotslip nao Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$p108_sequencial;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
