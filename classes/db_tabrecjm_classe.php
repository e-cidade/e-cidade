<?php
  ...
  class cl_tabrecjm { 
    ...
    var $campos = "
                  ...
                  k02_descrjm = varchar(40) = Descrição de juros e multa
                  ";
    ...
    function atualizacampos($exclusao=false) {
      if($exclusao==false){
        ...
        $this->k02_descrjm = ($this->k02_descrjm == ""?@$GLOBALS["HTTP_POST_VARS"]["k02_descrjm"]:$this->k02_descrjm);
      }else{
        $this->k02_codjm = ($this->k02_codjm == ""?@$GLOBALS["HTTP_POST_VARS"]["k02_codjm"]:$this->k02_codjm);
      }
    }
    ...
    function incluir ($k02_codjm){ 
        $this->atualizacampos();
        if($this->k02_descrjm == null ){ 
          $this->erro_sql = " Campo descrição do juros e multa não Informado.";
          $this->erro_campo = "k02_descrjm";
          $this->erro_banco = "";
      ...
      if(($resaco!=false)||($this->numrows!=0)){
        ...
        $resac = db_query("insert into db_acount values($acount,76,2013000,'','".AddSlashes(pg_result($resaco,0,'k02_descrjm'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
      }
      return true;
    } 
    ...
    function alterar ($k02_codjm=null) { 
      ...
      if(trim($this->k02_descrjm)!="" || isset($GLOBALS["HTTP_POST_VARS"]["k02_descrjm"])){ 
        $sql  .= $virgula." k02_descrjm = '$this->k02_descrjm' ";
        $virgula = ",";
        if(trim($this->k02_descrjm) == null ){ 
          $this->erro_sql = " Campo descrição de juros e multa não Informado.";
          $this->erro_campo = "k02_descrjm";
          $this->erro_banco = "";
          $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
          $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
      ...
      if($this->numrows>0){
        for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
          ...
          if(isset($GLOBALS["HTTP_POST_VARS"]["k02_descrjm"]) || $this->k02_descrjm != "")
            $resac = db_query("insert into db_acount values($acount,76,2013000,'".AddSlashes(pg_result($resaco,$conresaco,'k02_descrjm'))."','$this->k02_descrjm',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
        }
      }
      $result = db_query($sql);
      if($result==false){ 
    ...
    function excluir ($k02_codjm=null,$dbwhere=null) { 
      ...
      if(($resaco!=false)||($this->numrows!=0)){
        for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
          ...
          $resac = db_query("insert into db_acount values($acount,76,2013000,'','".AddSlashes(pg_result($resaco,$iresaco,'k02_descrjm'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
        }
      }
