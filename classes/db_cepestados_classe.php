...
class cl_cepestados { 
   ...
   function atualizacampos($exclusao=false) {
     if($exclusao==false){
       $this->cp03_sigla = ($this->cp03_sigla == ""?@$GLOBALS["HTTP_POST_VARS"]["cp03_sigla"]:$this->cp03_sigla);
       $this->cp03_estado = ($this->cp03_estado == ""?@$GLOBALS["HTTP_POST_VARS"]["cp03_estado"]:$this->cp03_estado);
     }else{
       $this->cp03_sigla = ($this->cp03_sigla == ""?@$GLOBALS["HTTP_POST_VARS"]["cp03_sigla"]:$this->cp03_sigla);
     }
   }
   // funcao para inclusao
   function incluir ($cp03_sigla){ 
      $this->atualizacampos();
     if($this->cp03_estado == null ){ 
       ...
       $this->cp03_sigla = $cp03_sigla; 
     if(($this->cp03_sigla == null) || ($this->cp03_sigla == "") ){ 
       $this->erro_sql = " Campo cp03_sigla nao declarado.";
     ...
     $this->erro_sql = "Inclusao efetuada com Sucesso\\n";
         $this->erro_sql .= "Valores : ".$this->cp03_sigla;
     $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
     $this->erro_status = "1";
   ...
   function alterar ($cp03_sigla=null) { 
      $this->atualizacampos();
     ...
     $sql .= " where ";
     if($cp03_sigla!=null){
       $sql .= " cp03_sigla = '$this->cp03_sigla'";
     }
     $resaco = $this->sql_record($this->sql_query_file($this->cp03_sigla));
     if($this->numrows>0){
   ...
   function excluir ($cp03_sigla=null,$dbwhere=null) { 
     if($dbwhere==null || $dbwhere==""){
       $resaco = $this->sql_record($this->sql_query_file($cp03_sigla));
     }else{ 
     ...
     if($result==false){ 
       ...
       $this->erro_sql .= "Valores : ".$cp03_sigla;
       $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       $this->erro_status = "0";
   ...
   function sql_query ( $cp03_sigla=null,$campos="*",$ordem=null,$dbwhere=""){ 
     ...
     if($dbwhere==""){
       if($cp03_sigla!=null ){
         $sql2 .= " where cepestados.cp03_sigla = '$cp03_sigla' "; 
       } 
     }else if($dbwhere != ""){
       $sql2 = " where $dbwhere";
     }
