<?
//MODULO: Habitacao
//CLASSE DA ENTIDADE habitgrupoprogramaformaavaliacao
class cl_habitgrupoprogramaformaavaliacao { 
   ...
   function cl_habitgrupoprogramaformaavaliacao() { 
     //classes dos rotulos dos campos
     $this->rotulo = new rotulo("habitgrupoprogramaformaavaliacao"); 
   ...
   function incluir ($ht06_sequencial){ 
     ...
     if($ht06_sequencial == "" || $ht06_sequencial == null ){
       $result = db_query("select nextval('habitgrupoprogramaformaavaliacao_ht06_sequencial_seq')"); 
       if($result==false){
         $this->erro_banco = str_replace("\n","",@pg_last_error());
         $this->erro_sql   = "Verifique o cadastro da sequencia: habitgrupoprogramaformaavaliacao_ht06_sequencial_seq do campo: ht06_sequencial"; 
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "0";
         return false; 
       ...
       $result = db_query("select last_value from habitgrupoprogramaformaavaliacao_ht06_sequencial_seq");
     ...
     $sql = "insert into habitgrupoprogramaformaavaliacao(
                                       ht06_sequencial 
   ...
   function alterar ($ht06_sequencial=null) { 
      $this->atualizacampos();
     $sql = " update habitgrupoprogramaformaavaliacao set ";
     $virgula = "";
   ...
   function excluir ($ht06_sequencial=null,$dbwhere=null) { 
     ...
     $sql = " delete from habitgrupoprogramaformaavaliacao
                    where ";
     $sql2 = "";
     if($dbwhere==null || $dbwhere ==""){
        if($ht06_sequencial != ""){
   ...
   function sql_record($sql) { 
     ...
     $this->numrows = pg_numrows($result);
      if($this->numrows==0){
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:habitgrupoprogramaformaavaliacao";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        $this->erro_status = "0";
        return false;
   ...
   function sql_query ( $ht06_sequencial=null,$campos="*",$ordem=null,$dbwhere=""){ 
     ...
     $sql .= " from habitgrupoprogramaformaavaliacao ";
     $sql .= "      inner join habittipogrupoprograma  on  habittipogrupoprograma.ht02_sequencial = habitgrupoprogramaformaavaliacao.ht06_habittipogrupoprograma";
     $sql .= "      inner join habitformaavaliacao  on  habitformaavaliacao.ht07_sequencial = habitgrupoprogramaformaavaliacao.ht06_habitformaavaliacao";
     $sql2 = "";
     if($dbwhere==""){
       if($ht06_sequencial!=null ){
         $sql2 .= " where habitgrupoprogramaformaavaliacao.ht06_sequencial = $ht06_sequencial "; 
       } 
     }else if($dbwhere != ""){
       $sql2 = " where $dbwhere";
     }
