...
class cl_veiccadposto {
   ...
   function atualizacampos($exclusao=false) {
     if($exclusao==false){
       $this->ve29_codigo = ($this->ve29_codigo == ""?@$GLOBALS["HTTP_POST_VARS"]["ve29_codigo"]:$this->ve29_codigo);
       $this->ve29_tipo = ($this->ve29_tipo == ""?@$GLOBALS["HTTP_POST_VARS"]["ve29_tipo"]:$this->ve29_tipo);
     }else{
       $this->ve29_codigo = ($this->ve29_codigo == ""?@$GLOBALS["HTTP_POST_VARS"]["ve29_codigo"]:$this->ve29_codigo);
     }
   }
   ...
   function incluir ($ve29_codigo=null){
     ...
     if($ve29_codigo == "" || $ve29_codigo == null ){
       ...
       if(($result != false) && (pg_result($result,0,0) < $ve29_codigo)){
         $this->erro_sql = " Campo ve29_codigo maior que último número da sequencia.";
     ...
     if($result==false){
       ...
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         ...
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_banco = "Cadastro de Postos de Combustiveis já Cadastrado";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       }else{
         $this->erro_sql   = "Cadastro de Postos de Combustiveis ($this->ve29_codigo) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     $resaco = $this->sql_record($this->sql_query_file($this->ve29_codigo));
     if(($resaco!=false)||($this->numrows!=0)){
       $resac = db_query("select nextval('db_acount_id_acount_seq') as acount");
       $acount = pg_result($resac,0,0);
       $resac = db_query("insert into db_acountacesso values($acount,".db_getsession("DB_acessado").")");
       $resac = db_query("insert into db_acountkey values($acount,9234,'$this->ve29_codigo','I')");
       $resac = db_query("insert into db_acount values($acount,1586,9234,'','".AddSlashes(pg_result($resaco,0,'ve29_codigo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
   ...
   function alterar ($ve29_codigo=null) {
     ...
     $sql .= " where ";
     if($ve29_codigo!=null){
       $sql .= " ve29_codigo = $this->ve29_codigo";
     }
     $resaco = $this->sql_record($this->sql_query_file($this->ve29_codigo));
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         $acount = pg_result($resac,0,0);
         $resac = db_query("insert into db_acountacesso values($acount,".db_getsession("DB_acessado").")");
         $resac = db_query("insert into db_acountkey values($acount,9234,'$this->ve29_codigo','A')");
         if(isset($GLOBALS["HTTP_POST_VARS"]["ve29_codigo"]))
           $resac = db_query("insert into db_acount values($acount,1586,9234,'".AddSlashes(pg_result($resaco,$conresaco,'ve29_codigo'))."','$this->ve29_codigo',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["ve29_tipo"]))
           $resac = db_query("insert into db_acount values($acount,1586,9235,'".AddSlashes(pg_result($resaco,$conresaco,'ve29_tipo'))."','$this->ve29_tipo',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "Cadastro de Postos de Combustiveis nao foi Alterado. Alteracao Executada.\\n";
         $this->erro_sql .= "Valores : ".$this->ve29_codigo;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
   ...
   function excluir ($ve29_codigo=null,$dbwhere=null) {
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $acount = pg_result($resac,0,0);
         $resac = db_query("insert into db_acountacesso values($acount,".db_getsession("DB_acessado").")");
         $resac = db_query("insert into db_acountkey values($acount,9234,'$ve29_codigo','E')");
         $resac = db_query("insert into db_acount values($acount,1586,9234,'','".AddSlashes(pg_result($resaco,$iresaco,'ve29_codigo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,1586,9235,'','".AddSlashes(pg_result($resaco,$iresaco,'ve29_tipo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $sql = " delete from veiccadposto
                    where ";
     $sql2 = "";
     if($dbwhere==null || $dbwhere ==""){
        if($ve29_codigo != ""){
     ...
     if($result==false){
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "Cadastro de Postos de Combustiveis nao Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$ve29_codigo;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
   ...
   function sql_record($sql) {
     ...
     $this->numrows = pg_numrows($result);
      if($this->numrows==0){
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:veiccadposto";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        $this->erro_status = "0";
        return false;
   ...
   function sql_query_tip ( $ve29_codigo=null,$campos="*",$ordem=null,$dbwhere=""){
     ...
     $sql .= " from veiccadposto ";
     $sql .= " 		left join veiccadpostointerno on ve35_veiccadposto=ve29_codigo";
     $sql .= " 		left join db_depart on coddepto = ve35_depart";
     $sql .= " 		left join veiccadpostoexterno on ve34_veiccadposto=ve29_codigo";
     $sql .= " 		left join cgm on z01_numcgm = ve34_numcgm";
     $sql2 = "";
     if($dbwhere==""){
       if($ve29_codigo!=null ){
         $sql2 .= " where veiccadposto.ve29_codigo = $ve29_codigo ";
