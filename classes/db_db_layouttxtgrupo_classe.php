...
class cl_db_layouttxtgrupo { 
   ...
   function atualizacampos($exclusao=false) {
     if($exclusao==false){
       $this->db56_sequencial = ($this->db56_sequencial == ""?@$GLOBALS["HTTP_POST_VARS"]["db56_sequencial"]:$this->db56_sequencial);
       $this->db56_layouttxtgrupotipo = ($this->db56_layouttxtgrupotipo == ""?@$GLOBALS["HTTP_POST_VARS"]["db56_layouttxtgrupotipo"]:$this->db56_layouttxtgrupotipo);
       $this->db56_descr = ($this->db56_descr == ""?@$GLOBALS["HTTP_POST_VARS"]["db56_descr"]:$this->db56_descr);
     }else{
       $this->db56_sequencial = ($this->db56_sequencial == ""?@$GLOBALS["HTTP_POST_VARS"]["db56_sequencial"]:$this->db56_sequencial);
     }
   }
   // funcao para inclusao
   function incluir ($db56_sequencial){ 
      $this->atualizacampos();
     if($this->db56_layouttxtgrupotipo == null ){ 
     ...
     if($db56_sequencial == "" || $db56_sequencial == null ){
       $result = db_query("select nextval('db_layouttxtgrupo_db56_sequencial_seq')"); 
       if($result==false){
         $this->erro_banco = str_replace("\n","",@pg_last_error());
         $this->erro_sql   = "Verifique o cadastro da sequencia: db_layouttxtgrupo_db56_sequencial_seq do campo: db56_sequencial"; 
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "0";
         return false; 
       ...
       if(($result != false) && (pg_result($result,0,0) < $db56_sequencial)){
         ...
         $this->db56_sequencial = $db56_sequencial; 
       }
     }
     if(($this->db56_sequencial == null) || ($this->db56_sequencial == "") ){ 
       $this->erro_sql = " Campo db56_sequencial nao declarado.";
     ...
     $sql = "insert into db_layouttxtgrupo(
                                       db56_sequencial 
                                      ,db56_layouttxtgrupotipo 
                                      ,db56_descr 
                       )
                values (
                                $this->db56_sequencial 
                               ,$this->db56_layouttxtgrupotipo 
                               ,'$this->db56_descr' 
                      )";
     $result = db_query($sql); 
     if($result==false){ 
     ...
     $this->erro_sql = "Inclusao efetuada com Sucesso\\n";
         $this->erro_sql .= "Valores : ".$this->db56_sequencial;
     $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
     $this->erro_status = "1";
   ...
   function alterar ($db56_sequencial=null) { 
      $this->atualizacampos();
     ...
     $sql .= " where ";
     if($db56_sequencial!=null){
       $sql .= " db56_sequencial = $this->db56_sequencial";
     }
     $resaco = $this->sql_record($this->sql_query_file($this->db56_sequencial));
     if($this->numrows>0){
   ...
   function excluir ($db56_sequencial=null,$dbwhere=null) { 
     if($dbwhere==null || $dbwhere==""){
       $resaco = $this->sql_record($this->sql_query_file($db56_sequencial));
     }else{ 
     ...
     if($dbwhere==null || $dbwhere ==""){
        if($db56_sequencial != ""){
          ...
          $sql2 .= " db56_sequencial = $db56_sequencial ";
        }
     }else{
       $sql2 = $dbwhere;
     }
     ...
     if($result==false){ 
       ...
       $this->erro_sql .= "Valores : ".$db56_sequencial;
       $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       $this->erro_status = "0";
