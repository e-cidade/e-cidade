...
class cl_iptuant { 
   ...
   var $campos = "
                 j40_matric = int4 = Matricula 
                 j40_refant = varchar(20) = Ref. Anterior 
                 ";
   ...
   function atualizacampos($exclusao=false) {
     if($exclusao==false){
       $this->j40_matric = ($this->j40_matric == ""?@$GLOBALS["HTTP_POST_VARS"]["j40_matric"]:$this->j40_matric);
       $this->j40_refant = ($this->j40_refant == ""?@$GLOBALS["HTTP_POST_VARS"]["j40_refant"]:$this->j40_refant);
     }else{
       $this->j40_matric = ($this->j40_matric == ""?@$GLOBALS["HTTP_POST_VARS"]["j40_matric"]:$this->j40_matric);
     }
   }
   // funcao para inclusao
   function incluir ($j40_matric){ 
      $this->atualizacampos();
     if($this->j40_refant == null ){ 
       $this->erro_sql = " Campo Ref. Anterior nao Informado.";
       $this->erro_campo = "j40_refant";
       $this->erro_banco = "";
     ...
     if(($this->j40_matric == null) || ($this->j40_matric == "") ){ 
       $this->erro_sql = " Campo j40_matric nao declarado.";
     ...
     if($result==false){ 
       ...
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         $this->erro_sql   = " ($this->j40_matric) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     $this->erro_sql = "Inclusao efetuada com Sucesso\\n";
         $this->erro_sql .= "Valores : ".$this->j40_matric;
     $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
     $this->erro_status = "1";
   ...
   function alterar ($j40_matric=null) { 
      $this->atualizacampos();
     $sql = " update iptuant set ";
     $virgula = "";
     if(trim($this->j40_matric)!="" || isset($GLOBALS["HTTP_POST_VARS"]["j40_matric"])){ 
       $sql  .= $virgula." j40_matric = $this->j40_matric ";
       $virgula = ",";
       if(trim($this->j40_matric) == null ){ 
     ...
     if(trim($this->j40_refant)!="" || isset($GLOBALS["HTTP_POST_VARS"]["j40_refant"])){ 
       $sql  .= $virgula." j40_refant = '$this->j40_refant' ";
       $virgula = ",";
       if(trim($this->j40_refant) == null ){ 
         $this->erro_sql = " Campo Ref. Anterior nao Informado.";
         $this->erro_campo = "j40_refant";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     $sql .= " where ";
     if($j40_matric!=null){
       $sql .= " j40_matric = $this->j40_matric";
     }
     $resaco = $this->sql_record($this->sql_query_file($this->j40_matric));
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["j40_matric"]))
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["j40_refant"]))
   ...
   function excluir ($j40_matric=null,$dbwhere=null) { 
     ...
     if($result==false){ 
       ...
       $this->erro_sql .= "Valores : ".$j40_matric;
       $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       $this->erro_status = "0";
   ...
   function sql_query ( $j40_matric=null,$campos="*",$ordem=null,$dbwhere=""){ 
     $sql = "select ";
     if($campos != "*" ){
   ...
   function sql_query_file ( $j40_matric=null,$campos="*",$ordem=null,$dbwhere=""){ 
     $sql = "select ";
     if($campos != "*" ){
       $campos_sql = explode("#",$campos);
       $virgula = "";
