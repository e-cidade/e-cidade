...
class cl_ntf102025
{
  ...
  var $campos = "
                 ...
                 si143_codnotafiscal = int8 = C�digo  Identificador da Nota Fiscal
                 si143_codorgao = varchar(2) = C�digo do �rg�o
                 si143_nfnumero = varchar(20) = N�mero da nota  fiscal
                 si143_nfserie = varchar(8) = N�mero de s�rie  da nota fiscal
                 si143_tipodocumento = int8 = Tipo de  Documento do  credor
                 si143_nrodocumento = varchar(14) = N�mero do documento do credor
                 si143_nroinscestadual = varchar(30) = N�mero da  inscri��o estadual
                 si143_nroinscmunicipal = varchar(30) = N�mero da  Inscri��o  Municipal
                 si143_nomemunicipio = varchar(120) = Nome do  Munic�pio
                 si143_cepmunicipio = int8 = CEP do Munic�pio
                 si143_ufcredor = varchar(2) = UF da Inscri��o  do Credor
                 si143_notafiscaleletronica = int8 = Identifica se a  nota fiscal
                 si143_chaveacesso = varchar(44) = Chave para  consulta
                 si143_outraChaveAcesso = varchar(60) = Chave Nota Fiscal  Eletr�nica
                 si143_nfaidf = varchar(15) = N�mero da  Autoriza��o
                 si143_dtemissaonf = date = Data de emiss�o  da nota fiscal
                 ...
                 si143_mes = int8 = M�s
                 si143_instit = int8 = Institui��o
                 ";

  //funcao construtor da classe
  function cl_ntf102025()
  {
    //classes dos rotulos dos campos
    $this->rotulo = new rotulo("ntf102025");
  ...
  function incluir($si143_sequencial)
  {
    ...
    if ($si143_sequencial == "" || $si143_sequencial == null) {
      $result = db_query("select nextval('ntf102025_si143_sequencial_seq')");
      if ($result == false) {
        $this->erro_banco = str_replace("
", "", @pg_last_error());
        $this->erro_sql = "Verifique o cadastro da sequencia: ntf102025_si143_sequencial_seq do campo: si143_sequencial";
        $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
        $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
        $this->erro_status = "0";
      ...
      $result = db_query("select last_value from ntf102025_si143_sequencial_seq");
      if (($result != false) && (pg_result($result, 0, 0) < $si143_sequencial)) {
        $this->erro_sql = " Campo si143_sequencial maior que �ltimo n�mero da sequencia.";
...
", "", @pg_last_error());
      if (strpos(strtolower($this->erro_banco), "duplicate key") != 0) {
        $this->erro_sql = "ntf102025 ($this->si143_sequencial) nao Inclu�do. Inclusao Abortada.";
        $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
        $this->erro_banco = "ntf102025 j� Cadastrado";
        $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
      } else {
        $this->erro_sql = "ntf102025 ($this->si143_sequencial) nao Inclu�do. Inclusao Abortada.";
        $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
  ...
  function alterar($si143_sequencial = null)
  {
    ...
    if (trim($this->si143_nroinscestadual) != "" || isset($GLOBALS["HTTP_POST_VARS"]["si143_nroinscestadual"])) {
      ...
      $aco = $this->sql_record($this->sql_query_file($this->si143_sequencial));
    // if ($this->numrows > 0) {
    ...
    //       $sql2$resac = db_query("insert into db_acount values($acount,2010372,2011059,'" . AddSlashes(pg_result($resaco, $conresaco, 'si143_cepmunicipio')) . "','$this->si143_cepmunicipio'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
    ...
    if ($result == false) {
      $this->erro_banco = str_replace("
", "", @pg_last_error());
      $this->erro_sql = "ntf102025 nao Alterado. Alteracao Abortada.\n";
      $this->erro_sql .= "Valores : " . $this->si143_sequencial;
      $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
      $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
      $this->erro_status = "0";
      ...
      if (pg_affected_rows($result) == 0) {
        $this->erro_banco = "";
        $this->erro_sql = "ntf102025 nao foi Alterado. Alteracao Executada.\n";
        $this->erro_sql .= "Valores : " . $this->si143_sequencial;
        $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
        $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
        $this->erro_status = "1";
  ...
  function excluir($si143_sequencial = null, $dbwhere = null)
  {
    ...
    if ($result == false) {
      $this->erro_banco = str_replace("
", "", @pg_last_error());
      $this->erro_sql = "ntf102025 nao Exclu�do. Exclus�o Abortada.\n";
      ...
      if (pg_affected_rows($result) == 0) {
        $this->erro_banco = "";
        $this->erro_sql = "ntf102025 nao Encontrado. Exclus�o n�o Efetuada.\n";
        $this->erro_sql .= "Valores : " . $si143_sequencial;
        $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
        $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
        $this->erro_status = "1";
...
", "", @pg_last_error());
    ...
    if ($this->numrows == 0) {
      $this->erro_banco = "";
      $this->erro_sql = "Record Vazio na Tabela:ntf102025";
      $this->erro_msg = "Usu�rio: \n\n " . $this->erro_sql . " \n\n";
      $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
      $this->erro_status = "0";
  ...
  function sql_query($si143_sequencial = null, $campos = "*", $ordem = null, $dbwhere = "")
  {
    ...
    if ($dbwhere == "") {
      if ($si143_sequencial != null) {
        $sql2 .= " where ntf102025.si143_sequencial = $si143_sequencial ";
      }
    } else if ($dbwhere != "") {
      $sql2 = " where $dbwhere";
    }
