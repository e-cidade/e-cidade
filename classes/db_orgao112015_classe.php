<?
//MODULO: sicom
//CLASSE DA ENTIDADE orgao112015
class cl_orgao112015 { 
   ...
   function cl_orgao112015() { 
     //classes dos rotulos dos campos
     $this->rotulo = new rotulo("orgao112015"); 
   ...
   function incluir ($si15_sequencial){ 
     ...
     if($si15_sequencial == "" || $si15_sequencial == null ){
       $result = db_query("select nextval('orgao112015_si15_sequencial_seq')"); 
       if($result==false){
         $this->erro_banco = str_replace("\n","",@pg_last_error());
         $this->erro_sql   = "Verifique o cadastro da sequencia: orgao112015_si15_sequencial_seq do campo: si15_sequencial"; 
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "0";
         return false; 
       ...
       $result = db_query("select last_value from orgao112015_si15_sequencial_seq");
   ...
   function sql_record($sql) { 
     ...
     $this->numrows = pg_numrows($result);
      if($this->numrows==0){
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:orgao112015";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        $this->erro_status = "0";
        return false;
   ...
   function sql_query ( $si15_sequencial=null,$campos="*",$ordem=null,$dbwhere=""){ 
     ...
     $sql .= " from orgao112015 ";
     $sql .= "      left  join orgao102015  on  orgao102015.si14_sequencial = orgao112015.si15_reg10";
     $sql2 = "";
     if($dbwhere==""){
       if($si15_sequencial!=null ){
         $sql2 .= " where orgao112015.si15_sequencial = $si15_sequencial "; 
       } 
     }else if($dbwhere != ""){
       $sql2 = " where $dbwhere";
     }
