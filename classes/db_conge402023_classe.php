<?php
//MODULO: sicom
//CLASSE DA ENTIDADE conge402023
class cl_conge402023 {
  ...
  function __construct() {
    ...
    $this->rotulo = new rotulo("conge402023");
...
/*     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
  ...
  function sql_record($sql) {
     ...
     $this->numrows = pg_numrows($result);
      if ($this->numrows==0) {
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:conge402023";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        $this->erro_status = "0";
        return false;
  ...
  function sql_query ( $si185_sequencial=null,$campos="*",$ordem=null,$dbwhere="") {
     ...
     $sql .= " from conge402023 ";
     $sql2 = "";
     if ($dbwhere=="") {
       if ($si185_sequencial!=null ) {
         $sql2 .= " where conge402023.si185_sequencial = $si185_sequencial ";
       }
     } else if ($dbwhere != "") {
       $sql2 = " where $dbwhere";
     }
