...
class cl_rpsd102017 {
  ...
  function __construct() {
    ...
    $this->rotulo = new rotulo("rpsd102017");
  ...
  function incluir ($si189_sequencial) {
     ...
     if ($si189_sequencial == "" || $si189_sequencial == null ) {
       $result = db_query("select nextval('rpsd102017_si189_sequencial_seq')");
       if ($result==false) {
         $this->erro_banco = str_replace("\n","",@pg_last_error());
         $this->erro_sql   = "Verifique o cadastro da sequencia: rpsd102017_si189_sequencial_seq do campo: si189_sequencial";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "0";
         return false;
       ...
       $result = db_query("select last_value from rpsd102017_si189_sequencial_seq");
     ...
     /*if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if (($resaco!=false)||($this->numrows!=0)) {
         ...
         $resac = db_query("insert into db_acount values($acount,1010202,2012017,'','".AddSlashes(pg_result($resaco,0,'si189_vlpagorsp'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       ...
       if ($this->numrows>0) {
         ...
         for($conresaco=0;$conresaco<$this->numrows;$conresaco++) {
           ...
           if (isset($GLOBALS["HTTP_POST_VARS"]["si189_vlpagorsp"]) || $this->si189_vlpagorsp != "")
             $resac = db_query("insert into db_acount values($acount,1010202,2012017,'".AddSlashes(pg_result($resaco,$conresaco,'si189_vlpagorsp'))."','$this->si189_vlpagorsp',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
...
/*     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if (($resaco != false) || ($this->numrows!=0)) {
         ...
         for ($iresaco = 0; $iresaco < $this->numrows; $iresaco++) {
           ...
           $resac  = db_query("insert into db_acount values($acount,1010202,2012017,'','".AddSlashes(pg_result($resaco,$iresaco,'si189_vlpagorsp'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
  ...
  function sql_record($sql) {
     ...
     $this->numrows = pg_numrows($result);
      if ($this->numrows==0) {
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:rpsd102017";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        $this->erro_status = "0";
        return false;
  ...
  function sql_query ( $si189_sequencial=null,$campos="*",$ordem=null,$dbwhere="") {
     ...
     if ($dbwhere=="") {
       if ($si189_sequencial!=null ) {
         $sql2 .= " where rpsd102017.si189_sequencial = $si189_sequencial ";
       }
     } else if ($dbwhere != "") {
       $sql2 = " where $dbwhere";
     }
