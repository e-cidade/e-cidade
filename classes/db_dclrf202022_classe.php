<?
//MODULO: sicom
//CLASSE DA ENTIDADE dclrf202022
class cl_dclrf202022 {
  ...
  var $si191_dscnumeroinst = 0;
   ...
   function cl_dclrf202022() {
     ...
     $this->rotulo = new rotulo("dclrf202022");
...
//     }
	 ...
	 if($this->si191_mes == null ){
	   ...
	   if($si191_sequencial == "" || $si191_sequencial == null ){
		   $result = @pg_query("select nextval('dclrf202022_si191_sequencial_seq')");
		   if($result==false){
			   $this->erro_banco = str_replace("\n","",@pg_last_error());
			   $this->erro_sql   = "Verifique o cadastro da sequencia: dclrf202022_si191_sequencial_seq do campo: si191_sequencial";
			   $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
			   $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
			   $this->erro_status = "0";
			   return false;
		   ...
		   $result = @pg_query("select last_value from dclrf202022_si191_sequencial_seq");
     ...
     if($result==false){
       ...
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
		   $this->erro_sql   = "dclrf202022 ($this->si191_sequencial) nao Incluído. Inclusao Abortada.";
		   $this->erro_msg   = "Usuário: \n\n ".$this->erro_sql." \n\n";
		   $this->erro_banco = "dclrf202022 já Cadastrado";
		   $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \n\n ".$this->erro_banco." \n"));
	   }else{
		   $this->erro_sql   = "dclrf202022 ($this->si191_sequencial) nao Incluído. Inclusao Abortada.";
		   $this->erro_msg   = "Usuário: \n\n ".$this->erro_sql." \n\n";
   ...
   function sql_query ( $si191_sequencial = null,$campos="*",$ordem=null,$dbwhere=""){
     ...
     $sql .= " from dclrf202022 ";
	 $sql .= " 		left join dclrf102020 on dclrf102020.si157_sequencial = dclrf202022.si191_reg10 ";
     $sql2 = "";
     if($dbwhere==""){
       if( $si191_sequencial != null){
          $sql2 .= " where dclrf202022.si191_sequencial = $si191_sequencial";
       }
     }else if($dbwhere != ""){
       $sql2 = " where $dbwhere";
     }
