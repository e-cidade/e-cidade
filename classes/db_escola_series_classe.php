...
class cl_escola_series { 
   ...
   function atualizacampos($exclusao=false) {
     if($exclusao==false){
       $this->ed18_i_escolas = ($this->ed18_i_escolas == ""?@$GLOBALS["HTTP_POST_VARS"]["ed18_i_escolas"]:$this->ed18_i_escolas);
       $this->ed18_i_series = ($this->ed18_i_series == ""?@$GLOBALS["HTTP_POST_VARS"]["ed18_i_series"]:$this->ed18_i_series);
     }else{
     }
   }
   ...
   function alterar ( $escolas, $series ) {
      $this->atualizacampos();
     ...
     if($result==false){ 
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "Escola Séries nao foi Alterado. Alteracao Executada.\\n";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
         $this->numrows_alterar = 0;
   ...
   function excluir ( $dbwhere=null) {
     ...
     if($result==false){ 
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "Escola Séries nao Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
         $this->numrows_excluir = 0;
   ...
   function sql_query ( $oid = null,$campos="escola_series.oid,*",$ordem=null,$dbwhere=""){ 
     ...
     $sql .= " from escola_series ";
     $sql .= "      inner join escolas  on  escolas.ed02_i_codigo = escola_series.ed18_i_escolas";
     $sql .= "      inner join series  on  series.ed03_i_codigo = escola_series.ed18_i_series";
     ...
     if($dbwhere==""){
       if( $oid != "" && $oid != null){
          $sql2 = " where escola_series.oid = '$oid'";
       }
     }else if($dbwhere != ""){
       $sql2 = " where $dbwhere";
     }
