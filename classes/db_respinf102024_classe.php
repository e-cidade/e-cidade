<?
//MODULO: sicom
//CLASSE DA ENTIDADE respinf102024
class cl_respinf102024 {
  ...
  function cl_respinf102024() {
    //classes dos rotulos dos campos
    $this->rotulo = new rotulo("respinf102024");
  ...
  function incluir ($si197_sequencial){
    ...
    if($si197_sequencial == "" || $si197_sequencial == null ){
      $result = db_query("select nextval('respinf2024_si197_sequencial_seq')");
      if($result==false){
        $this->erro_banco = str_replace("
","",@pg_last_error());
        $this->erro_sql   = "Verifique o cadastro da sequencia: respinf2024";
        $this->erro_msg   = "Usuário: \n\n ".$this->erro_sql." \n\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \n\n ".$this->erro_banco." \n"));
        $this->erro_status = "0";
        return false;
      ...
      $result = db_query("select last_value from respinf2024");
...
","",@pg_last_error());
      if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
        $this->erro_sql   = "respinf2024";
        $this->erro_msg   = "Usuário: \n\n ".$this->erro_sql." \n\n";
        $this->erro_banco = "respinf2024";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \n\n ".$this->erro_banco." \n"));
      }else{
        $this->erro_sql   = "respinf2024";
        $this->erro_msg   = "Usuário: \n\n ".$this->erro_sql." \n\n";
...
","",@pg_last_error());
    ...
    if($this->numrows==0){
      $this->erro_banco = "";
      $this->erro_sql   = "Record Vazio na Tabela:respinf2024";
      $this->erro_msg   = "Usuário: \n\n ".$this->erro_sql." \n\n";
      $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \n\n ".$this->erro_banco." \n"));
      $this->erro_status = "0";
      return false;
  ...
  function sql_query ( $si197_sequencial=null,$campos="*",$ordem=null,$dbwhere=""){
    ...
    $sql .= " from respinf2024 ";
    $sql2 = "";
    if($dbwhere==""){
      if($si197_sequencial!=null ){
        $sql2 .= " where respinf2024.si197_sequencial = $si197_sequencial ";
      }
    }else if($dbwhere != ""){
      $sql2 = " where $dbwhere";
    }
