...
class cl_autprotpagordem {
  ...
  function incluir ($p107_sequencial) {
     ...
     if ($p107_sequencial == "" || $p107_sequencial == null ) {
       ...
       if (($result != false) && (pg_result($result,0,0) < $p107_sequencial)) {
         $this->erro_sql = " Campo p107_sequencial maior que último número da sequencia.";
     ...
     if ($result==false) {
       ...
       if ( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ) {
         $this->erro_sql   = "autprotpagordem ($this->p107_sequencial) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_banco = "autprotpagordem já Cadastrado";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       } else {
         $this->erro_sql   = "autprotpagordem ($this->p107_sequencial) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
  ...
  function alterar ($p107_sequencial=null) {
     ...
     if (trim($this->p107_sequencial)!="" || isset($GLOBALS["HTTP_POST_VARS"]["p107_sequencial"])) {
       ...
       if (trim($this->p107_sequencial) == null ) {
         $this->erro_sql = " Campo p107_sequencial não informado.";
  ...
  function excluir ($p107_sequencial=null,$dbwhere=null) {
     ...
     if ($result==false) {
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "autprotpagordem nao Excluído. Exclusão Abortada.\\n";
       ...
       if (pg_affected_rows($result)==0) {
         $this->erro_banco = "";
         $this->erro_sql = "autprotpagordem nao Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$p107_sequencial;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
