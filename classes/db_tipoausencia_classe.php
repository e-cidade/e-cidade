...
class cl_tipoausencia { 
   ...
   function atualizacampos($exclusao=false) {
     if($exclusao==false){
       ...
       $this->ed320_licenca = ($this->ed320_licenca == "f"?@$GLOBALS["HTTP_POST_VARS"]["ed320_licenca"]:$this->ed320_licenca);
     }else{
       $this->ed320_sequencial = ($this->ed320_sequencial == ""?@$GLOBALS["HTTP_POST_VARS"]["ed320_sequencial"]:$this->ed320_sequencial);
     }
   }
   ...
   function incluir ($ed320_sequencial){ 
     ...
     if($this->ed320_licenca == null ){ 
       $this->erro_sql = " Campo Licença não informado.";
       $this->erro_campo = "ed320_licenca";
       $this->erro_banco = "";
       $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       $this->erro_status = "0";
     ...
     if($ed320_sequencial == "" || $ed320_sequencial == null ){
       $result = db_query("select nextval('tipoausencia_ed320_sequencial_seq')"); 
       if($result==false){
         $this->erro_banco = str_replace("\n","",@pg_last_error());
         $this->erro_sql   = "Verifique o cadastro da sequencia: tipoausencia_ed320_sequencial_seq do campo: ed320_sequencial"; 
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "0";
         return false; 
       ...
       if(($result != false) && (pg_result($result,0,0) < $ed320_sequencial)){
         ...
         $this->ed320_sequencial = $ed320_sequencial; 
       }
     }
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if(($resaco!=false)||($this->numrows!=0)){
         ...
         $resac = db_query("insert into db_acount values($acount,3540,20989,'','".AddSlashes(pg_result($resaco,0,'ed320_licenca'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     return true;
   } 
   ...
   public function alterar ($ed320_sequencial=null) { 
     ...
     if(trim($this->ed320_licenca)!="" || isset($GLOBALS["HTTP_POST_VARS"]["ed320_licenca"])){ 
       $sql  .= $virgula." ed320_licenca = '$this->ed320_licenca' ";
       $virgula = ",";
       if(trim($this->ed320_licenca) == null ){ 
         $this->erro_sql = " Campo Licença não informado.";
         $this->erro_campo = "ed320_licenca";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if ($this->numrows > 0) {
         ...
         for ($conresaco = 0; $conresaco < $this->numrows; $conresaco++) {
           ...
           if (isset($GLOBALS["HTTP_POST_VARS"]["ed320_licenca"]) || $this->ed320_licenca != "")
             $resac = db_query("insert into db_acount values($acount,3540,20989,'".AddSlashes(pg_result($resaco,$conresaco,'ed320_licenca'))."','$this->ed320_licenca',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
     $result = db_query($sql);
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if (($resaco != false) || ($this->numrows!=0)) {
         ...
         for ($iresaco = 0; $iresaco < $this->numrows; $iresaco++) {
           ...
           $resac  = db_query("insert into db_acount values($acount,3540,20989,'','".AddSlashes(pg_result($resaco,$iresaco,'ed320_licenca'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
