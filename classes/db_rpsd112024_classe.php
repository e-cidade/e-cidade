...
class cl_rpsd112024 {
  ...
  function __construct() {
    ...
    $this->rotulo = new rotulo("rpsd112024");
  ...
  function incluir ($si190_sequencial) {
     ...
     if ($si190_sequencial == "" || $si190_sequencial == null ) {
       ...
       if ($result==false) {
         ...
         $this->erro_sql   = "Verifique o cadastro da sequencia: rpsd112024_si190_sequencial_seq do campo: si190_sequencial";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "0";
         return false;
       ...
       $result = db_query("select last_value from rpsd112024_si190_sequencial_seq");
    ...
    /* if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if (($resaco!=false)||($this->numrows!=0)) {
         ...
         $resac = db_query("insert into db_acountkey values($acount,2012024,'$this->si190_sequencial','I')");
         $resac = db_query("insert into db_acount values($acount,1010203,2012024,'','".AddSlashes(pg_result($resaco,0,'si190_sequencial'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         ...
         $resac = db_query("insert into db_acount values($acount,1010203,2012024,'','".AddSlashes(pg_result($resaco,0,'si190_codreduzidorsp'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     ...
     /*if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if ($this->numrows>0) {
         ...
         for($conresaco=0;$conresaco<$this->numrows;$conresaco++) {
           ...
           $acount = pg_result($resac,0,0);
           $resac = db_query("insert into db_acountacesso values($acount,".db_getsession("DB_acessado").")");
           $resac = db_query("insert into db_acountkey values($acount,2012024,'$this->si190_sequencial','A')");
           if (isset($GLOBALS["HTTP_POST_VARS"]["si190_sequencial"]) || $this->si190_sequencial != "")
             $resac = db_query("insert into db_acount values($acount,1010203,2012024,'".AddSlashes(pg_result($resaco,$conresaco,'si190_sequencial'))."','$this->si190_sequencial',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
           ...
           if (isset($GLOBALS["HTTP_POST_VARS"]["si190_codreduzidorsp"]) || $this->si190_codreduzidorsp != "")
             $resac = db_query("insert into db_acount values($acount,1010203,2012024,'".AddSlashes(pg_result($resaco,$conresaco,'si190_codreduzidorsp'))."','$this->si190_codreduzidorsp',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     ...
     /*if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if (($resaco != false) || ($this->numrows!=0)) {
         ...
         for ($iresaco = 0; $iresaco < $this->numrows; $iresaco++) {
           ...
           $acount = pg_result($resac,0,0);
           $resac  = db_query("insert into db_acountacesso values($acount,".db_getsession("DB_acessado").")");
           $resac  = db_query("insert into db_acountkey values($acount,2012024,'$si190_sequencial','E')");
           $resac  = db_query("insert into db_acount values($acount,1010203,2012024,'','".AddSlashes(pg_result($resaco,$iresaco,'si190_sequencial'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
           ...
           $resac  = db_query("insert into db_acount values($acount,1010203,2012024,'','".AddSlashes(pg_result($resaco,$iresaco,'si190_codreduzidorsp'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
  ...
  function sql_record($sql) {
     ...
     $this->numrows = pg_numrows($result);
      if ($this->numrows==0) {
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:rpsd112024";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        $this->erro_status = "0";
        return false;
  ...
  function sql_query ( $si190_sequencial=null,$campos="*",$ordem=null,$dbwhere="") {
     ...
     $sql .= " from rpsd112024 ";
     $sql .= "      inner join rpsd102024  on  rpsd102024.si189_sequencial = rpsd112024.si190_reg10";
     $sql2 = "";
     if ($dbwhere=="") {
       if ($si190_sequencial!=null ) {
         $sql2 .= " where rpsd112024.si190_sequencial = $si190_sequencial ";
       }
     } else if ($dbwhere != "") {
       $sql2 = " where $dbwhere";
     }
