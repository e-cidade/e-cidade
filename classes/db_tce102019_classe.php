...
class cl_tce102019 {
  ...
  function __construct() {
    ...
    $this->rotulo = new rotulo("tce102019");
...
/*     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
  ...
  function sql_record($sql) {
     ...
     $this->numrows = pg_numrows($result);
      if ($this->numrows==0) {
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:tce102019";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        $this->erro_status = "0";
        return false;
  ...
  function sql_query ( $si187_sequencial=null,$campos="*",$ordem=null,$dbwhere="") {
     ...
     if ($dbwhere=="") {
       if ($si187_sequencial!=null ) {
         $sql2 .= " where tce102019.si187_sequencial = $si187_sequencial ";
       }
     } else if ($dbwhere != "") {
       $sql2 = " where $dbwhere";
     }
