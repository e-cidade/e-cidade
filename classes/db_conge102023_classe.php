<?php
//MODULO: sicom
//CLASSE DA ENTIDADE conge102023
class cl_conge102023 {
  ...
  function __construct() {
    ...
    $this->rotulo = new rotulo("conge102023");
...
/*     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
  ...
  function sql_record($sql) {
     ...
     $this->numrows = pg_numrows($result);
      if ($this->numrows==0) {
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:conge102023";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        $this->erro_status = "0";
        return false;
  ...
  function sql_query ( $si182_sequencial=null,$campos="*",$ordem=null,$dbwhere="") {
     ...
     $sql .= " from conge102023 ";
     $sql2 = "";
     if ($dbwhere=="") {
       if ($si182_sequencial!=null ) {
         $sql2 .= " where conge102023.si182_sequencial = $si182_sequencial ";
       }
     } else if ($dbwhere != "") {
       $sql2 = " where $dbwhere";
     }
