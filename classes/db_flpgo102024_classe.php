    ...
    function incluir ($si195_sequencial){
        ...
        $result = db_query($sql);
            ...
            if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
                $this->erro_sql   = "flpgo102024 ($this->si195_sequencial) nao Incluído. Inclusao Abortada.";
                $this->erro_msg   = "Usuário: \n\n ".$this->erro_sql." \n\n";
                $this->erro_banco = "flpgo102024 já Cadastrado";
                $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \n\n ".$this->erro_banco." \n"));
            }else{
                $this->erro_sql   = "flpgo102024 ($this->si195_sequencial) nao Incluído. Inclusao Abortada.";
                $this->erro_msg   = "Usuário: \n\n ".$this->erro_sql." \n\n";
...
//        }
    ...
    function excluir ($si195_sequencial=null,$dbwhere=null) {
        ...
        if($result==false){
            $this->erro_banco = str_replace("","",@pg_last_error());
            $this->erro_sql   = "flpgo102024 nao Excluído. Exclusão Abortada.\n";
            ...
            if(pg_affected_rows($result)==0){
                $this->erro_banco = "";
                $this->erro_sql = "flpgo102024 nao Encontrado. Exclusão não Efetuada.\n";
                $this->erro_sql .= "Valores : ".$si195_sequencial;
                $this->erro_msg   = "Usuário: \n\n ".$this->erro_sql." \n\n";
                $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \n\n ".$this->erro_banco." \n"));
                $this->erro_status = "1";
