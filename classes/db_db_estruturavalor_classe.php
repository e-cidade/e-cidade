...
//CLASSE DA ENTIDADE db_estruturavalor
class cl_db_estruturavalor { 
   ...
   var $campos = "
                 ...
                 db121_descricao = varchar(50) = Descriçao da Estrutural 
                 db121_estruturavalorpai = int4 = Código do Estrutural Pai 
                 db121_nivel = int4 = Nivel da Estrutura 
                 db121_tipoconta = int4 = Tipo da Conta 
                 ";
   //funcao construtor da classe 
   function cl_db_estruturavalor() { 
   ...
   function atualizacampos($exclusao=false) {
     if($exclusao==false){
       $this->db121_sequencial = ($this->db121_sequencial == ""?@$GLOBALS["HTTP_POST_VARS"]["db121_sequencial"]:$this->db121_sequencial);
       $this->db121_db_estrutura = ($this->db121_db_estrutura == ""?@$GLOBALS["HTTP_POST_VARS"]["db121_db_estrutura"]:$this->db121_db_estrutura);
       $this->db121_estrutural = ($this->db121_estrutural == ""?@$GLOBALS["HTTP_POST_VARS"]["db121_estrutural"]:$this->db121_estrutural);
       $this->db121_descricao = ($this->db121_descricao == ""?@$GLOBALS["HTTP_POST_VARS"]["db121_descricao"]:$this->db121_descricao);
       $this->db121_estruturavalorpai = ($this->db121_estruturavalorpai == ""?@$GLOBALS["HTTP_POST_VARS"]["db121_estruturavalorpai"]:$this->db121_estruturavalorpai);
       $this->db121_nivel = ($this->db121_nivel == ""?@$GLOBALS["HTTP_POST_VARS"]["db121_nivel"]:$this->db121_nivel);
       $this->db121_tipoconta = ($this->db121_tipoconta == ""?@$GLOBALS["HTTP_POST_VARS"]["db121_tipoconta"]:$this->db121_tipoconta);
     }else{
       $this->db121_sequencial = ($this->db121_sequencial == ""?@$GLOBALS["HTTP_POST_VARS"]["db121_sequencial"]:$this->db121_sequencial);
     }
   }
   // funcao para inclusao
   function incluir ($db121_sequencial){ 
      $this->atualizacampos();
     if($this->db121_db_estrutura == null ){ 
     ...
     if($this->db121_estruturavalorpai == null ){ 
       $this->db121_estruturavalorpai = "0";
     }
     if($this->db121_nivel == null ){ 
     ...
     if($db121_sequencial == "" || $db121_sequencial == null ){
       $result = db_query("select nextval('db_estruturavalor_db121_sequencial_seq')"); 
       if($result==false){
         $this->erro_banco = str_replace("\n","",@pg_last_error());
         $this->erro_sql   = "Verifique o cadastro da sequencia: db_estruturavalor_db121_sequencial_seq do campo: db121_sequencial"; 
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "0";
         return false; 
       ...
       if(($result != false) && (pg_result($result,0,0) < $db121_sequencial)){
         ...
         $this->db121_sequencial = $db121_sequencial; 
       }
     }
     if(($this->db121_sequencial == null) || ($this->db121_sequencial == "") ){ 
       $this->erro_sql = " Campo db121_sequencial nao declarado.";
     ...
     $sql = "insert into db_estruturavalor(
                                       db121_sequencial 
                                      ,db121_db_estrutura 
                                      ,db121_estrutural 
                                      ,db121_descricao 
                ...
                values (
                                $this->db121_sequencial 
                               ,$this->db121_db_estrutura 
                               ,'$this->db121_estrutural' 
                               ,'$this->db121_descricao' 
                               ,$this->db121_estruturavalorpai 
                               ,$this->db121_nivel 
                               ,$this->db121_tipoconta 
                      )";
     $result = db_query($sql); 
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
     ...
     $this->erro_sql = "Inclusao efetuada com Sucesso\\n";
         $this->erro_sql .= "Valores : ".$this->db121_sequencial;
     $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
     $this->erro_status = "1";
   ...
   function alterar ($db121_sequencial=null) { 
      $this->atualizacampos();
     ...
     $sql .= " where ";
     if($db121_sequencial!=null){
       $sql .= " db121_sequencial = $this->db121_sequencial";
     }
     $resaco = $this->sql_record($this->sql_query_file($this->db121_sequencial));
     if($this->numrows>0){
     ...
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "Valores de Estruturais nao Alterado. Alteracao Abortada.\\n";
         $this->erro_sql .= "Valores : ".$this->db121_sequencial;
       $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       $this->erro_status = "0";
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "Valores de Estruturais nao foi Alterado. Alteracao Executada.\\n";
         $this->erro_sql .= "Valores : ".$this->db121_sequencial;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
   ...
   function excluir ($db121_sequencial=null,$dbwhere=null) { 
     if($dbwhere==null || $dbwhere==""){
       $resaco = $this->sql_record($this->sql_query_file($db121_sequencial));
     }else{ 
     ...
     if($dbwhere==null || $dbwhere ==""){
        if($db121_sequencial != ""){
          ...
          $sql2 .= " db121_sequencial = $db121_sequencial ";
        }
     }else{
       $sql2 = $dbwhere;
     }
     ...
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "Valores de Estruturais nao Excluído. Exclusão Abortada.\\n";
       $this->erro_sql .= "Valores : ".$db121_sequencial;
       $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       $this->erro_status = "0";
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "Valores de Estruturais nao Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$db121_sequencial;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
   ...
   // funcao do sql 
   function sql_query ( $db121_sequencial=null,$campos="*",$ordem=null,$dbwhere=""){ 
     $sql = "select ";
     if($campos != "*" ){
   ...
   // funcao do sql 
   function sql_query_file ( $db121_sequencial=null,$campos="*",$ordem=null,$dbwhere=""){ 
     $sql = "select ";
     if($campos != "*" ){
       $campos_sql = explode("#",$campos);
       $virgula = "";
