...
class cl_veicitensobrig { 
   ...
   function alterar ($ve09_sequencial=null) { 
     ...
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "Itens orbigatórios nao Alterado. Alteracao Abortada.\\n";
         $this->erro_sql .= "Valores : ".$this->ve09_sequencial;
       $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       $this->erro_status = "0";
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "Itens orbigatórios nao foi Alterado. Alteracao Executada.\\n";
         $this->erro_sql .= "Valores : ".$this->ve09_sequencial;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
   ...
   function excluir ($ve09_sequencial=null,$dbwhere=null) { 
     ...
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "Itens obrigatórios nao Excluído. Exclusão Abortada.\\n";
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "Itens obrigatórios nao Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$ve09_sequencial;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
   ...
   function sql_query_obrigatorio( $ve09_sequencial=null,$campos="*",$ordem=null,$dbwhere=""){
     ...
     /**
      * @contass
      * @ocorrência: 870 - Correção de tela de cadastro de veículos
      * @autor: rodrigo@contass
      * @problema: Foi liberado para o usuário inserir somente informações básicas no cadastro do veículo, porém, nas outras abas as informações não estão sendo carregadas caso todos os campos não tenha as informações preenchidas, ajustar para que mesmo sem os campos que não são obrigatórios o sistema execute o fluxo padrão.
      * @solução: Foi verificado que a consulta abaixo retornava os itens obrigatórios dos veículos que tinham no seu cadastro
      * a capacidade, categoriacnh, procedencia, potencia, categoria, tipo de abasteciamento e veicitensobrigbaixa. Entretanto,
      * esses ítens não são de cadastro obrigatório na tela de cadastro de veículos. Portanto, a solução foi colocar left join 
      * nessas tabelas.
      */
