<?
	//MODULO: sicom
	//CLASSE DA ENTIDADE balancete282024
	class cl_balancete282024
	{
		...
		function cl_balancete282024()
		{
			//classes dos rotulos dos campos
			$this->rotulo = new rotulo("balancete282024");
		...
		function incluir($si198_sequencial)
		{
			...
			if ($si198_sequencial == "" || $si198_sequencial == null) {
				$result = db_query("select nextval('balancete282024_si198_sequencial_seq')");
				if ($result == false) {
					$this->erro_banco = str_replace("", "", @pg_last_error());
					$this->erro_sql = "Verifique o cadastro da sequencia: balancete282024_si198_sequencial_seq do campo: si198_sequencial";
					$this->erro_msg = "Usuário: \n\n " . $this->erro_sql . " \n\n";
					$this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
					$this->erro_status = "0";
				...
				$result = db_query("select last_value from balancete282024_si198_sequencial_seq");
			...
			$sql = "insert into balancete282024(
                                       si198_sequencial
                                      ,si198_tiporegistro
                                      ,si198_contacontabil
                                      ,si198_codfundo
			...
			if ($result == false) {
				...
				if (strpos(strtolower($this->erro_banco), "duplicate key") != 0) {
					$this->erro_sql = "balancete282024 ($this->si198_sequencial) nao Incluído. Inclusao Abortada.";
					$this->erro_msg = "Usuário: \n\n " . $this->erro_sql . " \n\n";
					$this->erro_banco = "balancete282024 já Cadastrado";
					$this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
				} else {
					$this->erro_sql = "balancete282024 ($this->si198_sequencial) nao Incluído. Inclusao Abortada.";
					$this->erro_msg = "Usuário: \n\n " . $this->erro_sql . " \n\n";
		...
		{
			$this->atualizacampos();
			$sql = " update balancete282024 set ";
			$virgula = "";
...
", "", @pg_last_error());
				$this->erro_sql = "balancete282024 nao Alterado. Alteracao Abortada.\n";
				$this->erro_sql .= "Valores : " . $this->si198_sequencial;
				$this->erro_msg = "Usuário: \n\n " . $this->erro_sql . " \n\n";
				$this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
				$this->erro_status = "0";
				...
				if (pg_affected_rows($result) == 0) {
					$this->erro_banco = "";
					$this->erro_sql = "balancete282024 nao foi Alterado. Alteracao Executada.\n";
					$this->erro_sql .= "Valores : " . $this->si198_sequencial;
					$this->erro_msg = "Usuário: \n\n " . $this->erro_sql . " \n\n";
					$this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
					$this->erro_status = "1";
		...
		function excluir($si198_sequencial = null, $dbwhere = null)
		{
			...
			$sql = " delete from balancete282024
                    where ";
			$sql2 = "";
			if ($dbwhere == null || $dbwhere == "") {
				if ($si198_sequencial != "") {
			...
			if ($result == false) {
				$this->erro_banco = str_replace("
", "", @pg_last_error());
				$this->erro_sql = "balancete282024 nao Excluído. Exclusão Abortada.\n";
				...
				if (pg_affected_rows($result) == 0) {
					$this->erro_banco = "";
					$this->erro_sql = "balancete282024 nao Encontrado. Exclusão não Efetuada.\n";
					$this->erro_sql .= "Valores : " . $si198_sequencial;
					$this->erro_msg = "Usuário: \n\n " . $this->erro_sql . " \n\n";
					$this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
					$this->erro_status = "1";
...
", "", @pg_last_error());
			...
			if ($this->numrows == 0) {
				$this->erro_banco = "";
				$this->erro_sql = "Record Vazio na Tabela:balancete282024";
				$this->erro_msg = "Usuário: \n\n " . $this->erro_sql . " \n\n";
				$this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
				$this->erro_status = "0";
		...
		function sql_query($si198_sequencial = null, $campos = "*", $ordem = null, $dbwhere = "")
		{
			...
			$sql .= " from balancete282024 ";
			$sql2 = "";
			if ($dbwhere == "") {
				if ($si198_sequencial != null) {
					$sql2 .= " where balancete282024.si198_sequencial = $si198_sequencial ";
				}
			} else {
				if ($dbwhere != "") {
					$sql2 = " where $dbwhere";
