...
class cl_db_sysarqcamp
{
  ...
  function sql_query_modulo($codarq = null, $codcam = null, $seqarq = null, $campos = "*", $ordem = null, $dbwhere = "")
  {
    $sql = "select ";
    if ($campos != "*") {
      $campos_sql = split("#", $campos);
    ...
    $sql .= "      inner join db_sysarquivo  on  db_sysarquivo.codarq = db_sysarqcamp.codarq";
    $sql .= "      inner join db_syscampo   on  db_syscampo.codcam = db_sysarqcamp.codcam";
    $sql .= "      inner join db_sysarqmod  on  db_sysarqmod.codarq =  db_sysarqcamp.codarq";
    $sql .= "      inner join db_sysmodulo on db_sysarqmod.codmod = db_sysmodulo.codmod";
    $sql2 = "";
    if ($dbwhere == "") {
      if ($codarq != null) {
        $sql2 .= " where db_sysarqcamp.codarq = $codarq ";
