<?php
//MODULO: sicom
//CLASSE DA ENTIDADE iderp112022
class cl_iderp112022 {
  ...
  function __construct() {
    ...
    $this->rotulo = new rotulo("iderp112022");
  ...
  function incluir ($si180_sequencial) {
     ...
     if ($si180_sequencial == "" || $si180_sequencial == null ) {
       $result = db_query("select nextval('iderp112022_si180_sequencial_seq')");
       if ($result==false) {
         $this->erro_banco = str_replace("\n","",@pg_last_error());
         $this->erro_sql   = "Verifique o cadastro da sequencia: iderp112022_si180_sequencial_seq do campo: si180_sequencial";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "0";
         return false;
       ...
       $result = db_query("select last_value from iderp112022_si180_sequencial_seq");
...
/*     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
  ...
  function sql_record($sql) {
     ...
     $this->numrows = pg_numrows($result);
      if ($this->numrows==0) {
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:iderp112022";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        $this->erro_status = "0";
        return false;
  ...
  function sql_query ( $si180_sequencial=null,$campos="*",$ordem=null,$dbwhere="") {
     ...
     $sql .= " from iderp112022 ";
     $sql .= "      inner join iderp102020  on  iderp102020.si179_sequencial = iderp112022.si180_reg10";
     $sql2 = "";
     if ($dbwhere=="") {
       if ($si180_sequencial!=null ) {
         $sql2 .= " where iderp112022.si180_sequencial = $si180_sequencial ";
       }
     } else if ($dbwhere != "") {
       $sql2 = " where $dbwhere";
     }
