...
class cl_numpref {
   ...
   public $k03_diasvalidadecertidao = 0;
   public $k03_diasreemissaocertidao = 0;
   public $k03_toleranciacredito = 0;
   public $k03_ativo_integracao_pix = 'f';
   public $k03_instituicao_financeira = null;
   ...
   public $campos = "
                 ...
                 k03_ativo_integracao_pix = bool = Ativa integração com API PIX
                 k03_instituicao_financeira = int4 = Instituição financeira habilitada para integração com o PIX
                 ";
   ...
   function atualizacampos($exclusao=false) {
     if($exclusao==false){
       ...
       $this->k03_ativo_integracao_pix = ($this->k03_ativo_integracao_pix == "f"?@$GLOBALS["HTTP_POST_VARS"]["k03_ativo_integracao_pix"]:$this->k03_ativo_integracao_pix);
       $this->k03_instituicao_financeira = ($this->k03_instituicao_financeira == ""?@$GLOBALS["HTTP_POST_VARS"]["k03_instituicao_financeira"]:$this->k03_instituicao_financeira);
     }else{
       $this->k03_anousu = ($this->k03_anousu == ""?@$GLOBALS["HTTP_POST_VARS"]["k03_anousu"]:$this->k03_anousu);
       $this->k03_instit = ($this->k03_instit == ""?@$GLOBALS["HTTP_POST_VARS"]["k03_instit"]:$this->k03_instit);
     }
   ...
   function incluir ($k03_anousu,$k03_instit){
     ...
     if ($this->k03_ativo_integracao_pix === 't') {
         if (empty($this->k03_instituicao_financeira)) {
             $this->erro_sql = " Campo k03_instituicao_financeira nao declarado.";
             $this->erro_banco = "Instituição Financeira não informada";
             $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
             $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
             $this->erro_status = "0";
             return false;
     ...
     $sql = "insert into numpref(
                ...
                values (
                               ...
                               ,'$this->k03_ativo_integracao_pix'
                               ,$this->k03_instituicao_financeira
                      )";
     $result = db_query($sql);
     if($result==false){
       $this->erro_banco = str_replace("\n","",@pg_last_error());
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if(($resaco!=false)||($this->numrows!=0)){
         ...
         $resac = db_query("insert into db_acount values($acount,318,(select codcam from db_syscampo where nomecam = 'k03_ativo_integracao_pix'),'','".AddSlashes(pg_result($resaco,0,'k03_ativo_integracao_pix'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = db_query("insert into db_acount values($acount,318,(select codcam from db_syscampo where nomecam = 'k03_instituicao_financeira'),'','".AddSlashes(pg_result($resaco,0,'k03_instituicao_financeira'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     return true;
   }
   ...
   function alterar ($k03_anousu=null,$k03_instit=null) {
     ...
     if (!empty($this->k03_ativo_integracao_pix)) {
       ...
       if ($this->k03_ativo_integracao_pix === 't') {
           if (empty($this->k03_instituicao_financeira)) {
               $this->erro_sql = " Campo Instituição Financeira não informado.";
               $this->erro_campo = "k03_instituicao_financeira";
               $this->erro_banco = "";
               $this->erro_msg = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
               $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \\n\\n " . $this->erro_banco . " \\n"));
               $this->erro_status = "0";
           ...
           $sql .= $virgula . " k03_instituicao_financeira = $this->k03_instituicao_financeira ";
       }
       if ($this->k03_ativo_integracao_pix === 'f') {
           $sql .= $virgula . " k03_instituicao_financeira = null ";
       }
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if($this->numrows>0) {
           ...
           for ($conresaco = 0; $conresaco < $this->numrows; $conresaco++) {
               ...
               if (isset($GLOBALS["HTTP_POST_VARS"]["k03_ativo_integracao_pix"]) || $this->k03_ativo_integracao_pix != "")
                   $resac = db_query("insert into db_acount values($acount,318,(select codcam from db_syscampo where nomecam = 'k03_ativo_integracao_pix'),'" . AddSlashes(pg_result($resaco, $conresaco, 'k03_ativo_integracao_pix')) . "','$this->k03_ativo_integracao_pix'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");

               if (isset($GLOBALS["HTTP_POST_VARS"]["k03_instituicao_financeira"]) || $this->k03_instituicao_financeira != "")
                   $resac = db_query("insert into db_acount values($acount,318,(select codcam from db_syscampo where nomecam = 'k03_instituicao_financeira'),'" . AddSlashes(pg_result($resaco, $conresaco, 'k03_instituicao_financeira')) . "','$this->k03_instituicao_financeira'," . db_getsession('DB_datausu') . "," . db_getsession('DB_id_usuario') . ")");
           }
       }
     }
     $result = db_query($sql);
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if (($resaco != false) || ($this->numrows!=0)) {
         ...
         for ($iresaco = 0; $iresaco < $this->numrows; $iresaco++) {
           ...
           $resac  = db_query("insert into db_acount values($acount,318,(select codcam from db_syscampo where nomecam = 'k03_ativo_integracao_pix'),'','".AddSlashes(pg_result($resaco,$iresaco,'k03_ativo_integracao_pix'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
           $resac  = db_query("insert into db_acount values($acount,318,(select codcam from db_syscampo where nomecam = 'k03_instituicao_financeira'),'','".AddSlashes(pg_result($resaco,$iresaco,'k03_instituicao_financeira'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
   ...
   function sql_query ( $k03_anousu=null,$k03_instit=null,$campos="*",$ordem=null,$dbwhere=""){
     ...
     $sql .= " left join configuracoes_pix_banco_do_brasil on k177_instituicao_financeira = numpref.k03_instituicao_financeira";
     $sql2 = "";
     if($dbwhere==""){
       if($k03_anousu!=null ){
