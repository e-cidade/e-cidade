<?php
//MODULO: sicom
//CLASSE DA ENTIDADE conge202019
class cl_conge202019 {
  ...
  function __construct() {
    ...
    $this->rotulo = new rotulo("conge202019");
...
/*     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
  ...
  function sql_record($sql) {
     ...
     $this->numrows = pg_numrows($result);
      if ($this->numrows==0) {
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:conge202019";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        $this->erro_status = "0";
        return false;
  ...
  function sql_query ( $si183_sequencial=null,$campos="*",$ordem=null,$dbwhere="") {
     ...
     $sql .= " from conge202019 ";
     $sql2 = "";
     if ($dbwhere=="") {
       if ($si183_sequencial!=null ) {
         $sql2 .= " where conge202019.si183_sequencial = $si183_sequencial ";
       }
     } else if ($dbwhere != "") {
       $sql2 = " where $dbwhere";
     }
