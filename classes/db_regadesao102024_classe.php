<?
//MODULO: sicom
//CLASSE DA ENTIDADE regadesao102024
class cl_regadesao102024
{
  ...
  function cl_regadesao102024()
  {
    //classes dos rotulos dos campos
    $this->rotulo = new rotulo("regadesao102024");
  ...
  function incluir($si67_sequencial)
  {
    ...
    if ($si67_sequencial == "" || $si67_sequencial == null) {
      ...
      $result = db_query("select last_value from regadesao102024_si67_sequencial_seq");
    ...
    $sql = "insert into regadesao102024(
                                       si67_sequencial
                                      ,si67_tiporegistro
                                      ,si67_codorgao
                                      ,si67_codunidadesub
    ...
    if ($result == false) {
      ...
      if (strpos(strtolower($this->erro_banco), "duplicate key") != 0) {
        ...
        $this->erro_msg = "Usuário: \n\n " . $this->erro_sql . " \n\n";
        $this->erro_banco = "regadesao102024 já Cadastrado";
        $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
      } else {
        $this->erro_sql = "regadesao102024 ($this->si67_sequencial) nao Incluído. Inclusao Abortada.";
        $this->erro_msg = "Usuário: \n\n " . $this->erro_sql . " \n\n";
  ...
  {
    $this->atualizacampos();
    $sql = " update regadesao102024 set ";
    $virgula = "";
...
", "", @pg_last_error());
      $this->erro_sql = "regadesao102024 nao Alterado. Alteracao Abortada.\n";
      $this->erro_sql .= "Valores : " . $this->si67_sequencial;
      $this->erro_msg = "Usuário: \n\n " . $this->erro_sql . " \n\n";
      $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
      $this->erro_status = "0";
      ...
      if (pg_affected_rows($result) == 0) {
        $this->erro_banco = "";
        $this->erro_sql = "regadesao102024 nao foi Alterado. Alteracao Executada.\n";
        $this->erro_sql .= "Valores : " . $this->si67_sequencial;
        $this->erro_msg = "Usuário: \n\n " . $this->erro_sql . " \n\n";
        $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
        $this->erro_status = "1";
  ...
  function excluir($si67_sequencial = null, $dbwhere = null)
  {
    ...
    $sql = " delete from regadesao102024
                    where ";
    $sql2 = "";
    if ($dbwhere == null || $dbwhere == "") {
      if ($si67_sequencial != "") {
    ...
    if ($result == false) {
      $this->erro_banco = str_replace("
", "", @pg_last_error());
      $this->erro_sql = "regadesao102024 nao Excluído. Exclusão Abortada.\n";
      ...
      if (pg_affected_rows($result) == 0) {
        $this->erro_banco = "";
        $this->erro_sql = "regadesao102024 nao Encontrado. Exclusão não Efetuada.\n";
        $this->erro_sql .= "Valores : " . $si67_sequencial;
        $this->erro_msg = "Usuário: \n\n " . $this->erro_sql . " \n\n";
        $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
        $this->erro_status = "1";
...
", "", @pg_last_error());
    ...
    if ($this->numrows == 0) {
      $this->erro_banco = "";
      $this->erro_sql = "Record Vazio na Tabela:regadesao102024";
      $this->erro_msg = "Usuário: \n\n " . $this->erro_sql . " \n\n";
      $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
      $this->erro_status = "0";
      return false;
  ...
  function sql_query($si67_sequencial = null, $campos = "*", $ordem = null, $dbwhere = "")
  {
    ...
    $sql .= " from regadesao102024 ";
    $sql2 = "";
    if ($dbwhere == "") {
      if ($si67_sequencial != null) {
        $sql2 .= " where regadesao102024.si67_sequencial = $si67_sequencial ";
      }
    } else if ($dbwhere != "") {
      $sql2 = " where $dbwhere";
    }
