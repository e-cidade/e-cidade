...
class cl_issplaninscr { 
   ...
   function incluir ($q24_sequencial){ 
     ...
     if($q24_sequencial == "" || $q24_sequencial == null ){
      $ultimo_erro = pg_last_error();
       $result = db_query("select nextval('issplaninscr_q24_sequencial_seq')"); 
       if($result==false){
         $this->erro_banco = str_replace("\n","",@pg_last_error());
         $this->erro_sql   = "Verifique o cadastro da sequencia: issplaninscr_q24_sequencial_seq do campo: q24_sequencial. Ultimo Erro: {$ultimo_erro}"; 
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "0";
         return false; 
