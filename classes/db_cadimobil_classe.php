...
class cl_cadimobil { 
   ...
   function atualizacampos($exclusao=false) {
     if($exclusao==false){
       $this->j63_numcgm = ($this->j63_numcgm == ""?@$GLOBALS["HTTP_POST_VARS"]["j63_numcgm"]:$this->j63_numcgm);
     }else{
       $this->j63_numcgm = ($this->j63_numcgm == ""?@$GLOBALS["HTTP_POST_VARS"]["j63_numcgm"]:$this->j63_numcgm);
     }
   }
   // funcao para inclusao
   function incluir ($j63_numcgm){ 
      $this->atualizacampos();
   ...
   function alterar ($j63_numcgm=null) { 
      $this->atualizacampos();
     $sql = " update cadimobil set ";
     $virgula = "";
     if(trim($this->j63_numcgm)!="" || isset($GLOBALS["HTTP_POST_VARS"]["j63_numcgm"])){ 
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["j63_numcgm"]))
   ...
   function excluir ($j63_numcgm=null,$dbwhere=null) { 
   ...
   function sql_query ( $j63_numcgm=null,$campos="*",$ordem=null,$dbwhere=""){ 
     $sql = "select ";
     if($campos != "*" ){
   ...
   function sql_query_file ( $j63_numcgm=null,$campos="*",$ordem=null,$dbwhere=""){ 
     $sql = "select ";
     if($campos != "*" ){
       $campos_sql = explode("#",$campos);
       $virgula = "";
