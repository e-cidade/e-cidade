<?
//MODULO: sicom
//CLASSE DA ENTIDADE dispensa402025
class cl_dispensa402025 {
   ...
   function cl_dispensa402025() {
     //classes dos rotulos dos campos
     $this->rotulo = new rotulo("dispensa402025");
   ...
   function incluir ($si204_sequencial){
    ...
    if($this->si204_perctaxaadm == null ){
     ...
     if($si204_sequencial == "" || $si204_sequencial == null ){
       $result = db_query("select nextval('dispensa402025_si204_sequencial_seq')");
       if($result==false){
         $this->erro_banco = str_replace("
","",@pg_last_error());
         $this->erro_sql   = "Verifique o cadastro da sequencia: dispensa402025_si204_sequencial_seq do campo: si204_sequencial";
         $this->erro_msg   = "Usu�rio: \n\n ".$this->erro_sql." \n\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \n\n ".$this->erro_banco." \n"));
         $this->erro_status = "0";
         return false;
       ...
       $result = db_query("select last_value from dispensa402025_si204_sequencial_seq");
     ...
     $sql = "insert into dispensa402025(
                                      si204_sequencial
                                      ,si204_tiporegistro
                                      ,si204_codorgaoresp
...
","",@pg_last_error());
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         $this->erro_sql   = "dispensa402025 ($this->si75_sequencial) nao Inclu�do. Inclusao Abortada.";
         $this->erro_msg   = "Usu�rio: \n\n ".$this->erro_sql." \n\n";
         $this->erro_banco = "dispensa402025 j� Cadastrado";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \n\n ".$this->erro_banco." \n"));
       }else{
         $this->erro_sql   = "dispensa402025 ($this->si75_sequencial) nao Inclu�do. Inclusao Abortada.";
         $this->erro_msg   = "Usu�rio: \n\n ".$this->erro_sql." \n\n";
   ...
   function excluir ($si204_sequencial=null,$dbwhere=null) {
     ...
     $sql = " delete from dispensa402025
                    where ";
     $sql2 = "";
     ...
     if($result==false){
       $this->erro_banco = str_replace("
","",@pg_last_error());
       $this->erro_sql   = "dispensa402025 nao Exclu�do. Exclus�o Abortada.\n";
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "dispensa402025 nao Encontrado. Exclus�o n�o Efetuada.\n";
         $this->erro_sql .= "Valores : ".$si204_sequencial;
         $this->erro_msg   = "Usu�rio: \n\n ".$this->erro_sql." \n\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \n\n ".$this->erro_banco." \n"));
         $this->erro_status = "1";
...
","",@pg_last_error());
     ...
     $this->numrows = pg_numrows($result);
      if($this->numrows==0){
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:dispensa402025";
        $this->erro_msg   = "Usu�rio: \n\n ".$this->erro_sql." \n\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \n\n ".$this->erro_banco." \n"));
        $this->erro_status = "0";
        return false;
   ...
   function sql_query ( $si204_sequencial=null,$campos="*",$ordem=null,$dbwhere=""){
     ...
     $sql .= " from dispensa402025 ";
     $sql2 = "";
     if($dbwhere==""){
       if($si204_sequencial!=null ){
         $sql2 .= " where dispensa402025.si204_sequencial = $si204_sequencial ";
       }
     }else if($dbwhere != ""){
       $sql2 = " where $dbwhere";
     }
