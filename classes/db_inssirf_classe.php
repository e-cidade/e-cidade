...
class cl_inssirf
{
    ...
    function atualizacampos($exclusao = false)
    {
        if ($exclusao == false) {
            ...
            $this->r33_tiporegime = ($this->r33_tiporegime == "" ? @$GLOBALS["HTTP_POST_VARS"]["r33_tiporegime"] : $this->r33_tiporegime);
        } else {
            $this->r33_instit = ($this->r33_instit == "" ? @$GLOBALS["HTTP_POST_VARS"]["r33_instit"] : $this->r33_instit);
            $this->r33_codigo = ($this->r33_codigo == "" ? @$GLOBALS["HTTP_POST_VARS"]["r33_codigo"] : $this->r33_codigo);
        }
    ...
    function incluir($r33_codigo, $r33_instit)
    {
        ...
        if ($r33_codigo == "" || $r33_codigo == null) {
            ...
            if ($result == false) {
                ...
                $this->erro_sql   = "Verifique o cadastro da sequência: inssirf_r33_codigo_seq do campo: r33_codigo";
                $this->erro_msg   = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
                $this->erro_msg   .=  str_replace('"', "", str_replace("'", "",  "Administrador: \\n\\n " . $this->erro_banco . " \\n"));
                $this->erro_status = "0";
                return false;
            ...
            if (($result != false) && (pg_result($result, 0, 0) < $r33_codigo)) {
                $this->erro_sql = " Campo r33_codigo maior que último número da sequência.";
        ...
        if ($result == false) {
            ...
            if (strpos(strtolower($this->erro_banco), "duplicate key") != 0) {
                $this->erro_sql   = "Cadastro das Tabelas                               ($this->r33_codigo." - ".$this->r33_instit) não Incluído. Inclusao Abortada.";
                $this->erro_msg   = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
    ...
    function alterar($r33_codigo = null, $r33_instit = null)
    {
        ...
        if (trim($this->r33_tiporegime) != "" || isset($GLOBALS["HTTP_POST_VARS"]["r33_tiporegime"])) {
            $sql  .= $virgula . " r33_tiporegime = '$this->r33_tiporegime' ";
            $virgula = ",";
        }
        $sql .= " where ";
        if ($r33_codigo != null) {
        ...
        if ($result == false) {
            $this->erro_banco = str_replace("\n", "", @pg_last_error());
            $this->erro_sql   = "Cadastro das Tabelas                               não Alterado. Alteracao Abortada.\\n";
            $this->erro_sql .= "Valores : " . $this->r33_codigo . "-" . $this->r33_instit;
            $this->erro_msg   = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
            $this->erro_msg   .=  str_replace('"', "", str_replace("'", "",  "Administrador: \\n\\n " . $this->erro_banco . " \\n"));
            $this->erro_status = "0";
            ...
            if (pg_affected_rows($result) == 0) {
                $this->erro_banco = "";
                $this->erro_sql = "Cadastro das Tabelas                               não foi Alterado. Alteracao Executada.\\n";
                $this->erro_sql .= "Valores : " . $this->r33_codigo . "-" . $this->r33_instit;
                $this->erro_msg   = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
                $this->erro_msg   .=  str_replace('"', "", str_replace("'", "",  "Administrador: \\n\\n " . $this->erro_banco . " \\n"));
                $this->erro_status = "1";
