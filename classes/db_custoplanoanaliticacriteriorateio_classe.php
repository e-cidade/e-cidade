...
class cl_custoplanoanaliticacriteriorateio { 
   ...
   function atualizacampos($exclusao=false) {
     if($exclusao==false){
       ...
       $this->cc07_automatico = ($this->cc07_automatico == "f"?@$GLOBALS["HTTP_POST_VARS"]["cc07_automatico"]:$this->cc07_automatico);
     }else{
       $this->cc07_sequencial = ($this->cc07_sequencial == ""?@$GLOBALS["HTTP_POST_VARS"]["cc07_sequencial"]:$this->cc07_sequencial);
     }
   }
   ...
   function incluir ($cc07_sequencial){ 
     ...
     $sql = "insert into custoplanoanaliticacriteriorateio(
                                       cc07_sequencial 
   ...
   function sql_query ( $cc07_sequencial=null,$campos="*",$ordem=null,$dbwhere=""){ 
     ...
     $sql .= " from custoplanoanaliticacriteriorateio ";
     $sql .= "      inner join custoplanoanalitica  on  custoplanoanalitica.cc04_sequencial = custoplanoanaliticacriteriorateio.cc07_custoplanoanalitica";
     $sql .= "      inner join custocriteriorateio  on  custocriteriorateio.cc08_sequencial = custoplanoanaliticacriteriorateio.cc07_custocriteriorateio";
     ...
     if($dbwhere==""){
       if($cc07_sequencial!=null ){
         $sql2 .= " where custoplanoanaliticacriteriorateio.cc07_sequencial = $cc07_sequencial "; 
       } 
     }else if($dbwhere != ""){
       $sql2 = " where $dbwhere";
     }
