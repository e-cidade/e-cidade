...
class ArquivoRPLiquidacaoAdiantamento extends ArquivoBase
{
    ...
    public function validaTipo($codnota){
        ...
        $resultado = pg_fetch_all($sql);
        return ($resultado[0]["e177_codigo"] == 5) ? true : false;
    }
    ...
    public function buscaPagordemProcesso($codord){
        $sql = pg_query("SELECT * FROM pagordemprocesso WHERE e03_pagordem = {$codord}");
        $resultado = pg_fetch_all($sql);
        return $resultado[0];
    }
    ...
    {   
        ...
        foreach ($empenhos as $empenho) {
            ...
            $dadospagordemprocesso = $this->buscaPagordemProcesso($codord);
            
            $vt = $this->validaTipo($empenho->c66_codnota);            
            if(!$vt){continue;}

            $nomeServidor = $empenho->z01_nome; //!empty($empenho->nome_servidor) ? $empenho->nome_servidor : $empenho->nome_responsavel;
            $dadosLiquidacaoEmpenho = (object)[
                ...
                'Finalidade' => Helper::convertAndLimit($dadosordem["e50_obs"], 255),
                'AnoLiquidacaoRestosPagar' => $dadosordem["e50_anousu"],
                'CodigoOrgao' => $empenho->o58_orgao,
                'CodigoUnidadeOrcamentaria' => $empenho->o58_unidade,
                'ProcessoAdministrativo' => $dadospagordemprocesso["e03_numeroprocesso"],
            ];
        ...
        if(count($obj->LiquidacoesRestosPagarAdiantamentos) == 0){
            throw new \Exception(sprintf(
                'Não foi encontrado nenhum registro para o arquivo da Remessa de %s',
                'Restos a Pagar - Liquidação - Adiantamento'
            ));
