...
class Session extends \ECidade\V3\Extension\Session
{
  ...
  protected function setup()
  {
    ...
    if (!ini_get("session.use_cookies")) {
      throw new ResponseException('A diretiva de cookies para sessão n�o está habilitada nas configurações do PHP.');
    }

    if (ini_get('suhosin.session.encrypt')) {
      throw new ResponseException('Sistema n�o é compatível com a criptografia de sessão da extensão Suhosin.');
    }
