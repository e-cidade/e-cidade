...
class Manager extends AbstractManager
{
  ...
  public function unpack($file, $force = false)
  {
    ...
    if (pathinfo($file, PATHINFO_EXTENSION) != "gz") {
      throw new Exception("Arquivo com extensÃ£o invÃ¡lida, esperado tar.gz");
    }
    ...
    if ($extensionData->exists() && !$force) {
      throw new Exception("ExtensÃ£o jÃ¡ descompactada.");
    }
  ...
  public function install($id, $user = null)
  {
    ...
    if (!$extensionData->exists()) {
      throw new Exception("ExtensÃ£o não descompactada: '$id'");
    }

    if ($extensionData->isUserType() && empty($user)) {
      throw new Exception("Usuário da extensÃ£o '$id' não informado");
    }

    if ($extensionData->isEnabled($user)) {
      throw new Exception("ExtensÃ£o jÃ¡ instalada: '$id'" . ($extensionData->isUserType() ? " usuário '$user'" : null));
    }
  ...
  public function uninstall($id, $user = null)
  {
    ...
    if (false === $extensionData->exists()) {
      throw new Exception("ExtensÃ£o não instalada: $id");
    }
    ...
    if (false === $extensionData->isEnabled($user)) {
      throw new Exception("ExtensÃ£o desativada: $id");
    }
