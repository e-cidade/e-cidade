<?php
  ...
  global $contapagina;
        ...
        ////////// MODELO 18  -  REQUISI√á√ÉO DE SA√çDA DE MATERIAIS 
...
//verifica se algum item tem observa√ß√£o
...
if ($cont_obs > 0){
// 8 itens por folha se algum item conter observa√ß√£o
    $quant_itens = 8;
...
if ($iPag_a==true && $iPag_b==true){
  ...
  //14 itens por folha se n„o conter observa√ß√£o em nenhum item
    $quant_itens = 20;
    $qReg=$this->linhasdositens;
    for ($i=0; $i < $qReg; $i++){
       $contaobs++;
...
for ($i = 0;$i < $iVias;$i++){
      ...
      $comeco = 0;
          ...
          $this->objpdf->text(110,$xlin-13,'ANULA«√O DE SOLICITA«√O DE MATERIAIS N'.chr(176).' '.$this->Rnumero);
          ...
          $this->objpdf->text($xcol+2,$xlin+9,'CÛdigo ');
          $this->objpdf->Setfont('Arial','',8);
          ...
          $this->objpdf->text($xcol+105,$xlin+9,'CÛdigo ');
          $this->objpdf->Setfont('Arial','',8);
          ...
          $this->objpdf->Setfont('Arial','b',8);
          $this->objpdf->text($xcol+2,$xlin+25,'CODIGO');
          $this->objpdf->text($xcol+32,$xlin+25,'DESCRI«√O');
          ...
          $this->objpdf->text($xcol+183,$xlin+30,'ANULADO');
 ...
 $result_endent = pg_exec("select j14_nome as j14_nome_almox, numero as numero_almox, compl as compl_almox, j13_descr as j13_descr_almox, fonedepto as fone_almox, ramaldepto as ramal_almox, faxdepto as fax_almox
        ...
        if (pg_numrows($result_endent) > 0) {
          ...
          $this->objpdf->Setfont('Arial','b',8);
          $this->objpdf->text($xcol+2,$xlin+215,'Endere√ßo de Entrega dos Materiais');
          ...
          $this->objpdf->text($xcol+92,$xlin+255,'Respons·vel Almoxarifado');
          $this->objpdf->Setfont('Arial','',8);
          $this->objpdf->sety($xlin+220);
          $this->objpdf->setx($xcol+1);
