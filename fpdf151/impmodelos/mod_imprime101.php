...
for ($i = 0;$i < 2;$i++){
  ...
  $this->objpdf->text(150,$xlin-13,'VÁLIDO PARA PAGAMENTO ATÉ: ');
  ...
  $sqlhist = "select distinct
                  ...
                  from db_reciboweb
                       ...
                       left join proced on v01_proced = v03_codigo
                 where k99_numpre_n = {$this->numnov_recibo}
                 order by v01_exerc,v01_numpar";
  $result = db_query($sqlhist);

  $aResults = db_utils::getCollectionByRecord($result);
  $sHistorico = "Referência: \n";
  foreach($aResults as $obj){
    $sHistorico .= "{$obj->v01_numpar}/{$obj->v01_exerc} {$obj->v03_descr}. Venc.: ".db_formatar($obj->v01_dtvenc, 'd')." | ";
  }
  $sHistoricoGeral = substr($this->historico."\n\n".$sHistorico,0,810);

  $this->objpdf->multicell(150,2,"HISTÓRICO :  ".$sHistoricoGeral);
  $this->objpdf->SetX($xcol+3);
  //dados do desconto
