...
$this->objpdf->text(130, $xlin - 12, 'PROCESSO DE COMPRA N' . CHR(176));
$this->objpdf->text(175, $xlin - 12, db_formatar($this->Snumero, 's', '0', 6, 'e'));
...
$this->objpdf->text(130, $xlin - 3, 'USUÁRIO');
$this->objpdf->text(142, $xlin - 3, ': ' . substr($this->Susuarioger, 0, 40));
...
/*  if ((isset($servico) && (trim($servico) == "f" || trim($servico) == "")) || !isset($servico)) {
  ...
  if ($reservado == "t")
    $this->objpdf->Row(array($item, $quantitem, $unid, '[ITEM ME/EPP] ' . $scodpcmater . $descricaoitem . " - SOLICITAÇÃO: {$iCodigoSolicita} PA: {$sProcessoAdministrativo}" . $barran, $valoritem, $valimp), 3, false, $distanciar, 0, true);
  else
