...
for ($i = 0; $i < 2; $i++) {
    ...
    $this->objpdf->Setfont('Arial', '', 8);
    $this->objpdf->text($xcol + 116, $xlin + 40.5, '14 - AGENTES NOCIVOS');
    $this->objpdf->Setfont('Arial', 'B', 10);
    $this->objpdf->text($xcol + 155, $xlin + 40.5, 'R$' . db_formatar($this->agentes_nocivos, 'f'));
    ...
    $this->objpdf->Setfont('Arial', '', 8);
    $this->objpdf->text($xcol + 116, $xlin + 54.5, '16 - TOTAL LÍQUIDO');
    $this->objpdf->Setfont('Arial', 'B', 10);
    $this->objpdf->text($xcol + 155, $xlin + 54.5, 'R$' . db_formatar($liquido, 'f'));
    $this->objpdf->rect($xcol + 115, $xlin + 50, 35, $alt);
    $this->objpdf->rect($xcol + 150, $xlin + 50, 50, $alt);

    $this->objpdf->Setfont('Arial', '', 8);
    $this->objpdf->text($xcol + 116, $xlin + 61.5, '17 - ATUAL. MONETÁRIA');
    $this->objpdf->Setfont('Arial', 'B', 10);
    $this->objpdf->text($xcol + 155, $xlin + 61.5, 'R$' . db_formatar($this->atu_monetaria, 'f'));
    $this->objpdf->rect($xcol + 115, $xlin + 57, 35, $alt);
    $this->objpdf->rect($xcol+150,$xlin+57,50,$alt);

    $this->objpdf->Setfont('Arial', '', 8);
    $this->objpdf->text($xcol + 116, $xlin + 68.5, '18 - JUROS/MULTA');
    $this->objpdf->Setfont('Arial', 'B', 10);
    $this->objpdf->text($xcol + 155, $xlin + 68.5, 'R$' . db_formatar($this->juros, 'f'));
    $this->objpdf->rect($xcol + 115, $xlin + 64, 35, $alt);
    $this->objpdf->rect($xcol + 150, $xlin + 64, 50, $alt);

    $this->objpdf->Setfont('Arial', '', 8);
    $this->objpdf->text($xcol + 116, $xlin + 75.5, '19 - CAMPO EXTRA');
    $this->objpdf->Setfont('Arial', 'B', 10);
    $this->objpdf->text($xcol + 155, $xlin + 75.5, 'R$' . db_formatar($this->campoextra, 'f'));
    $this->objpdf->rect($xcol + 115, $xlin + 71, 35, $alt);
    $this->objpdf->rect($xcol + 150, $xlin + 71, 50, $alt);

    $liquido += $this->liquido + $this->campoextra;

    $this->objpdf->Setfont('Arial','',8);
    $this->objpdf->text($xcol+116,$xlin+82.5,'20 - TOTAL');
    $this->objpdf->Setfont('Arial','B',10);
    $this->objpdf->text($xcol+155,$xlin+82.5,'R$'.db_formatar($liquido,'f'));
    $this->objpdf->rect($xcol+115,$xlin+85,35,$alt);
    $this->objpdf->rect($xcol+150,$xlin+85,50,$alt);

    $this->objpdf->rect($xcol + 115, $xlin + 85, 85, 37);
