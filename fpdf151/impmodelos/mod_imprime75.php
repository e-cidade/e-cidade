...
$xlin = 19;
$xcol = 4;
$iAltQuadroPrincipal = 90;

for ($i = 0; $i < 3; $i++){
	
	if($i == 2){
		
		$iAltQuadroPrincipal = 105;
	}
  ...
  $this->objpdf->roundedrect($xcol-2, $xlin-18, 206, $iAltQuadroPrincipal, 2,'DF','1234');
  ...
  $this->objpdf->text(178, $xlin - 4, ($i+1). 'ª Via '.$str_via );
  ...
  $this->objpdf->Roundedrect($xcol, $xlin - 2, $xcol + 119, 16, 2, 'DF','1234');
  $this->objpdf->Setfont('Arial','',6);
  $this->objpdf->text($xcol + 2, $xlin , 'Identificação:');
  
  $this->objpdf->Setfont('Arial','',8);
  $this->objpdf->text($xcol +  2, $xlin +  3, 'Nome :');
  $this->objpdf->text($xcol + 17, $xlin +  3, $this->nome);
  $this->objpdf->text($xcol +  2, $xlin +  6, 'Endereço :');
  $this->objpdf->text($xcol + 17, $xlin +  6, $this->ender);
  $this->objpdf->text($xcol +  2, $xlin +  9, 'Bairro :');
  $this->objpdf->text($xcol + 17, $xlin +  9, $this->bairrocontri);
  $this->objpdf->text($xcol +  2, $xlin + 12, 'Município : ');
  $this->objpdf->text($xcol + 17, $xlin + 12, "{$this->munic}");
  $this->objpdf->text($xcol + 75, $xlin +  9, 'CEP :  ');
  $this->objpdf->text($xcol + 82, $xlin +  9, $this->cep);
  
  $this->objpdf->text($xcol + 128, $xlin - 4     , 'Data :'. date("d-m-Y",db_getsession("DB_datausu")). 'Hora: '.date("H:i:s"));
  $this->objpdf->text($xcol + 75 , $xlin + 12, 'CNPJ/CPF:');
  $this->objpdf->text($xcol + 90 , $xlin + 12, db_formatar($this->cgccpf,(strlen($this->cgccpf)<12?'cpf':'cnpj')));
  
  $this->objpdf->Setfont('Arial','',6);
  
  $this->objpdf->Roundedrect($xcol + 126, $xlin - 2, 76 , 16, 2, 'DF','1234');
  
  $this->objpdf->text($xcol + 128, $xlin + 1, $this->identifica_dados);
  $this->objpdf->text($xcol + 128, $xlin + 1, $this->tipoinscr);
  $this->objpdf->text($xcol + 145, $xlin + 1, $this->nrinscr);
  
  $this->objpdf->text($xcol + 128, $xlin +  4, $this->tipolograd);
  $this->objpdf->text($xcol + 145, $xlin +  4, $this->nomepri);
  $this->objpdf->text($xcol + 128, $xlin +  7, $this->tipocompl);
  $this->objpdf->text($xcol + 145, $xlin +  7, $this->nrpri."      ".$this->complpri);
  $this->objpdf->text($xcol + 128, $xlin + 10, $this->tipobairro );
  $this->objpdf->text($xcol + 145, $xlin + 10, $this->bairropri);
  
  $this->objpdf->Roundedrect($xcol, $xlin + 15, 202, 20, 2, 'DF', '1234');
  
  $this->objpdf->sety($xlin + 16);
  $maiscol = 0;
  $yy = $this->objpdf->gety();
  
   if($this->linhasdadospagto >= 11){
     db_redireciona('db_erros.php?fechar=true&db_erro=O numero de receitas ultrapassou o espaço limite do carne.  Gere um novo recibo!');
     break;
   }
  ...
  $this->objpdf->Roundedrect($xcol, $xlin + 36, 176, 23.2, 2, 'DF', '1234');
  $this->objpdf->SetY($xlin + 36);
  $this->objpdf->SetX($xcol+3);
  $this->objpdf->multicell(150,4,'HISTÓRICO :   '.$this->historico);
  $this->objpdf->SetX($xcol+3);
  //dados do desconto

  $this->objpdf->Roundedrect(181, $xlin + 36  , 25, 7, 2, 'DF', '1234');
  $this->objpdf->Roundedrect(181, $xlin + 44  , 25, 7, 2, 'DF', '1234');
  $this->objpdf->Roundedrect(181, $xlin + 52  , 25, 7, 2, 'DF', '1234');

  $this->objpdf->Setfont('Arial','',6);
  $this->objpdf->text(182, $xlin + 38.5, '( = ) Valor Documento');
  $this->objpdf->text(182, $xlin +   46, '( - ) Desconto ');
  $this->objpdf->text(182, $xlin +   54, '( + ) Mora / Multa');
  ...
  if(isset($this->lEmiteVal)){
	...
	$this->objpdf->setxy(181, $xlin + 36);
	$this->objpdf->cell(25,9,$totalrec,0,0,"R");
	$this->objpdf->setxy(181, $xlin + 44);
	$this->objpdf->cell(25,9,$totaldesc,0,0,"R");
	$this->objpdf->setxy(181, $xlin + 52);
  ...
  $this->objpdf->Roundedrect(125, $xlin + 60, 32, 7, 2, 'DF','1234');
  $this->objpdf->Roundedrect(158, $xlin + 60, 22, 7, 2, 'DF','1234');
  $this->objpdf->Roundedrect(181, $xlin + 60, 25, 7, 2, 'DF','1234');
  
  $this->objpdf->text(129, $xlin + 62, 'Código de Arrecadação');
  $this->objpdf->text(160, $xlin + 62, 'Vencimento');
  $this->objpdf->text(183, $xlin + 62, '( = ) Valor Cobrado R$');
  
  $this->objpdf->setfont('Arial','',9);
  $this->objpdf->text(160, $xlin + 65,$this->dtvenc);
  $this->objpdf->text(126, $xlin + 65,$this->numpre);
  
  $this->objpdf->setfont('Arial','b',10);
  $this->objpdf->setxy(181,($xlin + 60));
  $this->objpdf->cell(25,9,$valtotal,0,0,"R");
  
  $this->objpdf->SetFont('Arial','B',5);
  $this->objpdf->text(140, $xlin + 70, "A   U   T   E   N   T   I   C   A   Ç   Ã   O      M   E   C   Â   N   I   C   A");

  if (isset($this->k12_codautent)){
     $this->objpdf->SetFont('Arial', '', 8);
     $this->objpdf->text(138, $xlin + 80, $this->k12_codautent);
  }
  ...
  if( $i == 2 ){
  	...
  	$this->objpdf->setfont('Arial','',10);
    $this->objpdf->text(20, $xlin + 68, @$this->linhadigitavel );
  	$this->objpdf->int25(10, $xlin + 70, $this->codigobarras, 15, 0.341);
  }
  
  if ($i != 2) {
  	
    $this->objpdf->setDash(1, 1);
    $this->objpdf->line(2, $this->objpdf->getY() + 15 , 208, $this->objpdf->getY() + 15 );
    $this->objpdf->setDash();
  }
  
  //$xlin =  130;
  $xlin = $this->objpdf->getY() + 35 ;
}
...
if ($this->loteamento == true) {
  ...
  for ($reg=0; $reg < pg_numrows($resultrecibo); $reg++) {
    ...
    if(($this->objpdf->gety() > $this->objpdf->h-40) or $reg == 0) {
      ...
      //      $this->objpdf->cell(15,$alt,"DESC VLR",0,0,"R",0);
    ...
    //$vlrcor      = substr($fc_calcula,14,13);
    //$descvlrcor  = $vlrcor * $descvlr / 100;
    
    //$vlrjuros    = substr($fc_calcula,27,13);
    //$descjuros   = $vlrjuros * $descjur / 100;
    
    //$vlrmulta    = substr($fc_calcula,40,13);
    //$descmulta   = $vlrmulta * $descmul / 100;
