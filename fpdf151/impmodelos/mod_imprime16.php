...
//		$this->objpdf->roundedrect(10,07,190,183,2,'DF','1234');
	...
	$this->objpdf->Setfont('Arial','',6);
		$this->objpdf->text($xcol+2,$xlin+11,'Admissão:');
		$this->objpdf->Setfont('Arial','B',7);
		$admiss = implode('/', array_reverse((explode('-', $this->admiss))));
		$this->objpdf->text($xcol+13,$xlin+11, $admiss);
	
	$this->objpdf->Setfont('Arial','',6);
		$this->objpdf->text($xcol+27,$xlin+11,'CPF:');
		$this->objpdf->Setfont('Arial','B',7);
		$this->objpdf->text($xcol+32,$xlin+11, $this->cgm);	

	$this->objpdf->Setfont('Arial','',6);
		$this->objpdf->text($xcol+53,$xlin+11,'PIS/PASEP:');
		$this->objpdf->Setfont('Arial','B',7);
		$this->objpdf->text($xcol+66,$xlin+11, $this->pis);
		
	$this->objpdf->Setfont('Arial','',6);
		$this->objpdf->text($xcol+85,$xlin+11,'Vinculo:');
		$this->objpdf->Setfont('Arial','B',7);
		$this->objpdf->text($xcol+93,$xlin+11, $this->vinculo);	
