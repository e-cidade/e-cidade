...
///////////////////////////////////////  MODELO 92  -  SOLICITAÇÃO DE COMPRA  ////////////////////////////////
...
$this->objpdf->MultiCell(202, 4, 'QUANDO NECESSÁRIO FRETE, ESTE CORRERÁ POR CONTA DO FORNECEDOR', 0, "C", 0);
...
for ($i = 0; $i < $this->linhasdasdotac; $i++) {
  ...
  $reservado  = pg_result($this->recorddasdotac, $i, 'pc11_reservado');
  ...
  if (trim($dotacao) != "") {
    if (!in_array($dotacao . $danousu, $arr_antigadotac)) {
      $arr_antigadotac[$dotacao . $danousu][$i] = $dotacao . $danousu;
      $this->objpdf->Setfont('Arial', 'b', 7);
      if (!in_array($estrutu, $arr_antigaestru) && trim($estrutu) != "") {
        $arr_antigaestru[$estrutu][$i] = $estrutu;
  ...
  if ($reservado == 't')
    $this->objpdf->Row(array($item, $quantitem, $unid, '[ITEM ME/EPP] ' . $scodpcmater . $descricaoitem, $valoritem, $valimp), 3, false, $distanciar, 0, 0, true);
  else
