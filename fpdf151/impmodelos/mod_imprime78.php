...
foreach ($this->aLinhasRelatorio as $iInd => $oLinhaRelatorio) {
	...
	if ( $iRecurso != $oLinhaRelatorio->rh72_recurso ) {
		...
		$this->objpdf->ln();
		cabecalhoRelatorio($this->objpdf, $sFonte, $iAlt, $this->lRetencao, $this->lLotacao);
	...
	if ($this->lLotacao == true) {
		...
		$this->objpdf->SetFont($sFonte,"",$iFonte - 1);
	...
	if ($this->lRetencao == "t") {
	  ...
	  foreach($oLinhaRelatorio->aDescontos as $oRetencao) {
		...
		if ($this->lLotacao == true) {
			...
			$this->objpdf->cell(25, $iAlt, "", 0, 0, "C");
			$this->objpdf->cell(15, $iAlt, $oRetencao->rh73_rubric, 1, 0, "C");
			$this->objpdf->cell(75, $iAlt, $oRetencao->rh27_descr, 1, 0, "L");
...
function cabecalhoRelatorio (&$oPdf, $sFonte, $iAlt, $lRetencoes, $lLotacao) {

	if ($lLotacao == true) {
		$oPdf->SetFont($sFonte,"b",8);
		$oPdf->cell(8, $iAlt*2, "Seq", "TBR", 0, "C", 1);
		$oPdf->cell(8, $iAlt*2, "Lot", "TBR", 0, "C", 1);
		$oPdf->cell(17, $iAlt*2, "Empenho", 1, 0, "C", 1);
		$oPdf->cell(90, $iAlt, "Item", 1, 0, "C", 1);
		...
		$iAltura = $oPdf->GetY()-4;
		$oPdf->setxy(43, $iAltura);
		$oPdf->cell(15 , $iAlt, "Código", 1, 0, "C", 1);
		$oPdf->cell(74.7, $iAlt, "Descrição", 1, 0, "C", 1);
		$oPdf->setxy(158, $iAltura);
	...
	if ($lRetencoes == "t") {
		if ($lLotacao == true) {
			$oPdf->cell(33, $iAlt, "", "T", 0, "C" ,1);
			$oPdf->cell(90, $iAlt, "Descontos", 1, 0, "C", 1);
			$oPdf->cell(15, $iAlt*2, "Valor", 1, 0, "C", 1);
			$oPdf->cell(145, $iAlt*2, "", "TLB", 1, "C" ,1);
			$oPdf->sety($oPdf->getY() -4);
			$oPdf->cell(33, $iAlt, "", "B", 0, "C", 1);
			$oPdf->cell(15, $iAlt, "Código", 1, 0, "C", 1);
			$oPdf->cell(74.7, $iAlt, "Descrição", 1, 1, "C", 1);
		} else {
