...
class Oc18009FcParcelamento extends PostgresMigration
{
  ...
  public function up()
  {
    $sql = <<<SQL
        ...
        CREATE OR REPLACE FUNCTION public.fc_parcelamento(integer, date, date, integer, integer, double precision, integer, integer, integer, integer, double precision, double precision, text, integer)
...
declare
  ...
  v_gerahonorario                  boolean default false;
  v_recHonorarioInstituicao        integer;
...
-- 6 = tentando parcelar mais de um numpre (debito)
    ...
    select db21_honorarioadvocaticio 
      into v_recHonorarioInstituicao 
      from db_config 
    ...
    loop
      ...
      if v_record_origem.k03_tipo in (5,15) then

        -- 5 divida ativa e CDA
        v_parcnormal = true;

      elsif v_record_origem.k03_tipo in (18) then
      ...
      IF (v_record_origem.k03_tipo in (15,18)) AND (v_recHonorarioInstituicao > 0 ) THEN
         v_gerahonorario = true;
      end if;
    ...
    for v_record_origem in select distinct
                            ...
                            where k03_tipodebito not in (18) 
    loop
    ...
    v_comando = v_comando || '                                    where k03_tipo in (5,15)                                                                                      \n';
    if v_juronaultima is true then
...
--    perform fc_corrigeparcelamento();
    ...
    if round(v_total,2) <> round(v_resto,2) then
      return '[16] - total gerado da soma das parcelas inconsistente!'||v_total||' - Resto: '||v_resto;
    end if;
    -- and v_desconto in (17,18)
    if v_gerahonorario  is true then 
         insert into arrecad (
        select k00_numpre,
                               k00_numpar,
                               k00_numcgm,
                               k00_dtoper,
                               v_recHonorarioInstituicao,
                               k00_hist,
                               round(k00_valor*0.1,2),
                               k00_dtvenc,                               
                               k00_numtot,
                               k00_numdig,
                               k00_tipo,
                               k00_tipojm from arrecad where k00_numpre = v_numpre );
     end if;
