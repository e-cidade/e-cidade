...
class M8644AutorizacaoHorasExtras extends PostgresMigration
{
  public function up()
  {
    $this->execute("insert into db_syscampo values(1009360,'rh200_autorizahoraextra','bool','Controla a permiss�o de horas extras para o servidores no ponto eletr�nico.','true', 'Calcular H.E. Somente Com Autoriza��o',1,'f','f','f',5,'text','Calcular H.E. Somente Com Autoriza��o')");
  ...
  public function adicionarNaturezaAssentamento() {
    ...
    if(empty($naturezasTipoAssentamentos)) {
      ...
      $this->table('naturezatipoassentamento', array('schema'=>'pessoal'))
        ->insert(array('rh159_sequencial', 'rh159_descricao'), array(array(7, 'Autoriza��o H.E.')
        ))->saveData();
    }
