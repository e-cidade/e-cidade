...
DECLARE
  ...
  iInstit               integer;
  iExerc                integer;
  iRecHonorarioInstituicao      integer;
  ...
  nDescCorrecao         numeric(15,2) default 0;
  nDescJuros            numeric(15,2) default 0;
  nDescMulta            numeric(15,2) default 0;
  ...
  TEM_DESCONTO_MANUAL   boolean default false;
...
BEGIN
  ...
  select db21_honorarioadvocaticio 
    into iRecHonorarioInstituicao 
    from db_config 
  where prefeitura = true and codigo = iInstit ;
 ...
 perform 1
  ...
  if not found then
     ...
     if lRaise is true then
       perform fc_debug('<recibo> N�o encontrados registros do numpre na tabela db_reciboweb' , lRaise, false, false);
       perform fc_debug('<recibo> '                                                           , lRaise, false, false);
...
--    NUMBCO = RECORD_NUMPRE.K99_NUMBCO;
  ...
  select coalesce(sum(k128_correcao),0) as k128_correcao,
         coalesce(sum(k128_juros),0) as k128_juros,
         coalesce(sum(k128_multa),0) as k128_multa
         into nDescCorrecao,
         nDescJuros,
         nDescMulta
         from
         (select abatimentoarreckey.* from abatimentoarreckey
    ...
    inner join tipoabatimento               on tipoabatimento.k126_sequencial     = abatimento.k125_tipoabatimento
    inner join abatimentosituacao           on abatimentosituacao.k165_sequencial = abatimento.k125_abatimentosituacao
    left  join arrecad                      on arrecad.k00_numpre                 = arreckey.k00_numpre and arrecad.k00_numpar = arreckey.k00_numpar and arrecad.k00_receit = arreckey.k00_receit
    where arreckey.k00_numpre                = RECORD_NUMPRE.K99_NUMPRE and arreckey.k00_numpar = RECORD_NUMPRE.K99_NUMPAR
    and abatimento.k125_tipoabatimento     = 2
    and abatimento.k125_abatimentosituacao = 1) as x;

    if nDescCorrecao <> 0 OR nDescJuros <> 0 OR nDescMulta <> 0 then
      TEM_DESCONTO_MANUAL = TRUE;
    end if;
    if lRaise is true then
      perform fc_debug('<recibo> TEM_DESCONTO_MANUAL: '||TEM_DESCONTO_MANUAL, lRaise, false, false);
      perform fc_debug('<recibo> NUMPRE: '||RECORD_NUMPRE.K99_NUMPRE, lRaise, false, false);
      perform fc_debug('<recibo> NUMPAR: '||RECORD_NUMPRE.K99_NUMPAR, lRaise, false, false);
    end if;

  if lRaise is true then
    perform fc_debug('<descontomanual> JUROS: Desconto '||nDescJuros,lRaise, false, false);
    perform fc_debug('<descontomanual> MULTA: Desconto '||nDescMulta,lRaise, false, false);
    perform fc_debug('<descontomanual> VALOR: Desconto '||nDescCorrecao,lRaise, false, false);
    perform fc_debug('<descontomanual> ---------------'                                                ,lRaise, false, false);
  end if;
 ...
 */
        ...
        --Se o tipo for CDA ou Inicial Foro, insere os calculos advocaticios
        --iRecHonorarioInstituicao retorna código da receita de honorários da Instituição
        IF (ARRETIPO = 19 OR ARRETIPO = 34) AND (iRecHonorarioInstituicao > 0 ) THEN                                      
              INSERT INTO RECIBOPAGA ( k00_numcgm,
                                       k00_dtoper,
                                       k00_receit,
                                       k00_hist  ,
                              ...
                              VALUES ( NUMCGM,
                                       DTEMITE,
                                       iRecHonorarioInstituicao,
                                       V_K00_HIST + 100,
                                       ROUND(((VALOR_RECEITA+CORRECAO)*0.10),2),
                                       DATAVENC,
                                       RECORD_NUMPRE.K99_NUMPRE,
                                       NUMPAR,
                                       NUMTOT,
...
-- INCLUIDO VARIAVEL DESCONTO NO DB_RECIBOWEB
              --Incluindo na recibopaga os descontos concedidos na rotina de desconto manual
              if TEM_DESCONTO_MANUAL is true then
                  if nDescCorrecao <> 0 then
                      if lRaise is true then

                        perform fc_debug('<recibo> DESCONTO MANUAL - VALOR CORRIGIDO', lRaise, false, false);
                        ...
                        perform fc_debug('<recibo> 06 - Receita ....: '||RECEITA, lRaise, false, false);
                        perform fc_debug('<recibo> 06 - Historico ..: 918', lRaise, false, false);
                        perform fc_debug('<recibo> 06 - Valor ......: '||(nDescCorrecao * -1), lRaise, false, false);
                        perform fc_debug('<recibo> ', lRaise, false, false);
                  ...
                  if nDescJuros <> 0 then

                      if lRaise is true then

                        perform fc_debug('<recibo> DESCONTO MANUAL - VALOR JUROS', lRaise, false, false);
                        perform fc_debug('<recibo> 06 - inserindo na recibopaga... ', lRaise, false, false);
                        perform fc_debug('<recibo> 06 - Numpre .....: '||RECORD_NUMPRE.K99_NUMPRE, lRaise, false, false);
                        perform fc_debug('<recibo> 06 - Numpar .....: '||NUMPAR, lRaise, false, false);
                        perform fc_debug('<recibo> 06 - Receita ....: '||K03_RECJUR, lRaise, false, false);
                        perform fc_debug('<recibo> 06 - Historico ..: 918', lRaise, false, false);
                        perform fc_debug('<recibo> 06 - Valor ......: '||(nDescJuros * -1), lRaise, false, false);
                        perform fc_debug('<recibo> ', lRaise, false, false);
                  ...
                  if nDescMulta <> 0 then
                      if lRaise is true then

                        perform fc_debug('<recibo> DESCONTO MANUAL - VALOR MULTA', lRaise, false, false);
                        perform fc_debug('<recibo> 06 - inserindo na recibopaga... ', lRaise, false, false);
                        perform fc_debug('<recibo> 06 - Numpre .....: '||RECORD_NUMPRE.K99_NUMPRE, lRaise, false, false);
                        perform fc_debug('<recibo> 06 - Numpar .....: '||NUMPAR, lRaise, false, false);
                        perform fc_debug('<recibo> 06 - Receita ....: '||K03_RECJUR, lRaise, false, false);
                        perform fc_debug('<recibo> 06 - Historico ..: 918', lRaise, false, false);
                        perform fc_debug('<recibo> 06 - Valor ......: '||(nDescMulta * -1), lRaise, false, false);
                        perform fc_debug('<recibo> ', lRaise, false, false);
  ...
  ELSE
      ...
      if lRaise is true then
        perform fc_debug('<recibo> N�o encontrados registros na tabela arrecad'                , lRaise, false, false);
        perform fc_debug('<recibo> '                                                           , lRaise, false, false);
