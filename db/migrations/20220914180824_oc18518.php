...
class Oc18518 extends PostgresMigration
{
    public function up()
    {
        $jsonDados = file_get_contents('db/migrations/20220914180824_oc18518.json');
    ...
    public function insertFontes(array $fontes)
    {
        ...
        if (strlen($fontes[0]) < 5) {
            $this->execute("UPDATE orctiporec SET o15_datalimite = '2022-12-31' WHERE o15_codigo = {$fontes[0]}");
        }
...
//        foreach ($dados->fontes as $fonte) {
//
//            $this->execute("DELETE FROM orctiporec WHERE o15_codigo = {$fonte->FonteEcidade}");
//            $this->execute("DELETE FROM db_estruturavalor WHERE db121_estrutural = '{$fonte->FonteEcidade}'");
//        }
    }
}
