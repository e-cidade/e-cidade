    ...
    $sql = "
                    ...
                    loop
                      ...
                      insert into arrepaga ( k00_numcgm,
                                           ...
                                           order by k00_numpre,
                                                    k00_numpar,
                                                    k00_receit;
                -- REGISTRA OS DESCONTOS INTEGRAIS
                      for q_disrec_desc in
                                  SELECT a.k00_numpre,
                                         a.k00_numpar,
                                         a.k00_receit,
                                      (SELECT sum(b.k00_valor)
                                       FROM recibopaga b
                                       WHERE b.k00_numnov = r_idret.numpre
                                           AND b.k00_receit = a.k00_receit
                                           AND b.k00_valor > 0) AS k00_valor
                                  FROM recibopaga a
                                  WHERE a.k00_numnov = r_idret.numpre
                                  GROUP BY a.k00_numpre,
                                           a.k00_numpar,
                                           a.k00_receit
                                  HAVING sum(a.k00_valor) = 0.00::float8
                                  union
                                  SELECT a.k00_numpre,
                                         a.k00_numpar,
                                         a.k00_receit,
                                      (SELECT sum(b.k00_valor)
                                       FROM recibopaga b
                                       WHERE b.k00_numnov = (select k127_numpreoriginal from abatimentorecibo where k127_numprerecibo = r_idret.numpre)
                                           AND b.k00_receit = a.k00_receit
                                           AND b.k00_valor > 0) AS k00_valor
                                  FROM recibopaga a
                                  WHERE a.k00_numnov = (select k127_numpreoriginal from abatimentorecibo where k127_numprerecibo = r_idret.numpre)
                                  GROUP BY a.k00_numpre,
                                           a.k00_numpar,
                                           a.k00_receit
                                  HAVING sum(a.k00_valor) = 0.00::float8
                      ...
                      loop
                          insert into disrec_desconto_integral (
                                  codcla,
                                  k00_receit,
                                  vlrrec,
                                  idret,
                                  ...
                                  vcodcla,
                                  q_disrec_desc.k00_receit,
                                  q_disrec_desc.k00_valor,
                                  r_idret.idret,
                                  r_idret.instit
                                );
                      end loop;
                ...
                -- caso positivo passa para o proximo e deixa inconsistente
                      ...
                      */
                      ...
                      -- REGISTRA OS DESCONTOS INTEGRAIS
                      if lRaise is true then
                          perform fc_debug('  <BaixaBanco>  - Registrando desconto integral 2 do idret '||r_idret.idret||' (numpre '||r_idret.numpre||' numpar '||r_idret.numpar||')',lRaise,false,false);
                        end if;
                      for q_disrec_desc in
                                  SELECT a.k00_numpre,
                                         a.k00_numpar,
                         ...
                         for rDadosDesconto in  select sum(k00_valor) as k00_valor,
                                                  ...
                                                  /*select sum(recibopaga.k00_valor) as k00_valor,
                                                          ...
                                                          select sum(k00_valor)  as k00_valor,
                                                                 ...
                                                                 new.k12_receit
                                                            from recibopaga
                                                           where k00_numnov in ( SELECT DISTINCT k127_numpreoriginal AS k00_numpre
                                                                                  FROM corcla
                                                                                  INNER JOIN disrec ON disrec.codcla = corcla.k12_codcla
                                                                                  INNER JOIN disbanco ON disbanco.idret = disrec.idret
                                                                                  INNER JOIN abatimentorecibo ON k127_numprerecibo = k00_numpre
                                                                                  WHERE corcla.k12_id = new.k12_id
                                                                                      AND corcla.k12_data = new.k12_data
                                                                                      AND corcla.k12_autent = new.k12_autent  )
                                                             and recibopaga.k00_receit  = new.k12_receit
  ...
  private function _alterFcAutenClass()
  {
    $sql = "
        ...
        IF NOT IDTERM IS NULL THEN
           ...
           FOR GRAVA_CORNUMP IN SELECT K00_RECEIT,ROUND(SUM(VLRREC),2) AS SUM
              FROM DISREC
