...
class S2230 extends AbstractMigration
{
    ...
    public function up()
    {
        $sql = <<<SQL
        BEGIN;
        update avaliacao set db101_cargadados = 'SELECT DISTINCT * FROM
        ...
        UNION SELECT cgm.z01_cgccpf AS cpftrab,
                    rhpessoal.rh01_regist AS matricula,
                    tpcontra.h13_categoria AS codcateg,
                    NULL AS dtiniafast,
                    \'15\' AS codmotafast,
        ...
        WHERE date_part(\'month\',cadferia.r30_per1i::date) = date_part(\'month\',fc_getsession(\'DB_datausu\')::date)
            AND date_part(\'year\',cadferia.r30_per1i::date) = fc_getsession(\'DB_anousu\')::int)) AS x' where db101_sequencial = 4000108;
SQL;
        $this->execute($sql);
    }
}
