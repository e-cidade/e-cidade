...
BEGIN
    ...
    ELSE
        SELECT CASE
            WHEN (SELECT DISTINCT j31_codigo
                  FROM caracter
                  INNER JOIN carconstr ON j48_caract = j31_codigo
                  INNER JOIN iptuconstr ON (j39_matric, j39_idcons) = (j48_matric, j48_idcons)
                  WHERE j48_matric = iMatricula AND j31_grupo = 58 AND j39_idprinc IS TRUE) > 0
            THEN (SELECT j31_codigo
                  FROM caracter
                  INNER JOIN carconstr ON j48_caract = j31_codigo
                  WHERE j48_matric = iMatricula AND j31_grupo = 58)
            ELSE (SELECT j31_codigo
                  FROM caracter
