-- FUNCTION: public.fc_issqn(integer, date, integer, date, boolean, boolean, integer, character varying, integer, integer)
 -- DROP FUNCTION IF EXISTS public.fc_issqn(integer, date, integer, date, boolean, boolean, integer, character varying, integer, integer);

CREATE OR REPLACE FUNCTION PUBLIC.FC_ISSQN(integer, date, integer, date, boolean, boolean, integer, CHARACTER varying, integer, integer) RETURNS CHARACTER varying LANGUAGE 'plpgsql' COST 100 VOLATILE AS $BODY$
declare
  ...
  lIsencao                  boolean default false;
      ...
      for v_record_tipcalc in execute v_text loop
        ...
        if coalesce(v_quant,0) >= coalesce(v_q81_qini,0) and coalesce(v_quant,0) <= coalesce(v_q81_qfim,999999) then
          ...
          if v_record_tipcalc.q81_gera = 1 and to_number(substr(dInicioAtividade,1,4),'99999') < iAnousu then
            ...
            v_manual = v_manual || '\n';
            raise notice 'nao vai processar...';

          else

            perform fc_debug('entrou no gera 2 do if - verificando pagamentos ',lRaise,false,false);
            raise notice 'entrou no gera 2 do if - verificando pagamentos ';
            ...
            if v_continuar then
              ...
              v_manual = v_manual || 'inserindo na tabela tudo... sequencia:'||v_sequencia||'\n';
              v_manual = v_manual || 'q81_val: '||v_q81_val||' - v_base: '|| v_base||' - uqtab: '||v_uqtab||' - uqcad: '||v_uqcad||' - qprovisorio: '||v_qprovisorio||' - valinflator: '||v_valinflator||'\n';
              ...
              if lInscricaoMei and v_record_tipcalc.q81_cadcalc = 1 then
                v_manual = v_manual ||'Inscricao optante pelo MEI, nao calculando alvara.'||'\n';
                perform fc_debug('Inscricao optante pelo MEI, nao calculando alvara.',lRaise,false,false);
                continue;
              end if;

              --
              -- Se tiver imunidade ou isencao de 100% o calculo do alvara nao deve ser efetuado
              -- calculo alvara
...
ALTER FUNCTION PUBLIC.FC_ISSQN(integer, date, integer, date, boolean, boolean, integer, CHARACTER varying, integer, integer) OWNER TO DBPORTAL;
