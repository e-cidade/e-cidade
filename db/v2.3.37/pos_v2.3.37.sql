insert into db_versao (db30_codver, db30_codversao, db30_codrelease, db30_data, db30_obs)  values (353, 3, 37, '2015-04-27', 'Tarefas: 97615, 97762, 97764, 97766, 97770, 97771, 97772, 97773, 97774, 97775, 97776, 97777, 97778, 97781, 97782, 97783, 97784, 97785, 97786, 97787, 97788, 97789, 97790, 97791, 97792, 97793, 97795, 97796, 97797, 97798, 97799, 97800, 97801, 97802, 97804, 97806, 97807, 97808, 97810, 97811, 97812, 97813, 97814, 97815, 97816, 97817, 97818, 97819, 97820, 97821, 97822, 97823, 97825, 97826, 97827, 97830, 97831, 97832, 97833, 97834, 97835, 97842, 97844, 97845, 97846, 97847, 97848, 97849, 97850, 97853, 97855, 97858, 97859, 97860, 97861, 97862, 97863, 97864, 97865, 97866, 97867, 97868, 97869, 97870, 97871, 97873, 97874, 97875, 97876, 97877, 97878, 97879, 97880, 97881, 97882, 97883, 97884, 97885, 97886, 97887, 97888, 97889, 97890, 97891, 97892, 97893, 97894, 97896, 97897, 97898, 97899, 97900, 97901, 97902, 97903, 97904, 97906, 97909, 97910, 97911, 97912, 97913, 97914, 97915, 97916, 97917, 97918, 97919, 97920, 97921, 97922, 97923, 97925, 97927, 97930, 97931, 97932, 97933, 97934, 97935, 97937, 97938, 97939, 97940');-- 00 Liberado para execucao
 ...
 */
  if lAtivaPgtoParcial is true then
   ...
   /**
     ...
     * sao encontrada as origens excluindo juros, multa e desconto
     */
    ...
    loop
      ...
      /**
       * Verificamos se o somatorio da estrutura do débito (numpre, numpar, receita) é diferente
       * do somatorio da estrutura da recibopaga, pois podem haver casos onde é gerado um recibo
       * e após é alterada a origem do débito (ex.: receita) e quando este é pago, a rotina não
       * encontrará a origem para baixá-lo.
       * Para estes casos será gerada uma inconsistência do recibo.
       */
      select sum(count_arrecad) + sum(count_arrecant) + sum(count_arreold) + sum(count_arreprescr),
             sum(count_recibopaga)
        into iCountDebitoOrigem, iCountRecibopaga
        from (select  ( select count(*)
                          from arrecad
                         where arrecad.k00_numpre = reciboorigem.k00_numpre
                           and arrecad.k00_numpar = reciboorigem.k00_numpar
                           and arrecad.k00_receit = reciboorigem.k00_receit
                       ) as count_arrecad,

                       ( select count(*)
                           from arrecant
                          where arrecant.k00_numpre = reciboorigem.k00_numpre
                            and arrecant.k00_numpar = reciboorigem.k00_numpar
                            and arrecant.k00_receit = reciboorigem.k00_receit
                        ) as count_arrecant,

                       ( select count(*)
                           from arreold
                          where arreold.k00_numpre = reciboorigem.k00_numpre
                            and arreold.k00_numpar = reciboorigem.k00_numpar
                            and arreold.k00_receit = reciboorigem.k00_receit
                        ) as count_arreold,

                       ( select count(*)
                           from arreprescr
                          where arreprescr.k30_numpre = reciboorigem.k00_numpre
                            and arreprescr.k30_numpar = reciboorigem.k00_numpar
                            and arreprescr.k30_receit = reciboorigem.k00_receit
                        ) as count_arreprescr,

                       count(*) as count_recibopaga

                 from recibopaga reciboorigem
                where reciboorigem.k00_numnov = r_idret.k00_numpre
                  and reciboorigem.k00_hist  not in ( 918, 400, 401 )
                group by reciboorigem.k00_numpre,
                         reciboorigem.k00_numpar,
                         reciboorigem.k00_receit ) as origem;

      if ( iCountDebitoOrigem <> iCountRecibopaga ) then
...
begin
   ...
   bSubRaise := ( case when fc_getsession('DB_debugon') is null then false else true end );
  ...
  if bRaise then
   ...
   select case
    ...
    if not found OR nValorInflator is null then
...
$$  language 'plpgsql';create or replace function fc_iptu_get_fator_profundidade_guaiba_2015(integer, integer, boolean) returns numeric as
$$
declare
...
begin
  ...
  select sum(j34_area)
    into nAreaLote
    from lote
         inner join iptubase b on lote.j34_idbql = b.j01_idbql
   where b.j01_baixa is null
     and j34_setor||j34_quadra||j34_lote in (select j34_setor||j34_quadra||j34_lote
                                             from lote
  ...
  perform fc_debug( '<iptu_get_fator_profundidade> nTestada: ' || nTestada || '  nAreaLote: '||nAreaLote, lRaise);
...
$$ language 'plpgsql';create or replace function fc_iptu_taxaaforamento_jaguarao_2015(integer,numeric,integer,numeric,numeric,boolean) returns boolean as
$$
declare
  ...
  nMetrosReferencia numeric;
  nTotalAreaConstr  numeric;

  nInflator         numeric;
  nInflator1993     numeric;
  nAreaTerreno      numeric;

  rDadosTaxa        record;

  nTotalMatriculasLote numeric;

begin

  perform fc_debug( 'CALCULANDO TAXA DE AFORAMENTO ...');
  perform fc_debug( ' receita   - ' || iReceita  );
  perform fc_debug( ' aliq      - ' || iAliquota );
  perform fc_debug( ' historico - ' || iHistCalc );
  ...
  perform 1
    from carlote
         inner join caracter on caracter.j31_codigo = carlote.j35_caract
   where caracter.j31_grupo  = 56
     and caracter.j31_codigo = 140
     and carlote.j35_idbql   = rDadosTaxa.idbql;

  if found then

    perform fc_debug('Não deve calcular taxa de aforamento.');
    return false;
  end if;
 ...
 select j34_setor
    ...
    where j01_matric = rDadosTaxa.matric;

  perform fc_debug('Zona ' || iSetor);

  select i02_valor
    into nInflator1993
    from infla
   where infla.i02_codigo = 'URM'
     and extract (year from infla.i02_data) = '1993';

  select i02_valor
    into nInflator
    from infla
   where infla.i02_codigo = 'URM'
     and extract (year from infla.i02_data) = iAnousu;

  --37 valor em cruzeiros da vigencia da lei em 1993
  nValorBase := ( 37 / nInflator1993 ) * nInflator;

  perform fc_debug('Valor base ' || nValorBase);

  if iSetor <> '99' then

    perform fc_debug('Calculando aforamento Urbano');

    select j36_testad
      into nMetrosReferencia
      from testpri
           inner join testada on testada.j36_idbql = testpri.j49_idbql
     where testpri.j49_idbql = rDadosTaxa.idbql;

  else

    perform fc_debug('Calculando aforamento Rural');

    select areat
      into nAreaTerreno
      from tmpdadosiptu;

    nMetrosReferencia := nAreaTerreno / 100;

  end if;

  perform fc_debug('Metros referencia '|| nMetrosReferencia);

  nValTaxa := nMetrosReferencia * nValorBase;

  if iPercIsen > 0 then
   nValTaxa := nValTaxa * (100 - iPercIsen) / 100;
  end if;

  select count(*) as total_matriculas_lote
    into nTotalMatriculasLote
    from iptubase
   where j01_idbql = rDadosTaxa.idbql
     and j01_baixa is null;

  nValTaxa = nValTaxa / nTotalMatriculasLote;

  perform fc_debug('Valor da taxa de aforamento: ' || nValTaxa );
...
$$ language 'plpgsql';create or replace function fc_iptu_taxalimpeza_jaguarao_2015(integer,numeric,integer,numeric,numeric,boolean) returns boolean as
$$
declare
  ...
  iAnousu              integer default 0;
  iSetor               integer default 0;
  nMetrosTestada       numeric;
  nPercentual          numeric default 20;
  nInflator            numeric;
  nTotalMatriculasLote numeric;
...
begin
  ...
  perform fc_debug( 'CALCULANDO TAXA DE LIMPEZA ...');
  perform fc_debug( ' receita   - ' || iReceita  );
  perform fc_debug( ' aliq      - ' || iAliquota );
  perform fc_debug( ' historico - ' || iHistCalc );
  ...
  perform fc_debug('Percentual: '     || nPercentual);
  perform fc_debug('Metros testada: ' || nMetrosTestada);
  ...
  nValTaxa = ((20 * nInflator) / 100) * nMetrosTestada;
  ...
  perform fc_debug('Valor da taxa de limpeza: ' || nValTaxa );
...
$$ language 'plpgsql';create or replace function fc_iptu_taxalixo_jaguarao_2015(integer,numeric,integer,numeric,numeric,boolean) returns boolean as
$$
declare
...
begin
  ...
  perform fc_debug( 'CALCULANDO TAXA DE COLETA DE LIXO ...');
  perform fc_debug( ' receita   - ' || iReceita  );
  perform fc_debug( ' aliq      - ' || iAliquota );
  perform fc_debug( ' historico - ' || iHistCalc );
  ...
  if not found OR lPredial is false then
    return false;
  end if;

  perform fc_debug( 'PREDIAL ' || lPredial);
  ...
  nValTaxa = ((nPercentual * nInflator) / 100) * rDadosTaxa.totareaconst;

  if iPercIsen > 0 then
   nValTaxa := nValTaxa * (100 - iPercIsen) / 100;
  end if;

  perform fc_debug('Valor da taxa de lixo: ' || nValTaxa );
...
$$ language 'plpgsql';create or replace function fc_iptu_taxamanutencaocadastral_jaguarao_2015(integer,numeric,integer,numeric,numeric,boolean) returns boolean as
$$
declare
...
begin
  ...
  perform fc_debug( 'CALCULANDO TAXA DE MANUTENCAO CADASTRAL ...');
  perform fc_debug( ' receita   - ' || iReceita  );
  perform fc_debug( ' aliq      - ' || iAliquota );
  perform fc_debug( ' historico - ' || iHistCalc );
  ...
  perform fc_debug('Valor da taxa de matenutencao: ' || nValTaxa );
...
$$
language 'sql';insert into db_versaoant (db31_codver,db31_data) values (353, current_date);
...
select setval ('db_versaocpdarq_db34_codarq_seq',(select max (db34_codarq) from db_versaocpdarq));create table bkp_db_permissao_20150427_103927 as select * from db_permissao;
