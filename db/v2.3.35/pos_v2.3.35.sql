...
declare
  ...
  iAnousu    alias for $2;
  parcelaini alias for $3; -- Não utilizada no escopo
  parcelafim alias for $4; -- Não utilizada no escopo
...
begin
  ...
  /**
   * Consultamos as caracteristicas de muro, passeio e pavimentaçao do imovel para determinar
  ...
  /**
   * Se qualquer uma das características não for informada, não será aplicada nenhuma correção sobre a alíquota
   */
  if iCarPavimentacao != 0 and iCarPasseio != 0 and iCarMuro != 0 then
    ...
    if iCarPavimentacao = 7 then
      ...
      if iCarMuro = 142 or iCarPasseio = 140 then
        ...
        /**
         * Comparamos o ano atual com o ano coletado anteriormente, para sabermos sua diferença.
      ...
      /**
       * Se o lote for desprovido de pavimentaçao, a aliquota sera reduzida em 0.02%
       */
...
begin
   ...
   */
   ...
   where caracter.j31_grupo = 44
    ...
    rtp_iptu_calculavvt.rtMsgerro := 'Sem posição fiscal definida [ IDBQL : '||coalesce(iIdbql,0)||' ] ';
    rtp_iptu_calculavvt.rbErro    := 't';
    return rtp_iptu_calculavvt;
  end if;
	...
	if not found or nAreaTerreno = 0 then
	  ...
	  rtp_iptu_calculavvt.rtMsgerro := 'Área não encontrado para terreno [ IDBQL : '||coalesce(iIdbql,0)||' ] ';
    rtp_iptu_calculavvt.rbErro    := 't';
    return rtp_iptu_calculavvt;
	end if;
    ...
    rtp_iptu_calculavvt.rtMsgerro := 'Valor não encontrado para o setor de localização [ IDBQL : '||coalesce(iIdbql,0)||' ] ';
    rtp_iptu_calculavvt.rbErro    := 't';
    return rtp_iptu_calculavvt;
	end if;
   ...
   where carlote.j35_idbql = iIdbql;
	...
	if not found or nFatorSituacao = 0 then
	  ...
	  rtp_iptu_calculavvt.rtMsgerro := 'Fator de situacao não encontrado para o lote [ IDBQL : '||coalesce(iIdbql,0)||' ] ';
    rtp_iptu_calculavvt.rbErro    := 't';
    return rtp_iptu_calculavvt;
	end if;
   ...
   where carlote.j35_idbql = iIdbql;
	...
	if not found or nFatorCaracteristica = 0 then
	  ...
	  rtp_iptu_calculavvt.rtMsgerro := 'Fator de caracteristica não encontrado para o lote [ IDBQL : '||coalesce(iIdbql,0)||' ] ';
    rtp_iptu_calculavvt.rbErro    := 't';
    return rtp_iptu_calculavvt;
	end if;
   ...
   where carlote.j35_idbql = iIdbql;
	...
	if not found or nFatorNivel = 0 then
	  ...
	  rtp_iptu_calculavvt.rtMsgerro := 'Fator de nivel não encontrado para o lote [ IDBQL : '||coalesce(iIdbql,0)||' ] ';
    rtp_iptu_calculavvt.rbErro    := 't';
    return rtp_iptu_calculavvt;
	end if;
   ...
   where carlote.j35_idbql = iIdbql;
	...
	if not found or nFatorNroFrentes = 0 then
	  ...
	  rtp_iptu_calculavvt.rtMsgerro := 'Fator de nro de frentes não encontrado para o lote [ IDBQL : '||coalesce(iIdbql,0)||' ] ';
    rtp_iptu_calculavvt.rbErro    := 't';
    return rtp_iptu_calculavvt;
	end if;
  ...
  if nAreaTerreno > 4000 then
    ...
    if ( cPlanta = '0174' or cPlanta = '174A' ) or cQuadra = 'AREA' or iCaractAreaLote = 600 then
      ...
      if not found or nFatorGleba = 0 then
        ...
        rtp_iptu_calculavvt.rtMsgerro := 'Fator de gleba não encontrado para o lote [ IDBQL : '||coalesce(iIdbql,0)||' ] ';
        rtp_iptu_calculavvt.rbErro    := 't';
        return rtp_iptu_calculavvt;
