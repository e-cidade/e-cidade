...
require_once(DB_ROOT ."fpdf151/fpdf.php");
require_once(DB_ROOT ."fpdf151/impcarne.php");
@require_once(DB_ROOT ."fpdf151/scpdf.php");

require_once(DB_MODEL."model/regraEmissao.model.php");
require_once(DB_MODEL."model/dataManager.php");

require_once(DB_CLASSES."classes/db_cancdebitos_classe.php");
require_once(DB_CLASSES."classes/db_cancdebitosproc_classe.php");
require_once(DB_CLASSES."classes/db_cancdebitosreg_classe.php");
require_once(DB_CLASSES."classes/db_cancdebitosprocreg_classe.php");
require_once(DB_CLASSES."classes/db_cancdebitosconcarpeculiar_classe.php");
require_once(DB_CLASSES."classes/db_cancdebitosprocconcarpeculiar_classe.php");
require_once(DB_CLASSES."classes/db_arrecant_classe.php");
require_once(DB_CLASSES."classes/db_arrehist_classe.php");
require_once(DB_CLASSES."classes/db_issvar_classe.php");
require_once(DB_CLASSES."classes/db_issvarsemmov_classe.php");
require_once(DB_CLASSES."classes/db_issvarsemmovreg_classe.php");
require_once(DB_CLASSES."classes/db_issplan_classe.php");
require_once(DB_CLASSES."classes/db_issplannumpre_classe.php");

require_once(DB_MODEL."model/cancelamentoDebitos.model.php");
$oCancelaDebito = new cancelamentoDebitos();

require_once(DB_CLASSES."classes/db_arreinscr_classe.php");
$clarreinscr = new cl_arreinscr();

require_once(DB_CLASSES."classes/db_arrecad_classe.php");
...
try {
  ...
  if ( $iLinhasIntegraCadastro > 0 ) {
      ...
      $sSqlInsereCgm = " insert into cgm ( z01_numcgm,
                                           ...
                                           nextval('cgm_z01_numcgm_seq'),
																					 ".dbValida(substr($oIntegraCadastro->nome              ,0,40),'string').",
																			     ".dbValida($oIntegraCadastro->cpf_cnpj                       ,'string').",
																					 ".dbValida(substr($oIntegraCadastro->cidade            ,0,40),'string').",
																					 ".dbValida($oIntegraCadastro->logradouro                     ,'string').",
																					 ".dbValida($oIntegraCadastro->numero                         ,'int').",
																					 ".dbValida(substr($oIntegraCadastro->complemento       ,0,20),'string').",
																					 ".dbValida(substr($oIntegraCadastro->bairro            ,0,40),'string').",
																					 ".dbValida($oIntegraCadastro->estado                         ,'string').",
  ...
  if ( $lInsereConfig ) {
	...
	$sSqlAtivPrefeitura .= "          substr(q03_descr, 0, 30)       			  as descricao,		   									   		                        ";
	$sSqlAtivPrefeitura .= "          q71_estrutural  			  as codigo_cnae,      									   	                     	  ";
	$sSqlAtivPrefeitura .= "          substr(q71_descr, 0,30)       			  as descricao_cnae,   									   		                      ";
	...
	$rsAtivPrefeitura      = db_query($connOrigem,$sSqlAtivPrefeitura);
	...
	$rsGraficaPrefeitura      = db_query($connOrigem,$sSqlGraficaPrefeitura);
  ...
  $rsDadosEmpresa 	   = db_query($connOrigem,$sSqlDadosEmpresa) or die("Erro:{$sSqlDadosEmpresa}");
    	...
    	unset($oEmpresa);
 	    ...
 	    if ( count($aListaEmpresa) == $iLoteEmpresa || ($iInd+1) == $iLinhasDadosEmpresa ) {
		    ...
		    foreach ( $aListaEmpresa as $iIndEmp => $oEmpresa ) {
			    ...
			    $rsAidofEmpPrefeitura     = db_query($connOrigem,$sSqlAidofEmpPrefeitura);
			    ...
			    $rsEstimativaEmpPrefeitura     = db_query($connOrigem,$sSqlEstimativaEmpPrefeitura);
			    ...
			    if ( count($aListaEstimativaEmpPrefeitura) > 0 ) {
			      ...
			      foreach ( $aListaEstimativa as $oEstimativa ) {
			        ...
			        $rsEstimativaDetalhePrefeitura     = db_query($connOrigem,$sSqlEstimativaDetalhePrefeitura);
	...
	db_logTitulo(" PROCESSA RECIBOS",$sArquivoLog,$iParamLog);

	$sSqlIntegraRecibo  = " select integra_recibo.*,                            ";
  ...
  $sSqlIntegraRecibo .= "        integra_cadastro.cpf_cnpj     as cpfcnpj     ";
	...
	if ( $iLinhasIntegraRecibo > 0 ) {
      ...
      $rsConsultaCgm = db_query($connOrigem,$sSqlConsultaCgm);
      ...
      if (!$rsAlteraIntegraRecibo) {
        throw new Exception("ERRO-45: ".pg_last_error($connDestino)." ".$sSqlAlteraIntegraRecibo);
			...
			$sSqlConsultaReciboAnulado = " select *
			                                 from integra_recibo_anulado
		                                  where integra_recibo = {$oIntegraRecibo->sequencial}";
      ...
      if ( $oIntegraRecibo->tipo_boleto == 'T' &&
         ( $oIntegraRecibo->valor_imposto == 0 ||
           trim($oIntegraRecibo->valor_imposto) == '' ) ) {
      	continue;
      }
      ...
      if ( $oIntegraRecibo->valor_imposto == 0 || trim($oIntegraRecibo->valor_imposto) == '' ) {
        ...
        $rsVerificaIss   =  db_query($connOrigem,$sSqlConsultaIssVar);
      ...
      $rsConsultaIssvar    = db_query($connOrigem,$sSqlConsultaIssVar);
      ...
      if ( pg_num_rows($rsConsultaIssvar) > 0 && $sTipoBoleto == 'P' ) {
        ...
        db_query($connOrigem,$sSqlDeletaVariavelArrecad);

      }

      $oDaoIssVar->q05_numpre = $oIntegraRecibo->numdoc;
		  ...
		  $oDaoIssVar->q05_histor = "Importado do SigCorp - Tipo de Boleto: {$oIntegraRecibo->tipo_boleto}";
      ...
      $oDaoIssVar->q05_vlrinf = "null";
		  ...
		  if ( trim($oIntegraRecibo->inscricao) != '' ) {
		    ...
		    $oDaoIssVar->incluir_issvar_complementar(null,$oIntegraRecibo->inscricao,null,$sTipoBoleto);
		  } else {

	    	db_log("Incluindo ISSQN complementar para CGM : {$iNumCgm}.",$sArquivoLog,2);
	      $oDaoIssVar->incluir_issvar_complementar(null,null,$iNumCgm,$sTipoBoleto);
		  }
	    ...
	    $sSqlIncluiRecibo  = "insert into db_reciboweb ( k99_numpre,
	                                                     k99_numpar,
	                                                     k99_numpre_n,
	                                                     k99_codbco,
	                                                     k99_codage,
	                                                     ...
	                                                     k99_tipo,
	                                                     k99_origem ) ";
	    $sSqlIncluiRecibo .= "                  values ( {$oIntegraRecibo->numdoc},
	                                                     {$oDaoIssVar->q05_numpar},
	                                                     {$oIntegraRecibo->numdoc},
	                                                     0,
	                                                     '',
	                                                     '',
	                                                     0,
	    ...
	    $rsIncluiRecibo    = db_query($connOrigem,$sSqlIncluiRecibo);

	    if ($rsIncluiRecibo == false) {
	    	throw new Exception("ERRO-43: $sSqlIncluiRecibo ");
	    }

	    $sSqlIncluiReciboCodBar = "insert into recibocodbar ( k00_numpre,
	                                                          k00_codbar,
	                                                          k00_linhadigitavel )
	                                                 values ( {$oIntegraRecibo->numdoc},
	                                                         '{$oIntegraRecibo->cod_barras}',
	                                                         '{$oIntegraRecibo->cod_barras}')";

	    $rsIncluiReciboCodBar   = db_query($connOrigem,$sSqlIncluiReciboCodBar);

	    if ($rsIncluiReciboCodBar == false) {
	    	throw new Exception("ERRO-44: $sSqlIncluiReciboCodBar ");
	    }


	    if ( trim($oIntegraRecibo->numbanco) != '' ) {

	      //
	      $sSqlIncluiArrebanco = " insert into arrebanco ( k00_numpre,
                                                         k00_numpar,
                                                         k00_codbco,
                                                         k00_codage,
                                                         k00_numbco
	                                                       ...
	                                                       {$oIntegraRecibo->numdoc},
	                                                       {$oIntegraRecibo->mes_competencia},
                                                         {$oBancoAgencia->banco},
                                                         '{$oBancoAgencia->agencia}',
                                                         '{$oIntegraRecibo->numbanco}'
	                                                     ); ";

	      if ( !db_query($connOrigem,$sSqlIncluiArrebanco) ) {
	        throw new Exception("ERRO: Erro ao gerar arrebanco, {$sSqlIncluiArrebanco}");
	      }
        ...
        $sSqlReceitaDebito = " select arrecad.*
	                              ...
	                              where issvar.q05_codigo = {$oDaoIssVar->q05_codigo}";

	      $rsReceitaDebito   = db_query($connOrigem,$sSqlReceitaDebito);
	      ...
	      try {
	      	$oCancelaDebito->setArreHistTXT("Cancelado pela importação SIGCORP");
	...
	$sSqlIntegraReciboAnulado  = " select integra_recibo_anulado.sequencial as seqanu, ";
  $sSqlIntegraReciboAnulado .= "        integra_recibo.*                             ";
	$sSqlIntegraReciboAnulado .= "   from integra_recibo_anulado                       ";
	$sSqlIntegraReciboAnulado .= "        inner join integra_recibo on integra_recibo.sequencial = integra_recibo_anulado.integra_recibo";
	...
	if ( $iLinhasIntegraReciboAnulado > 0 ) {
	  ...
	  for ( $iIndRecibo=0; $iIndRecibo < $iLinhasIntegraReciboAnulado; $iIndRecibo++ ) {
	    ...
	    $sSqlReceitaDebito = " select k00_receit
	                             from arrecad
	                            where k00_numpre = {$oIntegraReciboAnulado->numdoc}
	                              and k00_numpar = {$oIntegraReciboAnulado->mes_competencia}";
	    $rsReceitaDebito   = db_query($connOrigem,$sSqlReceitaDebito);
        ...
        continue;
	    	...
	    	$iCodReceitaDebito = db_utils::fieldsMemory($rsReceitaDebito,0)->k00_receit;
	    }

	    $aDadosDebitos['Numpre']  = $oIntegraReciboAnulado->numdoc;
	    $aDadosDebitos['Numpar']  = $oIntegraReciboAnulado->mes_competencia;
	    $aDadosDebitos['Receita'] = $iCodReceitaDebito;
      $aDebitos = array($aDadosDebitos);
  ...
  if ( $iLinhasReciboSemBaixa > 0 ) {
    ...
    for ( $iInd=0; $iInd < $iLinhasReciboSemBaixa; $iInd++ ) {
	    ...
	    $rsSituacaoDebito    = db_query($connOrigem,$sSqlSituacaoDebito);
...
if ( $lErro ) {
	...
	db_logTitulo(" FIM PROCESSAMENO COM ERRO",$sArquivoLog,$iParamLog);
