    ...
    for($w=0; $w<sizeof($this->identifica); $w++){
       ...
       <td>
         <table class="bordas" id="<?=$chave?>" border="0" style="border: 3px outset #666666; border-bottom-width: 0px; border-right-width: 1px ;border-right-color: #000000; border-top-color: #3c3c3c; border-right-style: inset; " cellpadding="3" cellspacing="0" >
           <tr>
             <td nowrap>
               <input readonly <?=$disabled?>  name="<?=$chave?>" class="nomes"  style="font-weight:bold; color:<?=$cortexto?>; background-color:<?=$corfundo?>;" type="text"  value="<?=$this->identifica[$chave]?>" title="<?=$this->title[$chave]?>" size="<?=$sizecampo?>"  onClick="<?=$funcao_js?> mo_camada('<?=$chave?>'); ">
                   </td>
           </tr>
         </table>
       </td>
 ...
 <?
    ...
    for($w=0; $w<sizeof($this->identifica); $w++){
       ...
       <div class="tabela" id="div_<?=$chave?>" style="position:absolute; left:<?=$this->abas_left?>px; top:<?=$this->abas_top?>px;  visibility: visible;">
          <iframe  id='<?=$chave?>' name="iframe_<?=$chave?>" class="bordasi" <?=$src?> frameborder="0" marginwidth="0" leftmargin="0" topmargin="0" width="<?=$this->iframe_width?>"  height="<?=$this->iframe_height?>" scrolling="<?=$this->scrolling?>">
          </iframe>
          </div>
...
//|30|//A princípio, utilizado somente para quando for rubrica (Módulo Pessoal), setando true, completará com zeros à esquerda formando um código com 4 caracteres
  ...
  function funcao_gera_formulario() {
    ...
    echo "            <input name=\"".$this->nome_botao."\" type=\"button\" value=\"Lançar\" >\n";
    echo "        </b></td>\n";
    echo "        </tr>\n";
    echo "        <tr>\n ";
    ...
    echo "  if(F.length==0){\n";
    echo "    alert('Cadastre um ítem para prosseguir.');\n";
...
*/
    ...
    if($this->mostrar_botao_lancar == true)
      echo "  document.form1.".$this->nome_botao.".onclick = '';\n";
    echo "  if(chave){\n";
    echo "    js_OpenJanelaIframe('".($this->localjan != "CurrentWindow.corpo"?$this->localjan:"CurrentWindow.corpo")."','".$this->nomeiframe."','".$this->func_arquivo."?funcao_js=parent.".$this->funcao_js."|".$this->codigo."|".$this->descr.$this->passar_query_string_para_func."'".($this->parametros != ""?$this->parametros:"").",'Pesquisa',true".($this->top!=null?",'".$this->CurrentWindow."'":"").");\n";
    echo "  }else{\n";
...
class cl_formulario_rel_pes {
  ...
  var $mostaln = false; // Mostrar se é em ordem ordem ascendente ou descendente.
...
//    }
  ...
  function monta_form_unicocampo($campo,$ncampo,$dcampo,$tabela,$tamanho=8){
    ...
    echo "
          <tr>
            <td align='right' nowrap title='".$$Tncampo."' >
         ";
  ...
  function monta_form_intervalos($campo1,$campo2,$campo4,$ncampo,$tabela,$tamanho=8){
    ...
    echo "
          <tr>
            <td align='right' nowrap title='".$campo4."' ><b>
         ";
  ...
  function monta_select_tdrtable($arraydados,$nomecampo,$labelimp,$titleimp="",$onchang=""){
    ...
    if(count($arraydados) > 1){
      echo "
            <tr>
              <td align='right' nowrap title='".$titleimp."' >
    ...
    if($this->tipofol == true || $this->tipopon == true){
      if(trim($this->complementar) != ""){
        if((isset($GLOBALS[$this->tfonome]) && $GLOBALS[$this->tfonome] == $this->complementar) || (isset($GLOBALS[$this->tponome]) && $GLOBALS[$this->tponome] == $this->complementar)){
          $result_complementar = pg_exec("select distinct r48_semest as semestralidade from gerfcom  where r48_anousu = ".$$campoano." and r48_mesusu = ".$$campomes);
  ...
  $this->monta_select_tdrtable($this->arr_mostord,$this->mornome,$this->tipordem.":",$this->tipordem);
    ...
    if($this->usarprevid == true){
      ...
      $result_previd = pg_exec("select distinct ".$this->camposprevid." from inssirf ".$whereprevid." order by r33_codtab");
      if(pg_num_rows($result_previd) > 0){
        echo "
              <tr>
                <td align='right' nowrap title='Tabelas de Previdência / INSS' >
    ...
    if($this->selregi == true || $this->selorga == true || $this->sellota == true || $this->selrubr == true || $this->selloca == true || $this->selcarg == true){
      echo "
            <tr>
    ...
    if($this->testarescisaoregi == true){
      $registrosselecionados = "";
            ...
            for($i=0; $i<count($arr_registrosselecion); $i++){
                    $result_rescisoes = pg_exec("select rh05_recis from rhpessoalmov inner join rhpesrescisao on rh05_seqpes = rh02_seqpes where rh02_anousu = ".$ano." and rh02_mesusu = ".$mes." and rh02_regist = ".$arr_registrosselecion[$i]);
     ...
     $query_rescisao = "&testarescisao=".$this->testarescisaoregi;
      ...
      $this->clarquivo_auxiliar->funcao_gera_formulario();
      echo "
                </table>
              </td>
            </tr>
