...
require_once("./classes/db_padroes_classe.php");
require_once("./classes/db_cargo_classe.php");
require_once("./classes/db_cfpess_classe.php");
require_once("./classes/db_cedulas_classe.php");
...
require_once("./classes/db_cheques_classe.php");
require_once("./classes/db_desconto_classe.php");
require_once("./classes/db_rubricas_classe.php");
require_once("./classes/db_pesdiver_classe.php");
require_once("./classes/db_bases_classe.php");
require_once("./classes/db_basesr_classe.php");
require_once("./classes/db_efetiv_classe.php");
require_once("./classes/db_historic_classe.php");
require_once("./classes/db_eventos_classe.php");
require_once("./classes/db_folhaemp_classe.php");
require_once("./classes/db_inssirf_classe.php");
require_once("./classes/db_landesc_classe.php");
require_once("./classes/db_lotativ_classe.php");
require_once("./classes/db_pensao_classe.php");
require_once("./classes/db_pensaocontabancaria_classe.php");
require_once("./classes/db_pontofa_classe.php");
require_once("./classes/db_pontofx_classe.php");
require_once("./classes/db_depend_classe.php");
require_once("./classes/db_rhdepend_classe.php");
require_once("./classes/db_progress_classe.php");
require_once("./classes/db_rescisao_classe.php");
require_once("./classes/db_reposic_classe.php");
...
require_once("./classes/db_rhinssoutros_classe.php");
require_once("./classes/db_rhpesponto_classe.php");
require_once("./classes/db_rhpesprogres_classe.php");
...
require_once("./classes/db_rhvisavalecad_classe.php");
require_once("./classes/db_rhpesrubcalc_classe.php");
require_once("./classes/db_rhpescargo_classe.php");
require_once("./classes/db_funcao_classe.php");
require_once("./classes/db_vtfempr_classe.php");
require_once("./classes/db_vtffunc_classe.php");
require_once("./classes/db_vtfdias_classe.php");
require_once("./classes/db_pensaoretencao_classe.php");
require_once("./classes/db_planosaude_classe.php");
require_once("./dbforms/db_funcoes.php");
require_once("./libs/db_libpessoal.php");
require_once("./classes/db_rhpessoalmovcontabancaria_classe.php");
...
if (isset($desprocess) && $desprocess == "true") {
    ...
    $arr_tabelas_exclui = array("AFASTA","AJUSTEIR","BASESR","BASES","CADFERIA","CALFOLHA","CEDULAS","CFPESS","CHEQUES","DEPEND","DESCONTO","PESDIVER","EFETIV","FOLHAEMP","GERFSAL","GERFFER","GERFS13","GERFADI","GERFFX","GERFRES","GERFCOM","HISTORIC","INSSIRF","IPE","LANDESC","LOTATIV","PENSAORETENCAO","PENSAO","PONTOFX","PONTOFS","PONTOFA","PONTOF13","PONTOFR","PONTOFE","PONTOCOM","VTFDIAS","VTFFUNC ","PREVIDEN","REPOSIC","PESSOAL","PROGRESS","PADROES","CARGO","FUNCAO","RUBRICAS","RESCISAO","MOVCASADASSEFIP","TIPOASSEEXTERNO","CODMOVSEFIP","VTFEMPR","RHPESSOALMOV","RHVISAVALECAD","TIPOASSEFINANCEIRO","PLANOSAUDE"
    );
    $arr_siglass_exclui = array("r45","r61","r09","r08","r30","r51","r05","r11","r12","r03","r27","r07","r57","r42","r14","r31","r35","r22","r53","r20","r48","r25","r33","r36","r28","r41","rh77","r52","r90","r10","r21","r34","r19","r29","r47","r63","r17","r60","r64","r01","r24","r02","r65","r37","r06","r59","r67","rh167","r66","r16","rh02","rh49","rh165","r75"
    );
    $exist_virada = false;
    ...
    if (!$sqlerro) {
        ...
        for ($i = 0; $i < count($arr_tabelas_exclui); $i++) {
            ...
            if (db_at(strtoupper($sigla), "R61_R51_R05_R12_R03_R27_R57_R42_R25_R28_R41_R52_R63_R17_R60_R64_R65_R37_R66_R67_R75_") != '0' && $exist_virada == true) {
                continue;
            }
    ...
    function db_incluir_sql($sql1, $table, $trigger)
    {
        ...
        for ($ii = 0; $ii < $colunas; $ii++) {
            ...
            if (strpos($dcoluna, "anousu") || (strpos($dcoluna, "ano") && $trigger == "baserhcadregime")) {
                ...
                $dcoluna == "rh44_seqpes" || $dcoluna == "rh03_seqpes" || $dcoluna == "rh05_seqpes" || $dcoluna == "rh20_seqpes"
                ...
                // Testa se a tabela planosaude
            } else if ($dcoluna == "r75_sequencial") {
                $dcoluna = "nextval('planosaude_r75_sequencial_seq')";
            }
  ...
  and pensaocontabancaria.rh139_numcgm = p.r52_numcgm )         ";
    ...
    if ($sqlerro == false) {
        ...
        $sWherePlanoSaude  = "     r75_anousu = " . $ano;
        $sWherePlanoSaude .= " and r75_mesusu = " . $mes;
        $sWherePlanoSaude .= " and exists (select 1 ";
        $sWherePlanoSaude .= "               from rhpessoal ";
        $sWherePlanoSaude .= "              where rh01_regist = r75_regist ";
        $sWherePlanoSaude .= "                and rh01_instit = " . db_getsession("DB_instit") . ")";
        $result_test = $clplanosaude->sql_record($clplanosaude->sql_query_file(null, "*", null, $sWherePlanoSaude));

        if ($clplanosaude->numrows == 0) {

            $sWherePlanoSaude  = "     r75_anousu = " . $dataii_ano;
            $sWherePlanoSaude .= " and r75_mesusu = " . $dataii_mes;
            $sWherePlanoSaude .= " and exists (select 1 ";
            $sWherePlanoSaude .= "               from rhpessoal ";
            $sWherePlanoSaude .= "              where rh01_regist = r75_regist ";
            $sWherePlanoSaude .= "                and rh01_instit = " . db_getsession("DB_instit") . ")";
            ...
            $sql1 = $clplanosaude->sql_query_file(null, "*", null, $sWherePlanoSaude);

            $retorno = db_incluir_sql($sql1, $clplanosaude, "planosaude");
