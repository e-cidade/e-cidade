  ...
  switch($chave) {
    case "hist":
      if($chave_valor!= "" && $lista == ""){
        $sql = "select (c03_codigo || '##' || c03_descr) as db_codigo,c03_codigo,c03_descr
	          from hist
      ...
      $sql = "select (c03_codigo || '##' || c03_descr) as db_codigo,c03_codigo,c03_descr
	          from hist
		      where c03_codigo like '".$chave_valor."%'
		      order by c03_codigo";
      
	  break;
    case "descrhist":
      $sql = "select (c03_descr || '##' || c03_codigo) as db_codigo,c03_codigo,c03_descr
	          from hist
			  where upper(c03_descr) like upper('".$chave_valor."%')
		      order by c03_descr";
	  break;
   }
?>
